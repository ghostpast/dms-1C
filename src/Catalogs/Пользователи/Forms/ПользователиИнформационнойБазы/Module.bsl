...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПользователиСервер.П_ЭтоПолноправныйПользователь(, Истина) Тогда
		ВызватьИсключение "Недостаточно прав для открытия списка пользователей информационной базы.";
	КонецЕсли;

	ПользователиСервер.П_НайтиНеоднозначныхПользователейИБ(Неопределено);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ДобавленПользовательИБ" ИЛИ ИмяСобытия = "ИзмененПользовательИБ" ИЛИ ИмяСобытия = "УдаленПользовательИБ" ИЛИ ИмяСобытия = "ОчищеноСопоставлениеСНесуществующимПользователемИБ" Тогда
		ЗаполнитьПользователейИБ();
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтменитьСопоставление(Команда)
	...
	Кнопки.Добавить("ОтменитьСопоставление", "Отменить сопоставление");
	Кнопки.Добавить("ОставитьСопоставление", "Оставить сопоставление");

	ПоказатьВопрос(
		Новый ОписаниеОповещения("ОтменитьСопоставлениеПродолжение", ЭтотОбъект),
		"Отмена сопоставления пользователя информационной базы с пользователем в справочнике.
		           |
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", "<Нет данных>");
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат","БЛ=Нет; БИ=Да");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат", "БЛ=; БИ=Да");
КонецПроцедуры
...
Процедура ЗаполнитьПользователейИБ()
	...
	ТекущийИдентификаторПользователяИБ = ?(Строка = Неопределено, ПустойУникальныйИдентификатор, Строка.ИдентификаторПользователяИБ);
	...
	Для Каждого ПользовательИБ Из ВсеПользователиИБ Цикл
		...
		Если СвойстваПользовательИБ = Неопределено Тогда
			СвойстваПользовательИБ = ПользователиСервер.П_НовоеОписаниеПользователяИБ();
		КонецЕсли;
...
Процедура УдалитьПользователяИБ(ИдентификаторПользователяИБ, Отказ)
	Попытка
		ПользователиСервер.П_УдалитьПользователяИБ(ИдентификаторПользователяИБ);
	Исключение
		БазоваяПодсистемаСервер.ОН_СообщитьПользователю(ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке()),,,, Отказ);
	КонецПопытки;
КонецПроцедуры
...
Процедура ОткрытьПользователяПоСсылке()
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Ссылка) Тогда
		ОткрытьФорму(?(ТекущиеДанные.СопоставленСВнешнимПользователем, "Справочник.ВнешниеПользователи.ФормаОбъекта", "Справочник.Пользователи.ФормаОбъекта"), Новый Структура("Ключ", ТекущиеДанные.Ссылка));
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура УдалитьТекущегоПользователяИБ(УдалитьСтроку = Ложь)
	ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьТекущегоПользователяИБЗавершение", ЭтотОбъект, УдалитьСтроку), "Удалить пользователя информационной базы?", РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура УдалитьТекущегоПользователяИБЗавершение(Ответ, УдалитьСтроку) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		Отказ = Ложь;
		УдалитьПользователяИБ(Элементы.ПользователиИБ.ТекущиеДанные.ИдентификаторПользователяИБ, Отказ);
...
Процедура СопоставитьПользователяИБ(СНовым = Ложь)
	Если ТипыПользователей.Количество() > 1 Тогда
		ТипыПользователей.ПоказатьВыборЭлемента(Новый ОписаниеОповещения("СопоставитьПользователяИБДляТипаЭлемента", ЭтотОбъект, СНовым), "Выбор типа данных", ТипыПользователей[0]);
	Иначе
...
Процедура СопоставитьПользователяИБДляТипаЭлемента(ЭлементСписка, СНовым) Экспорт
	...
	Если Не СНовым Тогда
		...
		ОткрытьФорму("Справочник." + ИмяСправочника + ".ФормаВыбора", ПараметрыФормы,,,,, Новый ОписаниеОповещения("СопоставитьПользователяИБСЭлементом", ЭтотОбъект, ИмяСправочника));
	Иначе
...
Процедура СопоставитьПользователяИБСЭлементом(Элемент, ИмяСправочника) Экспорт
	...
	ПараметрыФормы.Вставить("ИдентификаторПользователяИБ", Элементы.ПользователиИБ.ТекущиеДанные.ИдентификаторПользователяИБ);
