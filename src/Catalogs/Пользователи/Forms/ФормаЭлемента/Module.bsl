...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПользователиКлиентСервер.сП_ВерсияПредприятияПоддерживаетВосстановлениеПаролей() Тогда
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	Если ТребуетсяЗаписьПользователяИБ(ЭтотОбъект) Тогда
		...
		Если ПользователиКлиентСервер.сП_ВерсияПредприятияПоддерживаетВосстановлениеПаролей() И ЗначениеЗаполнено(ИмяРеквизитаСПочтойДляВосстановленияПароля) Тогда
...
Процедура ВходВПрограммуРазрешенПриИзменении(Элемент)
	Если Объект.ПометкаУдаления И ВходВПрограммуРазрешен Тогда
		...
		ПоказатьПредупреждение(,
			"Чтобы разрешить вход в программу, требуется снять
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	ИначеЕсли Не ПользователиСервер.П_ЭтоПолноправныйПользователь() Тогда
		Рекомендация = Символы.ПС
			+ "Обратитесь к администратору, чтобы устранить различия.";
	Иначе
		Рекомендация = "";
	КонецЕсли;
...
Функция ТребуетсяЗаписьПользователяИБ(Форма, УчитыватьСтандартноеИмя = Истина)
	...
	Если ПользователиКлиентСервер.сП_ВерсияПредприятияПоддерживаетВосстановлениеПаролей() И Форма.ПользовательИБЗапрещеноВосстанавливатьПароль <> Шаблон.ЗапрещеноВосстанавливатьПароль Тогда
		Возврат Истина;
	КонецЕсли;
