...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если СписокОшибок.Количество() > 0 Тогда
		...
		Для каждого ОписаниеОшибки Из СписокОшибок Цикл
			БазоваяПодсистемаСервер.ОН_СообщитьПользователю(ОписаниеОшибки.ТекстСообщения,, ОписаниеОшибки.ИмяПоля);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверитьУникальностьЭлементов()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрСравнить(Выборка.Код, Код) = 0 Тогда
			Сообщение.ТекстСообщения	= СтрШаблон("С кодом %1 уже существует страна %2. Измените код или используйте уже существующие данные.", Код, Выборка.Наименование);
			Сообщение.ИмяПоля			= "Объект.Код";
		ИначеЕсли СтрСравнить(Выборка.Наименование, Наименование) = 0 Тогда
			Сообщение.ТекстСообщения	= СтрШаблон("Страна с наименованием %1 уже существует. Измените наименование или используйте уже существующие данные.", Выборка.Наименование);
			Сообщение.ИмяПоля			= "Объект.Наименование";
		ИначеЕсли ЗначениеЗаполнено(НаименованиеПолное) И СтрСравнить(Выборка.НаименованиеПолное, НаименованиеПолное) = 0 Тогда
			Сообщение.ТекстСообщения	= СтрШаблон("Уже существует страна %2 с полным наименованием %1. Измените полное наименование или используйте уже существующие данные.", НаименованиеПолное, Выборка.Наименование);
		...
		ИначеЕсли ЗначениеЗаполнено(КодАльфа2) И СтрСравнить(Выборка.КодАльфа2, КодАльфа2) = 0 Тогда
			Сообщение.ТекстСообщения	= СтрШаблон("С кодом Альфа-2 %1 уже существует страна %2. Измените код Альфа-2 или используйте уже существующие данные.", КодАльфа2, Выборка.Наименование);
		...
		ИначеЕсли ЗначениеЗаполнено(КодАльфа3) И СтрСравнить(Выборка.КодАльфа3, КодАльфа3) = 0 Тогда
			Сообщение.ТекстСообщения	= СтрШаблон("С кодом Альфа-3 %1 уже существует страна %2. Измените код Альфа-3 или используйте уже существующие данные.", КодАльфа3, Выборка.Наименование);
...
Процедура ПроверитьИзменениеПредопределенногоЭлемента()
	ПредыдущиеЗначения = БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта(Ссылка, "Код, Наименование");
	Если СтрСравнить(ПредыдущиеЗначения.Наименование, Наименование) <> 0 Тогда
		ТекстСообщения = СтрШаблон("Не допускается изменение наименования для страны %1", ПредыдущиеЗначения.Наименование);

		ВызватьИсключение ТекстСообщения;
	КонецЕсли;

	Если СтрСравнить(ПредыдущиеЗначения.Код, Код) <> 0 Тогда
		ТекстСообщения = СтрШаблон("Не допускается изменение кода для страны %1", ПредыдущиеЗначения.Наименование);
