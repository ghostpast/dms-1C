...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПредставлениеОбъектаКлассификатора = БазоваяПодсистемаСервер.ОН_ПредставлениеОбъекта(Метаданные.Справочники.СтраныМира);
...
Процедура КлассификаторВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Не РежимВыбора Тогда
		...
		Если СтраныНетВСпискеСтран(Страна.Наименование) Тогда
			ТекстВопроса = СтрШаблон("Добавить страну ""%1"" из классификатора в список стран мира?", Страна.Наименование);
...
Процедура ОповеститьОВыбореЭлементаКлассификатора(ИдентификаторСтрокиВыбора)
	...
	Если ВсеДанныеСтроки <> Неопределено Тогда
		...
		ИначеЕсли РежимДобавления И ДанныеВыбора.ДанныеОтличаются Тогда
			...
			ПоказатьВопрос(Оповещение, СтрШаблон("%1 уже присутствует в списке стран. Перезаполнить?", ДанныеСтроки.Наименование), РежимДиалогаВопрос.ДаНет);
...
Функция ДанныеВыбораЭлементаКлассификатора(Знач СведенияОСтране, ПоляКлассификатора)
	...
	Иначе
		ЗначенияСправочника = БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта(Ссылка, ПоляКлассификатора);
...
Функция СтраныНетВСпискеСтран(Страна)
	Запрос			= Новый Запрос;
	Запрос.Текст	= "ВЫБРАТЬ СтраныМира.Ссылка
