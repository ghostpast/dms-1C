...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных, ВызыватьИсключение) Экспорт
	...
	Идентификатор = ИдентификаторыОбъектовМетаданных(Массив, ВызыватьИсключение, Истина).Получить(ПолноеИмяОбъектаМетаданных);
...
Процедура ОбновитьСвойстваИдентификатора(Объект) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Объект) <> Тип("ДанныеФормыСтруктура") Тогда
			...
			Если КлючОбъектаМетаданных = Неопределено ИЛИ СвойстваОбъекта.БезКлючаОбъектаМетаданных И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Иначе
			...
			Если Свойства.Наименование              <> СвойстваОбъекта.Наименование
			 ...
			 ИЛИ СвойстваОбъекта.БезКлючаОбъектаМетаданных И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
...
Функция ИдентификаторыОбъектовМетаданныхСПопыткойПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент)
	...
	Попытка
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, Истина, ОдинЭлемент);
	Исключение
	...
	Если Идентификаторы = Неопределено Тогда
		ОбновитьДанныеСправочника();
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент);
	КонецЕсли;
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			Если ОбъектМетаданных = Неопределено Тогда
				Если Не ВызыватьИсключение Тогда
					Продолжить;
				КонецЕсли;
				ОписаниеОшибки = СтрШаблон("Объект метаданных не найден по полному имени:
					           |""%1"".",
