...
Процедура ПроверкаИспользования(ОбъектыРасширений = Ложь) Экспорт
	Если БазоваяПодсистемаСерверПовтИсп.СП_ОтключитьИдентификаторыОбъектовМетаданных() Тогда
		ТекстОшибки = СтрШаблон("Справочник ""%1"" не используется.", НазваниеСправочника(ОбъектыРасширений));
	...
	Если ПланыОбмена.ГлавныйУзел() = Неопределено И ЗначениеЗаполнено(БазоваяПодсистемаСервер.ОН_МенеджерОбъектаПоПолномуИмени("Константа.ГлавныйУзел").Получить()) Тогда
		ТекстОшибки = СтрШаблон("Справочник ""%1"" не может использоваться
...
Функция ДопустимыеКоллекции() Экспорт
	СвойстваКоллекций = БазоваяПодсистемаСерверПовтИсп.СП_СвойстваКоллекцийОбъектовМетаданных();
...
Процедура ВыполнитьОбновлениеДанных(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения = Ложь, СписокКритичныхИзменений = "", ОбъектыРасширений = Ложь) Экспорт
	...
	Если Не ОбъектыРасширений И Не БазоваяПодсистемаСервер.СП_ВерсияПрограммыОбновленаДинамически() Тогда
...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных, ВызыватьИсключение) Экспорт
	...
	Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
		...
		ИначеЕсли Не ВызыватьИсключение Тогда
			Возврат Null;
		Иначе
			ТекстОшибки = СтрШаблон("Некорректное значение параметра %1 в функции %2.
				           |Указан несуществующий объект метаданных: ""%3"".",
				"ОписаниеОбъектаМетаданных", "БазоваяПодсистемаСервер.ОН_ИдентификаторОбъектаМетаданных", ОписаниеОбъектаМетаданных);
	...
	ИначеЕсли Не ВызыватьИсключение Тогда
		Возврат Null;
	Иначе
		ТекстОшибки = СтрШаблон("Некорректный тип параметра %1 в функции %2:
			           |""%3"".",
			"ОписаниеОбъектаМетаданных", "БазоваяПодсистемаСервер.ОН_ИдентификаторОбъектаМетаданных", ТипОписанияОбъектаМетаданных);
	...
	Для Каждого ОписаниеОбъектаМетаданных Из ОписаниеОбъектовМетаданных Цикл
		...
		Если ТипОписанияОбъектаМетаданных = Тип("Тип") Тогда
			...
			ИначеЕсли Не ВызыватьИсключение Тогда
				Продолжить;
			Иначе
				ТекстОшибки = СтрШаблон("Некорректное значение параметра %1 функции %2:
					           |Указан несуществующий объект метаданных: ""%3"".",
					"ОписаниеОбъектовМетаданных", "ОбщегоНазначения.ИдентификаторыОбъектовМетаданных", ОписаниеОбъектаМетаданных);
		...
		ИначеЕсли Не ВызыватьИсключение Тогда
			Продолжить;
		Иначе
			ТекстОшибки = СтрШаблон("Некорректный тип параметра %1 функции %2:
				           |""%3"".",
				"ОписаниеОбъектовМетаданных", "ОбщегоНазначения.ИдентификаторыОбъектовМетаданных", ТипОписанияОбъектаМетаданных);
	...
	Для Каждого Идентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли Описание.Ключ <> Описание.ПолноеИмя Тогда
			Если РолиПоКлючам = Неопределено Тогда
				РолиПоКлючам = БазоваяПодсистемаСерверПовтИсп.СП_РолиПоКлючамОбъектовМетаданных();
			КонецЕсли;
			Результат.Вставить(Идентификатор, РолиПоКлючам.Получить(Описание.Ключ));
		Иначе
			Результат.Вставить(Идентификатор, БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоПолномуИмени(Описание.Ключ));
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьПереименование(Итог, ВерсияИБ, СтароеПолноеИмя, НовоеПолноеИмя, ИдентификаторБиблиотеки = "") Экспорт
	БазоваяПодсистемаСерверПовтИсп.СП_ИдентификаторыОбъектовМетаданныхПроверкаИспользования();

	СтароеИмяКоллекции = ВРег(ИмяКоллекции(СтароеПолноеИмя));
	НовоеИмяКоллекции  = ВРег(ИмяКоллекции(НовоеПолноеИмя));

	ЗаголовокОшибки = "Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных.";

	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтрШаблон("Не совпадают имена типов переименованного объекта метаданных.
			           |Прежний тип: ""%1"",
			           |новый тип: ""%2"".",
			СтароеПолноеИмя, НовоеПолноеИмя);
	...
	Если Итог.КоллекцииБезКлюча[СтароеИмяКоллекции] = Неопределено Тогда
		...
		СписокДопустимыхТипов	= ?(ЗначениеЗаполнено(СписокДопустимыхТипов), Лев(СписокДопустимыхТипов, СтрДлина(СписокДопустимыхТипов) - 1), "");

		ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтрШаблон("Для типа объекта метаданных ""%1"" не требуется описывать переименование,
			           ...
			           |%2.",
			СтароеПолноеИмя,СписокДопустимыхТипов);
	...
	Если ВерсияБиблиотеки = Неопределено Тогда
		ВерсияБиблиотеки = ОбновлениеВерсииИБСервер.сОИБ_ВерсияИБ(ИдентификаторБиблиотеки);
	...
	Результат = БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсии(ВерсияИБ, ВерсияБиблиотеки);
...
Процедура ОбновитьСвойстваИдентификатора(Объект) Экспорт
	...
	Иначе
		...
		СвойстваОбъектовМетаданных	= СвойстваОбъектовМетаданных(ОбъектыРасширений, СвойстваКоллекций.Скопировать(Строки));
...
Процедура ОбновитьДанные(ЕстьИзменения, ЕстьУдаленные, ТолькоПроверка, ЕстьКритичныеИзменения, СписокКритичныхИзменений, ОбъектыРасширений)
	Если ОбъектыРасширений Тогда
		...
		СвойстваРасширений.Вставить("ИменаПодключенныхРасширений", ИменаРасширений(ИсточникРасширенийКонфигурации.СеансАктивные, ИдентификаторыКлючейРасширений));
		СвойстваРасширений.Вставить("ИменаНеподключенныхРасширений", ИменаРасширений(ИсточникРасширенийКонфигурации.СеансОтключенные, ИдентификаторыКлючейРасширений));
	КонецЕсли;
	...
	Попытка
		...
		Если ЗначениеЗаполнено(СписокПереименованийОбъектовМетаданных) Тогда
			СписокКритичныхИзменений	= СтрШаблон("Переименование идентификаторов объектов метаданных %1 -> %2:","СтароеПолноеИмя", "НовоеПолноеИмя");
			СписокКритичныхИзменений	= СписокКритичныхИзменений + Символы.ПС + СписокПереименованийОбъектовМетаданных + Символы.ПС + Символы.ПС;
		КонецЕсли;
		...
		ЕстьТекущиеИзменения = Ложь;
		ОбновитьИдентификаторыОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений, СвойстваРасширений, ВерсияРасширений, ЕстьТекущиеИзменения, ТолькоПроверка);
...
Функция ВыгрузкаВсехИдентификаторов(ОбъектыРасширений = Ложь)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИдентификаторРасширения",?(ОбъектыРасширений, "Идентификаторы.ИдентификаторРасширения", """"""));
	...
	Для каждого Строка Из Выгрузка Цикл
		Если Не ЗначениеЗаполнено(Строка.Ссылка) Тогда
			ТекстОшибки = СтрШаблон("Данные справочника %1 повреждены.
...
Процедура ОбработатьИдентификаторыОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений, СвойстваРасширений, ЕстьУдаленные, ЕстьКритичныеИзменения, СписокПереименованийОбъектовМетаданных)
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если ОбъектыРасширений И (СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИмяРасширения)] <> Неопределено Или СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИдентификаторРасширения)] <> Неопределено)Тогда
		...
		Если ОбъектыРасширений И СвойстваРасширений.ИменаПодключенныхРасширений[НРег(Свойства.ИмяРасширения)] = Неопределено И СвойстваРасширений.ИменаПодключенныхРасширений[НРег(Свойства.ИдентификаторРасширения)] = Неопределено Тогда
		...
		ОбъектМетаданных		= ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, Свойства.ИмяРасширения, Свойства.ИдентификаторРасширения, Свойства.ХешСуммаРасширения);
...
Функция МетаданныеРасширенияНайтиПоПолномуИмени(Свойства)
	...
	Если НРег(РасширениеОбъекта.УникальныйИдентификатор) = НРег(Сред(Свойства.ИдентификаторРасширения, 3)) Или НРег(РасширениеОбъекта.Имя) = НРег(Сред(Свойства.ИмяРасширения, 3)) Тогда
...
Процедура ОбновитьИдентификаторыОбъектовМетаданных(Выгрузка, СвойстваОбъектовМетаданных, ОбъектыРасширений, СвойстваРасширений, ВерсияРасширений, ЕстьИзменения, ТолькоПроверка)
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если Не Свойства.ЭтоКоллекция Тогда
			...
			Если ОбъектыРасширений И Свойства.ПометкаУдаления = Ложь И СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИмяРасширения)] = Неопределено И СвойстваРасширений.ИменаНеподключенныхРасширений[НРег(Свойства.ИдентификаторРасширения)] = Неопределено Тогда
	...
	Если ОбъектыРасширений Тогда
		...
		Если Строки.Количество() > 0 И (Строки.Количество() <> 1 Или ЗначениеЗаполнено(Строки[0].ПолноеИмяОбъекта) И Строки[0].ВерсияРасширений = ВерсияРасширений) Тогда
			ОбновитьНаборЗаписей = Истина;
...
Процедура ОбновитьСвойстваПомеченногоНаУдаление(Свойства, СвойстваОбновлены = Ложь, ЕстьУдаленные = Ложь, ЭтоДубль = Ложь)
	...
	Если ЭтоДубль Тогда
		КлючОбъектаМетаданных = ?(ТипЗнч(Свойства.КлючОбъектаМетаданных) = Тип("ХранилищеЗначения"), Свойства.КлючОбъектаМетаданных.Получить(), Свойства.КлючОбъектаМетаданных);
	КонецЕсли;
	...
	Если НЕ Свойства.ПометкаУдаления
	 ...
	 ИЛИ ЭтоДубль И КлючОбъектаМетаданных <> Неопределено Тогда
...
Функция КлючОбъектаМетаданныхСоответствуетПолномуИмени(СвойстваИдентификатора)
	...
	Если КлючОбъектаМетаданных <> Неопределено И КлючОбъектаМетаданных <> Тип("Неопределено") Тогда
		...
		ОбъектМетаданных = ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, СвойстваИдентификатора.ИмяРасширения, СвойстваИдентификатора.ИдентификаторРасширения, СвойстваИдентификатора.ХешСуммаРасширения);
		Если ОбъектМетаданных <> Неопределено Тогда
	...
	Если ОбъектМетаданных = Неопределено
	   ...
	   И СтрНачинаетсяС(СвойстваИдентификатора.ПолноеИмяКоллекции, "? ") Тогда
		...
		РезультатПроверки.ОбъектМетаданныхУдаленного = БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоПолномуИмени(ПолноеИмяУдаленного(СвойстваИдентификатора.ПолноеИмяКоллекции));
	КонецЕсли;
...
Функция ОбъектМетаданныхПоКлючу(КлючОбъектаМетаданных, ИмяРасширения, ИдентификаторРасширения, ХешСуммаРасширения)
	...
	ИначеЕсли ТипКлючаОбъектаМетаданных = Тип("Строка") Тогда
		ОбъектМетаданных = БазоваяПодсистемаСерверПовтИсп.СП_РолиПоКлючамОбъектовМетаданных().Получить(КлючОбъектаМетаданных);
	КонецЕсли;
...
Функция КлючРоли(ОбъектМетаданныхРоль)
	...
	Если Позиция = 1 Тогда
		...
		Если БазоваяПодсистемаКлиентСервер.СФ_ЭтоУникальныйИдентификатор(Идентификатор) Тогда
			Возврат Идентификатор;
		КонецЕсли;
	КонецЕсли;

	ТекстОшибки = СтрШаблон("Не удалось получить ключ роли %1", ОбъектМетаданныхРоль.Имя);
...
Функция СвойстваОбъектовМетаданных(ОбъектыРасширений, СвойстваКоллекций = Неопределено, ИдентификаторыКлючейРасширений = Неопределено)
	...
	Если Не ОбъектыРасширений Или ЗначениеЗаполнено(ПараметрыСеанса.ПодключенныеРасширения) Тогда
		Для Каждого СвойстваКоллекции Из СвойстваКоллекций Цикл
			ДобавитьСвойстваОбъектовМетаданных(Метаданные[СвойстваКоллекции.Имя], СвойстваКоллекции, СвойстваОбъектовМетаданных, ИдентификаторыКлючейРасширений, ИменаПредопределенных);
		КонецЦикла;
...
Процедура ДобавитьСвойстваОбъектовМетаданных(Знач КоллекцияОбъектовМетаданных, Знач СвойстваКоллекции, Знач СвойстваОбъектовМетаданных, Знач ИдентификаторыКлючейРасширений, Знач ИменаПредопределенных, Знач ПолноеИмяРодителя = "", Знач ПолныйСинонимРодителя = "")
	...
	Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
		...
		Если СвойстваКоллекции.Имя = "Подсистемы" Тогда
			ДобавитьСвойстваОбъектовМетаданных(ОбъектМетаданных.Подсистемы, СвойстваКоллекции, СвойстваОбъектовМетаданных, ИдентификаторыКлючейРасширений, ИменаПредопределенных, ПолноеИмя, НоваяСтрока.ПолныйСиноним + ". ");
		КонецЕсли;
...
Функция МетаданныеНайтиПоПолномуИмени(ПолноеИмя)
	...
	Возврат БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоПолномуИмени(ПолноеИмя);
КонецФункции
...
Процедура ОбновитьСписокНовыхПодсистем(НовыеПодсистемы, ВсеНовыеПодсистемы)
	...
	Пока Индекс >= 0 Цикл
		...
		Если БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоПолномуИмени(ИмяПодсистемы) = Неопределено Тогда
...
Функция ИдентификаторыОбъектовМетаданныхСПопыткойПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент)
	...
	Попытка
		Идентификаторы = ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, Истина, ОдинЭлемент, Не ВызыватьИсключение);
	Исключение
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, ВызыватьИсключение, ОдинЭлемент, ПропускатьНеподдерживаемыеОбъекты)
	...
	Для Каждого ПолноеИмяОбъектаМетаданных Из ПолныеИменаОбъектовМетаданных Цикл
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			...
			ОбъектМетаданных = БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоПолномуИмени(ПолноеИмяОбъектаМетаданных);
			Если ОбъектМетаданных = Неопределено Тогда
	...
	Если КоличествоОшибок > 0 Тогда
		Если ОдинЭлемент Тогда
			ЗаголовокОшибки = СтрШаблон("Ошибка при выполнении функции %1.", "БазоваяПодсистемаСервер.ОН_ИдентификаторОбъектаМетаданных.");
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = СтрШаблон("Ошибка при выполнении функции %1.", "БазоваяПодсистемаСервер.ОН_ИдентификаторыОбъектовМетаданных.");
		Иначе
			ЗаголовокОшибки = СтрШаблон("Ошибка при выполнении функции %1.", "БазоваяПодсистемаСервер.ОН_ИдентификаторыОбъектовМетаданных.");
		КонецЕсли;
		...
		ИначеЕсли ВызыватьИсключение Тогда
			Если ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
			 Или ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
				ТекстОшибки = СтрШаблон("Ошибка при выполнении функции %1.
					           |
					           |Неверный идентификатор: пустая ссылка типа ""%2"".",
					"БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоИдентификатору", ТипЗнч(ТекущийИдентификатор));
			Иначе
				ТекстОшибки = СтрШаблон("Ошибка при выполнении функции %1.
					           |
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы, ИдентификаторыКонфигурации, ИдентификаторыРасширений, ВызыватьИсключение)
	...
	Выгрузка = Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки = СтрШаблон("Ошибка при выполнении функции %1.", "БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоИдентификатору");
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если РезультатПроверки.НеСоответствует Тогда
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				...
				Если Свойства.ОбъектРасширений Тогда
					...
					ИмяРасширения					= "";
					ИдентификаторРасширения			= ?(СтрНачинаетсяС(Свойства.ИдентификаторРасширения, "? "), Сред(Свойства.ИдентификаторРасширения, 3), Свойства.ИдентификаторРасширения);
					Если БазоваяПодсистемаКлиентСервер.СФ_ЭтоУникальныйИдентификатор(ИдентификаторРасширения) Тогда
		...
		Если Не Свойства.ОбъектРасширений И Свойства.ПометкаУдаления И Не КонфигурацияБазыДанныхИзмененаДинамически Тогда
			...
			ТекстОшибки = ЗаголовокОшибки + Символы.ПС + Символы.ПС + ОписаниеОшибки + БазоваяПодсистемаСервер.СП_УточнениеОшибкиПараметровРаботыПрограммыДляРазработчика();
...
Процедура ЗаменитьДублиПодчиненногоУзлаНайденныеПриЗагрузке(ТолькоПроверка, ЕстьИзменения)
	...
	Пока Истина Цикл
		...
		Если ТекущаяПопытка > 10 Тогда
			ТекстОшибки = "Не удалось выполнить замену дублей идентификаторов объектов метаданных.
