...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, ВызыватьИсключение)
	...
	Для Каждого ТекущийИдентификатор Из Идентификаторы Цикл
		...
		ИначеЕсли ТипЗнч(ТекущийИдентификатор) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
			...
			КонецЕсли;
		Иначе
			ВызватьИсключение СтрШаблон("Ошибка при выполнении функции БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоИдентификатору().
				           |
...
Функция ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы, ИдентификаторыКонфигурации, ИдентификаторыРасширений, ВызыватьИсключение)
	...
	Выгрузка		= Запрос.Выполнить().Выгрузить();
	ЗаголовокОшибки	= "Ошибка при выполнении функции БазоваяПодсистемаСервер.ОН_ОбъектМетаданныхПоИдентификатору().";
	...
	ВерсияПрограммыОбновленаДинамически = БазоваяПодсистемаСервер.СП_ВерсияПрограммыОбновленаДинамически();
		...
		Если РезультатПроверки.НеСоответствует Тогда
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
				Если Свойства.ОбъектРасширений И Не Свойства.ПометкаУдаления Тогда
					...
					Если ВызыватьИсключение Тогда
						...
						Если РасширенияКонфигурации.Получить(Отбор, ИсточникРасширенийКонфигурации.СеансОтключенные).Количество() > 0 Тогда
							ВызватьИсключение СтрШаблон("Расширение конфигурации %1 установлено, но отключено при запуске сеанса.
								           |Обычно, это значит, что при подключении расширения конфигурации возникла ошибка.",
								Свойства.ИмяРасширения);
						Иначе
							ВызватьИсключение СтрШаблон("Расширение конфигурации %1 установлено, но не подключено.
				...
				ИначеЕсли РезультатПроверки.КлючОбъектаМетаданных = Неопределено Тогда
					ОписаниеОшибки = СтрШаблон("Идентификатору ""%1""
