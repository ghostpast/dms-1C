...
Процедура ВыбратьИЗакрыть()
	...
	Родитель		= ВыбранноеПоле.Родитель;
...
Процедура ОбновитьКоллекцииПолей()
	КонструкторФормулСервер.сКФ_ОбновитьКоллекцииПолей(ЭтотОбъект, КоллекцииПолей(), "ДоступныеПоля");
КонецПроцедуры
...
Функция ДополнительноеПолеГруппыФормул()
	ТаблицаПолей		= КонструкторФормулСервер.сКФ_ТаблицаПолей();
	Поле				= ТаблицаПолей.Добавить();
	Поле.Идентификатор	= ИдентификаторГруппыФормул();
	Поле.Представление	= "Формулы";
	Поле.Порядок		= 99;

	Возврат КонструкторФормулСервер.сКФ_КоллекцияПолей(ТаблицаПолей, Неопределено);
КонецФункции
