...
Процедура ПереименоватьПоле(Команда)
	Если Не ЗначениеЗаполнено(ПредставлениеПоля) Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Необходимо ввести новый заголовок поля",, "ПредставлениеПоля");
...
Процедура УстановитьВысотуСтроки(Команда)
	Если Не ЗначениеЗаполнено(ВысотаСтроки) Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Необходимо ввести высоту строки",, "ВысотаСтроки");
...
Процедура УстановитьШиринуКолонки(Команда)
	Если Не ЗначениеЗаполнено(ШиринаКолонки) Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Необходимо ввести ширину колонки",, "ШиринаКолонки");
...
Процедура ОпределитьВидыИерархииЗначений()
	...
	Для Каждого ТипЗначения Из ТипыЗначений Цикл
		...
		Если Метаданные.Справочники.Содержит(МетаданныеОбъекта) И МетаданныеОбъекта.ВидИерархии = ВидыИерархии.ИерархияЭлементов Тогда
...
Процедура ПрименитьНастройки(ПереформироватьОтчет)
	...
	ВариантыОтчетовКлиент.сВО_ДобавитьНастройкиВСтек(ЭтотОбъект, КомпоновщикНастроек.Настройки, "Фильтровать", СвойстваЗаголовка.Текст);

	Результат = ВариантыОтчетовКлиент.сВО_РезультатКонтекстнойНастройки(КомпоновщикНастроек, "Фильтровать", ВладелецФормы.УникальныйИдентификатор);
