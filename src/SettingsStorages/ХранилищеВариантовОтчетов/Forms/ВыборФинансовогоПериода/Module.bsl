...
Процедура ВыбратьДень(Команда)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборФинансовогоПериодаДень", ПараметрыФормы, ЭтотОбъект,,,, ОписаниеОповещения);
КонецПроцедуры
...
Процедура УстановитьКартинкуПереходаКСтандартномуПериоду()
	Если БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсии(БазоваяПодсистемаСервер.СП_ВерсияРежимаСовместимости(), "8.3.17.0") >= 0 Тогда
