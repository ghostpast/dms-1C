...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИдентификаторКД <> Неопределено Тогда
		...
		НаименованиеПоУмолчанию	= ВариантыОтчетовКлиентСервер.О_ПредставлениеЭлементаУсловногоОформления(ЭлементОформления, Неопределено, "");
...
Процедура ОформляемоеПолеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Строка") И ВыбранноеЗначение <> "Еще" Тогда
	...
	ИначеЕсли ТипЗнч(ВыбранноеЗначение) = Тип("ПользовательскоеПолеВыражениеКомпоновкиДанных") Тогда
		...
		ДоступноеПоле = ВыбранноеЗначение;
		ВариантыОтчетовКлиент.сВО_ДобавитьФормулу(КомпоновщикНастроек.Настройки, Оформление.ДоступныеПоляПолей, ДоступноеПоле);
...
Функция ЭлементОформленияИсходный()
	...
	Иначе
		...
		ЭлементОформления = ВариантыОтчетовКлиентСервер.О_СкопироватьРекурсивно(Оформление, ЭлементОформленияИсточник, Оформление.Элементы, 0, Новый Соответствие);
	КонецЕсли;
...
Процедура ВыбратьИЗакрыть()
	...
	ОписаниеФормул = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(НастройкиОтчета, "ОписаниеФормул", Новый Массив);
...
Процедура ЗаполнитьСписокВыбораОформляемыхПолей(ЭлементОформления)
	...
	Для Индекс = 0 По Граница Цикл
		...
		Если Не ДоступноеПоле.Папка Тогда
			КартинкаПоля = ВариантыОтчетовКлиентСервер.сВО_КартинкаПоля(ДоступноеПоле.ТипЗначения);
...
Процедура ВыбратьОформляемоеПоле(Элемент, ВыбранноеЗначение, ИспользуемыеПоля)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборПоляОтчета", ПараметрыВыбора, Элемент, УникальныйИдентификатор);
КонецПроцедуры
...
Процедура ДобавитьДополнительноеПоле(Список)
	...
	Список.Добавить("Еще", "Еще...",, БиблиотекаКартинок.Пустая);
КонецПроцедуры
...
Процедура ДополнитьСписокВыбораОформляемыхПолей(Форма, Поле, ДоступноеПоле)
	...
	Если НайденноеПоле = Неопределено Тогда
		...
		КартинкаПоля = ВариантыОтчетовКлиентСервер.сВО_КартинкаПоля(ТипЗначенияПоля);
...
Процедура ИзменитьПоле(Элемент, Отказ, ЭтоДобавление = Истина)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборПоляОтчета",ПараметрыВыбора, ЭтотОбъект, УникальныйИдентификатор,,, Обработчик);
КонецПроцедуры
...
Процедура ПослеВыбораПоля(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("ПользовательскоеПолеВыражениеКомпоновкиДанных") Тогда
		ОбновитьСоставОписанияФормул(ВыбранноеЗначение);
		ВариантыОтчетовКлиент.сВО_ДобавитьФормулу(КомпоновщикНастроек.Настройки, ДоступныеПоля, ВыбранноеЗначение);
	КонецЕсли;
