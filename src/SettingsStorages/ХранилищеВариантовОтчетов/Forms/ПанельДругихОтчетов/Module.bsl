...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ГруппаДругиеВариантыОтчета.Заголовок = ОтчетНаименование + " (варианты отчета):";
...
Процедура Подключаемый_ВариантНажатие(Элемент)
	...
	ВариантыОтчетовКлиент.ВО_ОткрытьФормуОтчета(ВладелецФормы, Вариант, Новый Структура("Подсистема", ПодсистемаСсылка));
...
Процедура СохранитьНастройкиЭтойФормы()
	...
	БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеНастроекДанныхФорм, "СтандартныеПодсистемы.ВариантыОтчетов", "ПанельДругихОтчетов", НастройкиФормы, Неопределено, Неопределено, Ложь);
КонецПроцедуры
...
Процедура ПрочитатьНастройкиЭтойФормы()
	...
	НастройкиФормы = БазоваяПодсистемаСервер.ОН_ХранилищеЗагрузить(ХранилищеОбщихНастроек, "СтандартныеПодсистемы.ВариантыОтчетов", "ПанельДругихОтчетов", НастройкиПоУмолчанию, Неопределено, Неопределено);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, НастройкиФормы);
КонецПроцедуры

&НаСервере
Функция НастройкиПоУмолчанию()
	Возврат ВариантыОтчетовСервер.ВО_ГлобальныеНастройки().ДругиеОтчеты;
КонецФункции
...
Процедура ВывестиГиперссылкуВПанель(ТаблицаВывода, Вариант, Группа, ПоказыватьПодсказки)
	...
	Если ЗначениеЗаполнено(Вариант.Автор) Тогда
		Надпись.Подсказка = СокрЛ(Надпись.Подсказка + Символы.ПС) + "Автор: " + СокрЛП(Строка(Вариант.Автор));
	КонецЕсли;
...
Функция ПодсистемыРаздела(КорневойРаздел)
	...
	ДеревоПодсистем	= ВариантыОтчетовСерверПовтИсп.ВО_ПодсистемыТекущегоПользователя();
