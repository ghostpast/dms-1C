...
Процедура Подключаемый_ПредупредитьПослеОткрытияФормы()
	БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(ТекстПредупрежденияПриОткрытии);
КонецПроцедуры
...
Процедура ПодтвердитьИЗакрыть(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Модифицированность Тогда
		...
		Если ПроверятьКорректность Тогда
			ПоляТелефона				= КонтактнаяИнформацияКлиентСервер.УКИ_СтруктураПолейТелефона();
			...
			Если ИспользоватьДополнительныеПроверки Тогда
				СписокОшибок = КонтактнаяИнформацияКлиентСервер.УКИ_ОшибкиЗаполненияТелефона(ПоляТелефона, КонтактнаяИнформацияКлиент);
			Иначе
				СписокОшибок = КонтактнаяИнформацияКлиентСервер.УКИ_ОшибкиЗаполненияТелефона(ПоляТелефона, Неопределено);
			КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок)
	...
	Для Каждого Элемент Из СписокОшибок Цикл
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(Элемент.Представление,,, ПутьКДаннымФормыПоПутиXPath(Элемент.Значение));
	КонецЦикла;
КонецПроцедуры    
