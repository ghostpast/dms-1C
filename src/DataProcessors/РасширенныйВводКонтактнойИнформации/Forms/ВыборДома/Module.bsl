...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтрСравнить(Параметры.ТипДома, "Зем. участок") <> 0 Тогда
		ТипВыбора						= "СписокДомов";
	Иначе
		ТипВыбора						= "ЗемельныеУчастки";
		Заголовок						= "Выбрать земельный участок";
		Элементы.Найти.Подсказка		= "Поиск по номеру земельного участка";
		Элементы.Найти.ПодсказкаВвода	= "Номер земельного участка";
	КонецЕсли;
	...
	Если БазоваяПодсистемаСервер.ОН_ЭтоМобильныйКлиент() Тогда
		БазоваяПодсистемаКлиентСервер.ОН_УстановитьСвойствоЭлементаФормы(Элементы, "ФормаВыбрать", "Видимость", Ложь);
	КонецЕсли;
КонецПроцедуры
