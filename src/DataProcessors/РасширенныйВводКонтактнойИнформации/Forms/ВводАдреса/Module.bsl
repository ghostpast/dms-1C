		...
		Если Результат.Отказ Тогда
			...
			Иначе
				Если НЕ ПодборПоИндексуДоступен Тогда
					...
					БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(ТекстСообщения,,, "Объект.Индекс");
				Иначе
...
Функция РезультатВыбора(Контекст, ВозвращатьСписокЗначений = Ложь)
	...
	НаселенныйПунктДетально.Value = КонтактнаяИнформацияКлиентСервер.РСА_ПредставлениеАдреса(НаселенныйПунктДетально, ВидКИ.ВключатьСтрануВПредставление,, Контекст.ИсключатьГородИзМуниципальногоАдреса);
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Для Каждого Элемент Из СписокОшибок Цикл
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(Элемент.ТекстОшибки,,, Элемент.ИмяПоля);
	КонецЦикла;
КонецПроцедуры
...
Процедура ОбновитьПредставлениеАдреса()
	КонтактнаяИнформацияКлиентСервер.РСА_ОбновитьПредставлениеАдреса(НаселенныйПунктДетально, ВидКонтактнойИнформации.ВключатьСтрануВПредставление, ИсключатьГородИзМуниципальногоАдреса);
	ПредставлениеАдреса = НаселенныйПунктДетально.Value;
...
Функция ПредставлениеНаселенногоПунктаАдреса(Адрес)
	...
	Для каждого ИмяУровня Из КонтактнаяИнформацияКлиентСервер.РСА_ИменаУровнейАдреса(Адрес.AddressType, Ложь,, ИсключатьГородИзМуниципальногоАдреса) Цикл
