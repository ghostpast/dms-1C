...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Родитель) И Уровень > 1 Тогда
		...
		КраткоеПредставлениеОшибки = "Поле не содержит адресных сведений для выбора.";
		Возврат;
	КонецЕсли;
	...
	Если БазоваяПодсистемаСервер.ОН_ЭтоМобильныйКлиент() Тогда
		...
		БазоваяПодсистемаКлиентСервер.ОН_УстановитьСвойствоЭлементаФормы(Элементы,	"ВариантыАдресаКонтекстноеМенюГруппаНайти",	"Видимость",	Ложь);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроизвестиВыбор(Знач НомерСтроки)
	...
	ПредупреждениеНеактуальности = СтрШаблон("Адрес ""%1"" неактуален.
		           |Продолжить?", Данные.Представление);

	ЗаголовокПредупреждения = "Подтверждение";
...
Процедура ОповеститьВладельца(Знач Данные, Отказ = Ложь)
	...
	Иначе
		...
		Если Уровень = 7 Тогда
			Результат.КраткоеПредставлениеОшибки = "Данные об улицах для введенного населенного пункта отсутствуют.";
		Иначе
			Результат.КраткоеПредставлениеОшибки = "Нет доступных адресных сведений для выбора.";
		КонецЕсли;
	КонецЕсли;

	Результат.Муниципальный		= СтрСравнить("Муниципальный", ТипАдреса) = 0;
...
Функция ДанныеКлассификатораПоИдентификатору(Знач ПараметрыПоиска)
	...
	Результат.Вставить("КраткоеПредставлениеОшибки", "Поле не содержит адресных сведений для выбора");
	Результат.Вставить("Отказ", Ложь);
	...
	Если ДанныеКлассификатора.Отказ Тогда
		...
		Результат.КраткоеПредставлениеОшибки	= "Автоподбор и проверка адреса недоступны:" + Символы.ПС + ДанныеКлассификатора.КраткоеПредставлениеОшибки;
		Результат.Отказ							= Истина;

		Возврат Результат;
	ИначеЕсли ДанныеКлассификатора.Данные.Количество() = 0 Тогда
		Адрес = КонтактнаяИнформацияСервер.РСА_АдресПоИдентификатору(Родитель);
		...
		НаименованиеРегиона = КонтактнаяИнформацияСервер.РСА_РегионАдресаКонтактнойИнформации(Адрес);
		Если КонтактнаяИнформацияСервер.РСА_ЭтоГородФедеральногоЗначения(НаименованиеРегиона) Тогда
...
Функция ЗагрузитьДанныеИзКлассификатора(Позиция, СтрокаПоиска = "")
	...
	Если ДанныеИзКлассификатора.Данные.Количество() > 0 И Не ДанныеИзКлассификатора.Данные[0].РегионЗагружен Тогда
		...
		Если ДанныеИзКлассификатора.Данные.Количество() = 500 Тогда
			Элементы.Информация.Заголовок = "Показано только 500 адресных объектов";
			Элементы.Следующие.Доступность = Истина;
		Иначе
			Элементы.Информация.Заголовок	= СтрШаблон("Показано %1 адресных объектов", ДанныеИзКлассификатора.Данные.Количество());
