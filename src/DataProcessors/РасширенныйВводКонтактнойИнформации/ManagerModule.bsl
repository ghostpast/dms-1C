	...
	Для каждого ИмяПоля Из СписокПолей Цикл
		...
		Если ОбъектАдреса.Свойство(ИмяПоля) И ЗначениеЗаполнено(ОбъектАдреса[ИмяПоля]) Тогда
			Если СтрСравнить(ИмяПоля, "MunDistrict") = 0 Тогда
				НаименованиеУровня	= КонтактнаяИнформацияКлиентСервер.РСА_КраткоеНаписаниеМуниципальныхРайонов(ОбъектАдреса[ИмяПоля], ОбъектАдреса[ИмяПоля + "Type"]);
			ИначеЕсли СтрСравнить(ИмяПоля, "Settlement") = 0 Тогда
				НаименованиеУровня	= КонтактнаяИнформацияКлиентСервер.РСА_КраткоеНаписаниеПоселений(ОбъектАдреса[ИмяПоля], ОбъектАдреса[ИмяПоля + "Type"]);
			Иначе
