...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыДобавленияСпискаПолей										= КонструкторФормулСервер.сКФ_ПараметрыДобавленияСпискаПолей();
	...
	КонструкторФормулСервер.сКФ_ДобавитьСписокПолейНаФорму(ЭтотОбъект, ПараметрыДобавленияСпискаПолей);
	...
	ПредставлениеФормулы			= КонструкторФормулСервер.сКФ_ПредставлениеФормулы(ЭтотОбъект, Параметры.Формула);
...
Функция ПрочитатьСписокОперандов()
	...
	Если СписокОперандов = Неопределено Тогда
		СписокОперандов = КонструкторФормулСервер.сКФ_ТаблицаПолей();
	КонецЕсли;

	КомпоновщикНастроек = КонструкторФормулСервер.сКФ_КомпоновщикНастроекИсточникаПолей(СписокОперандов);

	Возврат КонструкторФормулСервер.сКФ_КоллекцияПолей(СписокОперандов, , Параметры.ИмяКоллекцииСКДОперандов);
КонецФункции
...
Функция ФормулаИзПредставления()
	Возврат КонструкторФормулСервер.сКФ_ФормулаИзПредставления(ЭтотОбъект, ПредставлениеФормулы);
КонецФункции

&НаКлиенте
Процедура Подключаемый_СписокПолейПередРазворачиванием(Элемент, Строка, Отказ)
	КонструкторФормулКлиент.КФ_СписокПолейПередРазворачиванием(ЭтотОбъект, Элемент, Строка, Отказ);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_РазвернутьТекущийЭлементСпискаПолей()
	КонструкторФормулКлиент.КФ_РазвернутьТекущийЭлементСпискаПолей(ЭтотОбъект);
КонецПроцедуры
...
Процедура ЗаполнитьСписокДоступныхПолей(ПараметрыЗаполнения)
	КонструкторФормулСервер.сКФ_ЗаполнитьСписокДоступныхПолей(ЭтотОбъект, ПараметрыЗаполнения);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СписокПолейНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	КонструкторФормулКлиент.КФ_СписокПолейНачалоПеретаскивания(ЭтотОбъект, Элемент, ПараметрыПеретаскивания, Выполнение);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СтрокаПоискаИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	КонструкторФормулКлиент.КФ_СтрокаПоискаИзменениеТекстаРедактирования(ЭтотОбъект, Элемент, Текст, СтандартнаяОбработка);
КонецПроцедуры
...
Процедура ВыполнитьПоискВСпискеПолей()
	КонструкторФормулСервер.сКФ_ВыполнитьПоискВСпискеПолей(ЭтотОбъект);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СтрокаПоискаОчистка(Элемент, СтандартнаяОбработка)
	КонструкторФормулКлиент.КФ_СтрокаПоискаИзменениеТекстаРедактирования(ЭтотОбъект, Элемент, "", СтандартнаяОбработка);
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_СписокПолейВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	СтандартнаяОбработка	= Ложь;
	ВыбранноеПоле			= КонструкторФормулКлиент.КФ_ВыбранноеПолеВСпискеПолей(ЭтотОбъект);
...
Процедура Подключаемый_ОператорыНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, Выполнение)
	Оператор							= КонструкторФормулКлиент.КФ_ВыбранноеПолеВСпискеПолей(ЭтотОбъект, ИмяСпискаОператоров());
...
Процедура Подключаемый_ОператорыОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	Если КонструкторФормулКлиент.КФ_ВыбранноеПолеВСпискеПолей(ЭтотОбъект, ИмяСпискаОператоров()).ПутьКДанным = "Формат" Тогда
...
Функция СписокОператоров()
	...
	Если Не ЗначениеЗаполнено(СписокОператоров) Тогда
		СписокОператоров = КонструкторФормулСервер.сКФ_СписокОператоров(?(ДляЗапроса, "ВсеОператорыСКД", Неопределено));
	КонецЕсли;

	Возврат КонструкторФормулСервер.сКФ_КоллекцияПолей(СписокОператоров, , Параметры.ИмяКоллекцииСКДОператоров);
КонецФункции

&НаКлиенте
Функция ВыражениеДляВставки(Оператор)
	Возврат КонструкторФормулКлиент.КФ_ВыражениеДляВставки(Оператор);
КонецФункции

&НаСервере
Функция ВыражениеДляПроверки()
	Возврат КонструкторФормулСервер.сКФ_ВыражениеДляПроверки(ЭтотОбъект, ПредставлениеФормулы, ИмяСпискаОперандов());
КонецФункции

&НаСервере
Процедура ПроверитьВыражениеДляЗапроса()
	Выражение = КонструкторФормулСервер.сКФ_ФормулаИзПредставления(ЭтотОбъект, ПредставлениеФормулы);
