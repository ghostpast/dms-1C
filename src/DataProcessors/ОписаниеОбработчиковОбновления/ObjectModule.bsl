...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	БазоваяПодсистемаСервер.ОН_УдалитьНепроверяемыеРеквизитыИзМассива(ПроверяемыеРеквизиты, МассивНепроверяемыхРеквизитов);
КонецПроцедуры
...
Процедура ЗагрузитьОбработчики() Экспорт
	...
	ОписаниеПодсистем		= БазоваяПодсистемаСерверПовтИсп.СП_ОписанияПодсистем();
	ПорядокБиблиотеки		= 1;
...
Процедура ЗагрузитьОбработчикиПодсистем(ОбработчикиПодсистем)
	...
	Если ТипЗнч(ОбработчикиПодсистем) = Тип("Массив") Тогда
		...
		Для Каждого Библиотека Из ОбработчикиПодсистем Цикл
			...
			ЕщеСведения											= Новый Структура("РежимВыполненияОтложенныхОбработчиков,ПорядокБиблиотеки");
...
Функция ОбновитьСведенияОКонфликтахОбработчиков() Экспорт
	...
	Для Каждого Приоритет Из Приоритеты Цикл
		Если Приоритет.Порядок <> "Любой" И Приоритет.ПорядокАвто = "Любой" И НЕ Приоритет.ПовторнаяЗапись Тогда
			Приоритет.НеЛюбой = 1;
		КонецЕсли;
		Если Приоритет.Порядок = "До" И Приоритет.ПорядокАвто = "После" ИЛИ Приоритет.Порядок = "После" И Приоритет.ПорядокАвто = "До" Тогда
...
Функция ПостроитьОчередь(БросатьИсключение = Ложь) Экспорт
	...
	Для НомерОчереди = 1 По МаксимальнаяОчередь Цикл
		Если НаличиеОбработчиковВОчереди.Получить(НомерОчереди) = Неопределено Или НаличиеОбработчиковВОчереди[НомерОчереди].Количество() = 0 Тогда
...
Функция ЕстьЦиклВыполненияОбработчиков(ПроверяемыйОбработчик = Неопределено, Знач ПриоритетыОбработчика = Неопределено, СообщатьОбОшибках = Ложь)
	...
	Если ЕстьЦикл Тогда
		Для Каждого ЦиклГрафа Из ЦиклыГрафа Цикл
			...
			ТекстСообщения = "Обнаружен цикл порядка выполнения:" + Символы.ПС + "%Путь%";
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Путь%", ТекстПути);

			ДобавитьОшибку(ТекстПути, ТекстСообщения);
			Если СообщатьОбОшибках Тогда
				БазоваяПодсистемаСервер.ОН_СообщитьПользователю(ТекстСообщения);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Процедура ИнициализацияКонстант()
	СтатусНетПроблем		= "Нет проблем";
	СтатусНеобходимАнализ	= "Необходим анализ";
...
Процедура ЗагрузитьТаблицуОбработчиков(Библиотека, ДополнительныеСведения)
	Для Каждого ОписаниеОбработчика Из Библиотека.Обработчики Цикл
		...
		НовоеОписание.ИмяБиблиотеки					= СтрЗаменить(Библиотека.ИмяОсновногоСерверногоМодуля, "ОбновлениеВерсииИБСервер", "");
...
Функция ЗапросВременныхТаблиц()
	...
	Запрос.УстановитьПараметр("СтандартнаяПроцедураПроверки",	"ОбновлениеВерсииИБСервер.ОИБ_ДанныеОбновленыНаНовуюВерсиюПрограммы");
...
Функция ТекстЗапросаВременныхТаблиц()
	...
	ТекстЗапросаЧитаемыеОбъекты = "ВЫБРАТЬ
	...
	ТекстЗапросаПриоритетыОбновленияТипов = "ВЫБРАТЬ
...
Процедура СдвинутьРекурсивноСвязанныеОбработчики(ОчередьОбработчиков, НаличиеОбработчиковВОчереди, Обработчик, МаксимальнаяОчередь, ДанныеРекурсии)
	...
	Для каждого СвязанныйОбработчик Из СвязанныеОбработчики Цикл
		...
		Если СвязанныйОбработчик.Значение.ПорядокПомещенияВОчередь = "Любой" Тогда
			Если СвязанныйОбработчик.Значение.ПовторнаяЗаписьОбъекта И ОчередьОбработчиков[Обработчик].Очередь = ОчередьОбработчиков[СвязанныйОбработчик.Ключ].Очередь Тогда
		...
		ИначеЕсли СвязанныйОбработчик.Значение.ПорядокПомещенияВОчередь = "После" Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения = "Ошибка при формировании очереди.";
...
Функция ЕстьОшибкиВПостроенииОчереди(Запрос, СообщатьОбОшибках = Истина)
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения = "Ошибка в алгоритме построения очереди: обработчик %Обработчик1% должен быть отнесен к очереди 1.";
		Если Выборка.Проблема = "ПроблемаВПорядкеСледованияОбработчиков" Тогда
			ТекстСообщения	= "Ошибка в алгоритме построения очереди: обработчики %Обработчик1% и %Обработчик2% размещены в неправильном порядке.";
...
Функция МожноСтроитьОчередь()
	...
	Пока Выборка.Следующий() Цикл
		ТекстСообщения	= "Не завершены работы по паре обработчиков %Обработчик% - %СвязанныйОбработчик%.";
		ТекстСообщения	= ТекстСообщения + Символы.ПС + "Статус проблемы: %ПорядокПомещенияВОчередь%";
		...
		ТекстСообщения	= СтрЗаменить(ТекстСообщения, "%ПорядокПомещенияВОчередь%", "Не задан приоритет выполнения.");
		ДобавитьОшибку(Выборка.Обработчик1, ТекстСообщения);
	КонецЦикла;
...
Процедура ВывестиСообщениеОбОшибках(БросатьИсключение = Истина)
	Если БросатьИсключение Тогда
		ИмяСобытия						= "Построение очереди обработчиков обновления";
		...
		Для Каждого Ошибка Из Ошибки Цикл
			Если НЕ ПустаяСтрока(Ошибка.Обработчик) Тогда
				...
				КонецЦикла;
			КонецЕсли;
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, ОбъектМетаданных, Ошибка.Обработчик, Ошибка.Сообщение);
		КонецЦикла;
		ТекстИсключения = "При построении очереди отложенных обработчиков обнаружены ошибки.
	...
	Иначе
		Для Каждого Ошибка Из Ошибки Цикл
			БазоваяПодсистемаСервер.ОН_СообщитьПользователю(Ошибка.Сообщение);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьНомерОчереди(ИтерацииОбновления)
	...
	Для Каждого Библиотека Из ИтерацииОбновления Цикл
		...
		ОтложенныеОбработчики = Библиотека.Обработчики.НайтиСтроки(Отбор); // см. ОбновлениеВерсииИБСервер.ОИБ_НоваяТаблицаОбработчиковОбновления()
