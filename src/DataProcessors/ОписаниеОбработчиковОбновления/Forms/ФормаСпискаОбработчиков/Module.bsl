...
Процедура ПриОткрытии(Отказ)
	...
	ВызватьИсключение "Работа в web-клиенте не поддерживается.";
	#КонецЕсли
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если Модифицированность И НЕ ЗавершениеРаботы Тогда
		...
		ПоказатьВопрос(ОбработчикОтвета, "Данные были изменены. Сохранить изменения в репозиторий?", РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
КонецПроцедуры
...
Процедура КаталогSRCНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	#Если НЕ ВебКлиент Тогда
		...
		БазоваяПодсистемаКлиент.ФС_ВыбратьКаталог(ОбработчикВыбораФайла, "Укажите каталог SRC", Объект.КаталогSRC);
	#КонецЕсли
КонецПроцедуры
...
Процедура КаталогФайловОшибокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	#Если НЕ ВебКлиент Тогда
		...
		БазоваяПодсистемаКлиент.ФС_ВыбратьКаталог(ОбработчикВыбораФайла, "Укажите каталог файлов ошибок", КаталогФайловОшибок);
	#КонецЕсли
КонецПроцедуры
...
Процедура ОбработчикиОбновленияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса		= "Удалить описание обработчика?";
...
Процедура СохранитьВРепозиторийВсеОбработчики(Команда)
	...
	ТекстВопроса		= "Внимание! Будут перезаписаны все процедуры описания обработчиков обновления.
						|Продолжить?";
	ПоказатьВопрос(ОбработчикОтвета, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
	Элементы.ГруппаНастройкиЗагрузкиВыгрузки.Скрыть();
	Модифицированность = Ложь;
...
Функция ПравильноУказанКаталогSRC()
	...
	Если НЕ ЗначениеЗаполнено(Объект.КаталогSRC) Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Не указан путь к каталогу SRC",,, "Объект.КаталогSRC");
		Результат = Ложь;
	КонецЕсли;

	КаталогНаДиске = Новый Файл(Объект.КаталогSRC);
	Если НЕ КаталогНаДиске.Существует() Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Не найден каталог SRC",,, "Объект.КаталогSRC");
		Результат = Ложь;
	КонецЕсли;
...
Процедура УстановитьНомерСборкиОбработчикам(Команда)
	...
	ПоказатьВводЧисла(ОбработчикОтвета, Число(НомераВерсии[3])+1, "Новый номер сборки");
КонецПроцедуры
...
Процедура СохранитьВФайл(Команда)
    #Если НЕ ВебКлиент Тогда
		...
		БазоваяПодсистемаКлиент.ФС_ПоказатьДиалогВыбора(ОбработчикВыбораФайла, ДиалогВыбораФайла);
	#КонецЕсли
КонецПроцедуры
...
Процедура ЗагрузитьИзФайла(Команда)
	#Если НЕ ВебКлиент Тогда
		...
		ПараметрыЗагрузки.Диалог.Заголовок		= "Укажите файл";
...
Процедура ПоказатьПриоритетыКонфликтыОбработчика(Команда)
	Если Элементы.ОбработчикиОбновления.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,"Не выбраны обработчики обновления");
		Возврат;
	КонецЕсли;

	ДанныеТаблиц = ВыгрузитьТаблицыВMXL(Элементы.ОбработчикиОбновления.ТекущиеДанные.Ссылка, "ПриоритетыВыполнения,КонфликтыОбработчиков");
	Для Каждого Таблица Из ДанныеТаблиц Цикл
		Таблица.Значение.Показать(Таблица.Ключ);
	КонецЦикла;
КонецПроцедуры
...
Процедура ПоказатьОбъектыОбработчика(Команда)
	Если Элементы.ОбработчикиОбновления.ТекущиеДанные = Неопределено Тогда
		ПоказатьПредупреждение(,"Не выбран обработчики обновления");
...
Процедура ВыполнитьТестПостроенияОчереди(Команда)
	...
	Если Команда <> Неопределено Тогда
		...
		ОформитьГруппуНастроек();
		Если НЕ ЗначениеЗаполнено(КаталогФайловОшибок) Тогда
			БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Не указан каталог файлов ошибок.",,, "КаталогФайловОшибок");
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", "Изменена процедура проверки");
	#КонецОбласти
...
Процедура ВыполнитьЗагрузкуОбработчиков()
	ПараметрыФормы			= Новый Структура("ТекстСообщения", "Выполняется загрузка обработчиков");
...
Функция ДобавитьПроцедуруМодуля(Код)
	...
	КомментарийПроцедуры = "// см. ОбновлениеВерсииИБСервер.ПриДобавленииОбработчиковОбновления
	|%1";
	...
	Если ЭтоМодульМенеджера И СтрокиМодуля.Количество() = 0 Тогда
		...
		Шаблон = "В модуле ""%1"" добавлена процедура ""%2""";
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(СтрШаблон(Шаблон, Код.ИмяМодуля, ИмяПроцедуры));

		Возврат Истина;
	КонецЕсли;

	ЕстьПроцедураОписания = СтрокиМодуля.Найти(ПростойЗаголовокПроцедурыОписания) <> Неопределено ИЛИ СтрокиМодуля.Найти("Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт") <> Неопределено;
	...
	Если Код.ЭтоПроцедураОписания Тогда
		Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
			...
			НайденныйЗаголовокПроцедуры	= СтрШаблон(ШаблонЗаголовка, ИмяБиблиотеки);
	...
	Если ЕстьПереданнаяПроцедура ИЛИ ЕстьПроцедураОписания ИЛИ ЕстьБиблиотечнаяПроцедураОписания И ПереданаПростаяПроцедура Тогда
	...
	ТекстОбласти = "#Область ОбновлениеИнформационнойБазы
	|
	|%1
	|
	|#КонецОбласти";
	...
	Если НЕ ДобавленаПроцедура И НЕ ПустаяСтрока(Код.ПроцедураОбработки) Тогда
		...
		Если Индекс <> Неопределено Тогда
			ТекстОбласти	= СтрШаблон(ТекстОбласти, ТекстПроцедуры);
			Индекс			= Индекс + 1;
			СтрокиМодуля.Вставить(Индекс, "");
	...
	Если ДобавленаПроцедура Тогда
		...
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(СтрШаблон(Шаблон, Код.ИмяМодуля, ИмяПроцедуры));
	Иначе
		Шаблон = "В модуле ""%1"" не удалось добавить процедуру ""%2""
		|Возможно в модуле не задана область ""%3"".
		|Добавьте процедуру или область вручную и повторите запись обработчиков.";
		ТекстСообщения = СтрШаблон(Шаблон, Код.ИмяМодуля, ИмяПроцедуры, "ОбновлениеИнформационнойБазы");
...
Процедура ЗаменитьПроцедуруМодуля(Код)
	...
	ЭтоМодульОбновления			= СтрНачинаетсяС(Код.ИмяМодуля, "ОбновлениеВерсииИБСервер");
	#Если НЕ ВебКлиент Тогда
	...
	Пока СтрокаМодуля <> Неопределено Цикл
		...
		Если Код.ЭтоПроцедураОписания И СтрНайти(СтрокаМодуля, "--// Добавляет в список процедуры-обработчики обновления данных ИБ") > 0 Тогда
			СтрокаМодуля = ПропуститьСтрокиДо(Модуль, "Обработчики - см. ОбновлениеВерсииИБСервер.ОИБ_НоваяТаблицаОбработчиковОбновления");
			Если СтрНайти(СтрокаМодуля, "Процедура") = 0 Тогда
				СтрокаМодуля	= Модуль.ПрочитатьСтроку();
				СтрокаМодуля	= "// см. ОбновлениеВерсииИБСервер.ПриДобавленииОбработчиковОбновления";
			КонецЕсли;
		КонецЕсли;
		#КонецОбласти
		...
		Если НЕ ВыполненаЗамена И Код.ЭтоПроцедураОписания И (СтрНайти(СтрокаМодуля, "Процедура ОписаниеОбработчиковОбновления") > 0 ИЛИ СтрНайти(СтрокаМодуля, "Процедура ПриДобавленииОбработчиковОбновления") > 0) И СтрНайти(СтрокаМодуля, БиблиотечныйЗаголовокПроцедуры) = 0 Тогда
			Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
				БиблиотечныйЗаголовокПроцедуры = СтрШаблон("Процедура ОписаниеОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
				Если СтрНайти(СтрокаМодуля, БиблиотечныйЗаголовокПроцедуры) > 0 Тогда
					ЭтоБиблиотечнаяПроцедураОписания = Истина;

					Прервать;
				КонецЕсли;
				БиблиотечныйЗаголовокПроцедуры = СтрШаблон("Процедура ПриДобавленииОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
				Если СтрНайти(СтрокаМодуля, БиблиотечныйЗаголовокПроцедуры) > 0 Тогда
					ЭтоБиблиотечнаяПроцедураОписания = Истина;
		...
		Если СокрЛП(СтрокаМодуля) = Код.ЗаголовокПроцедуры
			ИЛИ (НЕ ЭтоМодульОбновления И Код.ЭтоПроцедураОписания И (СокрЛП(СтрокаМодуля) = ПростойЗаголовокПроцедурыОписания ИЛИ СокрЛП(СтрокаМодуля) = "Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт"))
			ИЛИ ЭтоБиблиотечнаяПроцедураОписания И Код.ЭтоПроцедураОписания И (Код.ЗаголовокПроцедуры = БиблиотечныйЗаголовокПроцедуры ИЛИ Код.ЗаголовокПроцедуры = ПростойЗаголовокПроцедурыОписания) Тогда
	...
	Если УстановленаВерсия Тогда
		Шаблон = "В модуле ""%1"" установлена новая версия ""%2""";
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(СтрШаблон(Шаблон, Код.ИмяМодуля, Код.Версия));
	КонецЕсли;
	Если ВыполненаЗамена Тогда
		Шаблон		= "В модуле ""%1"" заменена процедура ""%2""";
...
Процедура СохранитьВерсиюВКореньКонфигурации()
	...
	Если УстановленаВерсия Тогда
		Шаблон		= "Установлена новая версия ""%1"" в корне конфигурации";
		НовыйТекст	= НовыйФайл.ПолучитьТекст();
		ЗаписатьТекстМодуля(НовыйТекст, ПолноеИмяФайла);
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(СтрШаблон(Шаблон, НоваяВерсияКонфигурации));
	КонецЕсли;
...
Процедура Подключаемый_ОткрытьРедакторОписанияОбработчика()
	...
	ОткрытьФорму(ИмяФормыОбработчика, ПараметрыФормы, ЭтотОбъект, ЭтотОбъект.УникальныйИдентификатор,,, ОбработчикЗакрытияФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Функция ПоместитьДанныеОбработчикаВХранилище(СсылкаОбработчика, Адрес = Неопределено, ИсточникКопирования = Неопределено)
	...
	Иначе
		...
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(ЕщеКонфликты, ТабЧасти.КонфликтыОбработчиков);
	КонецЕсли;
...
Функция ОбновитьДанныеОбработчика(АдресОбработчика)
	...
	Если ОписаниеОбработчика <> Неопределено Тогда
		...
		Если МодулиПодсистем <> Неопределено И НовоеОписание.Подсистема <> "" Тогда
			...
			ОписаниеОбработчика.ИмяБиблиотеки					= СтрЗаменить(ОписаниеОбработчика.ИмяОсновногоСерверногоМодуля, "ОбновлениеВерсииИБСервер", "");
		КонецЕсли;
...
Функция ПостроитьОчередьНаСервере()
	ИтерацииОбновления	= ОбновлениеВерсииИБСервер.сОИБ_ИтерацииОбновления();
...
Процедура ЗаполнитьБыстрыеОтборы(Знач ОписаниеОбработчиков = Неопределено)
	...
	ДобавитьБыстрыйОтбор(ОписаниеОбработчиков, МассивСтрок, "ИзмененаПроцедураПроверки",	"Изменена процедура проверки",	WebЦвета.Золотистый);
	...
	ТекстСтатуса		= СтрШаблон("%1 (%2)", НСтр("ru = 'Все'"), ОписаниеОбработчиков.Количество());
...
Функция ОписанияОбработчиковПоМодулямМенеджеров(АдресНастроек)
	...
	Для Каждого ИмяКонфигурации Из ИменаКонфигураций Цикл
		...
		Если РазметкаКода.УстановитьТэги Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ИмяБиблиотеки = СтрЗаменить(ИмяОсновногоСерверногоМодуля, "ОбновлениеВерсииИБСервер", "");
		...
		Иначе
			БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(МодулиОбработчиков, ВсеМодулиОбработчиков);
		КонецЕсли;

		Если ОбработчикиКОписанию = Неопределено Тогда
			ОбработчикиКОписанию = ОбработчикиМодуля;
		Иначе
			БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(ОбработчикиМодуля, ОбработчикиКОписанию);
		КонецЕсли;
	КонецЦикла;
	#КонецОбласти
	...
	Если РазметкаКода.УстановитьТэги И НЕ ТолькоВыделенные Тогда
		...
		Если ВыделенныеОбработчики.Количество() > 0 Тогда
			БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ИзмененныеОбработчики, ВыделенныеОбработчики, Истина);
		КонецЕсли;
		Для Каждого Измененный Из ИзмененныеОбработчики Цикл
			...
			Для Каждого Конфликт Из КонфликтыОбработчика Цикл
				...
				Если Обработчик <> Неопределено Тогда
					...
					БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(Буфер, КонфликтующиеОбработчики);
				КонецЕсли;
			КонецЦикла;
			БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(КонфликтующиеОбработчики, ВсеМодулиОбработчиков);
		КонецЦикла;
	КонецЕсли;
	#КонецОбласти
	...
	Если ТолькоВыделенные И НЕ РасширенныйРежимФормы Тогда
		КодОбработчиков = ОписаниеКодаМодуля("Выделенные обработчики");
		Для Каждого КодМодуля Из ТекстыМодулей Цикл
...
Процедура ЗаполнитьОписание(ОписаниеКонфигураций, ИменаКонфигураций)
	ПодсистемыКонфигурации = БазоваяПодсистемаСерверПовтИсп.СП_ОписанияПодсистем();
...
Процедура ДобавитьВерсиюМодуля(ВерсииМодулей, ОбработчикиМодуля)
	...
	Если ЭтоРазрабатываемаяБиблиотека И Число(ПерваяЦифраМаксимум) >= Число(ПерваяЦифраКонфигурации) И Максимум3Последних > Конфигурация3Последних Тогда
...
Функция ТекстГруппыВызововРекурсивно(Ветка, РазметкаКода)
	...
	Для Каждого МодульОбработчика Из Ветка.Строки Цикл
		...
		ТекстВызова		= Символы.Таб + СтрШаблон("%1.%2;", МодульОбработчика.МодульОбработчика, ИмяПроцедуры);
...
Функция ТекстОписанияОбработчика(Обработчик, РазметкаКода, ТэгиОбработчика)
	...
	Иначе
		...
		СтрокиОбработчика.Добавить("Обработчик.Комментарий = """ + ТекстКомментария +""";");
	КонецЕсли;
...
Функция ТекстИнформацииОбОбъектах(Обработчик, ИмяНабораОбъектов, РазметкаКода, ТэгиОбработчика)
	...
	Если ИмяНабораОбъектов = "БлокируемыеОбъекты" Тогда
		...
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(БлокируемыеОбъекты, ОбъектыОбработчика);
...
Функция ЗаполнитьТэгиОбъектов(РеквизитИмяОбъекта, ОбъектыОбработчика, РазметкаКода, ВнешниеТэги = Неопределено, СортироватьПоТэгам = Истина)
	...
	Для Каждого ИспользуемыйОбъект Из ОбъектыОбработчика Цикл
		...
		Если ОписаниеТэгов <> Неопределено Тогда
			БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(Тэги, СтрРазделить(ОписаниеТэгов.Тэги,",", Ложь), Истина);
		КонецЕсли;
...
Функция ЗаполнитьТэгиМодулейПоПодсистемам(МодулиМенеджеров, РазметкаКода)
	...
	Для Каждого МодульОбъекта Из МодулиМенеджеров Цикл
		...
		ОписаниеТэгов = РазметкаКода.Тэги.ПоОбъектам.Найти(ИмяОбъекта, "Имя");
		Если ОписаниеТэгов <> Неопределено Тогда
			ТэгиМодуля = БазоваяПодсистемаСервер.ОН_СкопироватьРекурсивно(ВнешниеТэги); // Массив -
...
Функция ЛишниеТэгиВОбласти(ВнешниеТэги, РазметкаКода)
	...
	Для Каждого ЛишнийТэг Из ЛишниеТэги Цикл
		...
		Для Каждого Имя Из БиблиотекиТэга Цикл
			...
			Если ТэгиВложенныхБиблиотек <> Неопределено Тогда
				БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ВложенныеТэги, ТэгиВложенныхБиблиотек, Истина);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ЛишниеТэги, ВложенныеТэги, Истина);
	...
	Если ТэгиВложенныхБиблиотек <> Неопределено И НЕ РазметкаКода.ЭтоМодульМенеджера Тогда
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ЛишниеТэги, ТэгиВложенныхБиблиотек, Истина);
	КонецЕсли;
...
Функция ПолучитьСведенияОТегахВырезки(Настройки)
	...
	Для Каждого ОбъектБиблиотеки Из ОбъектыБиблиотек Цикл
		...
		ТэгиОбъекта				= Новый Массив;
		ИсключитьИзКонфигурации	= БазоваяПодсистемаКлиентСервер.ОН_СвернутьМассив(СтрРазделить(ИсключитьИзСтрокой, ",",Ложь));
...
Функция ПолучитьСведенияОБиблиотеках(Настройки)
	...
	БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ВложенностьБиблиотек, СтрРазделить(БиблиотекиСтрокой, ","), Истина);
	...
	Для Каждого Описание Из ОписаниеТэгов["configurations"] Цикл
		...
		Если РасширяемаяКонфигурация <> Неопределено Тогда
			...
			Если Тэги <> Неопределено Тогда
				...
				Иначе
					БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(Конфигурация, Тэги, Истина);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПрочитатьФайлYaml(ИмяФайла)
	...
	Исключение
		...
		ТекстСообщения	= Символы.ПС + СтрШаблон("Не удалось прочитать файл %1.", ИмяФайла);
		ТекстСообщения	= ТекстСообщения  + Символы.ПС + "По причине" + ":" + Символы.ПС;
...
Процедура ПрочитатьСтрокиYAML(Результат, ТекстФайла, НомерСтроки = 0, Уровень = 0, ЧтениеМногострочнойСтроки = Ложь)
	...
	Исключение
		ТекстОшибки		= ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстСообщения	= Символы.ПС + СтрШаблон("Не удалось разобрать строку: %1", СтрокаФайла);
...
Процедура ЗаписатьОшибку(ИмяОбъекта, ОписаниеОшибки = "", ИнформацияОбОшибке = Неопределено, Ключ = Неопределено)
	...
	Ошибка.ТипОшибки				= "Ошибка построения очереди отложенных обработчиков обновления";
	...
	Если ЗначениеЗаполнено(КаталогФайловОшибок) Тогда
		...
		ИмяФайлаОшибки = ИмяФайлаОшибки + Формат(БазоваяПодсистемаКлиент.ОН_ДатаСеанса(), "ДФ=yyyyMMddHHmmss") + "_" + Строка(Новый УникальныйИдентификатор())+".xml";
...
Функция СоздатьОписаниеОшибки()
	...
	#Если Клиент Или ТолстыйКлиентУправляемоеПриложение Или ТонкийКлиент Или ВебКлиент  Тогда
		...
		Ошибка.Вставить("КлиентТекущаяДата",					БазоваяПодсистемаКлиент.ОН_ДатаСеанса());
	#КонецЕсли
...
Процедура УстановитьУпрощенныйРежим()
	...
	Элементы.ПереключитьРежимФормы.Заголовок	= "Перейти в расширенный режим";
...
Процедура УстановитьРасширенныйРежим()
	...
	Элементы.ПереключитьРежимФормы.Заголовок	= "Перейти в упрощенный режим";
	УстановитьВидимостьЭлементовРежимаФормы();
КонецПроцедуры
...
#Если Не ВебКлиент Тогда
	ПутьККаталогу = БазоваяПодсистемаКлиентСервер.ОН_ДобавитьКонечныйРазделительПути(ПолучитьИмяВременногоФайла());
...
Функция ФильтрФайловРезервнойКопии()
	Возврат "Описание обработчиков (*.handlers)|*.handlers|Все файлы (*.*)|*.*";
КонецФункции
...
Процедура ДиалогВыбораФайлаЗавершение(ВыбранныйФайл, ДополнительныеПараметры) Экспорт
	Если ВыбранныйФайл <> Неопределено Тогда
		...
		БазоваяПодсистемаКлиент.ФС_СохранитьФайл(Неопределено, Адрес, ИмяФайлаРезервнойКопии, ПараметрыСохранения);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗагрузитьДанныеИзФайла(Адрес)
	...
	БазоваяПодсистемаСервер.ФС_УдалитьВременныйФайл(ВременныйФайл);
...
Процедура ОтключитьОтборОбработчиков(ИмяПоля)
	...
	Если ЗначениеЗаполнено(Элементы.ОбработчикиОбновления.ОтборСтрок) И Элементы.ОбработчикиОбновления.ОтборСтрок.Свойство(ИмяПоля) Тогда
		ТекущийОтбор = Новый Структура(Элементы.ОбработчикиОбновления.ОтборСтрок);
...
Функция ДанныеОбработчика(ИмяТЧ, Ссылка)
	...
	ИначеЕсли ИмяТЧ = "КонфликтыОбработчиков" Тогда
		...
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(ДанныеТЧ2, ДанныеТЧ);
	КонецЕсли;
...
Процедура РассчитатьОчередьНаСервере()
	ИтерацииОбновления = Неопределено; // ОбновлениеВерсииИБСервер.сОИБ_ИтерацииОбновления();
