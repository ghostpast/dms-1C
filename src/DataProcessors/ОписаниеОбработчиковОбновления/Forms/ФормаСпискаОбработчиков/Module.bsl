...
Процедура СохранитьВРепозиторий(Команда)
	Если НЕ ЗначениеЗаполнено(Объект.КаталогSRC) Тогда
		...
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю("Не указан путь к каталогу SRC.",,, "Объект.КаталогSRC");
...
Процедура УстановитьНомерСборкиОбработчикам(Команда)
	...
	ПоказатьВводЧисла(ОбработчикОтвета, Число(НомераВерсии[3]), "Укажите новый номер сборки");
КонецПроцедуры
...
Функция ДобавитьПроцедуруМодуля(Код)
	...
	Если Код.ЭтоПроцедураОписания Тогда
		Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
			ЗаголовокПроцедуры = СтрШаблон("Процедура ОписаниеОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
			Если СтрокиМодуля.Найти(ЗаголовокПроцедуры) <> Неопределено Тогда
				ЕстьПроцедураОписания = Истина;

				Прервать;
			КонецЕсли;
			ЗаголовокПроцедуры = СтрШаблон("Процедура ПриДобавленииОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
			Если СтрокиМодуля.Найти(ЗаголовокПроцедуры) <> Неопределено Тогда
				ЕстьПроцедураОписания = Истина;
				Прервать;
			КонецЕсли;
	...
	Если СтрокиМодуля.Найти(Код.ЗаголовокПроцедуры) <> Неопределено ИЛИ СтрокиМодуля.Найти("Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт") <> Неопределено ИЛИ ЕстьПроцедураОписания Тогда
	...
	Иначе
		...
		ТекстСообщения = СтрШаблон(Шаблон, Код.ИмяМодуля, ИмяПроцедуры);
...
Процедура ЗаменитьПроцедуруМодуля(Код)
	...
	Пока СтрокаМодуля <> Неопределено Цикл
		...
		Если НЕ ВыполненаЗамена И Код.ЭтоПроцедураОписания И (СтрНайти(СтрокаМодуля, "Процедура ОписаниеОбработчиковОбновления") > 0 ИЛИ СтрНайти(СтрокаМодуля, "Процедура ПриДобавленииОбработчиковОбновления") > 0) И СтрНайти(СтрокаМодуля, ЗаголовокПроцедуры) = 0 Тогда
			Для Каждого ИмяБиблиотеки Из Код.ИменаБиблиотек Цикл
				ЗаголовокПроцедуры = СтрШаблон("Процедура ОписаниеОбработчиковОбновления%1(Обработчики) Экспорт", ИмяБиблиотеки);
		...
		Если СокрЛП(СтрокаМодуля) = ЗаголовокПроцедуры ИЛИ (Код.ЭтоПроцедураОписания И СокрЛП(СтрокаМодуля) = "Процедура ОписаниеОбработчиковОбновления(Обработчики) Экспорт") ИЛИ ЭтоПроцедураОписания Тогда
...
Функция ТекстИнформацииОбОбъектах(Обработчик, ИмяНабораОбъектов, РазметкаКода, ТэгиОбработчика)
	...
	Если ИмяНабораОбъектов = "БлокируемыеОбъекты" Тогда
		...
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(ИзменяемыеОбъекты, ОбъектыОбработчика);
...
Функция ВыгрузитьТаблицыВMXL(Ссылка = Неопределено, КаталогВыгрузкиТЧ = "")
	...
	Для Каждого ТЧ Из Обработка.ТабличныеЧасти Цикл
		Если Ссылка <> Неопределено И (ТЧ.Имя = "ОбработчикиОбновления" ИЛИ ТЧ.Имя = "ЧитаемыеОбъекты" ИЛИ ТЧ.Имя = "ИзменяемыеОбъекты" ИЛИ ТЧ.Имя = "БлокируемыеОбъекты" ИЛИ ТЧ.Имя = "Ошибки") Тогда
			Продолжить;
		КонецЕсли;
