...
Функция ПравильноУказанКаталогSRC()
	...
	Если НЕ ЗначениеЗаполнено(Объект.КаталогSRC) Тогда
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю("Не указан путь к каталогу SRC",,, "Объект.КаталогSRC", Ложь);
		Результат = Ложь;
	КонецЕсли;

	КаталогНаДиске = Новый Файл(Объект.КаталогSRC);
	Если НЕ КаталогНаДиске.Существует() Тогда
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю("Не найден каталог SRC",,, "Объект.КаталогSRC", Ложь);
		Результат = Ложь;
	КонецЕсли;
...
Процедура ВыполнитьТестПостроенияОчереди(Команда)
	...
	Если Команда <> Неопределено Тогда
		...
		ОформитьГруппуНастроек();
		Если НЕ ЗначениеЗаполнено(КаталогФайловОшибок) Тогда
			БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю("Не указан каталог файлов ошибок.",,, "КаталогФайловОшибок");
...
Функция ДобавитьПроцедуруМодуля(Код)
	...
	Если ЭтоМодульМенеджера И СтрокиМодуля.Количество() = 0 Тогда
		...
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(СтрШаблон(Шаблон, Код.ИмяМодуля, ИмяПроцедуры), Неопределено, "", "", Ложь);
...
Процедура ЗаменитьПроцедуруМодуля(Код)
	...
	Если УстановленаВерсия Тогда
		...
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(СтрШаблон(Шаблон, Код.ИмяМодуля, Код.Версия), Неопределено, "", "", Ложь);
	КонецЕсли;
...
Процедура СохранитьВерсиюВКореньКонфигурации()
	...
	Если УстановленаВерсия Тогда
		...
		ЗаписатьТекстМодуля(НовыйТекст, ПолноеИмяФайла);
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(СтрШаблон(Шаблон, НоваяВерсияКонфигурации), Неопределено, "", "", Ложь);
	КонецЕсли;
