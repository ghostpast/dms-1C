...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивПорядка.Добавить("Текущий");
	МассивПорядка.Добавить("Конфликтующий");
	МассивПорядка.Добавить("Любой");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(ОбработчикОтвета, "Данные были изменены. Сохранить изменения?", РежимДиалогаВопрос.ДаНетОтмена);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроцедураПроверкиПриИзменении(Элемент)
	...
	Если Элемент.ТекстРедактирования <> "ОбновлениеВерсииИБСервер.ОИБ_ДанныеОбновленыНаНовуюВерсиюПрограммы" Тогда
...
Процедура ПроцедураПроверкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ЧитаемыеОбъекты.НайтиСтроки(Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0 ИЛИ Объект.БлокируемыеОбъекты.Количество() > 0 Тогда
		ТекстСообщения = "Процедура ""%1"" не предназначена для проверки блокировки в обработчиках, которые контролируют блокировку читаемых данных или прочих объектов.";
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%1", "ОбновлениеВерсииИБСервер.ОИБ_ДанныеОбновленыНаНовуюВерсиюПрограммы");
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(ТекстСообщения,,"ПроцедураПроверки","Объект");

		Возврат;
	КонецЕсли;

	Объект.ОбработчикиОбновления[0].ПроцедураПроверки	= "ОбновлениеВерсииИБСервер.ОИБ_ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Элементы.ПроцедураПроверки.ОтметкаНезаполненного	= Ложь;

	ТекущееЗначениеПроцедурыПроверки					= "ОбновлениеВерсииИБСервер.ОИБ_ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура КомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	БазоваяПодсистемаКлиент.ОН_ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Элементы.Комментарий.ТекстРедактирования, "Служебный комментарий");
КонецПроцедуры
...
Процедура ПриоритетыВыполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ПриоритетыВыполненияОбъектыПересечения" Тогда
		...
		ОткрытьФорму(ИмяФормыОбработчика, ПараметрыФормы, ЭтотОбъект, ЭтотОбъект.УникальныйИдентификатор,,,, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьТипыРеквизитаВБлокируемыеОбъекты(Команда)
	...
	ИменаОбъектов.ПоказатьВыборЭлемента(ОбработчикВыбора, "Выберите объект метаданных");
КонецПроцедуры
...
Функция ЗаполненоПравильно()
	...
	Если ТекущийОбработчик.РежимВыполнения = "Отложенно" И Объект.ИзменяемыеОбъекты.Количество() = 0 Тогда
		ТекстСообщения							= "Отложенный обработчик не может быть записан с пустой табличной частью ""Изменяемые объекты""";
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(ТекстСообщения);
...
Функция ПредставлениеОбъектаМетаданных(СсылкаОбъектаМетаданных)
	...
	Иначе
		ПолноеИмя = БазоваяПодсистемаСервер.ОН_ЗначениеРеквизитаОбъекта(СсылкаОбъектаМетаданных, ИмяРеквизита);
	КонецЕсли;
...
Функция ИмяОсновногоОбъектаМетаданных(ИмяПроцедуры, ЕдинственноеЧисло, МножественноеЧисло)
	...
	ИначеЕсли ЧастиИмени.Количество() = 3 Тогда
		...
		Исключение
			ТекстСообщения = "Не удалось распознать вид объекта метаданных ""%1""
				|Вид объекта должен быть во множественном числе.";
			ТекстСообщения = СтрШаблон(ТекстСообщения, ЧастиИмени[0]);
...
Процедура ВыборОбъектаМетаданных(Элемент, ТекущееЗначение = "", ИмяТаблицы = Неопределено)
	...
	ОткрытьФорму(ИмяФормыВыбора, ПараметрыФормы, Элемент, ЭтотОбъект.УникальныйИдентификатор,,, ОбработчикВыбора, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
		...
		ИначеЕсли ОбъектыПересечения.Количество() > 1 Тогда
			Для Каждого ОбъектПересечения Из ОбъектыПересечения Цикл
				Если ИмяОсновногоОбъекта <> ОбъектПересечения.ОбъектМетаданных Тогда
					НоваяСтрока.ОбъектыПересечения = СтрШаблон("%1 +%2", ОбъектПересечения.ОбъектМетаданных, ОбъектыПересечения.Количество()-1);
...
Процедура ЗаполнитьПересеченияОбработчика(ТекущийОбработчик, КонфликтующийОбработчик)
	...
	Для Каждого Конфликт Из Объект.КонфликтыОбработчиков Цикл
		Отбор				= Новый Структура("ОбъектМетаданных,ПроцедураПисатель,ПроцедураЧитательИлиПисатель2", Конфликт.ОбъектМетаданных, Конфликт.ПроцедураЧитательИлиПисатель2, Конфликт.ПроцедураПисатель);
		ОбратныеКонфликты	= Пересечения.НайтиСтроки(Отбор);
		Если Конфликт.ОбработчикПисатель = ТекущийОбработчик И Конфликт.ОбработчикЧитательИлиПисатель2 = КонфликтующийОбработчик ИЛИ Конфликт.ОбработчикПисатель = КонфликтующийОбработчик И Конфликт.ОбработчикЧитательИлиПисатель2 = ТекущийОбработчик И ОбратныеКонфликты.Количество() = 0 Тогда
...
Процедура УстановитьОтметкуНезаполненногоПроцедураПроверки()
	...
	Если ЗначениеЗаполнено(Объект.ОбработчикиОбновления[0].Процедура) ИЛИ Объект.ОбработчикиОбновления[0].РежимВыполнения = "Монопольно" ИЛИ Объект.ОбработчикиОбновления[0].РежимВыполнения = "Оперативно" ИЛИ НЕ (Объект.БлокируемыеОбъекты.Количество() > 0 ИЛИ Объект.ЧитаемыеОбъекты.НайтиСтроки(Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0 ИЛИ Объект.ИзменяемыеОбъекты.НайтиСтроки(Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0) Тогда
		Отметка = Ложь;
	КонецЕсли;
...
Процедура ВыбратьРеквизитОбъектаМетаданных(Реквизиты, ИмяТаблицы, ИмяОбъекта)
	...
	Реквизиты.ПоказатьВыборЭлемента(ОбработчикВыбора, "Выберите реквизит объекта");
КонецПроцедуры
	...
	Для Каждого Коллекция Из ЕщеРеквизитыОбъекта Цикл
		...
		Для Каждого Реквизит Из Коллекция.Значение Цикл
			...
			Для Каждого Тип Из Реквизит.Тип.Типы() Цикл
				...
				Если БазоваяПодсистемаСервер.ОН_ЭтоСсылка(Тип) Тогда // добавим его в список
...
Процедура ДобавитьТипыРеквизитаВТаблицу(ИмяТаблицы, ПолноеИмяРеквизита)
	...
	Для Каждого Тип Из Реквизит.Тип.Типы() Цикл
		ИмяОбъекта							= СтрЗаменить(БазоваяПодсистемаСервер.ОН_СтроковоеПредставлениеТипа(Тип),"Ссылка","");
...
Функция ТекстПредупрежденияИзмененаПроцедураПроверки(Вопрос = Ложь)
	ТекстШапка = "Использование своей процедуры проверки, отличной от стандартной, допустимо
		|только в крайних случаях, когда ее возможностей недостаточно.";
	ТекстШапка = СтрСоединить(СтрРазделить(ТекстШапка, Символы.ПС), " ");

	ТекстПродолжение = "При этом необходимо соблюдать следующие правила при ее написании:
	...
	Если Вопрос Тогда
		ТекстПредупреждения = ТекстПредупреждения + Символы.ПС + Символы.ПС + "Продолжить?";
	КонецЕсли;
...
Процедура ПроцедураПроверкиОкончаниеВводаТекстаПослеВопроса(Результат, Текст) Экспорт
	...
	Элементы.ДекорацияПредупреждение.Видимость = (ТекущееЗначениеПроцедурыПроверки <> "ВнешняяОбработка.ОписаниеОбработчиковОбновления.Форма.ОбъектыПересеченияОбработчика");
