...
Процедура ПроцедураПроверкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Объект.ЧитаемыеОбъекты.НайтиСтроки(Новый Структура("БлокироватьИнтерфейс", Истина)).Количество() > 0 ИЛИ Объект.БлокируемыеОбъекты.Количество() > 0 Тогда
		...
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(ТекстСообщения,,"ПроцедураПроверки","Объект", Ложь);
...
Процедура КомментарийНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	БазоваяПодсистемаКлиент.ОН_ПоказатьФормуРедактированияМногострочногоТекста(Оповещение, Элементы.Комментарий.ТекстРедактирования, НСтр("ru='Служебный комментарий'"));
КонецПроцедуры
...
Функция ЗаполненоПравильно()
	...
	Если ТекущийОбработчик.РежимВыполнения = "Отложенно" И Объект.ИзменяемыеОбъекты.Количество() = 0 Тогда
		...
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(ТекстСообщения, Неопределено, "", "", Ложь);
...
Процедура УстановитьПорядокПоУмолчанию()
	...
	ВыборПорядка.Вставить("До",		"Текущий");
	ВыборПорядка.Вставить("После",	"Конфликтующий");
