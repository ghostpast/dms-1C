...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не БазоваяПодсистемаКлиентСервер.ОН_СпискиЗначенийИдентичны(СобытияПоУмолчанию, События) Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	Если ИмяСобытия = "ВыборЗначенийЭлементовОтбораЖурналаРегистрации" И Источник.УникальныйИдентификатор = ЭтотОбъект.УникальныйИдентификатор Тогда
...
Процедура УстановитьОтборИЗакрытьФорму(Команда)
	ОповеститьОВыборе(Новый Структура("Событие, Отбор", "УстановленОтборЖурналаРегистрации", ПолучитьОтборЖурналаРегистрации()));
КонецПроцедуры
...
Процедура ЗаполнитьПараметрыОтбора()
	...
	Если ЗначениеЗаполнено(РабочиеСерверы) Или ЗначениеЗаполнено(ОсновныеIPПорты) Или ЗначениеЗаполнено(ВспомогательныеIPПорты) Или ЗначениеЗаполнено(Комментарий) Тогда
