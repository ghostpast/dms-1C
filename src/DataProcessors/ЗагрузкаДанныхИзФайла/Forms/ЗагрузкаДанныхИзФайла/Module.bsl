...
Процедура ВывестиСообщениеОбОшибке(ТекстОшибкиДляПользователя, ТехническаяИнформация)
	...
	БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(ТекстСообщенияОбОшибке);
КонецПроцедуры
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = СтрШаблон("Подсистема %1: Выполнение серверного метода загрузка данных из файла", "ЗагрузкаДанныхИзФайла");
...
Процедура ПослеСозданияОтчета(Задание, ДополнительныеРезультаты) Экспорт
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю(Задание.КраткоеПредставлениеОшибки);
