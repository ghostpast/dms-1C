...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли НЕ ПользователиСервер.П_ЭтоПолноправныйПользователь() Тогда
		ВызватьИсключение("Недостаточно прав для открытия загрузки данных из файла");
	КонецЕсли;
	...
	ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
		...
		ТаблицаИнформацияПоКолонкам = БазоваяПодсистемаСервер.ОН_ХранилищеЗагрузить(ХранилищеОбщихНастроек, "ЗагрузкаДанныхИзФайла", ИмяОбъектаСопоставления, Неопределено, ИмяПользователя(), Неопределено); // ТаблицаЗначений
		...
		Если Параметры.Свойство("КолонкиМакета") И Параметры.КолонкиМакета <> Неопределено Тогда
			...
			ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ДобавитьСтатистическуюИнформацию("РежимЗапуска.ЗагрузкаВТабличнуюЧасть.ДинамическийМакет",, Параметры.ПолноеИмяТабличнойЧасти);
		Иначе
			...
			ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ДобавитьСтатистическуюИнформацию("РежимЗапуска.ЗагрузкаВТабличнуюЧасть.СтатическийМакет",, Параметры.ПолноеИмяТабличнойЧасти);
			Если Отказ Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		ТекстПредупреждения = "Введенные данные не будут записаны.";
	...
	Если Отказ Тогда
		...
		ТекстВопроса = "Введенные данные не будут записаны. Закрыть форму?";
...
Процедура СписокКолонокСопоставленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СопоставитьПоКолонке.Количество() = 0 Тогда
		Для каждого СведенияОКолонке Из ИнформацияПоКолонкам Цикл
			Если (ТипЗнч(СведенияОКолонке.ТипКолонки) = Тип("Строка") И СведенияОКолонке.ТипКолонки.КвалификаторыСтроки.Длина = 0) ИЛИ СтрНачинаетсяС(СведенияОКолонке.ИмяКолонки, "Свойство_") Тогда
					Продолжить;
			КонецЕсли;
...
Процедура ОтменитьСопоставление(Команда)
	...
	ПоказатьВопрос(Оповещение, "Отменить сопоставление?", РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура ШагНазад()
	Если Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.ЗаполнениеТаблицыДанными Тогда
		...
		ЭтотОбъект.Заголовок						= "Загрузка данных в справочник";
		ОчиститьТаблицу();
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных
		...
		Элементы.Далее.Заголовок					= ?(ТипЗагрузки = "ВставкаИзБуфераОбмена", "Вставить в список", "Далее >");
...
Процедура ЗагрузитьШаблонИзФайла(Команда)
	...
	ЗагрузкаДанныхИзФайлаКлиент.ЗДФ_ДиалогЗагрузкиФайла(Оповещение, ИмяФайла);
КонецПроцедуры
...
Процедура ПереходКСледующемуШагуЗагрузкиДанных()
	...
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.РезультатыСопоставления Тогда
		Элементы.СтраницыПомощника.ТекущаяСтраница	= Элементы.СопоставлениеЗагружаемыхДанных;
		Элементы.ВставитьВСписок.Видимость			= Ложь;
		Элементы.Далее.Заголовок					= "Вставить в список";
		Элементы.Далее.КнопкаПоУмолчанию			= Истина;
		Элементы.Назад.Заголовок					= "< В начало";
	ИначеЕсли Элементы.СтраницыПомощника.ТекущаяСтраница = Элементы.СопоставлениеЗагружаемыхДанных Тогда
		...
		Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			Если Строки.Количество() > 0 Тогда
				...
				ПоказатьВопрос(Оповещение, "Строки, в которых не заполнены обязательные колонки, будут пропущены." + Символы.ПС + "Продолжить?", РежимДиалогаВопрос.ДаНет);
			...
			Закрыть(АдресЗагруженныхДанных);
		ИначеЕсли ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
			Элементы.Назад.Заголовок = "< В начало_";
			ЗакрытьФормуИВернутьМассивСсылок();
		Иначе
...
Процедура ВыполнитьСопоставление()
	...
	ПоказатьОповещениеПользователя("Выполнено сопоставление",, "Сопоставлено элементов:" + " " + Строка(КоличествоСопоставленныхПоКолонкам));
	ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
КонецПроцедуры
...
Процедура ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла()
	...
	ТекстВсе = СтрШаблон("Все (%1)", Статистика.Всего);

	Элементы.СоздаватьЕслиНеСопоставлено.Заголовок = "Несопоставленные (" + Статистика.НеСопоставленных + ")";
	Элементы.ОбновлятьСуществующие.Заголовок       = "Сопоставленные элементы (" + Строка(Статистика.Сопоставленных) + ")";

	СписокВыбора = Элементы.ФильтрТаблицаСопоставления.СписокВыбора;
	СписокВыбора.Очистить();
	СписокВыбора.Добавить("Все", ТекстВсе, Истина);
	СписокВыбора.Добавить("Несопоставленные",	СтрШаблон("Несопоставленные (%1 из %2)", Статистика.НеСопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Сопоставленные",		СтрШаблон("Сопоставленные (%1 из %2)", Статистика.Сопоставленных, Статистика.Всего));
	СписокВыбора.Добавить("Неоднозначные",		СтрШаблон("Неоднозначные (%1 из %2)",Статистика.Неоднозначных, Статистика.Всего));

	Если Статистика.Неоднозначных > 0 Тогда
		Элементы.ОписаниеНеоднозначность.Видимость = Истина;
		Элементы.ОписаниеНеоднозначность.Заголовок = СтрШаблон("(неоднозначностей: %1)", Статистика.Неоднозначных);
	Иначе
...
Процедура ВыгрузитьШаблонВФайлЗавершение(Подключено, ДополнительныеПараметры) Экспорт
	Если Подключено Тогда
		...
		ДиалогВыбораФайла.Фильтр						= "Книга Excel 97 (*.xls)|*.xls|Книга Excel 2007 (*.xlsx)|*.xlsx|Электронная таблица OpenDocument (*.ods)|*.ods|Текстовый документ c разделителями (*.csv)|*.csv|Табличный документ (*.mxl)|*.mxl";
		ДиалогВыбораФайла.Расширение					= "xls";
		ДиалогВыбораФайла.МножественныйВыбор			= Ложь;
		ДиалогВыбораФайла.ИндексФильтра					= 0;
		ДиалогВыбораФайла.ПолноеИмяФайла				= ИмяФайла;
		БазоваяПодсистемаКлиент.ФС_ПоказатьДиалогВыбора(Оповещение, ДиалогВыбораФайла);
	Иначе
...
Процедура ОповеститьФормыОбИзменении(МассивСсылок)
	БазоваяПодсистемаКлиент.СП_ОповеститьФормыОбИзменении(МассивСсылок);
КонецПроцедуры
...
Процедура ИнициализацияВставкиИзБуфераОбмена()
	...
	Если Параметры.Свойство("ПредставлениеПоля") Тогда
		Заголовок = СтрШаблон("Вставка из буфера обмена (%1)", Параметры.ПредставлениеПоля);
	Иначе
		Заголовок = "Вставка из буфера обмена";
	КонецЕсли;

	ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ДобавитьСтатистическуюИнформацию("РежимЗапуска.ВставкаИзБуфераОбмена");
...
Процедура УстановитьВидимостьЭлементовФормы()
	Заголовок = ?(ПустаяСтрока(Параметры.Заголовок), "Загрузка данных в справочник", Параметры.Заголовок);

	Если БазоваяПодсистемаСервер.ОН_ЭтоВебКлиент() Тогда
		Элементы.СтраницаВариантЗаполнениеТаблицы.Видимость			= Ложь;
		Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница			= Элементы.СтраницаВариантЗагрузкаИзФайла;
		Элементы.ВариантЗагрузки.Видимость							= Ложь;
		Элементы.ПояснениеДляВыбораСправочникаДляЗагрузки.Заголовок	= "Выбор справочника для загрузки данных из электронных таблиц, расположенных во внешних файлах 
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаКлиенте()
	...
	ВысотаШапкиШаблонаСДанными					= ?(ЗагрузкаДанныхИзФайлаКлиентСервер.ЗДФ_КолонкиИмеютГруппировку(ИнформацияПоКолонкам), 2, 1);
КонецПроцедуры
...
Процедура ВыполнитьШагЗаполнениеТаблицыДаннымиНаСервере(ОписаниеСтрокиВыбора)
	...
	КонецЕсли;
	ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ДобавитьСтатистическуюИнформацию("РежимЗапуска.ЗагрузкаВСправочник." + ИмяОбъектаСопоставления,, ТипЗагрузки);
	...
	КонецЕсли;
	Заголовок = СтрШаблон("Загрузка данных в справочник ""%1""", ЗаголовокОкна);
КонецПроцедуры
...
Процедура СформироватьМакетПоТипуЗагрузки()
	...
	ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
		...
		Обработки.ЗагрузкаДанныхИзФайла.ПараметрыЗагрузкиИзФайлаВнешняяОбработка(ИдентификаторКоманды, ВнешняяОбработкаСсылка, ПараметрыЗагрузкиПоУмолчанию);
	КонецЕсли;

	ИнформацияПоКолонкамТаблица = БазоваяПодсистемаСервер.ОН_ХранилищеЗагрузить(ХранилищеОбщихНастроек, "ЗагрузкаДанныхИзФайла", ИмяОбъектаСопоставления, Неопределено, ИмяПользователя(), Неопределено);
...
Процедура СопоставитьЗагружаемыеДанныеНаСервере()
	ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ДобавитьСтатистическуюИнформацию(?(ВариантЗагрузки = 0, "ВариантЗагрузки.ЗаполнениеТаблицы", "ВариантЗагрузки.ИзВнешнегоФайла"));

	ТаблицаИнформацияПоКолонкам = РеквизитФормыВЗначение("ИнформацияПоКолонкам");
	Если ТипЗагрузки = "ТабличнаяЧасть" Тогда
		АдресЗагруженныхДанных   = "";
		АдресКопииТабличнойЧасти = "";
		СписокНеоднозначностей   =  ЗагрузкаДанныхИзФайлаСервер.ЗДФ_НовыйСписокНеоднозначностей();
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеСопоставленияНаСервере(АдресРезультата)
	...
	Если Результат.Количество() = ТаблицаСопоставления.Количество() Тогда
		Пояснение = СтрШаблон("Данные сопоставлены. При необходимости возможно сопоставить данные вручную, заполнив колонку ""%1"".", ПараметрыЗагрузки.ПредставлениеОбъекта);
		ФильтрТаблицаСопоставления = Элементы.ФильтрТаблицаСопоставления.СписокВыбора.НайтиПоЗначению("");
	Иначе
		Пояснение = СтрШаблон(Элементы.ПояснениеДляПрикладнойЗагрузки.Заголовок, ПараметрыЗагрузки.ПредставлениеОбъекта);
	КонецЕсли;
...
Процедура СопоставитьЗагружаемыеДанные()
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		Если ПустаяСтрока(ШаблонСДаннымиОднаКолонка) Тогда
			ПоказатьПредупреждение(, ("Для вставки сопоставленных данных в список, заполните текстовое поле."));
	...
	ИначеЕсли ТаблицаСДаннымиПустая() Тогда
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, ("Для сопоставления и загрузки данных, заполните таблицу."));
		Иначе
			ПоказатьПредупреждение(, ("Невозможно выполнить сопоставление данных, т.к данные не были загружены в таблицу. 
	...
	Если СписокНезаполненныхКолонок.Количество() > 0 Тогда
		Если СписокНезаполненныхКолонок.Количество() = 1 Тогда
			ТекстПроКолонки = "Обязательная колонка""" + " " + СписокНезаполненныхКолонок[0] + """ содержит незаполненные строки, эти строки будут пропущены при загрузке.";
		Иначе
			ТекстПроКолонки = "Обязательные колонки""" + " " + СтрСоединить(СписокНезаполненныхКолонок,", ") + """ содержат незаполненные строки, эти строки будут пропущены при загрузке.";
		КонецЕсли;
		ТекстПроКолонки = ТекстПроКолонки + Символы.ПС + "Продолжить?";
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхПослеПроверки()
	...
	Иначе
		...
		БазоваяПодсистемаКлиент.ДО_ОжидатьЗавершение(ФоновоеЗадание, Обработчик, НастройкиОжидания);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаписатьЗагружаемыеДанныеКлиент()
	...
	Элементы.ОткрытьСправочникПослеЗакрытияПомощника.Заголовок	= СтрШаблон(Элементы.ОткрытьСправочникПослеЗакрытияПомощника.Заголовок, ПредставлениеОбъекта);
	Элементы.ПояснениеОтчетаОЗагрузке.Заголовок					= СтрШаблон(Элементы.ПояснениеОтчетаОЗагрузке.Заголовок, ПредставлениеОбъекта);
...
Процедура ПоказатьОтчет(АдресРезультата)
	...
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("ВсеЭлементы",	"Все (" + Отчет.Всего + ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Новые",			"Новые (" + Отчет.Создано+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Обновленные",	"Обновленные (" + Отчет.Обновлено+ ")");
	Элементы.ФильтрОтчет.СписокВыбора.Добавить("Пропущенные",	"Пропущенные (" + Отчет.Пропущено+ ")");
...
Процедура ВывестиСообщениеОбОшибке(ТекстОшибкиДляПользователя, ТехническаяИнформация)
	...
	ТекстСообщенияОбОшибке = СтрШаблон(ТекстСообщенияОбОшибке, ТехническаяИнформация);
	БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(ТекстСообщенияОбОшибке, Неопределено, "", "", Ложь);
КонецПроцедуры
...
Процедура ВыполнитьШагСопоставлениеЗагружаемыхДанныхКлиент()
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		...
		Если Статистика.Сопоставленных > 0 Тогда
			ТекстНайдено										= "Из %1 введенных строк в список будут вставлены: %2.";
			Элементы.НадписьРезультатСопоставления.Заголовок 	= СтрШаблон(ТекстНайдено, Статистика.Всего, Статистика.Сопоставленных);

			Если Статистика.Неоднозначных > 0 И Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = "Будет пропущено строк: %3
				|  - Нет данных в программе: %1
				|  - Несколько вариантов для вставки: %2";
				ТекстНеНайдено = СтрШаблон(ТекстНеНайдено, Статистика.НеНайдено, Статистика.Неоднозначных, Статистика.Несопоставленных);
			ИначеЕсли Статистика.Неоднозначных > 0 Тогда
				ТекстНеНайдено = "Строки, для которых в программе имеется несколько вариантов, будут пропущены: %1";
				ТекстНеНайдено = СтрШаблон(ТекстНеНайдено, Статистика.Неоднозначных);
			ИначеЕсли Статистика.НеНайдено > 0 Тогда
				ТекстНеНайдено = "Строки, для которых в программе нет соответствующих данных, будут пропущены: %1";
				ТекстНеНайдено = СтрШаблон(ТекстНеНайдено, Статистика.НеНайдено);
			КонецЕсли;
			ТекстНеНайдено											= ТекстНеНайдено + Символы.ПС + "Для просмотра пропущенных строк и подбора данных для вставки нажмите ""Далее"".";
			...
			ПоказатьСтатистикуПоСопоставлениюЗагрузкаИзФайла();
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПоискаСсылок, Ложь, "Далее >");
		Иначе
	...
	Иначе
		...
		Если ТипЗагрузки = "УниверсальнаяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Истина, Элементы.ПояснениеДляСопоставленияДанных, Истина, "Загрузить данные >");
		ИначеЕсли ТипЗагрузки = "ТабличнаяЧасть" Тогда
			...
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляТабличнойЧасти, Истина, "Загрузить данные");
			УстановитьОформлениеДляПолейСНеоднозначностью(Новый Структура("РезультатСопоставленияСтроки", "Неоднозначность"));
		ИначеЕсли ТипЗагрузки = "ВнешняяЗагрузка" Тогда
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, "Загрузить данные >");
		Иначе
			УстановитьОформлениеДляСтраницыСопоставления(Ложь, Элементы.ПояснениеДляПрикладнойЗагрузки, Ложь, "Загрузить данные >");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПодготовитьСписокДляОповещения(СопоставленныеДанные, СсылкиНаОбъектыОповещения)
	...
	СсылкиНаОбъектыОповещения = БазоваяПодсистемаСервер.СП_ПодготовитьОповещениеФормОбИзменении(СсылкиНаОбъекты);
КонецПроцедуры
...
Процедура УстановитьОформлениеДанных(СписокКолонок = Неопределено)
	...
	Если ЗначениеЗаполнено(СписокКолонок) Тогда
		Для каждого ИмяКолонки Из СписокКолонок Цикл
			...
			ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", "<неоднозначность>");
		КонецЦикла;
	Иначе
		ЭлементУсловногоОформления		= УсловноеОформление.Элементы.Добавить();
		ПолеОформления					= ЭлементУсловногоОформления.Поля.Элементы.Добавить();
...
Функция УсловияПоВыбраннымКолонкам(ИмяСправочника)
	...
	ВидыКонтактнойИнформации = КонтактнаяИнформацияСервер.УКИ_ВидыКонтактнойИнформацииОбъекта(Справочники[ИмяСправочника].ПустаяСсылка());
		...
		Если Элемент.Пометка Тогда
			...
			Если Колонка <> Неопределено Тогда
				...
				Если СтрНачинаетсяС(Колонка.ИмяКолонки, "КонтактнаяИнформация_") Тогда
					ИмяВидаКИ		= БазоваяПодсистемаСервер.СП_ПреобразоватьАдаптированноеНаименованиеКолонкиВСтроку(Сред(Колонка.ИмяКолонки, СтрДлина("КонтактнаяИнформация_") + 1));
...
Процедура ВыполнитьСопоставлениеПоВыбранномуРеквизиту(КоличествоСопоставленных = 0, СписокКолонокСопоставления = "")
	...
	Для каждого Колонка Из СопоставитьПоКолонке Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ДобавитьСтатистическуюИнформацию("СопоставлениеКолонок", КоличествоСопоставленных, СписокКолонокСопоставления);
...
Процедура ПоместитьДанныеВТаблицуСопоставления(АдресЗагруженныхДанных, АдресКопииТабличнойЧасти, СписокНеоднозначностей)
	...
	Для каждого Строка Из ТабличнаяЧасть Цикл
		...
		Для каждого Колонка Из ТабличнаяЧасть.Колонки Цикл
			...
			Если ЗначениеЗаполнено(КолонкиОбязательныеДляЗаполнения.Получить(Колонка.Имя)) И ЗаполненыВсеОбязательныеКолонки И НЕ ЗначениеЗаполнено(Строка[Колонка.Имя]) Тогда
...
Процедура ЗаписатьСопоставленныеДанныеПрикладнаяЗагрузка(СопоставленныеДанные)
	...
	ПараметрыЗагрузки						= ЗагрузкаДанныхИзФайлаСервер.ЗДФ_НастройкиЗагрузкиДанных();
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	Если Элементы.СтраницыЗаполненияДанных.ТекущаяСтраница = Элементы.СтраницаВариантЗагрузкаИзФайла Тогда
		ТекстПодсказки = "Для загрузки данных необходимо, сохранить бланк в файл для заполнения в другой программе. 
	...
	Иначе
		ТекстПодсказки = "Для заполнения таблицы скопируйте данные в таблицу из внешнего файла через буфер обмена." + Символы.ПС;
	КонецЕсли;
	...
	Если ОбязательныеКолонки.Количество() > 0 Тогда
		...
		Если ОбязательныеКолонки.Количество() = 1 Тогда
			ТекстПодсказки = ТекстПодсказки + "Колонка, обязательная для заполнения:" + " " + СписокКолонок;
		Иначе
			ТекстПодсказки = ТекстПодсказки + "Колонки, обязательные для заполнения:" + " " + СписокКолонок;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьСтандартныеКолонкиВТаблицуСопоставления(ВременнаяТЗ, СтруктураОбъектаСопоставления, ДобавитьИдентификатор, ДобавитьОписаниеОшибки, ДобавитьРезультатСопоставленияСтроки, ДобавитьСписокНеоднозначностей)
	Если ДобавитьИдентификатор Тогда
		ВременнаяТЗ.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число"), "п/п");
	КонецЕсли;
	...
	Если ДобавитьРезультатСопоставленияСтроки Тогда
		ВременнаяТЗ.Колонки.Добавить("РезультатСопоставленияСтроки", Новый ОписаниеТипов("Строка"), "Результат");
	КонецЕсли;
	Если ДобавитьОписаниеОшибки Тогда
		ВременнаяТЗ.Колонки.Добавить("ОписаниеОшибки", Новый ОписаниеТипов("Строка"), "Причина");
	КонецЕсли;
...
&НаСервере
Процедура ДобавитьСтандартныеКолонкиВМассивРеквизитов(МассивРеквизитов, ОписаниеТипаОбъектаСопоставления , ДобавитьИдентификатор, ДобавитьОписаниеОшибки, ДобавитьРезультатСопоставленияСтроки, ДобавитьСписокНеоднозначностей)
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонках()
	...
	Синоним = ?(ЗначениеЗаполнено(ПараметрыЗагрузки.ПредставлениеОбъекта), ПараметрыЗагрузки.ПредставлениеОбъекта, ОбъектМетаданных.Представление());

	ВременнаяТЗ  = ЗагрузкаДанныхИзФайлаСервер.ЗДФ_ОписаниеЗагружаемыхДанныхДляСправочников(ВременнаяТЗ, ОписаниеТипаОбъектаСопоставления, Синоним);
...
Процедура СоздатьТаблицуСопоставленияПоИнформацииОКолонкахДляТЧ()
	...
	Для каждого Колонка Из РеквизитыТЧ Цикл
		...
		ТипРеквизита = ?(Колонка.Тип.СодержитТип(Тип("УникальныйИдентификатор")), Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(36)), Колонка.Тип);
		...
		Если СтрНачинаетсяС(Колонка.Имя, "ТЧ_") Тогда
			...
			Если КолонкиГруппировки.Количество() = 1 Тогда
				...
				Если КолонкаУровень2 <> Неопределено Тогда
					...
					Если ТипКолонки <> Неопределено И СтрНайти(ТипКолонки.ПолноеИмя(), "Справочник") > 0 Тогда
						НовыйЭлемент.Заголовок	= "Данные из файла";
					Иначе
...
Процедура ОчисткаШаблонСДанными()
	НомерСтрокиСШапкойТаблицы	= ?(ЗагрузкаДанныхИзФайлаКлиентСервер.ЗДФ_КолонкиИмеютГруппировку(ИнформацияПоКолонкам), 2, 1);
...
Процедура ПослеВыбораФайлаДляСохранения(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		...
		ВыбранныйФайл	= БазоваяПодсистемаКлиентСервер.ОН_РазложитьПолноеИмяФайла(ПутьКФайлу);
		РасширениеФайла	= БазоваяПодсистемаКлиентСервер.ОН_РасширениеБезТочки(ВыбранныйФайл.Расширение);
		...
		Если ЗначениеЗаполнено(ВыбранныйФайл.Имя) Тогда
			...
			Иначе
				...
				Иначе
					ПоказатьПредупреждение(, "Шаблон файла не был сохранен.");
...
Процедура ЗагрузитьДанныеИзФайлаВШаблон(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		...
		Расширение									= БазоваяПодсистемаКлиентСервер.ОН_РасширениеБезТочки(БазоваяПодсистемаКлиентСервер.ОН_ПолучитьРасширениеИмениФайла(ИмяФайла));
...
Процедура ПослеВыбораРасширенияФайла(Результат, Параметр) Экспорт
	Если ЗначениеЗаполнено(Результат) Тогда
		...
		БазоваяПодсистемаКлиент.ФС_СохранитьФайл(Неопределено, АдресВоВременномХранилище, ИмяОбъектаСопоставления + "." + Результат, ПараметрыСохраненияФайла);
	КонецЕсли;
КонецПроцедуры
...
Процедура СохранитьШаблонВоВременноеХранилище(РасширениеФайла, АдресВоВременномХранилище)
	...
	БазоваяПодсистемаСервер.ФС_УдалитьВременныйФайл(ИмяФайла);
КонецПроцедуры
...
Процедура ОбновитьНаименованияКолонокТаблицыСопоставления()
	Для каждого СтрокаТаблицы Из ИнформацияПоКолонкам Цикл
		...
		Если Колонка <> Неопределено Тогда
			Колонка.Заголовок = ?(НЕ ПустаяСтрока(СтрокаТаблицы.Синоним), СтрокаТаблицы.Синоним + " (" + СтрокаТаблицы.ПредставлениеКолонки +")", СтрокаТаблицы.ПредставлениеКолонки);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ИзменитьБланкПоИнформацииПоКолонкам(Бланк = Неопределено, СохранитьНастройки = Ложь)
	...
	Если СохранитьНастройки Тогда
		БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеОбщихНастроек, "ЗагрузкаДанныхИзФайла", ИмяОбъектаСопоставления, ТаблицаКолонок, Неопределено, ИмяПользователя(), Ложь);
	КонецЕсли;
...
Функция ЗагрузитьФайлСДаннымиВТабличныйДокументНаСервере(АдресВременногоХранилища, Расширение)
	...
	ПараметрыВыполненияВФоне = БазоваяПодсистемаСервер.ДО_ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = "Подсистема ЗагрузкаДанныхИзФайла: Выполнение серверного метода загрузка данных из файла";

	ФоновоеЗадание = БазоваяПодсистемаСервер.ДО_ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайла.ЗагрузитьФайлВТаблицу", ПараметрыВызоваСервера, ПараметрыВыполненияВФоне);
...
Процедура ПослеЗагрузкиФайлаСДаннымиВТабличныйДокумент(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		СопоставитьЗагружаемыеДанные();
	Иначе
		ВывестиСообщениеОбОшибке("Не удалось произвести загрузку данных.", ФоновоеЗадание.КраткоеПредставлениеОшибки);
	КонецЕсли;
КонецПроцедуры
...
Функция СопоставитьЗагружаемыеДанныеНаСервереУниверсальнаяЗагрузка()
	...
	ПараметрыВыполненияВФоне								= БазоваяПодсистемаСервер.ДО_ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания	= "Заполнение таблицы сопоставления загруженными данными из файла.";

	ФоновоеЗадание = БазоваяПодсистемаСервер.ДО_ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайла.ЗаполнитьТаблицуСопоставленияДаннымиИзШаблонаФон", ПараметрыВызоваСервера, ПараметрыВыполненияВФоне);
...
Процедура ПослеСопоставленияЗагружаемыхДанных(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке("Не удалось произвести сопоставление данных.", ФоновоеЗадание.КраткоеПредставлениеОшибки);
	КонецЕсли;
КонецПроцедуры
...
Функция ЗаписатьЗагружаемыеДанныеОтчетУниверсальнаяЗагрузка()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания	= "Запись загруженных данных из файла";

	Возврат БазоваяПодсистемаСервер.ДО_ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайла.ЗаписатьСопоставленныеДанные", ПараметрыВызоваСервера, ПараметрыВыполнения);
КонецФункции
...
Процедура ПослеЗаписиЗагружаемыхДанныхОтчет(ФоновоеЗадание, ДополнительныеПараметры) Экспорт
	...
	Если ФоновоеЗадание.Статус = "Ошибка" Тогда
		ВывестиСообщениеОбОшибке("Не удалось произвести запись данных.", ФоновоеЗадание.КраткоеПредставлениеОшибки);
...
Функция СформироватьОтчетОЗагрузке(ТипОтчета = "ВсеЭлементы",  РассчитыватьПроцентПрогресса = Ложь)
	...
	ПараметрыВыполнения								= БазоваяПодсистемаСервер.ДО_ПараметрыВыполненияВФоне(ЭтотОбъект.УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания	= "Создание отчета о загрузке данных из файла";

	Возврат БазоваяПодсистемаСервер.ДО_ВыполнитьВФоне("Обработки.ЗагрузкаДанныхИзФайла.СформироватьОтчетОЗагрузкеФон", ПараметрыВызоваСервера, ПараметрыВыполнения);
КонецФункции
...
Процедура ПослеСозданияОтчета(Задание, ДополнительныеРезультаты) Экспорт
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(Задание.КраткоеПредставлениеОшибки, Неопределено, "", "", Ложь);
...
Функция ПрочитатьПрогресс(ИдентификаторЗадания)
	Возврат БазоваяПодсистемаСервер.ДО_ПрочитатьПрогресс(ИдентификаторЗадания);
КонецФункции
