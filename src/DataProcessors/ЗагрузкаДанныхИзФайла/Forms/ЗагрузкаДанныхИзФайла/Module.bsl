...
Процедура СопоставитьЗагружаемыеДанные()
	Если ТипЗагрузки = "ВставкаИзБуфераОбмена" Тогда
		Если ПустаяСтрока(ШаблонСДаннымиОднаКолонка) Тогда
			ПоказатьПредупреждение(, ("Для вставки сопоставленных данных в список, необходимо заполнить текстовое поле."));
	...
	ИначеЕсли ТаблицаСДаннымиПустая() Тогда
		Если ВариантЗагрузки = 0 Тогда
			ПоказатьПредупреждение(, ("Для сопоставления и загрузки данных, необходимо заполнить таблицу."));
		Иначе
...
Процедура ПоказатьИнформационнуюСтрокуПроОбязательныеКолонки()
	...
	Иначе
		ТекстПодсказки = "Для заполнения таблицы необходимо скопировать данные в таблицу из внешнего файла через буфер обмена." + Символы.ПС;
	КонецЕсли;
