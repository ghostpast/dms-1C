...
Процедура СброситьНастройки(Команда)
	...
	ПоказатьВопрос(Оповещение, "Установить настройки колонок в первоначальное состояние?", РежимДиалогаВопрос.ДаНет);
КонецПроцедуры
...
Процедура УстановитьОформлениеДанных()
	...
	ЭлементОтбора.Использование	= Истина;
	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", "Стандартное наименование");
...
Процедура СброситьНастройкиКолонок(ИмяОбъектаСопоставления)
	БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеОбщихНастроек, "ЗагрузкаДанныхИзФайла", ИмяОбъектаСопоставления, Неопределено, Неопределено, ИмяПользователя(), Ложь);
