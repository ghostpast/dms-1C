...
Функция сП_ОбъектАвторизацииИспользуется(Знач СсылкаНаОбъектАвторизации, Знач СсылкаНаТекущегоВнешнегоПользователя, НайденныйВнешнийПользователь = Неопределено, ЕстьПравоДобавленияВнешнегоПользователя = Ложь, ТекстОшибки = "") Экспорт
	...
	НачатьТранзакцию();
	Попытка
		ОбъектАвторизации = сП_ОбъектАвторизации(СсылкаНаОбъектАвторизации, СсылкаНаТекущегоВнешнегоПользователя);

		Если ОбъектАвторизации.Используется Тогда
			НайденныйВнешнийПользователь = ОбъектАвторизации.Ссылка;
		КонецЕсли;
	...
	Возврат ОбъектАвторизации.Используется;
КонецФункции

Функция сП_ОбъектАвторизации(СсылкаНаОбъектАвторизации, СсылкаНаТекущегоВнешнегоПользователя)
	Запрос			= Новый Запрос;
	Запрос.Текст	=
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	ВнешниеПользователи.Ссылка
	...
	ОбъектАвторизации = Новый Структура;
	ОбъектАвторизации.Вставить("Используется", Выборка.Следующий());
	ОбъектАвторизации.Вставить("Ссылка", Выборка.Ссылка);
