...
Процедура сП_УстановитьНачальныеНастройки(Знач ИмяПользователя, ЭтоВнешнийПользователь = Ложь) Экспорт
	...
	Если Не (БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсии(Информация.ВерсияПриложения, "8.3.15.0") >= 0 И Метаданные.РежимСовместимости = Метаданные.СвойстваОбъектов.РежимСовместимости.НеИспользовать) Тогда
...
Процедура сП_ЗапомнитьСвойстваПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ПользовательОбъект.ЭтоНовый() Или ПользовательОбъект.Ссылка <> СтарыйПользователь.Ссылка Тогда
		...
		СтарыйПользователь.ИдентификаторПользователяСервиса	= Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
...
Функция сП_ОписаниеПользователя(Пользователь) Экспорт
	...
	СвойстваПользователя = ?(ТипЗнч(Пользователь) = Тип("СправочникОбъект.Пользователи"),
		Пользователь,
		БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта(Пользователь,
			" Наименование,
			| ИдентификаторПользователяИБ,
			| Фотография,
			| ПометкаУдаления,
...
Процедура сП_ЗаполнитьНазначениеГруппВнешнихПользователей() Экспорт
	Запрос			= Новый Запрос;
	Запрос.Текст	=
	"ВЫБРАТЬ
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбновлениеВерсииИБСервер.ОИБ_ЗаписатьДанные(ГруппаОбъект);
	КонецЦикла;
КонецПроцедуры
...
Процедура сП_ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура					= "ПользователиСервер.сП_ЗаполнитьНазначениеГруппВнешнихПользователей";
