...
Функция П_СвойстваПроверяемогоПользователяИБ(Пользователь) Экспорт
...
Функция П_СоздатьАдминистратора(ПользовательИБ = Неопределено) Экспорт
	...
	Иначе
		Если Не сП_РолиАдминистратораДоступны(ПользовательИБ) Тогда
			ТекстОшибки = СтрШаблон("Невозможно создать пользователя в справочнике для пользователя
...
Процедура П_УдалитьПользователяИБ(Знач ИмяИлиИдентификатор) Экспорт
	...
	Если СвойстваУдаляемогоПользователяИБ = Неопределено Тогда
		ТекстОшибки = СтрШаблон("Пользователь информационной базы ""%1"" не существует.", ИмяИлиИдентификатор);
	...
	Исключение
		ТекстОшибки = П_ОписаниеОшибкиПриЗаписиПользователяИБ(
	...
	Если ТипЗнч(Приемник) = Тип("ПользовательИнформационнойБазы")
	 ...
	 Или ТипЗнч(Источник) = Тип("ПользовательИнформационнойБазы")
	   ...
	   И ТипЗнч(Приемник) <> Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ТекстОшибки = СтрШаблон("Недопустимое значение параметра %1 или %2
			           |в процедуре %3 общего модуля %4.", "Приемник", "Источник", "П_СкопироватьСвойстваПользователяИБ", "Пользователи");
					...
					Если Свойство = "Имя"
					   И Приемник[Свойство] <> ЗначениеСвойства Тогда
						...
						Если СтрДлина(ЗначениеСвойства) > 64 Тогда
							ТекстОшибки = СтрШаблон("Ошибка записи пользователя информационной базы
								           |Имя (для входа): ""%1""
...
Процедура П_УстановитьНастройкиВхода(СохраняемыеНастройки, ДляВнешнихПользователей = Ложь) Экспорт
	Блокировка = Новый БлокировкаДанных();
	...
	Попытка
		...
		Для Каждого СохраняемаяНастройка Из СохраняемыеНастройки Цикл
			Если Не НастройкиВхода.Свойство(СохраняемаяНастройка.Ключ) Или ТипЗнч(НастройкиВхода[СохраняемаяНастройка.Ключ]) <> ТипЗнч(СохраняемаяНастройка.Значение) Тогда
				Продолжить;
			КонецЕсли;
...
Функция П_НовоеОписаниеНастроекВхода() Экспорт
...
Функция сП_АвторизоватьТекущегоПользователя(ПриНачалеРаботыСистемы = Ложь, РегистрироватьВЖурнале = Ложь) Экспорт
	...
	Если ПустаяСтрока(ТекущийПользовательИБ.Имя) Тогда
		...
		Исключение
			...
			ШаблонОшибки		= СтрШаблон("Не удалось установить параметр сеанса %1 по причине:
				           |""%2"".
				           |
				           |Обратитесь к администратору.", "ТекущийПользователь");

			Возврат сП_КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(ИнформацияОбОшибке, ШаблонОшибки, РегистрироватьВЖурнале);
		КонецПопытки;
...
Функция сП_ПроверитьПраваПользователя(ПользовательИБ, РежимПроверки, ЭтоВнешнийПользователь, ВызыватьИсключение = Истина, РегистрироватьВЖурнале = Истина) Экспорт
	...
	НедоступныеРоли = сП_НедоступныеРолиПоТипуПользователя(ЭтоВнешнийПользователь);
...
Функция сП_ЗначенияПараметровСеансаТекущийПользователь()
	ЗаголовокОшибки = СтрШаблон("Не удалось установить параметр сеанса %1.", "ТекущийПользователь") + Символы.ПС;
...
Функция сП_НастройкиВхода() Экспорт
	Настройки = П_НовоеОписаниеНастроекВхода();
	...
	Для Каждого ПодготовленнаяНастройка Из ПодготовленныеНастройки Цикл
		Если Не СохраненныеНастройки.Свойство(ПодготовленнаяНастройка.Ключ) Или ТипЗнч(СохраненныеНастройки[ПодготовленнаяНастройка.Ключ]) <> Тип("Структура") Тогда
			Продолжить;
		КонецЕсли;
		...
		Для Каждого НачальнаяНастройка Из НачальныеНастройки Цикл
			Если Не ТекущиеНастройки.Свойство(НачальнаяНастройка.Ключ) Или ТипЗнч(ТекущиеНастройки[НачальнаяНастройка.Ключ]) <> ТипЗнч(НачальнаяНастройка.Значение) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура сП_ПроверитьВходВПрограммуРазрешен(ОшибкаАвторизации)
	...
	Недействителен				= БазоваяПодсистемаСервер.ОН_ЗначениеРеквизитаОбъекта(АвторизованныйПользователь, "Недействителен");

	Если ПользовательИБ = Неопределено Или Не ЗначениеЗаполнено(ПользовательИБ.Имя) Или П_ВходВПрограммуРазрешен(ПользовательИБ) И (сП_РолиАдминистратораДоступны(ПользовательИБ) Или Недействителен <> Истина) Тогда
		Возврат;
	КонецЕсли;
...
Функция сП_ОшибкаДлиныИлиСложностиПароля(Пароль, Знач МинимальнаяДлинаПароля, СложныйПароль, ИмяДляВхода) Экспорт
	...
	Если СложныйПароль И СтрНайти(ВРег(Пароль), ВРег(ИмяДляВхода)) Тогда
		Возврат "Пароль содержит Имя (для входа)."
			+ Символы.ПС + Символы.ПС
			+ "Надежный пароль:
		           |- имеет не менее 7 символов;
		           |- содержит любые 3 из 4-х типов символов: заглавные
	...
	Попытка
		...
		ИначеЕсли Не (ВызовИзФормыСменаПароля И Параметры.ТолькоПроверить) Тогда
			...
			Иначе
				...
				сП_ОбновитьСведенияОбИспользованииПароля(СведенияОПользователе, СохраняемоеЗначениеПароля);
...
Процедура сП_НачатьОбработкуПользователяИБ(ПользовательОбъект, ПараметрыОбработки,УдалениеПользователяИзСправочника = Ложь) Экспорт
	...
	Если НЕ ОписаниеПользователяИБ.Свойство("Действие") Тогда
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           |В параметре %2 не указано свойство %3.", ПользовательОбъект.Ссылка, "ОписаниеПользователяИБ", "Действие");
	...
	Если ОписаниеПользователяИБ.Действие <> "Записать"
	   И ОписаниеПользователяИБ.Действие <> "Удалить" Тогда
		...
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           ...
			           |неверное значение ""%3"" свойства %4.", ПользовательОбъект.Ссылка, "ОписаниеПользователяИБ", ОписаниеПользователяИБ.Действие, "Действие");
	...
	Если ОписаниеПользователяИБ.Действие = "Записать"
	   ...
	   И ОписаниеПользователяИБ.УникальныйИдентификатор <> ПараметрыОбработки.СтарыйПользователь.ИдентификаторПользователяИБ Тогда
		...
		Если сП_ПользовательПоИдентификаторуСуществует(ОписаниеПользователяИБ.УникальныйИдентификатор, ПользовательОбъект.Ссылка, НайденныйПользователь) Тогда
			ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
...
Процедура сП_УстановкаПараметровСеанса(Знач ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
		ТекстОшибки = СтрШаблон("Не удалось установить параметр сеанса %1 по причине:
			           |""%2"".
			           |
			           |Обратитесь к администратору.", "ТекущийПользователь", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
Процедура сП_ЗаписатьПользователяИБ(ПользовательОбъект, ПараметрыОбработки)
	...
	Если ПарольЗадан Или Не ХешПароляЗадан И ПроверитьПустойПароль Тогда
		ИмяДляВхода = ?(ОписаниеПользователяИБ.Свойство("Имя"), ОписаниеПользователяИБ.Имя, ?(ПараметрыОбработки.СтарыйПользовательИБСуществует, ПараметрыОбработки.ОписаниеСтарогоПользователяИБ.Имя, ""));
...
Процедура сП_ПроверитьИзмененияРеквизитовПользователя(ПользовательОбъект, ПараметрыОбработки)
	...
	Если АвтоРеквизиты.ИдентификаторПользователяИБ <> ПользовательОбъект.ИдентификаторПользователяИБ Тогда
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           ...
			           |Обновление реквизита выполняется автоматически.", ПользовательОбъект.Ссылка, "ИдентификаторПользователяИБ");
	...
	Если Не БазоваяПодсистемаСервер.ОН_ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ, ПользовательОбъект.СвойстваПользователяИБ) Тогда
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           |Реквизит %2 не допускается изменять.
			           |Обновление реквизита выполняется автоматически.", ПользовательОбъект.Ссылка, "СвойстваПользователяИБ");
	...
	Если СтарыйПользователь.ПометкаУдаления = Ложь
	   ...
	   И П_ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           |Нельзя помечать на удаление пользователя, которому разрешен вход в программу.", ПользовательОбъект.Ссылка);
	...
	Если СтарыйПользователь.Недействителен = Ложь
	   ...
	   И П_ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           |Нельзя пометить недействительным пользователя, которому разрешен вход в программу.", ПользовательОбъект.Ссылка);
	...
	Если СтарыйПользователь.Подготовлен = Ложь
	   ...
	   И П_ВходВПрограммуРазрешен(ПользовательОбъект.ИдентификаторПользователяИБ) Тогда
		...
		ТекстОшибки = СтрШаблон("Ошибка при записи пользователя ""%1"".
			           |Нельзя пометить подготовленным пользователя, которому разрешен вход в программу.", ПользовательОбъект.Ссылка);
	...
	Если Не ОписаниеПользователяИБ.Свойство("Пароль") Или Не ЗначениеЗаполнено(ОписаниеПользователяИБ.Пароль) Тогда
		...
		Если ЗначениеЗаполнено(НастройкиВхода.МинимальнаяДлинаПароля) Тогда
			...
			Если ПарольЗадан Или ПроверитьПустойПароль Тогда
				БазоваяПодсистемаСервер.ОН_СообщитьПользователю("Установите пароль.",, "СменитьПароль",, Отказ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура сП_ОбработатьИнтерфейсРолей(Действие, Параметры) Экспорт
	...
	Иначе
		ТекстОшибки = СтрШаблон("Ошибка в процедуре %1
			           |Неверное значение параметра Действие: ""%2"".", "ПользователиСервер.сП_ОбработатьИнтерфейсРолей", Действие);
...
Процедура сП_ОбновитьДеревоРолей(Параметры)
	...
	сП_ПодготовитьДеревоРолей(ДеревоРолей.Строки, СкрытьРольПолныеПрава, ПоказатьТолькоВыбранныеРоли, Параметры.КоллекцияРолей, ?(Параметры.Свойство("СтандартныеРолиРасширений"), Параметры.СтандартныеРолиРасширений, Неопределено));
...
Процедура сП_ОбновитьСоставРолей(Параметры)
	...
	Если ИдентификаторСтроки = Неопределено Тогда
		...
		Если Параметры.Свойство("ЗапретИзмененияАдминистративногоДоступа") И Параметры.ЗапретИзмененияАдминистративногоДоступа Тогда
			...
			КонецЕсли;
		КонецЕсли;
		сП_ЗаполнитьСтандартныеРолиРасширений(Параметры);
...
Процедура сП_ПодготовитьДеревоРолей(Знач Коллекция, Знач СкрытьРольПолныеПрава, Знач ПоказатьТолькоВыбранныеРоли, КоллекцияРолей, СтандартныеРолиРасширений)
	Индекс = Коллекция.Количество()-1;

	Пока Индекс >= 0 Цикл
		Строка = Коллекция[Индекс];

		сП_ПодготовитьДеревоРолей(Строка.Строки, СкрытьРольПолныеПрава, ПоказатьТолькоВыбранныеРоли, КоллекцияРолей, СтандартныеРолиРасширений);

		Если Строка.ЭтоРоль Тогда
			Если СкрытьРольПолныеПрава И (ВРег(Строка.Имя) = ВРег("ПолныеПрава") ИЛИ ВРег(Строка.Имя) = ВРег("АдминистраторСистемы") ИЛИ СтандартныеРолиРасширений <> Неопределено И (СтандартныеРолиРасширений.ПолныеПрава.Найти(Строка.Имя) <> Неопределено Или СтандартныеРолиРасширений.АдминистраторСистемы.Найти(Строка.Имя) <> Неопределено)) Тогда
...
Функция сП_ОписаниеПользователя(Пользователь) Экспорт
	...
	ЗначениеКонтактнойИнформации = КонтактнаяИнформацияСервер.УКИ_КонтактнаяИнформацияОбъекта(Пользователь,,,Ложь);

	ВидТелефон		= КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("ТелефонПользователя");
	ВидАдресПочты	= КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("EmailПользователя");
	...
	Если Не ЗначениеЗаполнено(Результат.ИдентификаторПользователяИБ) Тогда
		ПользовательСсылка = ?(ТипЗнч(Пользователь) = Тип("СправочникОбъект.Пользователи"), Пользователь.Ссылка, Пользователь);
		Если ПользовательСсылка = П_СсылкаНеуказанногоПользователя() Тогда
...
Функция сП_НедоступныеРолиПоТипуПользователя(ДляВнешнихПользователей) Экспорт
	...
	ИначеЕсли П_ЭтоПолноправныйПользователь(, Истина) Тогда
	...
	Возврат ПользователиСерверПовтИсп.сП_НедоступныеРоли(НазначениеРолейПользователя);
КонецФункции

Процедура сП_ОбновитьСведенияОбИспользованииПароля(СведенияОПользователе, СохраняемоеЗначениеПароля)
	...
	Если ПользователиСерверПовтИсп.сП_Настройки().ОбщиеНастройкиВхода И ЗначениеЗаполнено(НастройкиВхода.ЗапретитьПовторениеПароляСредиПоследних) Тогда
	...
	ДатаНачалаИспользованияПароля = Неопределено;
...
Процедура сП_ЗаполнитьСтандартныеРолиРасширений(Параметры)
...
Функция сП_ДоступноРедактированиеПочтыДляВосстановленияПароля(УровеньДоступа, Объект) Экспорт
	Если П_ЭтоПолноправныйПользователь() Или УровеньДоступа.ИзменениеТекущего Или Не ЗначениеЗаполнено(Объект.Ссылка) Или (УровеньДоступа.УправлениеСписком И Объект.Подготовлен) Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура сП_ОбновитьПредопределенныеВидыКонтактнойИнформацииПользователей() Экспорт
	ПараметрыВида									= КонтактнаяИнформацияСервер.УКИ_ПараметрыВидаКонтактнойИнформации("АдресЭлектроннойПочты");
	ПараметрыВида.Вид								= КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("EmailПользователя");
	ПараметрыВида.МожноИзменятьСпособРедактирования	= Истина;
	ПараметрыВида.РазрешитьВводНесколькихЗначений	= Истина;
	ПараметрыВида.Порядок							= 1;
	КонтактнаяИнформацияСервер.УКИ_УстановитьСвойстваВидаКонтактнойИнформации(ПараметрыВида);

	ПараметрыВида									= КонтактнаяИнформацияСервер.УКИ_ПараметрыВидаКонтактнойИнформации("Телефон");
	ПараметрыВида.Вид								= КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("ТелефонПользователя");
	ПараметрыВида.МожноИзменятьСпособРедактирования	= Истина;
	ПараметрыВида.РазрешитьВводНесколькихЗначений	= Истина;
...
Процедура сП_ПеренестиНастройкиДлиныИСложностиПаролейКонфигуратора() Экспорт
	...
	ИмяКонстанты = ?(Метаданные.Константы.Найти("УдалитьНастройкиВходаПользователей") = Неопределено, "НастройкиВходаПользователей", "УдалитьНастройкиВходаПользователей");
...
Функция сП_ХешСуммаРолейПользователяИБ(ПользовательИБ)
...
Процедура сП_ПриДобавленииСерверныхОповещений(Оповещения) Экспорт
	Оповещение						= СерверныеОповещенияСервер.СО_НовоеСерверноеОповещение("СтандартныеПодсистемы.Пользователи.ИзменениеРолейПользователяИлиБлокировка");
	Оповещение.ИмяМодуляОтправки	= "ПользователиСервер";
	Оповещение.ИмяМодуляПолучения	= "ПользователиКлиент";
	Оповещение.Параметры			= сП_ХешСуммаРолейПользователяИБ(ПользователиИнформационнойБазы.ТекущийПользователь());
...
Процедура сП_ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий		= "Перенос настроек входа пользователей";
КонецПроцедуры
...
Функция сП_ПользователиДляВключенияВосстановленияПароля() Экспорт
	ПользователиИБСПочтой = сП_ПользователиCЭлектроннойПочтойДляВосстановления();

	Если ПолучитьФункциональнуюОпцию("ИспользоватьВнешнихПользователей") Тогда
		Возврат сП_ПользователиДляВключенияВосстановленияПароляСУчетомВнешнихПользователей(ПользователиИБСПочтой);
	КонецЕсли;
	...
	КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("EmailПользователя"));

	РезультатЗапроса = Запрос.Выполнить().Выгрузить();

	Возврат сП_СписокПользователейДляОбновления(ПользователиИБСПочтой, РезультатЗапроса);
КонецФункции

Функция сП_ПользователиДляВключенияВосстановленияПароляСУчетомВнешнихПользователей(ПользователиИБСПочтой)
	ТипыВнешнихПользователей = Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.Типы();
	...
	Для каждого ТипВнешнегоПользователя Из ТипыВнешнихПользователей Цикл
		Если Не КонтактнаяИнформацияСервер.УКИ_СодержитКонтактнуюИнформацию(ТипВнешнегоПользователя) Тогда
			Продолжить;
		КонецЕсли;
	...
	Запрос.Параметры.Вставить("ТипКонтактнойИнформации", Перечисления.ТипыКонтактнойИнформации["АдресЭлектроннойПочты"]);
	Запрос.УстановитьПараметр("ВидКонтактнойИнформации", КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("EmailПользователя"));
...
Функция сП_СписокПользователейДляОбновления(ПользователиИБСПочтой, РезультатЗапроса)
	...
	Для Каждого ПользователяДляОбновления Из РезультатЗапроса Цикл
		...
		ТекстСообщения = СтрШаблон("Для пользователя ""%1"" не может быть установлена почта %2
			           |для восстановления пароля, т.к. она используется для пользователя
			           |%3 (%4)", Строка(ПользователяДляОбновления.Ссылка), СведенияОПользователе.АдресЭлектроннойПочты, СведенияОПользователе.Имя, СведенияОПользователе.ПолноеИмя);

		ЗаписьЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Предупреждение, Метаданные.Справочники.Пользователи, ПользователяДляОбновления.Ссылка, ТекстСообщения);
	КонецЦикла;

	Возврат СписокПользователей;
КонецФункции

Функция сП_ПользователиCЭлектроннойПочтойДляВосстановления()
	ПользователиИБСПочтой = Новый Соответствие;

	ПользователиИБ = ПользователиИнформационнойБазы.ПолучитьПользователей();
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		СведенияОПользователе = сП_СведенияОПользователеДляОбновленияПочтыДляВосстановления();
...
Функция сП_СведенияОПользователеДляОбновленияПочтыДляВосстановления()
	Результат = Новый Структура;
	Результат.Вставить("АдресЭлектроннойПочты",		"");
...
Функция сП_ИнтерактивноЗапрашиватьВводПароля(УровеньДоступа, Объект) Экспорт
	Если П_ЭтоПолноправныйПользователь() Или УровеньДоступа.УправлениеСписком Или НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Возврат Ложь;
...
Функция сП_ОбновитьПочтуДляВосстановленияПароля(ПользовательСсылка, ОбъектАвторизации = Неопределено) Экспорт
	Результат =  Новый Структура;
	Результат.Вставить("Статус",      "ОбновлениеНеТребуется");
	Результат.Вставить("ТекстОшибки", "");

	ИдентификаторПользователяИБ				= БазоваяПодсистемаСервер.ОН_ЗначениеРеквизитаОбъекта(ПользовательСсылка, "ИдентификаторПользователяИБ");

	ПользовательИБ = П_СвойстваПользователяИБ(ИдентификаторПользователяИБ);
	...
	Если ОбъектАвторизации = Неопределено Тогда
		...
		ТипИлиВидЭлектроннойПочты  = КонтактнаяИнформацияСервер.УКИ_ВидКонтактнойИнформацииПоИмени("EmailПользователя");
		ПолноеИмяОбъектаМетаданных = Метаданные.Справочники.Пользователи.ПолноеИмя();
	Иначе
		Если Не КонтактнаяИнформацияСервер.УКИ_СодержитКонтактнуюИнформацию(ОбъектАвторизации) Тогда
			Возврат Результат;
		КонецЕсли;
		ВладелецКИ                 = ОбъектАвторизации;
		ТипИлиВидЭлектроннойПочты  = Перечисления.ТипыКонтактнойИнформации["АдресЭлектроннойПочты"];
	...
	Попытка
		...
		КонтактнаяИнформацияПользователя = КонтактнаяИнформацияСервер.УКИ_КонтактнаяИнформацияОбъекта(ВладелецКИ, ТипИлиВидЭлектроннойПочты, ТекущаяДатаСеанса(), Ложь);
		...
		Если КонтактнаяИнформацияПользователя.Количество() > 0 Тогда
			...
			Если ЗначениеЗаполнено(СтрокаСПочтой.Значение) Тогда
				ПочтаДляВосстановленияПароля = КонтактнаяИнформацияСервер.УКИ_АдресЭлектроннойПочты(СтрокаСПочтой.Значение);
				Если Не БазоваяПодсистемаКлиентСервер.ОН_АдресЭлектроннойПочтыСоответствуетТребованиям(ПочтаДляВосстановленияПароля) Тогда
					ПочтаДляВосстановленияПароля = "";
				КонецЕсли;
			КонецЕсли;

			Если ПустаяСтрока(ПочтаДляВосстановленияПароля) И ЗначениеЗаполнено(СтрокаСПочтой.ЗначенияПолей) Тогда
				ПочтаДляВосстановленияПароля = КонтактнаяИнформацияСервер.УКИ_АдресЭлектроннойПочты(СтрокаСПочтой.ЗначенияПолей);
				Если Не БазоваяПодсистемаКлиентСервер.ОН_АдресЭлектроннойПочтыСоответствуетТребованиям(ПочтаДляВосстановленияПароля) Тогда
					ПочтаДляВосстановленияПароля = "";
				КонецЕсли;
			КонецЕсли;
			...
			Если БазоваяПодсистемаКлиентСервер.ОН_АдресЭлектроннойПочтыСоответствуетТребованиям(ПочтаДляВосстановленияПароля) Тогда
				ПользовательИБ.АдресЭлектроннойПочты          = ПочтаДляВосстановленияПароля;
				ПользовательИБ.ЗапрещеноВосстанавливатьПароль = Ложь;
				П_УстановитьСвойстваПользователяИБ(ИдентификаторПользователяИБ, ПользовательИБ);
			КонецЕсли;
		КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтрШаблон("Не удалось обработать пользователя %1 по причине: %2", ПользовательСсылка, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));

		ЗаписьЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Предупреждение, Метаданные.Справочники.Пользователи, ПользовательСсылка, ТекстСообщения);

		Если ЗначениеЗаполнено(ПочтаДляВосстановленияПароля) И СтрНайти(ТекстСообщения, ПочтаДляВосстановленияПароля) > 0 Тогда
			Уточнение = "Адрес электронной почты %1 занят другим пользователем и не может быть использован для восстановления пароля.";
			Уточнение = СтрШаблон(Уточнение, ПочтаДляВосстановленияПароля);
			ОбновлениеВерсииИБСервер.ОИБ_ЗарегистрироватьПроблемуСДанными(ПользовательСсылка, Уточнение);
		КонецЕсли;
	КонецПопытки;
