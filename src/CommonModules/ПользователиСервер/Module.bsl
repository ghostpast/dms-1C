...
Процедура сП_ПроверитьВходВПрограммуРазрешен(ОшибкаАвторизации)
	...
	Если ПользовательИБ = Неопределено Или П_ВходВПрограммуРазрешен(ПользовательИБ) И (сП_РолиАдминистратораДоступны(ПользовательИБ) Или Недействителен <> Истина) Тогда
		Возврат;
	КонецЕсли;
...
Функция сП_СписокПользователейДляОбновления(ПользователиИБСПочтой, РезультатЗапроса)
	...
	Для Каждого ПользователяДляОбновления Из РезультатЗапроса Цикл
		...
		ТекстСообщения = СтрШаблон("Для пользователя %1 не может быть установлена почта %2 для восстановления пароля, т.к. она используется для пользователя %3(%4)", ПользователяДляОбновления.Ссылка, СведенияОПользователе.АдресЭлектроннойПочты, СведенияОПользователе.Имя, СведенияОПользователе.ПолноеИмя);
