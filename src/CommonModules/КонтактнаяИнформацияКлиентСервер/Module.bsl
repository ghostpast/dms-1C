...
Функция УКИ_НомерТелефонаСодержитНедопустимыеСимволы(Знач СтрокаПроверки)
	СписокДопустимыхСимволов = "+-.,() wp1234567890";

	Возврат СтрРазделить(СтрокаПроверки, СписокДопустимыхСимволов, Ложь).Количество() > 0;
КонецФункции

Функция УКИ_ОставитьТолькоЦифрыВСтроке(Знач Строка) Экспорт
...
Функция УКИ_ОшибкиЗаполненияТелефона(СведенияОТелефоне, МодульДополнительныхПроверок = Неопределено) Экспорт
	СписокОшибок		= Новый СписокЗначений;
	ПолныйНомерТелефона	= СведенияОТелефоне.КодСтраны + СведенияОТелефоне.КодГорода + СведенияОТелефоне.НомерТелефона;

	КодСтраныТолькоЦифры = УКИ_ОставитьТолькоЦифрыВСтроке(СведенияОТелефоне.КодСтраны);
	Если ЗначениеЗаполнено(СведенияОТелефоне.КодСтраны) И ПустаяСтрока(КодСтраныТолькоЦифры) Тогда
		СписокОшибок.Добавить("КодСтраны", "Код страны содержит недопустимые символы");
	КонецЕсли;

	НомерТелефонаТолькоЦифры = УКИ_ОставитьТолькоЦифрыВСтроке(СведенияОТелефоне.Представление);
	Если ПустаяСтрока(НомерТелефонаТолькоЦифры) Тогда
		СписокОшибок.Добавить("НомерТелефона", "Номер телефона не содержит цифр");
	КонецЕсли;

	ПолныйНомерТелефонаТолькоЦифры = УКИ_ОставитьТолькоЦифрыВСтроке(ПолныйНомерТелефона);
	Если СтрДлина(ПолныйНомерТелефонаТолькоЦифры) > 15 Тогда
		СписокОшибок.Добавить("НомерТелефона", "Номер телефона слишком длинный");
	КонецЕсли;

	Если ЗначениеЗаполнено(СведенияОТелефоне.КодСтраны) И УКИ_НомерТелефонаСодержитНедопустимыеСимволы(СведенияОТелефоне.КодСтраны) Тогда
		СписокОшибок.Добавить("КодСтраны", "Код страны содержит недопустимые символы");
	КонецЕсли;

	Если ЗначениеЗаполнено(СведенияОТелефоне.КодГорода) И УКИ_НомерТелефонаСодержитНедопустимыеСимволы(СведенияОТелефоне.КодГорода) Тогда
		СписокОшибок.Добавить("КодГорода", "Код города содержит недопустимые символы");
	КонецЕсли;

	Если ЗначениеЗаполнено(СведенияОТелефоне.НомерТелефона) И УКИ_НомерТелефонаСодержитНедопустимыеСимволы(СведенияОТелефоне.НомерТелефона) Тогда
...
Процедура РСА_ОбновитьПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление, ИсключатьГород = Ложь) Экспорт
	Представление	= РСА_ПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление,, ИсключатьГород);
	Адрес.Value		= Представление;
КонецПроцедуры

Функция РСА_ПредставлениеАдреса(Адрес, ВключатьСтрануВПредставление, ТипАдреса = Неопределено, ИсключатьГород = Ложь) Экспорт
	...
	Для каждого ИмяУровня Из РСА_ИменаУровнейАдреса(ТипАдреса, Истина,, ИсключатьГород) Цикл
...
Функция РСА_ИменаУровнейАдреса(ТипАдреса, ВключатьУровеньУлицы, ВключатьУровеньДома = Ложь, ИсключитьУровеньГорода = Ложь) Экспорт
	Уровни = Новый Массив;
...
Процедура РСА_ПроверитьКорректностьКодовСтраныИГорода(ПоляТелефона, СписокОшибок) Экспорт
	...
	Если КодСтраны = "7" ИЛИ КодСтраны = "+7" Тогда
		Если СтрДлина(УКИ_ОставитьТолькоЦифрыВСтроке(НомерТелефона)) > 7 Тогда
			СписокОшибок.Добавить("НомерТелефона", "В России номер телефона не может быть больше 7 цифр");
		КонецЕсли;
	КонецЕсли;

	Если СтрНачинаетсяС(КодГорода, "9") И СтрДлина(КодГорода) <> 3 Тогда
		СписокОшибок.Добавить("НомерТелефона", "В России номера мобильных телефонов должны содержать 3 цифры");
	КонецЕсли;
КонецПроцедуры

Функция РСА_ВозвращаемыеДанныеОбАдресе() Экспорт
	Результат = Новый Структура;
