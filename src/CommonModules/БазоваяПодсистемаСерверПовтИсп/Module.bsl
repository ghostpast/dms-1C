...
Функция СП_ОписанияПодсистем() Экспорт
	...
	Для Каждого ИмяМодуля Из МодулиПодсистем Цикл
		...
		Если ОписанияПодсистем.ПоИменам.Получить(Описание.Имя) <> Неопределено Тогда
			ТекстОшибки = СтрШаблон("Ошибка при подготовке описаний подсистем:
				           ...
				           |указано имя подсистемы ""%2"", которое уже зарегистрировано ранее.", ИмяМодуля, "ПриДобавленииПодсистемы", Описание.Имя);
	...
	Иначе
		ТекстОшибки = СтрШаблон("Ошибка при подготовке описаний подсистем:
			           ...
			           |не существует описание подсистемы, совпадающей с именем конфигурации ""%2"".", "ПодсистемыКонфигурации.ПриДобавленииПодсистемы", Метаданные.Имя);
...
Функция СП_ОтключитьИдентификаторыОбъектовМетаданных() Экспорт
	...
	ВызватьИсключение СтрШаблон("Невозможно отключить справочник идентификаторов объектов метаданных,
...
Функция СП_ЭтоСеансДлительнойОперации() Экспорт
	УстановитьПривилегированныйРежим(Истина);
...
Функция СП_ИменаКоллекцийПоИменамБазовыхТипов() Экспорт
...
Функция СП_РолиПоКлючамОбъектовМетаданных() Экспорт
