...
Функция сВО_ЗначениеЯчейки(Ячейка, ДоступныеТипы, ДанныеРасшифровки) Экспорт
	...
	Если сВО_ЭтоЧисло(Ячейка.Текст) Тогда
	...
	Значение = сВО_СтрокаВДату(ЗначениеЯчейки.Значение);
...
Функция сВО_ЭтоЧисло(Знач ПроверяемоеЗначение)
...
Функция сВО_СтрокаВДату(Знач Значение)
	...
	Для НомерСимвола = 1 По КоличествоСимволов Цикл
		...
		Если сВО_ЭтоЧисло(Символ) Тогда
...
Функция сВО_ДанныеЭлементаРасшифровки(Форма, Расшифровка) Экспорт
	Если Не сВО_РежимВариантаОтчета(Форма.КлючТекущегоВарианта) Или Не Форма.НастройкиОтчета.РазрешеноИзменятьВарианты Или ТипЗнч(Расшифровка) <> Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция О_СведенияОПользовательскихНастройках(Настройки)
...
Функция О_ИдентификаторыСвойствНастроек(ДополнительныеСвойства)
	...
	Если ИдентификаторыСвойств.Количество() = 0 Тогда
		Возврат БазоваяПодсистемаСервер.ОН_СкопироватьРекурсивно(ИдентификаторыСвойствПоУмолчанию);
	КонецЕсли;
...
Процедура О_ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов, ВидыСтилизованныхЭлементов)
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл
		...
		Если Группа = Неопределено Тогда
			Группа				= О_ГруппаЭлементовФормыНастроек(Элементы, УзелИерархииЭлементов, ИмяГруппы);
		...
		О_ПодготовитьЭлементыФормыНастроекКРаспределению(ЭлементыНастроекГруппы, СвойстваГруппы.Группировка);
		О_РаспределитьЭлементыФормыНастроек(Форма.Элементы, Группа, ЭлементыНастроекГруппы);
	КонецЦикла;
...
Процедура О_ПодготовитьЭлементыФормыНастроекКРаспределению(ЭлементыНастроек, Группировка)
...
Процедура О_РаспределитьЭлементыФормыНастроек(Элементы, Знач Группа, ЭлементыНастроек)
	КоличествоКолонок = 0;
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл
		...
		Если Колонка = Неопределено Тогда
			Колонка				= О_ГруппаЭлементовФормыНастроек(Элементы, Группа, ИмяКолонки);
		...
		Если ТолькоПоляВвода Тогда
			О_РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Колонка, ЭлементыНастроек, НомерКолонки);
		...
		Для Каждого Признаки Из ПризнакиЭлементов Цикл
			...
			Родитель	= О_ИерархияЭлементовФормыНастроек(Элементы, Колонка, Признаки, НомерСтроки, НомерКолонки);

			О_РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Родитель, ЭлементыНастроек, НомерКолонки, Признаки.НомерГруппы);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

Процедура О_РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Родитель, ЭлементыНастроек, НомерКолонки, НомерГруппы = Неопределено)
	...
	Для Каждого СвойствоНастройки Из СвойстваНастроек Цикл
		...
		Группа				= О_ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяГруппы);
...
Функция О_ИерархияЭлементовФормыНастроек(Элементы, Родитель, Признаки, НомерСтроки, НомерКолонки)
	ИмяСтроки			= Родитель.Имя + "Строка" + НомерСтроки;
	Строка				= О_ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяСтроки);
	...
	Колонка				= О_ГруппаЭлементовФормыНастроек(Элементы, Строка, ИмяКолонки);
...
Функция О_ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяГруппы)
...
Процедура О_ВывестиПериодыНастроек(Форма, ЭлементыНастроек, ИменаРеквизитов)
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл
		...
		О_ИнициализироватьПериод(Форма, Элемент.ИндексНастройки);
		...
		О_ДобавитьЭлементПериода(Элементы, Группа, ШаблонИмениЭлемента, "ДатаНачала", Поле.Заголовок);
		...
		О_ДобавитьЭлементПериода(Элементы, Группа, ШаблонИмениЭлемента, "ДатаОкончания", Поле.Заголовок);
		О_ДобавитьКомандуВыбораПериода(Форма, Группа, ШаблонИмениЭлемента);
	КонецЦикла;
КонецПроцедуры
...
Процедура О_ДобавитьЭлементПериода(Элементы, Группа, ШаблонИмени, Свойство, ЗаголовокЭлементаНастройки)
	ИмяЭлемента = СтрШаблон(ШаблонИмени, "", Свойство);

	ШаблонЗаголовка	= "%1 (дата %2)";
	Заголовок	= СтрШаблон(ШаблонЗаголовка, ЗаголовокЭлементаНастройки, ?(СтрЗаканчиваетсяНа(Свойство, "Начала"), "начала", "окончания"));
...
Процедура О_ДобавитьКомандуВыбораПериода(Форма, Группа, ШаблонИмени)
...
Процедура О_ИнициализироватьПериод(Форма, Индекс)
	ПользовательскиеНастройки	= Форма.Отчет.КомпоновщикНастроек.ПользовательскиеНастройки;
