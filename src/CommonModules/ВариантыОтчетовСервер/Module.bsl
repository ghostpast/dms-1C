...
Функция ВО_ОписаниеВарианта(Настройки, Отчет, КлючВарианта) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтрШаблон("Недопустимое значение параметра %1 в функции %2:
				|вариант ""%3"" отсутствует в отчете ""%4"".",
			"КлючВарианта", "ВариантыОтчетовСервер.ВО_ОписаниеВарианта", КлючВариантаОтчета, МетаданныеОтчета.Имя);
	КонецЕсли;
...
Функция ВО_ПоляДляПоиска(ВариантОтчета)
	...
	Если СтрСравнить(Метаданные.ОсновнойЯзык.КодЯзыка, ТекущийЯзык().КодЯзыка) = 0 Или ЭтоПользовательскийВариантОтчета Тогда
...
Процедура ВО_НастроитьОтчетВМодулеМенеджера(Настройки, ОтчетМетаданные) Экспорт
	...
	Исключение
		ТекстОшибки = СтрШаблон("Недопустимое значение параметра %1 в процедуре %2.
			|Не удалось настроить варианты отчета из модуля менеджера по причине:
			|%3",
			"КлючВарианта", "ВариантыОтчетовСервер.ВО_НастроитьОтчетВМодулеМенеджера", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВО_ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, ОтчетМетаданные);
	КонецПопытки;
КонецПроцедуры
...
Функция ВО_ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтрШаблон("Недопустимое значение параметра %1 в функции %2.
			...
			"Отчет", "ВариантыОтчетовСервер.ВО_ОписаниеОтчета", Строка(Отчет), "Варианты отчетов");
	КонецЕсли;
...
Функция ВО_ЯзыкиПредставленийВариантовОтчетовДляЗаполнения()
	...
	Исключения.Добавить(Метаданные.ОсновнойЯзык.КодЯзыка);
...
Процедура ВО_ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий							= "Установка источника данных в настройках вариантов универсального отчета.
...
Функция ВО_НайтиВариантыОтчетов(Знач ПараметрыПоиска, Знач ПолучатьИтоговуюТаблицу = Ложь, Знач ПолучатьПодсветку = Ложь) Экспорт
	...
	Иначе
		...
		Если ПараметрыПоиска.Свойство("КлючВариантаБезУсловий") И ЗначениеЗаполнено(ПараметрыПоиска.КлючВариантаБезУсловий) Тогда
			Запрос.УстановитьПараметр("КлючВариантаБезУсловий", ПараметрыПоиска.КлючВариантаБезУсловий);
			ТекстЗапроса	= СтрЗаменить(ТекстЗапроса, "&ОтборКлючВариантаБезУсловий", "ВариантыОтчетов.КлючВарианта = &КлючВариантаБезУсловий");
		Иначе
...
Функция ВО_ТекстЗапросаОтчетовСУточненнымиОтборами()
	...
	ТекстЗапроса	= ТекстЗапросаПредварительный + "
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|" + ТекстЗапросаОбщий;
...
Функция ВО_ЭтоВнешнийОтчет(ОтчетОбъект)
	Если ОтчетОбъект = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ВО_ПодготовитьУсловиеПоискаПоСтроке(Знач Запрос, ТекстЗапроса, Знач ЕстьСтрокаПоиска, Знач ПараметрыПоиска, Знач ПравоЧтенияАвторов)
	Если ЕстьСтрокаПоиска Тогда
		...
		Для НомерСлова = 1 По СловаПоиска.Количество() Цикл
			...
			Запрос.УстановитьПараметр(ИмяСлова, "%" + БазоваяПодсистемаСервер.ОН_СформироватьСтрокуДляПоискаВЗапросе(Слово) + "%");
			ШаблонПоиска	= СтрЗаменить("&ИмяПоляПоиска ПОДОБНО &ИмяПараметра СПЕЦСИМВОЛ ""~""", "ИмяПараметра", ИмяСлова);
		...
		КонецЦикла;
		ВариантыИПодсистемыПоСтрокеПоиска		= "(" + СтрСоединить(ПоляПоиска, " ИЛИ ") + ")";
...
Функция ВО_ОписаниеНастроекВариантаОтчета(ИмяКаталога)
	...
	ШапкаОписанияОшибки = "Некорректный формат описания настроек:" + Символы.ПС;
	...
	Исключение
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтрШаблон("отсутствует файл %1.", ИмяФайла);
	...
	Если ДокументDOM.ДочерниеУзлы.Количество() = 0 Тогда
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтрШаблон("отсутствует элемент %1.", "SettingsDescription");
	...
	Если ИмяОтчета = Неопределено Тогда
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтрШаблон("отсутствует атрибут %1.", ИмяЭлемента);
	...
	Если Состав.Количество() = 0 Тогда
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтрШаблон("отсутствует элемент %1.", ИмяЭлемента);
	...
	Если КлючВарианта = Неопределено Тогда
		ОписаниеНастроек.ОписаниеОшибки = ШапкаОписанияОшибки + СтрШаблон("отсутствует атрибут %1 элемента %2.", ИмяАтрибута, ИмяЭлемента);
...
Функция ВО_ИнформацияОбОтчете(Знач ОтчетПолноеИмя, Знач ВызыватьИсключение = Ложь) Экспорт
	...
	Иначе
		Результат.ТекстОшибки = СтрШаблон("Неизвестный тип отчета ""%1"" (ожидался ""%2"" или ""%3"").", "Отчет", "ВнешнийОтчет", ОтчетПолноеИмя);
...
Функция ВО_ДоступноеНаименованиеВариантаОтчета(Отчет, Знач Наименование)
	...
	Запрос.УстановитьПараметр("Отчет",				Отчет);
	Запрос.УстановитьПараметр("ШаблонНаименования",	БазоваяПодсистемаСервер.ОН_СформироватьСтрокуДляПоискаВЗапросе(Наименование) + "%");
	...
	Для Каждого Пользователь Из ВыбранныеПользователи Цикл
		...
		Попытка
			...
			Если Не Запись.Видимость Тогда
				...
				ОбновлениеВерсииИБСервер.ОИБ_ЗаписатьНаборЗаписей(НаборЗаписей,,, ВключитьБизнесЛогику);
			КонецЕсли;
...
Функция ВО_НастройкиОтчета(ОтчетСсылка, КлючВарианта, ОтчетОбъект)
	...
	Исключение
		Если ВО_ЭтоВнешнийОтчет(ОтчетОбъект) Тогда
			НастройкиОтчета = ВариантыОтчетовКлиентСервер.О_НастройкиОтчетаПоУмолчанию();
		Иначе
			ВызватьИсключение;
		КонецЕсли;
...
Функция ВО_НастройкиФормыОтчета(ОтчетСсылка, КлючВарианта, ОтчетОбъект) Экспорт
	...
	Настройки = ВО_НастройкиОтчета(ОтчетСсылка, КлючВарианта, ОтчетОбъект);
	...
	Настройки.Вставить("СвойстваРезультата",										сВО_СвойстваРезультатаОтчета());
	...
	БазоваяПодсистемаКлиентСервер.ОН_ДополнитьСтруктуру(Настройки, ВО_ПараметрыКлиента());

	Возврат Настройки;
КонецФункции

Функция ВО_СохраняемыеПараметрыФормыОтчета(ПараметрыФормы) Экспорт
	...
	Если ТипЗнч(ПараметрыФормы.Отбор) = Тип("Структура") Тогда
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьСтруктуру(Параметры.Отбор, ПараметрыФормы.Отбор, Истина);
...
Процедура ВО_СформироватьОтчетВФоне(Параметры, АдресХранилища) Экспорт
	Формирование = ВО_СформироватьОтчет(Параметры, Ложь, Ложь);
...
Функция ВО_СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	Результат = Новый Структура("ТабличныйДокумент, Расшифровка,
	...
	Иначе
		Подключение = ВО_ПодключитьОтчетИЗагрузитьНастройки(Параметры);
	КонецЕсли;
	...
	ДопСвойства.Вставить("ИспользуемыеТаблицы", БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ИспользуемыеТаблицы", Новый Массив));
	...
	Попытка
		ИспользуемыеТаблицы = ВО_ИспользуемыеТаблицы(Результат.СхемаКД);
		...
		Результат.ДанныеЕщеОбновляются = ВО_ПроверитьИспользуемыеТаблицы(ИспользуемыеТаблицы, Ложь);
	Исключение
		ТекстОшибки = "Не удалось определить используемые таблицы:";
		ТекстОшибки = ТекстОшибки + Символы.ПС + ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВО_ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, Результат.СсылкаВарианта);
	КонецПопытки;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации("Варианты отчетов.Формирование отчета", УровеньЖурналаРегистрации.Ошибка, Результат.Метаданные, Результат.СсылкаВарианта, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ДопСвойства <> КомпоновщикНастроекКД.ПользовательскиеНастройки.ДополнительныеСвойства Тогда
		...
		БазоваяПодсистемаКлиентСервер.ОН_ДополнитьСтруктуру(НовыеДопСвойства, ДопСвойства, Ложь);
		ДопСвойства = НовыеДопСвойства;
	КонецЕсли;

	Модифицирован = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ДопСвойства, "ВариантМодифицирован");
	...
	Модифицированы = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ДопСвойства, "ПользовательскиеНастройкиМодифицированы");
	...
	Если ПолучатьФлажокПустой Тогда
		...
		ИначеЕсли Результат.ТабличныйДокумент.ВысотаТаблицы = 0 И Результат.ТабличныйДокумент.ШиринаТаблицы = 0 Тогда
			Пустой = Истина
		Иначе
			Пустой = О_ОтчетПустой(ОтчетОбъект);
		КонецЕсли;
		Результат.Вставить("Пустой", Пустой);
	КонецЕсли;

	НастройкиПечати = Результат.НастройкиФормы.Печать;
	НастройкиПечати.Вставить("КлючПараметровПечати", ВариантыОтчетовКлиентСервер.О_КлючУникальности(Результат.ПолноеИмя, Результат.КлючВарианта));
	...
	УК_УстановитьКолонтитулы(Результат.ТабличныйДокумент, НазваниеОтчета,, НастройкиКолонтитулов);
...
Функция ВО_ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
	...
	Если ТипЗнч(Результат.НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") И Результат.КлючВарианта = Неопределено И Результат.Объект = Неопределено И ТипЗнч(Результат.СсылкаВарианта) = ВО_ТипСсылкиДополнительногоОтчета() Тогда
		...
		ПодключениеОтчета			= ВО_ПодключитьОтчетОбъект(Результат.СсылкаОтчета, Истина);
		...
		Результат.СсылкаВарианта	= ВО_ВариантОтчета(Результат.СсылкаОтчета, Результат.КлючВарианта);
	...
	Если НадоПрочитатьСсылкуОтчета Или НадоПрочитатьНастройки Тогда
		...
		СвойстваВарианта	= БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта(Результат.СсылкаВарианта, ИменаСвойств);
		Результат.КлючВарианта = СвойстваВарианта.КлючВарианта;
		Если ЗначениеЗаполнено(СвойстваВарианта.Родитель) Тогда
			Если ТипЗнч(СвойстваВарианта.РодительКлючВарианта) = Тип("Строка") И ЗначениеЗаполнено(СвойстваВарианта.РодительКлючВарианта) Тогда
	...
	Результат.НастройкиФормы = ВО_НастройкиФормыОтчета(Результат.СсылкаОтчета, Результат.КлючВарианта, ОтчетОбъект);
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтрШаблон("Вариант ""%1"" (ключ ""%2"") не существует в схеме отчета ""%3"".", Строка(Результат.СсылкаВарианта), Результат.КлючВарианта, Строка(Результат.СсылкаОтчета));
	...
	Если Не АдресСхемыЗаполнен И ТипЗнч(ОтчетОбъект.СхемаКомпоновкиДанных) = Тип("СхемаКомпоновкиДанных") Тогда
		ИдентификаторФормы = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ИдентификаторФормы");
	...
	О_ЗаполнитьДополнительныеСвойства(ОтчетОбъект, Результат.НастройкиКД, Результат.КлючВарианта, КлючПредопределенногоВарианта);

	Если Результат.НастройкиФормы.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		КлючСхемы = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "КлючСхемы");
	...
	ФиксированныеНастройкиКД = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ФиксированныеНастройкиКД");
	...
	ВариантыОтчетовКлиентСервер.О_ЗагрузитьНастройки(КомпоновщикНастроекКД, Результат.НастройкиКД, Результат.ПользовательскиеНастройкиКД);
...
Процедура сВО_ИнициализироватьЗаголовкиОтчета(Форма) Экспорт
	...
	Если Не ВариантыОтчетовКлиентСервер.сВО_РежимВариантаОтчета(Форма.КлючТекущегоВарианта) Или Не НастройкиОтчета.РазрешеноИзменятьВарианты Или НастройкиОтчета.ОтключитьСтандартноеКонтекстноеМеню Тогда
		Возврат;
	КонецЕсли;
	...
	Для НомерСтроки = 1 По РезультатОтчета.ВысотаТаблицы Цикл
		...
		Если сВО_ПрерватьСканированиеРезультатаОтчета(НачальнаяЯчейка, ИндексСтруктурыОтчета, ГраницаЗаголовкаРаздела) Тогда
			Прервать;
		КонецЕсли;

		Если Не НачальнаяЯчейка.РежимИзмененияРазмераКолонки = РежимИзмененияРазмера.БыстроеИзменение Тогда
			Продолжить;
		КонецЕсли;
		...
		Для НомерКолонки = 1 По РезультатОтчета.ШиринаТаблицы Цикл
			...
			Если Не Ячейка.РежимИзмененияРазмераКолонки = РежимИзмененияРазмера.БыстроеИзменение Тогда
				Продолжить;
			КонецЕсли;
...
Функция сВО_ПрерватьСканированиеРезультатаОтчета(Ячейка, ИндексСтруктурыОтчета, ГраницаЗаголовкаРаздела)
	...
	Если ГраницаЗаголовкаРаздела = Неопределено И Не Ячейка.РежимИзмененияРазмераКолонки = РежимИзмененияРазмера.БыстроеИзменение Тогда
		Возврат Ложь;
	КонецЕсли;

	Если ГраницаЗаголовкаРаздела = Неопределено Или Ячейка.РежимИзмененияРазмераКолонки = РежимИзмененияРазмера.БыстроеИзменение И Ячейка.Верх - ГраницаЗаголовкаРаздела = 1 Тогда
...
Функция сВО_ИндексСтруктурыОтчета(Форма) Экспорт
	...
	Если ИндексСтруктурыОтчета = Неопределено Или Форма.ВариантМодифицирован Тогда
		...
		сВО_ЗаполнитьСвойстваМакетовЗаголовковОтчета(НастройкиОтчета.АдресСхемы, ИндексСтруктурыОтчета);
	...
	СвойстваРезультата.ОсновныеПоля		= сВО_ОсновныеПоляОтчета(Форма);
...
Процедура сВО_ЗаполнитьСвойстваМакетовЗаголовковОтчета(АдресСхемы, ИндексСтруктурыОтчета)
	Схема = ПолучитьИзВременногоХранилища(АдресСхемы);
...
Процедура сВО_ОпределитьДоступныеДействияПолейРазделаОтчета(ИндексСтруктурыОтчета, ПорядокРаздела, ЭтоРесурс = Ложь)
	...
	Для Каждого ПорядокГруппировки Из ПорядокГруппировок Цикл
		...
		Для Каждого ПолеГруппировки Из ПоляГруппировки Цикл
			...
			Индекс.ПереместитьПолеВыше				= Не ЭтоПолеГруппировкиКолонки И ПорядокРаздела = 1 И Не ЭтоРесурс И Индекс.СодержитРодительскиеГруппировки И ПоляГруппировкиВыше.Найти(Не ПолеГруппировки.Период, "Период") = Неопределено;

			Индекс.ПереместитьПолеНиже				= Не ЭтоПолеГруппировкиКолонки И ПорядокРаздела = 1 И Не ЭтоРесурс И Индекс.СодержитДочерниеГруппировки И ПоляГруппировкиНиже.Найти(Не ПолеГруппировки.Период, "Период") = Неопределено;
...
Процедура сВО_ПодготовитьНастройкиОтчетаКРасшифровкеПоДетальнымЗаписям(Форма, НастройкиВарианта, ПрименяемыеНастройки) Экспорт
	ДействиеОбработкиРасшифровки = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ПрименяемыеНастройки.ДополнительныеСвойства, "ДействиеОбработкиРасшифровки");

	Если Не ЗначениеЗаполнено(ДействиеОбработкиРасшифровки) Тогда
		Возврат;
	КонецЕсли;

	СвойстваЗаголовка = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ПрименяемыеНастройки.ДополнительныеСвойства, "СвойстваЗаголовка");
	...
	сВО_УдалитьНеактуальныеРазделаОтчета(Настройки, СвойстваЗаголовка);
	сВО_ПодготовитьСтруктуруРазделаОтчетаКРасшифровке(Настройки);

	НастройкиВарианта = Настройки;
КонецПроцедуры

Процедура сВО_УдалитьНеактуальныеРазделаОтчета(Настройки, СвойстваЗаголовка)
	ИндексРаздела = Настройки.Структура.Количество() - 1;

	Пока ИндексРаздела >= 0 Цикл
		Раздел = Настройки.Структура[ИндексРаздела];

		Если Не Раздел.Использование Или ТипЗнч(Раздел) <> Тип("ГруппировкаКомпоновкиДанных") И ТипЗнч(Раздел) <> Тип("ТаблицаКомпоновкиДанных") Тогда
...
Процедура сВО_ПодготовитьСтруктуруРазделаОтчетаКРасшифровке(Настройки)
...
Процедура О_УстановитьФиксированныеОтборы(СтруктураОтборов, НастройкиКД, НастройкиОтчета) Экспорт
	...
	Для Каждого КлючИЗначение Из СтруктураОтборов Цикл
		...
		Если О_ЭтоОтборОбработчикаРасшифровкиПоДетальнымЗаписям(КлючИЗначение.Значение) Тогда
		...
		БазоваяПодсистемаКлиентСервер.ОН_УстановитьЭлементОтбора(ОтборыКД, Имя, Значение, ВидСравненияКД, Представление, Истина, РежимОтображения, Идентификатор);
	КонецЦикла;
КонецПроцедуры
...
Функция О_ДоступныеНастройки(ПараметрыЗагрузки, НастройкиОтчета) Экспорт
	...
	Если НастройкиОтчета.События.ПередЗагрузкойНастроекВКомпоновщик Тогда
		Если ТипЗнч(НастройкиОтчета.НовыеНастройкиXML) = Тип("Строка") Тогда
			Попытка
				Настройки = БазоваяПодсистемаСервер.ОН_ЗначениеИзСтрокиXML(НастройкиОтчета.НовыеНастройкиXML);
		...
		Если ТипЗнч(НастройкиОтчета.НовыеПользовательскиеНастройкиXML) = Тип("Строка") Тогда
			Попытка
				ПользовательскиеНастройки = БазоваяПодсистемаСервер.ОН_ЗначениеИзСтрокиXML(НастройкиОтчета.НовыеПользовательскиеНастройкиXML);
			Исключение
				ПользовательскиеНастройки = Неопределено;
			КонецПопытки;
...
Процедура О_ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов, ВидыСтилизованныхЭлементов)
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл
		...
		О_УточнитьСвойстваГруппыУзлаИерархииЭлементовФормыНастроек(Группа, СвойстваГруппы);
	...
	О_УточнитьСвойстваГруппУзлаИерархииЭлементовФормыНастроек(УзелИерархииЭлементов);
...
Функция О_ОтчетПустой(ОтчетОбъект, ПроцессорКД = Неопределено) Экспорт
	Если ПроцессорКД = Неопределено Тогда
		...
		Если О_ЕстьВнешнийНаборДанных(МакетКД.НаборыДанных) Тогда
			Возврат Ложь;
		КонецЕсли;
...
Функция О_ЕстьВнешнийНаборДанных(НаборыДанных)
	Для Каждого НаборДанных Из НаборыДанных Цикл
		...
		ИначеЕсли ТипЗнч(НаборДанных) = Тип("НаборДанныхОбъединениеМакетаКомпоновкиДанных") Тогда
			Если О_ЕстьВнешнийНаборДанных(НаборДанных.Элементы) Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция О_ЭтоОтборОбработчикаРасшифровкиПоДетальнымЗаписям(Отбор)
	Если ТипЗнч(Отбор) <> Тип("Структура") Тогда
		Возврат Ложь;
	КонецЕсли;

	СвойстваОтбора = ВариантыОтчетовКлиентСервер.сВО_СвойстваОтбораОбработчикаРасшифровкиПоДетальнымЗаписям();
...
Процедура О_ЗаполнитьДополнительноеСвойство(Настройки, ИмяСвойства, ЗначениеСвойства, ЗначениеПоУмолчанию = "")
...
Процедура О_ЗаполнитьДополнительныеСвойства(ОтчетОбъект, НовыеНастройки, Знач КлючВарианта, Знач КлючПредопределенногоВарианта, Знач КонтекстВарианта = "", Знач ФормаПараметрыОтбор = Неопределено) Экспорт
	Если ТипЗнч(НовыеНастройки) = Тип("НастройкиКомпоновкиДанных") Тогда
		О_ЗаполнитьДополнительноеСвойство(НовыеНастройки, "КлючВарианта", КлючВарианта);
		О_ЗаполнитьДополнительноеСвойство(НовыеНастройки, "КлючПредопределенногоВарианта", КлючПредопределенногоВарианта);
		О_ЗаполнитьДополнительноеСвойство(НовыеНастройки, "КонтекстВарианта", КонтекстВарианта);
		О_ЗаполнитьДополнительноеСвойство(НовыеНастройки, "ФормаПараметрыОтбор", ФормаПараметрыОтбор, Новый Структура);
	КонецЕсли;

	ТекущиеНастройки = ОтчетОбъект.КомпоновщикНастроек.Настройки;
	О_ЗаполнитьДополнительноеСвойство(ТекущиеНастройки, "КлючВарианта", КлючВарианта);
	О_ЗаполнитьДополнительноеСвойство(ТекущиеНастройки, "КлючПредопределенногоВарианта", КлючПредопределенногоВарианта);
	О_ЗаполнитьДополнительноеСвойство(ТекущиеНастройки, "КонтекстВарианта", КонтекстВарианта);
	О_ЗаполнитьДополнительноеСвойство(ТекущиеНастройки, "ФормаПараметрыОтбор", ФормаПараметрыОтбор, Новый Структура);
КонецПроцедуры

Процедура О_УточнитьСвойстваГруппУзлаИерархииЭлементовФормыНастроек(УзелИерархииЭлементов)
	ПоведениеПереопределено = О_ПоведениеГруппУзлаИерархииЭлементовФормыНастроекПереопределено(УзелИерархииЭлементов);
...
Процедура О_УточнитьСвойстваГруппыУзлаИерархииЭлементовФормыНастроек(Группа, СвойстваГруппы)
	ЗаголовокГруппы = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(СвойстваГруппы, "Заголовок", "");
	...
	ГруппаСвернута = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(СвойстваГруппы, "Свернута", Ложь);
...
Функция О_ПоведениеГруппУзлаИерархииЭлементовФормыНастроекПереопределено(УзелИерархииЭлементов)
	Для Каждого Группа Из УзелИерархииЭлементов.ПодчиненныеЭлементы Цикл
...
Процедура О_ВывестиКоманду(ФормаОтчета, КомандаИлиКоманды, ТипГруппы, ВНачалоГруппы = Ложь, ТолькоВоВсехДействиях = Ложь, СуффиксПодгруппы = "") Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон("При вызове процедуры ""%1"" передано недопустимо значение параметра ""%2"".", "ОтчетыСервер.ВывестиКоманду", "ТипГруппы");
	КонецЕсли;
	...
	Для Каждого Команда Из Команды Цикл
		...
		Если ГруппаЕще = Неопределено Тогда
			Кнопка.ПоложениеВКоманднойПанели = ?(ТолькоВоВсехДействиях, ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю, ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели);
...
Функция УК_ЗначениеСвойстваПоШаблону(Шаблон, НазваниеОтчета, Пользователь)
...
Процедура УК_УстановитьКолонтитулы(ТабличныйДокумент, НазваниеОтчета = "", Пользователь = Неопределено, НастройкиКолонтитулов = Неопределено) Экспорт
	Если Пользователь = Неопределено Тогда
		Пользователь = ПользователиСервер.сП_АвторизованныйПользователь();
	КонецЕсли;

	Если НастройкиКолонтитулов = Неопределено Тогда
		НастройкиКолонтитулов = УК_НастройкиКолонтитулов();
	КонецЕсли;

	Если Не УК_КолонтитулУстановлен(ТабличныйДокумент.ВерхнийКолонтитул) Тогда
		СвойстваКолонтитула = УК_СвойстваКолонтитула(НастройкиКолонтитулов.ВерхнийКолонтитул, НазваниеОтчета, Пользователь);
		ЗаполнитьЗначенияСвойств(ТабличныйДокумент.ВерхнийКолонтитул, СвойстваКолонтитула);
	КонецЕсли;

	Если Не УК_КолонтитулУстановлен(ТабличныйДокумент.НижнийКолонтитул) Тогда
		СвойстваКолонтитула = УК_СвойстваКолонтитула(НастройкиКолонтитулов.НижнийКолонтитул, НазваниеОтчета, Пользователь);
		ЗаполнитьЗначенияСвойств(ТабличныйДокумент.НижнийКолонтитул, СвойстваКолонтитула);
	КонецЕсли;
КонецПроцедуры

Функция УК_КолонтитулУстановлен(Колонтитул)
	Возврат ЗначениеЗаполнено(Колонтитул.ТекстСлева) Или ЗначениеЗаполнено(Колонтитул.ТекстВЦентре) Или ЗначениеЗаполнено(Колонтитул.ТекстСправа);
КонецФункции

Функция УК_СвойстваКолонтитула(НастройкиКолонтитула, НазваниеОтчета, Пользователь)
	СвойстваКолонтитула = Новый Структура;
	Если ЗначениеЗаполнено(НастройкиКолонтитула.ТекстСлева) Или ЗначениеЗаполнено(НастройкиКолонтитула.ТекстВЦентре) Или ЗначениеЗаполнено(НастройкиКолонтитула.ТекстСправа) Тогда
		...
		СвойстваКолонтитула.Вставить("ТекстСлева", УК_ЗначениеСвойстваПоШаблону(НастройкиКолонтитула.ТекстСлева, НазваниеОтчета, Пользователь));
		СвойстваКолонтитула.Вставить("ТекстВЦентре", УК_ЗначениеСвойстваПоШаблону(НастройкиКолонтитула.ТекстВЦентре, НазваниеОтчета, Пользователь));
		СвойстваКолонтитула.Вставить("ТекстСправа", УК_ЗначениеСвойстваПоШаблону(НастройкиКолонтитула.ТекстСправа, НазваниеОтчета, Пользователь));
