...
Процедура ВО_ПометитьНаУдалениеВариантыУдаленныхОтчетов(Режим, Результат)
	...
	Если Режим = "ОбщиеДанныеКонфигурации" Тогда
		...
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И ВариантыОтчетов.ТипОтчета = &ТипОтчета", "Истина");
		ИмяТаблицы = "Справочник.ПредопределенныеВариантыОтчетов";
	ИначеЕсли Режим = "ОбщиеДанныеРасширений" Тогда
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если ОписаниеОтчета.ИспользуетСКД Тогда
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки			= СтрШаблон("Не удалось прочитать список вариантов отчета %1:
							|%2", ОтчетМетаданные.Имя, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
					ВО_ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
...
Процедура сВО_ИнициализироватьЗаголовкиОтчета(Форма) Экспорт
	...
	Если Не сВО_РежимВариантаОтчета(Форма.КлючТекущегоВарианта) Или Не НастройкиОтчета.РазрешеноИзменятьВарианты Тогда
		Возврат;
	КонецЕсли;
...
Функция сВО_ЗаполнитьЗначенияФильтра(ПараметрыЗаполнения) Экспорт
	...
	Для НомерСтроки = СвойстваЗаголовка.Низ + 1 По Документ.ВысотаТаблицы Цикл
		...
		Если Индексы[ЗначениеЯчейки.Значение] <> Неопределено Или (КоличествоДоступныхТипов > 0 И Не ДоступныеТипы.СодержитТип(ТипЗнч(ЗначениеЯчейки.Значение))) Тогда
			Продолжить;
		КонецЕсли;
...
Функция сВО_ДанныеЭлементаРасшифровки(Форма, Расшифровка) Экспорт
	Если Не сВО_РежимВариантаОтчета(Форма.КлючТекущегоВарианта) Или Не Форма.НастройкиОтчета.РазрешеноИзменятьВарианты Или Не ТипЗнч(Расшифровка) = Тип("ИдентификаторРасшифровкиКомпоновкиДанных") Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Функция сВО_ИндексСтруктурыОтчета(Форма)
	НастройкиОтчета		= Форма.НастройкиОтчета;
	СвойстваРезультата	= НастройкиОтчета.СвойстваРезультата;
	...
	сВО_ИндексироватьРазделыОтчета(СвойстваРезультата, СвойстваРезультата.ИтоговыеНастройки, ИндексСтруктурыОтчета);
	сВО_ИндексироватьГруппировкиОтчета(СвойстваРезультата, ИндексСтруктурыОтчета);
...
Процедура сВО_ИндексироватьРазделыОтчета(СвойстваРезультата, Настройки, ИндексСтруктурыОтчета, ПорядокРаздела = 0)
	Для Каждого Раздел Из Настройки.Структура Цикл
		...
		Если ТипЗнч(Раздел) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
			сВО_ИндексироватьРазделыОтчета(СвойстваРезультата, Раздел.Настройки, ИндексСтруктурыОтчета, ПорядокРаздела);
		Иначе
			ПорядокРаздела = ПорядокРаздела + 1;
...
Процедура сВО_ИндексироватьГруппировкиОтчета(СвойстваРезультата, ИндексСтруктурыОтчета)
...
Функция сВО_ИндексГруппировкиОтчета(ИндексСтруктурыОтчета, ИндексРаздела, Настройки, Группировка, ПорядокГруппировки)
	...
	ИндексГруппировки.ПредставлениеГруппировки			= сВО_ПредставлениеПолейГруппировки(Группировка.ПоляГруппировки);
...
Функция сВО_НовыйИндексСтруктурыОтчета()
	ОписаниеЧисла			= Новый ОписаниеТипов("Число");
	ОписаниеСтроки			= Новый ОписаниеТипов("Строка");
	ОписаниеПризнака		= Новый ОписаниеТипов("Булево");
...
Процедура О_ИнициализироватьПредопределенныеПараметрыВывода(Контекст, Настройки) Экспорт
	Если Настройки = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура О_УстановитьСтандартныйЗаголовокОтчета(Заголовок, Контекст)
	...
	Если ЗначениеЗаполнено(Вариант) Тогда
		Заголовок.Значение = БазоваяПодсистемаСервер.ОН_ЗначениеРеквизитаОбъекта(Вариант, "Наименование");
	КонецЕсли;
...
Процедура О_ПерегруппироватьЭлементыФормыНастроек(Форма, Знач УзелИерархииЭлементов, СвойстваЭлементов, ИменаРеквизитов, ВидыСтилизованныхЭлементов)
	...
	О_УстановитьСвойстваЭлементовФормыНастроек(Форма, ЭлементыНастроек, СвойстваЭлементов, ЭтоОсновнаяФорма);
	...
	Для Каждого ИдентификаторГруппы Из ИдентификаторыГрупп Цикл
		...
		Если Группа = Неопределено Тогда
			Группа				= О_ГруппаЭлементовФормыНастроек(Элементы, УзелИерархииЭлементов, ИмяГруппы, ЭтоОсновнаяФорма);
		...
		О_ПодготовитьЭлементыФормыНастроекКРаспределению(ЭлементыНастроекГруппы, СвойстваГруппы.Группировка, ЭтоОсновнаяФорма, ДополнительныеСвойства);

		О_РаспределитьЭлементыФормыНастроек(Форма.Элементы, Группа, ЭлементыНастроекГруппы, ЭтоОсновнаяФорма);
	КонецЦикла;
	...
	О_ДобавитьОтступКолонок(Элементы, ЭлементыНастроек, УзелИерархииЭлементов, ЭтоОсновнаяФорма);
КонецПроцедуры

Процедура О_ДобавитьОтступКолонок(Элементы, ЭлементыНастроек, УзелИерархииЭлементов, ЭтоОсновнаяФорма)
	...
	Для НомерКолонки = 2 По КоличествоКолонок Цикл
		...
		ГруппаПолейКолонки				= О_ГруппаЭлементовФормыНастроек(Элементы, УзелИерархииЭлементов, ИмяГруппы, ЭтоОсновнаяФорма);
...
Процедура О_УстановитьСвойстваЭлементовФормыНастроек(Форма, ЭлементыНастроек, СвойстваЭлементов, ЭтоОсновнаяФорма)
	КомпоновщикНастроек = Форма.Отчет.КомпоновщикНастроек;
	...
	Для Каждого Элемент Из НайденныеЭлементы Цикл
		...
		Если ТипЗнч(Поле) = Тип("ДекорацияФормы") Тогда
			...
			Если Не ЭтоОсновнаяФорма Или Элемент.ЭтоЗначениеКакФлажок Тогда
				Поле.Заголовок = "     ";
			КонецЕсли;
		Иначе // Поле ввода.
			СвойстваПоля = СвойстваЭлементов.Поля.Найти(Элемент.ИндексНастройки, "ИндексНастройки");
...
Процедура О_РаспределитьЭлементыФормыНастроек(Элементы, Знач Группа, ЭлементыНастроек, ЭтоОсновнаяФорма)
	КоличествоКолонок = 0;
	...
	Для НомерКолонки = 1 По КоличествоКолонок Цикл
		...
		Если Колонка = Неопределено Тогда
			Колонка				= О_ГруппаЭлементовФормыНастроек(Элементы, Группа, ИмяКолонки, ЭтоОсновнаяФорма);
		...
		Если ТолькоПоляВвода Тогда
			О_РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Колонка, ЭлементыНастроек, НомерКолонки, ЭтоОсновнаяФорма);
		...
		Для Каждого Признаки Из ПризнакиЭлементов Цикл
			...
			Родитель	= О_ИерархияЭлементовФормыНастроек(Элементы, Колонка, Признаки, НомерСтроки, НомерКолонки, ЭтоОсновнаяФорма);

			О_РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Родитель, ЭлементыНастроек, НомерКолонки, ЭтоОсновнаяФорма, Признаки.НомерГруппы);
		КонецЦикла;
	КонецЦикла;
КонецПроцедуры

Процедура О_РаспределитьЭлементыФормыНастроекПоСвойствам(Элементы, Родитель, ЭлементыНастроек, НомерКолонки, ЭтоОсновнаяФорма, НомерГруппы = Неопределено)
	...
	Для Каждого СвойствоНастройки Из СвойстваНастроек Цикл
		...
		Группа				= О_ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяГруппы, ЭтоОсновнаяФорма);
...
Функция О_ИерархияЭлементовФормыНастроек(Элементы, Родитель, Признаки, НомерСтроки, НомерКолонки, ЭтоОсновнаяФорма)
	ИмяСтроки			= Родитель.Имя + "Строка" + НомерСтроки;
	Строка				= О_ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяСтроки, ЭтоОсновнаяФорма);
	...
	Колонка				= О_ГруппаЭлементовФормыНастроек(Элементы, Строка, ИмяКолонки, ЭтоОсновнаяФорма);
...
Функция О_ГруппаЭлементовФормыНастроек(Элементы, Родитель, ИмяГруппы, ЭтоОсновнаяФорма = Ложь)
...
Процедура О_ВывестиСтилизованныеЭлементыФормыНастроек(Форма, ЭлементыНастроек, ОписаниеНастроек, ИменаРеквизитов, ВидыЭлементов)
	...
	РеквизитыДобавляемые	= О_РеквизитыДобавляемыеЭлементовНастроек(ЭлементыНастроек, ВидыЭлементов, ИменаРеквизитов, ПутьКДаннымЭлементов);
	РеквизитыУдаляемые		= О_РеквизитыУдаляемыеЭлементовНастроек(ВидыЭлементов, ИменаРеквизитов, ПутьКДаннымЭлементов);
...
Функция О_РеквизитыДобавляемыеЭлементовНастроек(ЭлементыНастроек, ВидыЭлементов, ИменаРеквизитов, ПутьКДаннымЭлементов)
...
Функция О_РеквизитыУдаляемыеЭлементовНастроек(ВидыЭлементов, ИменаРеквизитов, ПутьКДаннымЭлементов)
...
Функция О_ЗаголовокЭлементаПериода(Свойство, ЗаголовокЭлементаНастройки)
	Возврат СтрШаблон("%1 (дата %2)", ЗаголовокЭлементаНастройки, ?(СтрЗаканчиваетсяНа(НРег(Свойство), "начала"), "начала", "окончания"));
КонецФункции
...
Процедура О_ДобавитьКомандуВыбораПериода(Форма, Группа, ШаблонИмени, Период, ЭтоСтандартноеПредставление)
	...
	ГруппаКнопки									= О_ГруппаЭлементовФормыНастроек(Форма.Элементы, Группа, ИмяГруппыКнопки);
