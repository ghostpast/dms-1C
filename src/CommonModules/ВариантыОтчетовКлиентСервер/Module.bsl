...
Функция сВО_ПолеОтчета(Поля, Поле, ПроверятьИспользование = Истина, ПолеОтчета = Неопределено) Экспорт
	сВО_НайтиПолеОтчета(Поля, Поле, ПроверятьИспользование, ПолеОтчета);
...
Процедура сВО_НайтиПолеОтчета(Поля, Поле, ПроверятьИспользование, НайденноеПоле)
	Для Каждого Элемент Из Поля.Элементы Цикл
		ТипЭлемента = ТипЗнч(Элемент);

		Если ТипЭлемента <> Тип("АвтоВыбранноеПолеКомпоновкиДанных") И Элемент.Поле = Поле И (Не ПроверятьИспользование Или ПроверятьИспользование И Элемент.Использование) Тогда
			НайденноеПоле = Элемент;

			Возврат;
		ИначеЕсли ТипЭлемента = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			сВО_НайтиПолеОтчета(Элемент, Поле, ПроверятьИспользование, НайденноеПоле);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция сВО_ГруппировкаФильтра(Настройки, СвойстваЗаголовка, ЭтоГруппировка = Ложь) Экспорт
	...
	Если ЭтоГруппировка Или СвойстваЗаголовка.КоличествоРазделов = 1 Тогда
...
Функция КФ_ЭлементыФормулы(Знач Формула) Экспорт
	Операнды = КФ_ПрочитатьОперандыИзФормулы(Формула);
	Для Каждого Операнд Из Операнды Цикл
		Формула = СтрЗаменить(Формула, Операнд, " ");
	КонецЦикла;
	Функции = Новый Массив;

	СтрокиСФункциями = КФ_СтрРазделитьСУчетомСтрок(Формула, "(", Ложь);

	Для Индекс = 0 По СтрокиСФункциями.ВГраница() Цикл
		СтрокаСФункцией	= СтрокиСФункциями[Индекс];
		ОперандыСтроки	= КФ_ОперандыСтроки(СтрокаСФункцией);
...
Функция КФ_ПрочитатьОперандыИзФормулы(Формула)
	Операнды = Новый Массив;
	Для Каждого Строка Из КФ_СтрРазделитьСУчетомСтрок(Формула, "[", Истина) Цикл
		ЧастиСтроки = КФ_СтрРазделитьСУчетомСтрок(Строка, "]", Истина);
...
Функция КФ_ОперандыСтроки(Знач Строка)
	Операнды = Новый Массив;

	Строка = СтрСоединить(КФ_СтрРазделитьСУчетомСтрок(Строка, "[", Истина), "*[");

	Для Каждого Операнд Из КФ_СтрРазделитьСУчетомСтрок(Строка, "/*-+=(),% ", Ложь) Цикл
...
Функция КФ_СтрРазделитьСУчетомСтрок(ИсходнаяСтрока, Разделитель, ВключатьПустые = Истина)
