	...
	Для Каждого Строка Из СтрРазделить(СписокАдресов, ";", Ложь) Цикл
		...
		Для Каждого АдресСПредставлением Из СтрРазделить(Строка, ",", Ложь) Цикл
			...
			Если СтрНайти(Адрес, "@") Или СтрНайти(Адрес, "<") Или СтрНайти(Адрес, ">") Тогда
				...
				Иначе
					Адрес			= "";
					ОписаниеОшибки	= "Адрес электронной почты содержит ошибки";
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрЗаменить(ЧастьПредставления, ",", "")) Тогда
			...
			Адрес			= "";
			ОписаниеОшибки	= "Строка адреса не соответствует формату ""Имя получателя <Адрес@Почты>""";
			СтруктураАдреса	= Новый Структура("Псевдоним, Адрес, ОписаниеОшибки", Псевдоним, Адрес, ОписаниеОшибки);
			Результат.Добавить(СтруктураАдреса);
		КонецЕсли;
	КонецЦикла;
...
Функция ОН_РазобратьСтрокуСПочтовымиАдресами(Знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	СообщениеНедопустимыеСимволы			= "Недопустимый символ ""%1"" в адресе электронной почты ""%2""";
	СообщениеНеверныйФорматПочтовогоАдреса	= "Некорректный адрес электронной почты ""%1""";
	...
	Для Каждого СтрокаАдреса Из МассивПодстрокКОбработке Цикл
		...
		Пока Индекс <= СтрДлина(СтрокаАдреса) Цикл
			...
			Если      Символ = " " Тогда
				Индекс = ? ((ОН_ПропуститьПробелы(СтрокаАдреса, Индекс, " ") - 1) > Индекс, ОН_ПропуститьПробелы(СтрокаАдреса, Индекс, " ") - 1, Индекс);
				...
				Если СтадияРазбора = 1 Тогда
					...
					Для ИндексПоискаНС = 1 По СтрДлина(Накопитель) Цикл
						Если СтрНайти(НедопустимыеСимволы, Сред(Накопитель, ИндексПоискаНС, 1)) > 0 И ВызыватьИсключение Тогда
							ВызватьИсключение СтрШаблон(СообщениеНедопустимыеСимволы, Сред(Накопитель, ИндексПоискаНС, 1),СтрокаАдреса);
						КонецЕсли;
					КонецЦикла;
		...
		Если НЕ (ПустаяСтрока(ПолноеИмяАдресата) И ПустаяСтрока(ПочтовыйАдрес)) Тогда
			Результат.Добавить(ОН_ПроверитьИПодготовитьПочтовыйАдрес(ПолноеИмяАдресата, ПочтовыйАдрес));
		КонецЕсли;
	КонецЦикла;
...
Функция ОН_ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	НедопустимыйСимволВИмениАдресата	= "Недопустимый символ в имени адресата.";
	НедопустимыйСимволВПочтовомАдресе	= "Недопустимый символ в почтовом адресе.";
	СимволыОбрамления					= "<>[]";
...
Функция ОН_ПропуститьПробелы(Знач Строка, Знач ТекущийИндекс, Знач ПропускаемыйСимвол)
