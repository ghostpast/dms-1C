...
Процедура ПередНачаломРаботыСистемы(Знач ОповещениеЗавершения = Неопределено) Экспорт
	...
	Параметры.Вставить("ОбработкаЗавершения", Новый ОписаниеОповещения("СП_ДействияПередНачаломРаботыСистемыОбработкаЗавершения", ЭтотОбъект));
	...
	Параметры.Вставить("ОбработкаПродолжения", Новый ОписаниеОповещения("СП_ДействияПередНачаломРаботыСистемыВИнтеграционнойПроцедуре", ЭтотОбъект));
...
Функция СП_ИнтерактивнаяОбработкаПередЗавершениемРаботыСистемы(Параметры)
	...
	Иначе
		...
		Параметры.НепрерывноеВыполнение = Ложь;
		ПодключитьОбработчикОжидания("СП_ОбработчикОжиданияИнтерактивнаяОбработкаПередЗавершениемРаботыСистемы", 0.1, Истина);
	КонецЕсли;
...
Процедура СП_УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	ПараметрыКлиента = ?(ПриЗапуске, БазоваяПодсистемаКлиентПовтИсп.СП_ПараметрыРаботыКлиентаПриЗапуске(), БазоваяПодсистемаКлиентПовтИсп.СП_ПараметрыРаботыКлиента());
	...
	Если ПараметрыКлиента.Свойство("РаботаСВнешнимиРесурсамиЗаблокирована") И ПараметрыКлиента.Свойство("РазделениеВключено") И Не ПараметрыКлиента.РазделениеВключено Тогда
		ЗаголовокПриложения = "[КОПИЯ] " + ЗаголовокПриложения;
	КонецЕсли;
...
Процедура ОН_СообщитьПользователю(Знач ТекстСообщенияПользователю,	Знач КлючДанных = Неопределено, Знач Поле = "", Знач ПутьКДанным = "", Отказ = Ложь) Экспорт
	Сообщение = БазоваяПодсистемаКлиентСервер.сОН_СообщениеПользователю(ТекстСообщенияПользователю, КлючДанных, Поле, ПутьКДанным, Отказ);
	Сообщение.Сообщить();
КонецПроцедуры
...
Процедура ЖР_ДобавитьСообщениеДляЖурналаРегистрации(Знач ИмяСобытия, Знач ПредставлениеУровня = "Информация", Знач Комментарий = "", Знач ДатаСобытия = "", Знач ЗаписатьСобытия = Ложь) Экспорт
	ИмяПроцедуры = "БазоваяПодсистемаКлиент.ЖР_ДобавитьСообщениеДляЖурналаРегистрации";
	БазоваяПодсистемаКлиентСервер.ОН_ПроверитьПараметр(ИмяПроцедуры, "ИмяСобытия", ИмяСобытия, Тип("Строка"));
	БазоваяПодсистемаКлиентСервер.ОН_ПроверитьПараметр(ИмяПроцедуры, "ПредставлениеУровня", ПредставлениеУровня, Тип("Строка"));
	БазоваяПодсистемаКлиентСервер.ОН_ПроверитьПараметр(ИмяПроцедуры, "Комментарий", Комментарий, Тип("Строка"));
	Если ДатаСобытия <> "" Тогда
		БазоваяПодсистемаКлиентСервер.ОН_ПроверитьПараметр(ИмяПроцедуры, "ДатаСобытия", ДатаСобытия, Тип("Дата"));
	КонецЕсли;
	...
	Если ЗаписатьСобытия Тогда
		БазоваяПодсистемаВызовСервера.ЖР_ЗаписатьСобытияВЖурналРегистрации(СообщенияДляЖурналаРегистрации);
...
Процедура ДО_ОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении = Неопределено, Знач ПараметрыОжидания = Неопределено) Экспорт
	...
	РасширенныеПараметры = ДО_ПараметрыОжидания(Неопределено);
	...
	Если ДлительнаяОперация.Статус <> "Выполняется" Тогда
		...
		ДО_ЗавершитьДлительнуюОперацию(РасширенныеПараметры, ДлительнаяОперация);
	...
	Если РасширенныеПараметры.ВыводитьОкноОжидания Тогда
		...
		ОткрытьФорму("ОбщаяФорма.ДлительнаяОперация", РасширенныеПараметры, ?(ПараметрыОжидания <> Неопределено, ПараметрыОжидания.ФормаВладелец, Неопределено),,,,ОповещениеОЗавершении);
	Иначе
		...
		РасширенныеПараметры.Вставить("Контроль",				ТекущаяДата() + РасширенныеПараметры.ТекущийИнтервал);
		...
		СерверныеОповещенияКлиент.СО_ПодключитьОбработчикПроверкиПолученияСерверныхОповещений();
	КонецЕсли;
КонецПроцедуры

Процедура ДО_ЗавершитьДлительнуюОперацию(Знач ДлительнаяОперация, Знач Статус)
	Если Статус.Статус = "Выполнено" Тогда
		ДО_ПоказатьОповещение(ДлительнаяОперация.ОповещениеПользователя);
	КонецЕсли;
...
Функция ДО_ПараметрыОжидания(ФормаВладелец) Экспорт
	...
	Результат.Вставить("ОтменятьПриЗакрытииФормыВладельца",	ТипЗнч(ФормаВладелец) = Тип("ФормаКлиентскогоПриложения") И ФормаВладелец.Открыта());
...
Функция ДО_АктивныеДлительныеОперации()
...
Процедура ДО_ПроверитьПараметрыОжидатьЗавершение(Знач ДлительнаяОперация, Знач ОповещениеОЗавершении, Знач ПараметрыОжидания)
	...
	Если ПараметрыОжидания <> Неопределено Тогда
		...
		СообщениеПроверки = СтрШаблон("Параметр %1 должен быть больше или равен 1", "ПараметрыОжидания.Интервал");

		БазоваяПодсистемаКлиентСервер.ОН_Проверить(ПараметрыОжидания.Интервал = 0 Или ПараметрыОжидания.Интервал >= 1, СообщениеПроверки, "ДлительныеОперацииКлиент.ОжидатьЗавершение");

		СообщениеПроверки = СтрШаблон("Если параметр %1 установлен в %2, то параметр %3 не поддерживается", "ПараметрыОжидания.ВыводитьОкноОжидания", "Истина", "ПараметрыОжидания.ОповещениеОПрогрессеВыполнения");

		БазоваяПодсистемаКлиентСервер.ОН_Проверить(Не (ПараметрыОжидания.ОповещениеОПрогрессеВыполнения <> Неопределено И ПараметрыОжидания.ВыводитьОкноОжидания), СообщениеПроверки, "ДлительныеОперацииКлиент.ОжидатьЗавершение");
	КонецЕсли;
КонецПроцедуры

Процедура ДО_ПоказатьОповещение(ОповещениеПользователя, ВладелецФормы = Неопределено) Экспорт
	...
	ПоказатьОповещениеПользователя(?(Оповещение.Текст <> Неопределено, Оповещение.Текст, "Действие выполнено"), НавигационнаяСсылкаОповещения, ПояснениеОповещения, Оповещение.Картинка, СтатусОповещения);
КонецПроцедуры

Процедура ДО_ПередПериодическойОтправкойДанныхКлиентаНаСервер(Параметры, ОбсужденияАктивны, Интервал) Экспорт
	Результат = ДО_ПараметрыПроверкиДлительныхОпераций(ОбсужденияАктивны, Интервал);
	Если Результат = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДО_ПослеПериодическогоПолученияДанныхКлиентаНаСервере(Результаты, ОбсужденияАктивны, Интервал) Экспорт
	РезультатыОпераций = Результаты.Получить("СтандартныеПодсистемы.БазоваяФункциональность.РезультатПроверкиДлительныхОпераций");

	Если РезультатыОпераций = Неопределено Тогда
		Возврат;
	КонецЕсли;

	ТекущиеДлительныеОперации	= ДО_АктивныеДлительныеОперации();
	...
	Для Каждого РезультатОперации Из РезультатыОпераций Цикл
		...
		ДО_ОбработатьРезультатОперации(АктивныеДлительныеОперации, Операция, Результат);
	КонецЦикла;
	...
	ДО_УточнитьИнтервалОбработчикаОжидания(Интервал, АктивныеДлительныеОперации, ОбсужденияАктивны);
КонецПроцедуры

Процедура ДО_ОбработатьРезультатОперации(АктивныеДлительныеОперации, Операция, Результат)
	Если АктивныеДлительныеОперации.Получить(Операция.ИдентификаторЗадания) = Неопределено Тогда
		Возврат;
	КонецЕсли;

	Попытка
		Если ДО_ОбработатьРезультатАктивнойОперации(Операция, Результат) Тогда
...
Функция ДО_ОбработатьРезультатАктивнойОперации(ДлительнаяОперация, Результат)
	...
	Если Результат.Статус <> "Выполняется" Тогда
		Если Результат.Статус <> "Выполнено"
		 ...
		 Или БазоваяПодсистемаВызовСервера.ДО_ФоновоеЗаданиеЗавершено(ДлительнаяОперация.ИдентификаторЗадания) Тогда
			ДО_ЗавершитьДлительнуюОперацию(ДлительнаяОперация, Результат);
	...
	КонецЕсли;
	ДлительнаяОперация.Контроль = ТекущаяДата() + ИнтервалОжидания;

	Возврат Ложь;
КонецФункции

Функция ДО_ПараметрыПроверкиДлительныхОпераций(ОбсужденияАктивны, Интервал)
	...
	Для Каждого ДлительнаяОперация Из АктивныеДлительныеОперации Цикл
		...
		Иначе
			ИнтервалКонтроляОбсуждений	= 30;
			ДатаКонтроля				= ДлительнаяОперация.Контроль + ?(Не ОбсужденияАктивны Или ДлительнаяОперация.ТекущийИнтервал > ИнтервалКонтроляОбсуждений, 0, ИнтервалКонтроляОбсуждений - ДлительнаяОперация.ТекущийИнтервал);
...
Процедура ДО_УточнитьИнтервалОбработчикаОжидания(Интервал, АктивныеДлительныеОперации, ОбсужденияАктивны)
	ТекущаяДата		= ТекущаяДата();
	НовыйИнтервал	= 120;
