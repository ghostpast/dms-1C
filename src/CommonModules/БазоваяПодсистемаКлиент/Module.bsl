...
Процедура ПередНачаломРаботыСистемы(Знач ОповещениеЗавершения = Неопределено) Экспорт
	...
	Если СтрНайти(ПараметрЗапуска, "ОтключитьЛогикуНачалаРаботыСистемы") > 0 Тогда
		Попытка
			...
			СвойстваКлиента.Вставить("ЭтоВебКлиент",							СП_ЭтоВебКлиент());
			...
			СвойстваКлиента.Вставить("ЭтоМобильныйКлиент",						СП_ЭтоМобильныйКлиент());
			СвойстваКлиента.Вставить("ИспользуемыйКлиент",						СП_ИспользуемыйКлиент());
			СвойстваКлиента.Вставить("КаталогПрограммы",						СП_ТекущийКаталогПрограммы());
			СвойстваКлиента.Вставить("ИдентификаторКлиента",					СП_ИдентификаторКлиента());
			...
			СвойстваКлиента.Вставить("РазрешениеОсновногоЭкрана",				СП_РазрешениеОсновногоЭкрана());
...
#КонецЕсли
	...
	Если Не СП_ПараметрКлиента("ДоступноИспользованиеРазделенныхДанных") Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура СП_ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке, Событие, ПрекратитьРаботу = Ложь)
	...
	БазоваяПодсистемаВызовСервера.СП_ЗаписатьОшибкуВЖурналРегистрацииПриЗапускеИлиЗавершении(ПрекратитьРаботу, Событие, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));

	ТекстПредупреждения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке) + Символы.ПС + Символы.ПС + "Техническая информация записана в журнал регистрации.";
...
Процедура сФС_ОткрытьПроводникПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		ФайлИнфо.НачатьПроверкуСуществования(Оповещение);
	Иначе
		ОписаниеОшибки = "Для того чтобы открыть папку, необходимо установить расширение для работы с 1С:Предприятием.";
