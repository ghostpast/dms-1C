...
Процедура СП_ИнтерактивнаяОбработкаНачальныхРегиональныхНастроекИБ(Параметры, Контекст) Экспорт
	...
	Форма = ОткрытьФорму("ОбщаяФорма.РегиональныеНастройки",,,,,, Новый ОписаниеОповещения("СП_ПослеЗакрытияФормыВыбораНачальныхРегиональныхНастроекИБ", ЭтотОбъект, Параметры));
	Если Форма = Неопределено Тогда
...
Процедура СП_ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	ПоказатьОповещениеПользователя("Установлено обновление программы", "e1cib/app/ОбщаяФорма.КонтрольДинамическогоОбновления", СообщениеПользователю, БиблиотекаКартинок.Предупреждение32, СтатусОповещенияПользователя.Важное);
