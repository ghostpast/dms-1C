...
Процедура СП_ПередЗапускомПрограммы()
	...
	Если ТекущийРежимЗапуска() = Неопределено И ДО_ЭтоЗапускБезВызоваПередЗапускомПрограммы() Тогда
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Если СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_14") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_15") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_16") Или СтрЗаканчиваетсяНа(ТекущийРежимСтрокой, "8_3_17") Тогда
			НеПоддерживаемаяВерсияПлатформы = "";
		Иначе
...
Функция СП_ТекстСообщенияПриДинамическомОбновлении(ДинамическиеИзмененияКонфигурации) Экспорт
	...
	Если ДинамическиеИзмененияКонфигурации.Исправления <> Неопределено Тогда
		...
		ИначеЕсли ДинамическиеИзмененияКонфигурации.Исправления.Добавлено = 1 Тогда
			ТекстСообщенияПатчи = "Установлено новое исправление (патч).Ссылка";
...
Функция СП_НастройкиФорматовСохраненияТабличногоДокумента() Экспорт
	...
	НовыйФормат.ТипФайлаТабличногоДокумента	= СП_ТипФайлаТабличногоДокументаPDF();
	...
	НовыйФормат.Представление				= СП_ПредставлениеТипаФайлаТабличногоДокументаPDF();
...
Функция СП_ТипФайлаТабличногоДокументаPDF() Экспорт
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ВерсияПлатформы = БазоваяПодсистемаКлиентСервер.ОН_ВерсияКонфигурацииБезНомераСборки(СистемнаяИнформация.ВерсияПриложения);

	Если БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсииБезНомераСборки(ВерсияПлатформы, "8.3.16") >= 0 Тогда
...
Функция СП_ПредставлениеТипаФайлаТабличногоДокументаPDF() Экспорт
	СистемнаяИнформация = Новый СистемнаяИнформация;
	ВерсияПлатформы = БазоваяПодсистемаКлиентСервер.ОН_ВерсияКонфигурацииБезНомераСборки(СистемнаяИнформация.ВерсияПриложения);

	Если БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсииБезНомераСборки(ВерсияПлатформы, "8.3.16") >= 0 Тогда
		Возврат "Документ PDF/A (.pdf)";
	Иначе
		Возврат "Документ PDF (.pdf)";
	КонецЕсли;
КонецФункции
...
Функция ДО_ПрочитатьПрогресс(Знач ИдентификаторЗадания) Экспорт
	Возврат ДО_ПрочитатьПрогрессИСообщения(ИдентификаторЗадания, "Прогресс").Прогресс;
КонецФункции
...
Функция ДО_ЗапуститьФоновоеЗаданиеСКонтекстомКлиента(ИмяПроцедуры, ПараметрыВыполнения, ПараметрыПроцедуры = Неопределено, БезопасныйРежим = Ложь) Экспорт
	...
	Если "РегистрыСведений.ПараметрыРаботыПрограммы.ОбработчикДлительнойОперацииОбновленияПараметровРаботыПрограммы" = ИмяПроцедуры Тогда
		ИмяПроцедурыФоновогоЗадания = "БазоваяПодсистемаСервер.ДО_ВыполнитьСКонтекстомКлиентаБезВызоваПередЗапускомПрограммы";
	Иначе
...
Функция ДО_ЭтоЗапускБезВызоваПередЗапускомПрограммы() Экспорт
	Если ТекущийРежимЗапуска() <> Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ФоновоеЗадание = Неопределено Или ФоновоеЗадание.ИмяМетода <> "БазоваяПодсистемаСервер.ДО_ВыполнитьСКонтекстомКлиентаБезВызоваПередЗапускомПрограммы" Тогда
		Возврат Ложь;
	КонецЕсли;
...
Процедура ДО_ВыполнитьСКонтекстомКлиентаБезВызоваПередЗапускомПрограммы(ВсеПараметры) Экспорт
...
Функция ОН_ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь, Знач КодЯзыка = Неопределено) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") Тогда
		...
		Исключение
			ТекстОшибки = СтрШаблон("Неверный первый параметр Ссылка в функции БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта:
	...
	Иначе // Если передана ссылка.
		...
		Исключение
			ВызватьИсключение "Неверный первый параметр Ссылка в функции БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта:
	...
	ИначеЕсли ТипЗнч(Реквизиты) = Тип("Массив") Или ТипЗнч(Реквизиты) = Тип("ФиксированныйМассив") Тогда
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Исключение
				...
				Если Результат.Ошибка Тогда
					ВызватьИсключение СтрШаблон("Неверный второй параметр Реквизиты в функции БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта: %1", Результат.ОписаниеОшибки);
				КонецЕсли;
		...
		КонецЦикла;
	Иначе
		ВызватьИсключение СтрШаблон("Неверный тип второго параметра Реквизиты в функции БазоваяПодсистемаСервер.ОН_ЗначенияРеквизитовОбъекта: %1", Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
