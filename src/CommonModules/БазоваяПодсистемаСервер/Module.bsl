...
Функция СП_ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы()
	Если ОН_ЭтоАвтономноеРабочееМесто()  Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Если ОбновлениеВерсииИБСервер.сОИБ_РежимОбновленияДанных() = "НачальноеЗаполнение" И Метаданные.Языки.Количество() > 1 Тогда
		Возврат Истина;
	КонецЕсли;
...
Процедура СП_ВосстановитьПредопределенныеЭлементы() Экспорт
	...
	НачатьТранзакцию();
	Попытка
		СП_УстановитьИнициализациюВсехПредопределенныхДанных();

		СП_СоздатьНедостающиеПредопределенныеДанные();
...
Процедура СП_УстановитьИнициализациюВсехПредопределенныхДанных()
...
Процедура СП_СоздатьНедостающиеПредопределенныеДанные()
	КоллекцииМетаданных = Новый Массив;
	КоллекцииМетаданных.Добавить(Метаданные.Справочники);
	КоллекцииМетаданных.Добавить(Метаданные.ПланыВидовХарактеристик);
	КоллекцииМетаданных.Добавить(Метаданные.ПланыСчетов);
	...
	Для Каждого Коллекция Из КоллекцииМетаданных Цикл
		...
		Для Каждого ОбъектМетаданных Из Коллекция Цикл
			...
			Если Коллекция = Метаданные.ПланыСчетов Или Коллекция = Метаданные.ПланыВидовРасчета Или Не ОбъектМетаданных.Иерархический Тогда
			...
			СП_СохранитьСуществующиеПредопределенныеОбъектыПередСозданиемНедостающих(ОбъектМетаданных, ПолноеИмя, ТаблицаИмен, Имена, Запрос, ОписанияСохраненных, ЭтоПланСчетов);
		КонецЦикла;
	КонецЦикла;
...
Процедура СП_СохранитьСуществующиеПредопределенныеОбъектыПередСозданиемНедостающих(ОбъектМетаданных, ПолноеИмя, ТаблицаИмен, Имена, Запрос, ОписанияСохраненных, ЭтоПланСчетов)
...
Функция ОН_ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь, КодЯзыка = Неопределено) Экспорт
...
Функция ОН_ЗначениеРеквизитаОбъекта(Ссылка, ИмяРеквизита, ВыбратьРазрешенные = Ложь, КодЯзыка = Неопределено) Экспорт
...
Процедура ОН_ПриНачалеВыполненияРегламентногоЗадания(РегламентноеЗадание = Неопределено) Экспорт
	...
	Если РегламентноеЗадание <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если СП_ТребуетсяУстановкаРегиональныхНастроекИнформационнойБазы() Тогда
		Текст = "Регламентное задание недоступно до установки начальных региональных настроек программы.
			                |Выполнение прервано.";
		РегламентныеЗаданияСервер.РЗ_ОтменитьВыполнениеЗадания(РегламентноеЗадание, Текст);
...
Процедура ОН_УстановитьСвойстваДинамическогоСписка(Список, СтруктураПараметров) Экспорт
...
Функция М_КодПервогоДополнительногоЯзыкаИнформационнойБазы() Экспорт
	Если Константы.ИспользоватьДополнительныйЯзык1.Получить() = Ложь Тогда
		Возврат "";
	КонецЕсли;
...
Функция М_КодВторогоДополнительногоЯзыкаИнформационнойБазы() Экспорт
	Если Константы.ИспользоватьДополнительныйЯзык2.Получить() = Ложь Тогда
		Возврат "";
	КонецЕсли;
...
Процедура М_ПерезаполнитьМультиязычныеСтрокиВОбъектах(Параметры, Адрес) Экспорт
	ОбновлениеВерсииИБСервер.сОИБ_НачальноеЗаполнениеПредопределенныхДанных(Истина);
КонецПроцедуры

Функция М_ЯзыкиКонфигурации() Экспорт
	Языки = Новый Массив;
...
Функция М_МультиязычныеРеквизитыОбъекта(ОбъектИлиСсылка) Экспорт
	ТипОбъекта = ТипЗнч(ОбъектИлиСсылка);

	Если ТипОбъекта = Тип("Строка") Тогда
		МетаданныеОбъекта = Метаданные.НайтиПоПолномуИмени(ОбъектИлиСсылка);
	ИначеЕсли ОН_ЭтоСсылка(ТипОбъекта) Тогда
		МетаданныеОбъекта = ОбъектИлиСсылка.Метаданные();
	Иначе
		МетаданныеОбъекта = ОбъектИлиСсылка;
	КонецЕсли;

	Возврат М_НаименованияЛокализуемыхРеквизитовОбъекта(МетаданныеОбъекта);
КонецФункции

Функция М_НаименованияЛокализуемыхРеквизитовОбъекта(ОбъектМетаданных, Префикс = "")
	СписокРеквизитовОбъекта = Новый Соответствие;
	Если М_МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Тогда
...
Функция М_МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Экспорт
	ПолноеИмя = ОбъектМетаданных.ПолноеИмя();
...
Функция М_ЯзыкПоКоду(Знач КодЯзыка) Экспорт
	Для каждого Язык Из Метаданные.Языки Цикл
...
Процедура М_ПриЧтенииПредставленийНаСервере(Объект) Экспорт
	Если СтрСравнить(ОН_КодОсновногоЯзыка(), ТекущийЯзык().КодЯзыка) = 0 Тогда
		Возврат;
	КонецЕсли;

	ОбъектМетаданных	= М_ОбъектМетаданных(Объект);
	ЭтоРегистр			= ОН_ЭтоРегистр(ОбъектМетаданных);

	Если ЭтоРегистр Или М_МультиязычныеСтрокиВРеквизитах(ОбъектМетаданных) Тогда
		// Объект.Ссылка.Метаданные()
		ИменаЛокализуемыхРеквизитов	= М_НаименованияЛокализуемыхРеквизитовОбъекта(ОбъектМетаданных);
		СуффиксТекущегоЯзыка		= "";

		Если СтрСравнить(Константы.ОсновнойЯзык.Получить(), ТекущийЯзык().КодЯзыка) <> 0 Тогда
			СуффиксТекущегоЯзыка = М_СуффиксЯзыка(ТекущийЯзык().КодЯзыка);
		КонецЕсли;
	...
	Для каждого Реквизит Из ОбъектМетаданных.ТабличныеЧасти.Представления.Реквизиты Цикл
		...
		Отбор			= Новый Структура;
		Отбор.Вставить("КодЯзыка", ОН_КодОсновногоЯзыка());
		...
		Иначе
			...
			Представление.КодЯзыка	= ОН_КодОсновногоЯзыка();
		КонецЕсли;
...
Функция М_ОбъектМетаданных(Объект) Экспорт
...
Функция М_СуффиксЯзыка(Язык) Экспорт
