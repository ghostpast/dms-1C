...
Функция ОИБ_ДоступнаПакетнаяРегистрация() Экспорт
	СистемнаяИнформация = Новый СистемнаяИнформация;

	Возврат БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.19.725") >= 0;
КонецФункции

Функция ОИБ_ЗаписыватьДанныеДляОбновленияВФайл(Параметры)
	// Зарезервировано для новых подсистем

	Возврат Ложь;
КонецФункции

Функция ОИБ_МодульОбменДаннымиСервер(ЗаписыватьДанныеДляОбновленияВФайл)
	// Зарезервировано для новых подсистем

	Возврат Неопределено;
КонецФункции

Функция ОИБ_НовыеПереиспользуемыеНаборы(Менеджер, Размер = 1000)
...
Процедура ОИБ_ЗавершитьРегистрациюПорцииДанных(ПараметрыРегистрации)
	Если ПараметрыРегистрации.ДоступнаПакетнаяРегистрация Тогда
		ОИБ_ЗарегистрироватьПакетДанных(ПараметрыРегистрации, ПараметрыРегистрации.ДанныеДляРегистрации, Истина);
	КонецЕсли;
КонецПроцедуры

Процедура ОИБ_ЗарегистрироватьПакетДанных(ПараметрыРегистрации, Данные, Принудительно = Ложь, РазмерПакета = 1000)
	Если Данные.Количество() >= РазмерПакета Или Принудительно И Данные.Количество() > 0 Тогда
		...
		ОИБ_ОсвободитьПереиспользуемыеНаборыВПараметрах(ПараметрыРегистрации);
	КонецЕсли;
КонецПроцедуры

Процедура ОИБ_ЗарегистрироватьИзмененияЭлементаДанных(Данные, ПараметрыРегистрации)
	Если ПараметрыРегистрации.ДоступнаПакетнаяРегистрация Тогда
		ПараметрыРегистрации.ДанныеДляРегистрации.Добавить(Данные);
		ОИБ_ЗарегистрироватьПакетДанных(ПараметрыРегистрации, ПараметрыРегистрации.ДанныеДляРегистрации);
	Иначе
		ПланыОбмена.ЗарегистрироватьИзменения(ПараметрыРегистрации.Узел, Данные);
		ОИБ_ОсвободитьПереиспользуемыеНаборыВПараметрах(ПараметрыРегистрации);
	КонецЕсли;

	ОИБ_УвеличитьКоличествоЗарегистрированныхДанных(ПараметрыРегистрации.Параметры, Данные, ПараметрыРегистрации.ПолноеИмяОбъекта);

	Если ПараметрыРегистрации.ЗаписыватьДанныеДляОбновленияВФайл Тогда
		ПараметрыРегистрации.МодульОбменДаннымиСервер.ЗаписатьДанныеДляОбновленияВФайл(ПараметрыРегистрации.Параметры, Данные, ПараметрыРегистрации.ВидДанных, ПараметрыРегистрации.ПолноеИмяОбъекта);
	КонецЕсли;
КонецПроцедуры

Процедура ОИБ_ОсвободитьПереиспользуемыеНаборы(ПереиспользуемыеНаборы)
	ПереиспользуемыеНаборы.Выдано = 0;
КонецПроцедуры

Процедура ОИБ_ОсвободитьПереиспользуемыеНаборыВПараметрах(Параметры)
	Если Параметры.Свойство("ПереиспользуемыеНаборы") Тогда
		ОИБ_ОсвободитьПереиспользуемыеНаборы(Параметры.ПереиспользуемыеНаборы);
	КонецЕсли;
КонецПроцедуры

Процедура ОИБ_УвеличитьКоличествоЗарегистрированныхДанных(Параметры, Данные, ПолноеИмяОбъекта)
...
Функция ОИБ_ОбъектРегистрируетсяНаПланеОбмена(Данные)
	ТипОбъекта	= ТипЗнч(Данные);

	Кеш				= ОбновлениеВерсииИБСерверПовтИсп.сОИБ_КешПроверкиЗарегистрированныхОбъектов();
...
Функция ОИБ_НовыеПараметрыРегистрации(Параметры, Узел, ВидДанных, ПолноеИмяОбъекта)
	...
	ПараметрыРегистрации.Вставить("ДоступнаПакетнаяРегистрация",		ОИБ_ДоступнаПакетнаяРегистрация());
	ПараметрыРегистрации.Вставить("ЗаписыватьДанныеДляОбновленияВФайл",	ОИБ_ЗаписыватьДанныеДляОбновленияВФайл(Параметры));
	ПараметрыРегистрации.Вставить("МодульОбменДаннымиСервер",			ОИБ_МодульОбменДаннымиСервер(ПараметрыРегистрации.ЗаписыватьДанныеДляОбновленияВФайл));

	МенеджерОбъекта = Неопределено;

	Если ЗначениеЗаполнено(ПолноеИмяОбъекта) Тогда
		МенеджерОбъекта = БазоваяПодсистемаСервер.ОН_МенеджерОбъектаПоПолномуИмени(ПолноеИмяОбъекта);

		Если БазоваяПодсистемаСервер.ОН_ЭтоРегистр(Метаданные.НайтиПоПолномуИмени(ПолноеИмяОбъекта)) Тогда
			ПараметрыРегистрации.Вставить("ПереиспользуемыеНаборы", ОИБ_НовыеПереиспользуемыеНаборы(МенеджерОбъекта));
		КонецЕсли;
	КонецЕсли;
...
Процедура ОИБ_ЗаписатьДанные(Знач Данные, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, Знач ВключитьБизнесЛогику = Ложь) Экспорт
	...
	Если ОИБ_ОбъектРегистрируетсяНаПланеОбмена(Данные) Тогда
...
Процедура ОИБ_ЗаписатьНаборЗаписей(Знач НаборЗаписей, Замещать = Истина, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, Знач ВключитьБизнесЛогику = Ложь) Экспорт
	...
	Если ОИБ_ОбъектРегистрируетсяНаПланеОбмена(НаборЗаписей) Тогда
...
Процедура ОИБ_ОтметитьВыполнениеОбработки(Данные, ДополнительныеПараметры = Неопределено, Очередь = Неопределено) Экспорт
	...
	Если (ТипЗнч(Данные) = Тип("Массив") Или ТипЗнч(Данные) = Тип("ТаблицаЗначений")) И Данные.Количество() = 0 Тогда
		ТекстИсключения	= "В процедуру %1 передан пустой массив. Не возможно отметить выполнение обработки.";
		ТекстИсключения	= СтрШаблон(ТекстИсключения, "ОбновлениеВерсииИБСервер.ОИБ_ОтметитьВыполнениеОбработки");

		ВызватьИсключение ТекстИсключения;
	КонецЕсли;

	Узел = ПланыОбмена.ОбновлениеИнформационнойБазы.УзелПоОчереди(Очередь, Ложь);
	...
	Иначе
		...
		Если ТипЗнч(Данные) <> Тип("Массив") Тогда
			...
			ОИБ_ЗаписатьПрогрессВыполненияОбработчика(Данные, Узел, МетаданныеОбъекта);
			ПланыОбмена.УдалитьРегистрациюИзменений(Узел, Набор);
			КопияДанных = Набор;
		Иначе
...
Функция ОИБ_ВыбратьИзмеренияНезависимогоРегистраСведенийДляОбработки(Очередь, ПолноеИмяОбъекта, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ТекущаяОчередь", ПланыОбмена.ОбновлениеИнформационнойБазы.УзелПоОчереди(Очередь, Ложь));
...
Функция ОИБ_СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли БазоваяПодсистемаСервер.ОН_ЭтоРегистр(МетаданныеОбъекта) Тогда
		...
		КонецЕсли;
	Иначе
		ТекстИсключения	= "Для этого типа метаданных не поддерживается проверка в функции %1.";
		ТекстИсключения = СтрШаблон(ТекстИсключения, "ОбновлениеВерсииИБСервер.ОИБ_СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных");
	...
	Иначе
		...
		Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбъектовМассив Цикл
			...
			ИначеЕсли БазоваяПодсистемаСервер.ОН_ЭтоРегистр(МетаданныеОбъекта) Тогда
				...
				ЕстьРегистры = Истина;
			Иначе
				ТекстИсключения	= "Для типа метаданных ""%1"" не поддерживается проверка в функции %2";
				ТекстИсключения	= СтрШаблон(ТекстИсключения, Строка(МетаданныеОбъекта), "ОбновлениеВерсииИБСервер.ОИБ_СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияСсылок");
...
Функция ОИБ_ЕстьДанныеДляОбработки(Очередь, ПолноеИмяМетаданныеОбъекта, Отбор = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = "Передан неправильный тип параметра ""%1"" в функцию %2";
		ТекстИсключения = СтрШаблон(ТекстИсключения, "ПолноеИмяМетаданныеОбъекта", "ОбновлениеВерсииИБСервер.ОИБ_ЕстьДанныеДляОбработки");
	...
	Для Каждого ОбрабатываемыйТип Из ПолныеИменаОбрабатываемыхОбъектов Цикл
		...
		ИначеЕсли Метаданные.РегистрыСведений.Содержит(МетаданныеОбъекта)
			...
			Если ПолныеИменаОбрабатываемыхОбъектов.Количество() > 1 Тогда
				ТекстИсключения = "В массиве имен в параметре ""%1"" в функцию %2 передан независимый регистр сведений.";
				ТекстИсключения = СтрШаблон(ТекстИсключения, "ПолноеИмяМетаданныеОбъекта", "ОбновлениеВерсииИБСервер.ОИБ_ЕстьДанныеДляОбработки");
		...
		Иначе
			ТекстИсключения = "Для типа метаданных ""%1"" не поддерживается проверка в функции %2";
			ТекстИсключения = СтрШаблон(ТекстИсключения, Строка(МетаданныеОбъекта), "ОбновлениеВерсииИБСервер.ОИБ_ЕстьДанныеДляОбработки");
	...
	БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ИтоговыйМассивЗапросов, ТекстыЗапросаРегистров);
	...
	Для Каждого ТекстЗапроса Из ИтоговыйМассивЗапросов Цикл
		...
		Иначе
			...
			Иначе
				Запрос.УстановитьПараметр("Узлы", ПланыОбмена.ОбновлениеИнформационнойБазы.УзелПоОчереди(Очередь, Ложь));
			КонецЕсли;
		КонецЕсли;
...
Функция ОИБ_ЭтоПростойИсточникДанных(ДополнительныеИсточникиДанных)
	...
	Если ПростойИсточник И СложныйИсточник Тогда
		Ошибка	= "Источник данных задан неверно (см. %1).";
		Ошибка	= СтрШаблон(Ошибка, "ОИБ_ДополнительныеПараметрыВыборкиДанныхДляОбработки()");
...
Функция ОИБ_ОбъектОбработан(Данные) Экспорт
	...
	Если МетаданныеИОтбор <> Неопределено И ДоступныеДляРедактирования <> Неопределено И ДоступныеДляРедактирования.Найти(МетаданныеИОтбор.Отбор) <> Неопределено Тогда
	...
	Если НезавершенныеОбработчики.Количество() > 0 Тогда
		...
		ЧастиИсключения.Добавить("Действия с объектом временно запрещены, так как не завершен переход на новую версию программы.
			|Это плановый процесс, который скоро завершится.");
		ЧастиИсключения.Добавить(СтрСоединить(СтрРазделить("Для включения возможности редактирования можно нажать Еще - Разблокировать.
			|Разблокировку следует применять только в крайних случаях, так как документ может быть
			|записан некорректно.", Символы.ПС), " "));
		ЧастиИсключения.Добавить("Следующие процедуры обработки данных не завершены");
...
Процедура ОИБ_ЗаписатьОбъект(Знач Объект, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, Знач ВключитьБизнесЛогику = Ложь, ДокументРежимЗаписи = Неопределено) Экспорт
	...
	Если ДокументРежимЗаписи <> Неопределено Тогда
		Если ТипЗнч(ДокументРежимЗаписи) <> Тип("РежимЗаписиДокумента") Тогда
			ТекстИсключения = СтрШаблон("Неправильный тип параметра %1", "ДокументРежимЗаписи");
	...
	Если ОИБ_ОбъектРегистрируетсяНаПланеОбмена(Объект) Тогда
...
Функция ОИБ_МетаданныеИОтборПоДанным(Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ТипДанных = Тип("ДанныеФормыСтруктура") Тогда
		...
		ИначеЕсли БазоваяПодсистемаКлиентСервер.ОН_ЕстьРеквизитИлиСвойствоОбъекта(Данные, "ИсходныйКлючЗаписи") Тогда
			...
			КонецЦикла;
		Иначе
			ТекстИсключения	= "Процедура %1 не может быть использована для этой формы.";
			ТекстИсключения	= СтрШаблон(ТекстИсключения, "ОбновлениеВерсииИБСервер.ОИБ_МетаданныеИОтборПоДанным");
		КонецЕсли;
	Иначе
		Если МетаданныеОбъекта = Неопределено Тогда
		...
		ИначеЕсли БазоваяПодсистемаСервер.ОН_ЭтоРегистр(МетаданныеОбъекта) Тогда
			...
			КонецЕсли;
		Иначе
			ТекстИсключения	= "Для этого типа метаданных не поддерживается анализ в функции %1.";
			ТекстИсключения	= СтрШаблон(ТекстИсключения, "ОбновлениеВерсииИБСервер.ОИБ_МетаданныеИОтборПоДанным");
...
Процедура ОИБ_ОтметитьКОбработке(ОсновныеПараметры, Данные, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ОсновныеПараметры.Свойство("ПерезапускОбновления") И ОсновныеПараметры.ПерезапускОбновления = Истина Тогда
		Узел = ПланыОбмена.ОбновлениеИнформационнойБазы.УзелПоОчереди(ОсновныеПараметры.Очередь, Истина);
	Иначе
		Узел = ПланыОбмена.ОбновлениеИнформационнойБазы.УзелПоОчереди(ОсновныеПараметры.Очередь, Ложь);
	КонецЕсли;
	...
	Если ДополнительныеПараметры.ЭтоДвижения Тогда
		...
		Если ДополнительныеПараметры.ОтметитьВсеРегистраторы Тогда
			...
			Иначе
				ТекстИсключения = "Для регистрации всех регистраторов регистра необходимо в параметре ""Данные"" передать объект метаданных документ или ссылку на документ.";
...
Процедура ОИБ_ЗарегистрироватьИзменения(Параметры, Узел, Данные, ВидДанных, ПолноеИмяОбъекта = "")
	ПараметрыРегистрации = ОИБ_НовыеПараметрыРегистрации(Параметры, Узел, ВидДанных, ПолноеИмяОбъекта);
	ОИБ_ЗарегистрироватьИзмененияЭлементаДанных(Данные, ПараметрыРегистрации);
	ОИБ_ЗавершитьРегистрациюПорцииДанных(ПараметрыРегистрации);
КонецПроцедуры

Процедура ОИБ_ЗаписатьПрогрессВыполненияОбработчика(Данные, Узел, МетаданныеОбъекта = Неопределено)
	...
	Иначе
		...
		Если БазоваяПодсистемаСервер.ОН_ЭтоОбъектСсылочногоТипа(МетаданныеОбъекта) Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяТаблицыИзменений", ПолноеИмя + ".Изменения");
...
Процедура ОИБ_ЗарегистрироватьПроблемуСДанными(ПроблемныйОбъект, УточнениеПроблемы, Параметры = Неопределено) Экспорт
	// Зарезервировано для новых подсистем
КонецПроцедуры

Процедура ОИБ_ЗаписатьСобытиеВЖурналРегистрации(Комментарий, Уровень = Неопределено, Параметры = Неопределено) Экспорт
	...
	Если Уровень = УровеньЖурналаРегистрации.Ошибка Или Уровень = Неопределено Тогда
		сОИБ_ДобавитьИнформациюОбОшибкеВОбработчике(ИмяОбработчика);
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка,,, Комментарий);
	ИначеЕсли Уровень = УровеньЖурналаРегистрации.Предупреждение Тогда
		сОИБ_ДобавитьИнформациюОбОшибкеВОбработчике(ИмяОбработчика);
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Предупреждение,,, Комментарий);
	Иначе
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Информация,,, Комментарий);
	КонецЕсли;
КонецПроцедуры

Процедура ОИБ_ПерезапуститьОтложенноеОбновление(Отбор = Неопределено) Экспорт
	Если БазоваяПодсистемаСервер.ОН_ЭтоПодчиненныйУзелРИБ() Тогда
		Возврат;
	КонецЕсли;

	Если ТипЗнч(Отбор) = Тип("Структура") И Отбор.Количество() = 0 Тогда
		ВызватьИсключение "В отборе не указан список подсистем, для которых требуется перезапустить отложенные обработчики.";
	КонецЕсли;
	...
	ОтборЗаданий.Вставить("ИмяМетода", "ОбновлениеВерсииИБСервер.сОИБ_ВыполнитьОтложенноеОбновление");
	...
	Если ТипЗнч(Отбор) = Тип("Массив") Тогда
		...
		Для Каждого Обработчик Из Отбор Цикл
			ОИБ_СброситьСостояниеОбработчика(Обработчик);
		...
		Для Каждого НезавершенныйОбработчик Из НезавершенныеОбработчики Цикл
			ОИБ_СброситьСостояниеОбработчика(НезавершенныйОбработчик.ИмяОбработчика);
	...
	ИначеЕсли ТипЗнч(Отбор) = Тип("Структура") Тогда
		...
		сОИБ_ОбновитьСписокВыполняемыхОбработчиковОбновления(ИтерацииОбновления, Ложь, "Отложенные");
	КонецЕсли;

	сОИБ_ЗаписатьСведенияОБлокируемыхОбъектах(СведенияОБлокируемыхОбъектах);
	сОИБ_ЗаполнитьДанныеДляПараллельногоОтложенногоОбновления(ПараметрыРегистрации);
	...
	Попытка
		...
		Для Каждого ВременнаяОчередь Из ВременныеОчереди Цикл
			...
			ОсновнаяОчередь			= ПланыОбмена.ОбновлениеИнформационнойБазы.УзелПоОчереди(Очередь, Ложь);
			...
			ОсновнаяОчередьОбъект.Наименование	= XMLСтрока(Очередь) + " Старая после перезапуска";
...
Процедура ОИБ_СброситьСостояниеОбработчика(Обработчик)
...
Функция сОИБ_НастройкиПодсистемы() Экспорт
	...
	УправлениеАвтоцентромСервер.ПриОпределенииНастроек(Настройки);
...
Процедура сОИБ_ЗавершитьВыполнениеОтложенногоОбработчикаОбновления(КонтекстОбработчика)
	...
	Если ПараллельныйРежим И ОбработчикОбновления.Статус = Перечисления.СтатусыОбработчиковОбновления.Ошибка И ОбработчикОбновления.ЧислоПопыток >= сОИБ_МаксимумПопытокОбновления(ОбработчикОбновления) Тогда
		...
		ТекстОшибки		= СтрШаблон(ШаблонОшибки, КонтекстОбработчика.ИмяОбработчика);
...
Процедура сОИБ_ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ИмяОбработчика)
	...
	Попытка
		...
		Если Параметры.ОбработкаЗавершена = Неопределено Тогда
			ТекстОшибки = "Обработчик обновления не инициализировал параметр %1.
				|Выполнение прервано из-за явной ошибки в коде обработчика.";
			ТекстОшибки = СтрШаблон(ТекстОшибки, "ОбработкаЗавершена");
...
Функция сОИБ_НайтиОбработчикОбновления(КонтекстОбработчика, ПараметрыОбновления = Неопределено)
	...
	Пока Истина Цикл
		...
		Если Результат.ОбработчикДляВыполнения = Неопределено И Результат.ЕстьНевыполненные И Не Результат.ПрерватьВыполнение Тогда
...
Процедура сОИБ_УстановитьПараметрыОбработчикаОбновления(ОбработчикОбновления, Отложенно = Ложь, Параллельно = Ложь)
	...
	ПараметрыОбработчикаОбновления.ИзменяемыеОбъекты						= сОИБ_УникальныеЗначенияЧерезЗапятую(ПолучениеСвойств.ИзменяемыеОбъекты);
...
Процедура сОИБ_ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, ПараметрыОбновления, СведенияОбОбновлении)
	...
	Попытка
		...
		Если КонтекстОбработчика.ЗаписыватьВЖурнал Тогда
			КонтекстОбработчика.ДетальноеОписаниеОбработчика = сОИБ_ПодготовитьДетальнуюИнформациюОХодеОбновления(ОбработчикОбновления, Параметры, ОбработчикОбновления.ИмяБиблиотеки, Истина);
		КонецЕсли;
...
Функция сОИБ_МаксимумПопытокОбновления(ОбработчикОбновления)
	Если ОбработчикОбновления.Многопоточный Тогда
		...
		КоличествоПотоков		= сОИБ_КоличествоПотоковОбновленияИнформационнойБазы();
...
Процедура сОИБ_ОбработатьИсключениеОбработчика(КонтекстОбработчика, ОбработчикОбновления, ИнформацияОбОшибке)
	...
	Иначе
		ОИБ_ЗаписатьСобытиеВЖурналРегистрации(ПодробноеПредставлениеОшибки,, КонтекстОбработчика.Параметры);
	КонецЕсли;
...
Процедура сОИБ_ЗаписатьДетальнуюИнформациюОХодеОбновления(ОписаниеОбработчика)
	...
	КопияОписания = БазоваяПодсистемаСервер.ОН_СкопироватьРекурсивно(ОписаниеОбработчика);
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		БазоваяПодсистемаСервер.ОН_ЗначениеВСтрокуXML(КопияОписания));
КонецПроцедуры
...
Функция сОИБ_ОстатокМассива(Массив, ЗначениеНачала)
	ОстатокМассива			= Новый Массив;
	ТочноеЗначениеНачала	= СокрЛП(ЗначениеНачала);
	ЗначениеНайдено			= Ложь;

	Для Каждого Элемент Из сОИБ_СтрРазделитьСокрЛП(Массив, ",") Цикл
...
Функция сОИБ_ПараметрыОбходаДанныхДляОбновления(ПараметрыПоиска)
	...
	Если Не ПустаяСтрока(ПолныеИменаОбъектов) И Не ПустаяСтрока(ПерваяСсылочная) Тогда // Есть ссылочные объекты.
		...
		ПолныеИменаОбъектовМассив = сОИБ_ОстатокМассива(ПолныеИменаОбъектов, ПерваяСсылочная);
	Иначе
		ПолныеИменаОбъектовМассив = сОИБ_СтрРазделитьСокрЛП(ПолныеИменаОбъектов, ",");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ПолныеИменаРегистровНачало) И Не ПустаяСтрока(ПерваяТабличная) Тогда // Есть табличные объекты.
		...
		ПолныеИменаРегистровНачалоМассив = сОИБ_ОстатокМассива(ПолныеИменаРегистровНачало, ПерваяТабличная);
	Иначе
		ПолныеИменаРегистровНачалоМассив = сОИБ_СтрРазделитьСокрЛП(ПолныеИменаРегистровНачало, ",");
	КонецЕсли;
...
Функция сОИБ_ПереданныеПараметрыОбработчикаОбновления(Параметры)
	...
	Для Каждого Параметр Из Параметры Цикл
		Если Параметр.Ключ <> "ОбработкаЗавершена" И Параметр.Ключ <> "ПрогрессВыполнения" И Параметр.Ключ <> "Очередь" И Параметр.Ключ <> "ОбновляемыеДанные" Тогда
...
Функция сОИБ_КоличествоПотоковОбновленияИнформационнойБазы() Экспорт
	Если сОИБ_РазрешеноМногопоточноеОбновление() Тогда
		...
		Если ПозицияПараметра > 0 Тогда
			...
			Исключение
				ТекстИсключения = "Укажите параметр запуска программы ""%1"" в формате
					|""%1=Х"", где ""Х"" - максимальное количество потоков обновления.";
				ТекстИсключения = СтрШаблон(ТекстИсключения, "ЧислоПотоковОбновления");
...
Функция сОИБ_РежимОбновленияДанных() Экспорт
	...
	Если НЕ СтандартнаяОбработка Тогда
		...
		Сообщение = СтрШаблон("Недопустимое значение параметра %1 в %2.
			|Ожидалось: %3, %4 или %5; передано значение: %6 (тип %7).", "РежимОбновленияДанных", "ПриОпределенииРежимаОбновленияДанных", "НачальноеЗаполнение", "ОбновлениеВерсии", "ПереходСДругойПрограммы", РежимОбновленияДанных, ТипЗнч(РежимОбновленияДанных));
		БазоваяПодсистемаКлиентСервер.ОН_Проверить(РежимОбновленияДанных = "НачальноеЗаполнение" Или РежимОбновленияДанных = "ОбновлениеВерсии" Или РежимОбновленияДанных = "ПереходСДругойПрограммы", Сообщение);
	...
	Если МакетОписаниеОбновлений <> Неопределено Тогда
		...
		Для каждого Область Из Макет.Области Цикл
			...
			Если ВесВерсии > ВесВерсииМетаданных Тогда
				ТекстИсключения = "В общем макете %1 для одного из разделов изменений
					|установлена версия выше, чем в метаданных. (%2, должна быть %3)";
				ТекстИсключения = СтрШаблон(ТекстИсключения, "ОписаниеИзмененийСистемы", Версия, Метаданные.Версия);
...
Процедура сОИБ_ПриДобавленииОбработчиковУстановкиПараметровСеанса(Обработчики) Экспорт
	...
	Обработчики.Вставить("ДлительныеОперации",				"БазоваяПодсистемаСервер.ДО_УстановкаПараметровСеанса");
КонецПроцедуры
		...
		Если сОИБ_ФорсироватьОбновление(СведенияОбОбновлении) Тогда
			...
			Пока ОбработчикиВыполнялись Цикл
				...
				Если ДатаПоследнейПроверки = Неопределено Или ТекущаяДатаСеанса() - ДатаПоследнейПроверки > 600 Тогда
					...
					сОИБ_ОчиститьПрогрессОбработкиЗаИнтервалыПредыдущегоДня();
				КонецЕсли;
...
Процедура сОИБ_ПроверитьПараметрыВыборки(ПараметрыВыборки)
	...
	Если Не ИзвестныйСпособ Тогда
		...
		ВызватьИсключение СтрШаблон(ШаблонСообщения, СпособВыборки, "Параметры.ПараметрыВыборки.СпособВыборки");
	КонецЕсли;
	...
	Если Не ТаблицыУказаны Тогда
		ШаблонСообщения = "Укажите обрабатываемые таблицы в процедуре регистрации данных к обновлению.
			|Указывается в ""%1"" и/или
			|""%2"".";

		ВызватьИсключение СтрШаблон(ШаблонСообщения, "Параметры.ПараметрыВыборки.ПолныеИменаОбъектов", "Параметры.ПараметрыВыборки.ПолныеИменаРегистров");
	КонецЕсли;
КонецПроцедуры
...
Функция сОИБ_ЗавершитьПотокиВыполнившиеФЗ(Потоки, Группы)
	...
	Пока Индекс >= 0 Цикл
		...
		Если Не ЗначениеЗаполнено(ИдентификаторЗадания) Или ЗаданиеВыполнено <> Ложь Тогда
			Выполнять = Не ПустаяСтрока(ОписаниеПотока.ПроцедураЗавершения) И Поток.ПараметрыПроцедурыЗавершения <> Неопределено И (Не ЗначениеЗаполнено(ИдентификаторЗадания) Или ЗаданиеВыполнено = Истина) И Поток.АдресРезультата <> Неопределено;
...
Функция сОИБ_НовоеОписаниеГруппПотоковОбработчиковОтложенногоОбновления() Экспорт
...
Функция сОИБ_ОбработчикиПереходаСДругойПрограммы(ПредыдущееИмяКонфигурации, ИтерацииОбновления)
	...
	ВсеНовыеПодсистемы		= сОИБ_СведенияОбОбновленииИнформационнойБазы().ВсеНовыеПодсистемы;
	...
	ОбработчикПерехода.Процедура					= "ОбновлениеВерсииИБСервер.сОИБ_НачальноеЗаполнениеПредопределенныхДанных";
	...
	Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
		...
		Для Каждого Обработчик Из ОбработчикиНачальногоЗаполнения Цикл
			...
			ПолноеИмяОбъекта	= сОИБ_ИмяОбъектаМетаданныхПоИмениМенеджера(ИмяМенеджера);
	...
	БазоваяПодсистемаКлиентСервер.ОН_ДополнитьМассив(ВыбранныеОбработчики, ОбработчикиПерехода.НайтиСтроки(Отбор), Истина);
...
Процедура сОИБ_ПерейтиСДругойПрограммы(ИтерацииОбновления)
	...
	ОбработчикиИНовыеОбъекты	= сОИБ_ОбработчикиПереходаСДругойПрограммы(ПредыдущееИмяКонфигурации, ИтерацииОбновления);
	...
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Попытка
			...
			Если Обработчик.Процедура = "ОбновлениеВерсииИБСервер.сОИБ_НачальноеЗаполнениеПредопределенныхДанных" Тогда
...
Процедура сОИБ_ВыполнитьДействияПриОбновленииИнформационнойБазы(ПараметрыОбновления, ДополнительныеПараметры)
	...
	Попытка
		Если РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
			сОИБ_ПерейтиСДругойПрограммы(ИтерацииОбновления);
			ИтерацииОбновления		= сОИБ_ИтерацииОбновления();
	...
	сОИБ_ОчиститьЗарегистрированныеПроблемыСДанными();
...
Процедура сОИБ_ОбновитьСписокВыполняемыхОбработчиковОбновления(ИтерацииОбновления, ПервыйОбменВРИБ = Ложь, ТипыОбработчиков = Неопределено) Экспорт
	...
	Попытка
		...
		ИначеЕсли ТолькоОтложенные Тогда
			...
			БазоваяПодсистемаКлиентСервер.ОН_ДополнитьТаблицу(МонопольныеОбработчики, ОперативныеОбработчики);
		...
		Для Каждого ИтерацияОбновления Из ИтерацииОбновления Цикл
			Если Не ТолькоМонопольные И ЗаполнитьВерсииПодсистем Тогда
				...
				Иначе
					Если БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсии(СохраненнаяВерсия, ИтерацияОбновления.ПредыдущаяВерсия) > 0 Тогда
						ВерсииПодсистемНаНачалоОбновления.Вставить(ИтерацияОбновления.Подсистема, ИтерацияОбновления.ПредыдущаяВерсия);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			ОбработчикиПоВерсиям.Колонки.Процедура.Имя	= "ИмяОбработчика";
				...
				сОИБ_ДобавитьОтложенныеОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям.Строки, ГруппаОбновления, ТекстОшибок, ТекущийРежимВыполнения);
		КонецЦикла;
...
Функция сОИБ_ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если сОИБ_ВерсияИБ(ИдентификаторБиблиотеки) <> ВерсияМетаданныхИБ Тогда
		сОИБ_УстановитьВерсиюИБ(ИдентификаторБиблиотеки, ВерсияМетаданныхИБ, ИтерацияОбновления.ЭтоОсновнаяКонфигурация, ОтметитьРегистрациюДанных);
	КонецЕсли;

	Возврат ВыполняемыеОбработчики;
КонецФункции

Процедура сОИБ_ЗаполнитьДанныеДляПараллельногоОтложенногоОбновления(Параметры) Экспорт
	...
	Если Не (БазоваяПодсистемаСерверПовтИсп.СП_ИспользуетсяРИБ("СФильтром") И БазоваяПодсистемаСервер.ОН_ЭтоПодчиненныйУзелРИБ()) И Не ПерезапускОбновления Тогда
		Запрос			= Новый Запрос;
		Запрос.Текст	=
		"ВЫБРАТЬ
	...
	Обработчики	= сОИБ_ОбработчикиДляОтложеннойРегистрацииДанных(Ложь, ПерезапускОбновления, РегистрируемыеОбработчики);
	Для Каждого Обработчик Из Обработчики Цикл
		...
		Попытка
			...
			Если Обработчик.Многопоточный Тогда
				сОИБ_ИсправитьПолныеИменаВПараметрахВыборки(ПараметрыОбработчикаСтруктура.ПараметрыВыборки);
			КонецЕсли;
...
Процедура сОИБ_ДобавитьОтложенныеОбработчики(ИмяБиблиотеки, ОбработчикиПоВерсиям, ГруппаОбновления, ТекстОшибок, ТекущийРежимВыполнения = "")
	ЭтоПодчиненныйУзелРИБ			= БазоваяПодсистемаСервер.ОН_ЭтоПодчиненныйУзелРИБ();
	...
	Для каждого Обработчик Из ИтерацияОбновления.Обработчики Цикл
		...
		Если ПустаяСтрока(Обработчик.Версия) Тогда
			Если Обработчик.НачальноеЗаполнение <> Истина Тогда
				ОписаниеОшибки = СтрШаблон("У обработчика не заполнено свойство %1 или свойство %2.", "Версия", "НачальноеЗаполнение");
			КонецЕсли;
		ИначеЕсли Обработчик.Версия <> "*" И Не СтрНачинаетсяС(Обработчик.Версия, "ОтладкаОбработчика") Тогда
			...
			Исключение
				НулеваяВерсия	= Ложь;
				ОписаниеОшибки	= СтрШаблон("У обработчика неправильно заполнено свойство %1: ""%2"".
			...
			Если НЕ ЗначениеЗаполнено(ОписаниеОшибки) И Обработчик.ВыполнятьВГруппеОбязательных <> Истина И Обработчик.Приоритет <> 0 Тогда
				ОписаниеОшибки = СтрШаблон("У обработчика неправильно заполнено свойство %1 или
		...
		Если Обработчик.РежимВыполнения <> "" И Обработчик.РежимВыполнения <> "Монопольно" И Обработчик.РежимВыполнения <> "Оперативно" И Обработчик.РежимВыполнения <> "Отложенно" Тогда
			ОписаниеОшибки = СтрШаблон("У обработчика ""%1"" неправильно заполнено свойство %2.
				           |Допустимое значение: ""%3"", ""%4"", ""%5"".", Обработчик.Процедура, "РежимВыполнения", "Монопольно", "Отложенно", "Оперативно");
		КонецЕсли;

		Если НЕ ЗначениеЗаполнено(ОписаниеОшибки) И Обработчик.Опциональный = Истина И Обработчик.НачальноеЗаполнение = Истина Тогда
			ОписаниеОшибки = СтрШаблон("У обработчика не правильно заполнено свойство %1 или
...
Процедура сОИБ_УстановитьВерсиюИБ(Знач ИдентификаторБиблиотеки, Знач НомерВерсии, Знач ЭтоОсновнаяКонфигурация, ВыполненаРегистрация = Неопределено) Экспорт
	НаборЗаписей	= РегистрыСведений.ВерсииПодсистем.СоздатьНаборЗаписей();
...
Функция сОИБ_ОбработчикиДляОтложеннойРегистрацииДанных(БезФильтра = Ложь, ПерезапускОбновления = Ложь, РегистрируемыеОбработчики = Неопределено) Экспорт
...
Процедура сОИБ_ЗаполнитьБлокируемыеОбъекты(СтрокаВерсия, СведенияОБлокируемыхОбъектах)
	Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(БлокируемыеОбъекты) И Не ЗначениеЗаполнено(ПроцедураПроверки) Тогда
			...
			ВызватьИсключение СтрШаблон(ТекстИсключения, Обработчик.ИмяОбработчика, "ПроцедураПроверки");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура сОИБ_ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков, ТекстОшибок)
	...
	Если Обработчик.МонопольныйРежим = Истина Тогда
		ТекстОшибки = СтрШаблон("У отложенного обработчика ""%1""
			|не должен быть установлен признак ""%2"".", Обработчик.ИмяОбработчика, "МонопольныйРежим");
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если РежимВыполненияОтложенныхОбработчиков = "Параллельно" И Обработчик.ЗапускатьТолькоВГлавномУзле И Обработчик.ЗапускатьИВПодчиненномУзлеРИБСФильтрами Тогда
		ТекстОшибки = СтрШаблон("У отложенного обработчика ""%1""
			...
			| - ""%3"".
			|
			|Данные свойства не могут одновременно принимать значение ""Истина"".", Обработчик.ИмяОбработчика, "ЗапускатьТолькоВГлавномУзле", "ЗапускатьИВПодчиненномУзлеРИБСФильтрами");
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
	...
	Если Обработчик.ОбщиеДанные = Истина Тогда
		ТекстОшибки = СтрШаблон("У отложенного обработчика ""%1""
			...
			|
			|Данное свойство не может принимать значение ""Истина"" у отложенного обработчика.", Обработчик.ИмяОбработчика, "ОбщиеДанные");
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
КонецПроцедуры
...
Процедура сОИБ_ЗавершитьРегистрациюДанныхОтложенногоОбновления(ОписаниеОбрабатываемыхДанных, АдресРезультата) Экспорт
	...
	Если ОписаниеОбрабатываемыхДанных.Свойство("ПорцииДляОбновления") Тогда
		сОИБ_ИсправитьПолныеИменаВПараметрахВыборки(ОписаниеОбрабатываемыхДанных.ПараметрыВыборки);
	КонецЕсли;
...
Процедура сОИБ_НачальноеЗаполнениеПредопределенныхДанных(ОбновитьМультиязычныеСтроки = Ложь, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ПроверятьНеобходимостьВыполнения = ДополнительныеПараметры <> Неопределено И ТипЗнч(ДополнительныеПараметры) = Тип("Структура") И ДополнительныеПараметры.Свойство("НовыеОбъекты");

	Для Каждого МетаданныеОбъекта Из ОбъектыСНачальнымЗаполнением Цикл
		Если ПроверятьНеобходимостьВыполнения И ДополнительныеПараметры.НовыеОбъекты.Найти(МетаданныеОбъекта.ПолноеИмя()) = Неопределено Тогда
			Продолжить;
		КонецЕсли;

		ПараметрыЗаполнения = сОИБ_НаборПараметровДляЗаполненияОбъекта(МетаданныеОбъекта);
		...
		СуществующиеЭлементы	= сОИБ_СуществующиеПоставляемыеЭлементы(ПредопределенныеДанные, НастройкиЗаполнения, МенеджерОбъекта, МетаданныеОбъекта);
		...
		Попытка
			Для каждого СтрокаТаблицы Из ПредопределенныеДанные Цикл
				...
				Если ПараметрыЗаполнения.ЛокализуемыеРеквизитыОбъекта.Количество() > 0 И ПараметрыЗаполнения.ЛокализуемыеРеквизитыОбъекта["Наименование"] <> Неопределено Тогда
					Если ПараметрыЗаполнения.ОбъектСодержитТЧПредставления Тогда
						сОИБ_НачальноеЗаполнениеТЧПредставления(ЗаполняемыйЭлемент, СтрокаТаблицы, ПараметрыЗаполнения);
					КонецЕсли;
				ИначеЕсли ОбновитьМультиязычныеСтроки И ПредопределенныеДанные.Колонки.Найти("Наименование") <> Неопределено И ЗначениеЗаполнено(СтрокаТаблицы["Наименование"]) Тогда
					ЗаполняемыйЭлемент["Наименование"] = СтрокаТаблицы["Наименование"];
				КонецЕсли;
...
Функция сОИБ_НастройкиПредопределенныхЭлементов(МенеджерОбъекта, ПредопределенныеДанные) Экспорт
	ПользовательскиеНастройкиЗаполненияЭлементов = сОИБ_ПользовательскиеНастройкиЗаполненияЭлементов();
	...
	НастройкиЗаполненияЭлементов													= сОИБ_НастройкиЗаполненияЭлементов();
	...
	НастройкиЗаполненияЭлементов.ЕстьКолонкаИмяПредопределенныхДанных				= ПредопределенныеДанные.Колонки.Найти("ИмяПредопределенныхДанных") <> Неопределено;
	НастройкиЗаполненияЭлементов.ПереопределяемыеНастройки.ИмяКлючевогоРеквизита	= ?(ЗначениеЗаполнено(НастройкиЗаполненияЭлементов.ПереопределяемыеНастройки.ИмяКлючевогоРеквизита), НастройкиЗаполненияЭлементов.ПереопределяемыеНастройки.ИмяКлючевогоРеквизита, "ИмяПредопределенныхДанных");
...
Процедура сОИБ_ЗапускРегистрацииДанныхОтложенногоОбновления(ИдентификаторФормы, АдресРезультата) Экспорт
	...
	Попытка
		ДоступнаПакетнаяРегистрация = ОИБ_ДоступнаПакетнаяРегистрация();
...
Процедура сОИБ_ИсправитьПолныеИменаВПараметрахВыборки(ПараметрыВыборки)
	ПараметрыВыборки.ПолныеИменаОбъектов	= сОИБ_УникальныеЗначенияЧерезЗапятую(ПараметрыВыборки.ПолныеИменаОбъектов);
	ПараметрыВыборки.ПолныеИменаРегистров	= сОИБ_УникальныеЗначенияЧерезЗапятую(ПараметрыВыборки.ПолныеИменаРегистров);
КонецПроцедуры

Функция сОИБ_УникальныеЗначенияЧерезЗапятую(Строка)
...
Функция сОИБ_НаборПараметровДляЗаполненияОбъекта(МетаданныеОбъекта) Экспорт
	Результат = Новый Структура;
	...
	Если ОбъектСодержитТЧПредставления И ПоддерживаетсяИерархия Тогда
		Если МетаданныеОбъекта.ТабличныеЧасти.Представления.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппыИЭлемента Или МетаданныеОбъекта.ТабличныеЧасти.Представления.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы Тогда
...
Функция сОИБ_НастройкиЗаполненияЭлементов() Экспорт
	НастройкиЗаполненияЭлементов = Новый Структура;
	НастройкиЗаполненияЭлементов.Вставить("ПереопределяемыеНастройки",				сОИБ_ПользовательскиеНастройкиЗаполненияЭлементов());
	НастройкиЗаполненияЭлементов.Вставить("ЕстьКолонкаИмяПредопределенныхДанных",	Ложь);

	Возврат НастройкиЗаполненияЭлементов;
КонецФункции

Функция сОИБ_ПользовательскиеНастройкиЗаполненияЭлементов()
	НастройкиЗаполненияЭлементов = Новый Структура;
...
Функция сОИБ_СуществующиеПоставляемыеЭлементы(ПредопределенныеДанные, НастройкиПредопределенныхЭлементов, Знач МенеджерОбъекта, МетаданныеОбъекта)
	...
	Если ИмяКлючевогоРеквизита <> "Ссылка" Тогда
		НаборКлючей.Колонки.Добавить("Ключ", Новый ОписаниеТипов("Строка",, Новый КвалификаторыСтроки((150))));
	Иначе
		Массив = Новый Массив;
	...
	Если СтрСравнить(ИмяКлючевогоРеквизита, "ИмяПредопределенныхДанных") <> 0 Тогда
		Для Каждого ПредопределенныйЭлемент Из ПредопределенныеДанные Цикл
			Если ЕстьКолонкаИмяПредопределенныхДанных И ЗначениеЗаполнено(ПредопределенныйЭлемент.ИмяПредопределенныхДанных) И ИменаПредопределенных.Найти(ПредопределенныйЭлемент.ИмяПредопределенныхДанных) <> Неопределено Тогда
		...
		Если НаборКлючей.Количество() > 0 Тогда
			...
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяПоля", "ТаблицаДанных." + ИмяКлючевогоРеквизита);
...
Функция сОИБ_РасшифровкаЯчейки(Статус, ЕстьОшибки, ПроблемаВДанных) Экспорт
	Массив = Новый Массив;
...
Функция сОИБ_КоличествоПроблемСДанными() Экспорт
	Возврат 0;

	// Зарезервировано для новых подсистем
КонецФункции

Процедура сОИБ_ДобавитьИнформациюОбОшибкеВОбработчике(ИмяОбработчика) Экспорт
	Если Не ЗначениеЗаполнено(ИмяОбработчика) Тогда
		ТекстОшибки = "Не удалось сохранить информацию об ошибке в обработчике
			|по причине:
	...
	Исключение
		ОтменитьТранзакцию();
		ИнформацияОбОшибке	= ИнформацияОбОшибке();
		ТекстОшибки			= "Не удалось сохранить информацию об ошибке в обработчике %1
			|по причине:
			|%2";
		ТекстОшибки = СтрШаблон(ТекстОшибки, ИмяОбработчика, ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	КонецПопытки
КонецПроцедуры

Процедура сОИБ_НачальноеЗаполнениеТЧПредставления(ЗаполняемыйЭлемент, СтрокаТаблицы, ПараметрыЗаполнения) Экспорт
	Языки							= БазоваяПодсистемаСервер.СП_ЯзыкиКонфигурации();
	ЛокализуемыеРеквизитыОбъекта	= ПараметрыЗаполнения.ЛокализуемыеРеквизитыОбъекта;

	Для каждого КодЯзыка Из Языки Цикл
		Если СтрСравнить(КодЯзыка, Метаданные.ОсновнойЯзык.КодЯзыка) = 0 Тогда
			Продолжить;
		КонецЕсли;
...
Процедура сОИБ_ОчиститьЗарегистрированныеПроблемыСДанными()
	// Зарезервировано для новых подсистем
КонецПроцедуры

Процедура сОИБ_ОчиститьПрогрессОбработкиЗаИнтервалыПредыдущегоДня()
	Запрос			= Новый Запрос;
	Запрос.Текст	= "ВЫБРАТЬ
