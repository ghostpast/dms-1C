...
Функция ОИБ_ОбъектОбработан(Данные) Экспорт
	...
	Для Каждого Обработчик Из ОбработчикиОбъекта Цикл
		...
		ИначеЕсли ТипЗнч(Данные) = Тип("Строка") Тогда
			Обработан = Ложь;
		Иначе
			Обработан = Вычислить(СвойстваОбработчика.ПроцедураПроверки + "(Параметры)");
		КонецЕсли;
...
Процедура сОИБ_ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, ПараметрыОбновления, СведенияОбОбновлении)
	...
	сОИБ_ЗавершитьПроцедуруОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления);
КонецПроцедуры
...
Процедура сОИБ_ОбновитьСписокВыполняемыхОбработчиковОбновления(ИтерацииОбновления, ПервыйОбменВРИБ = Ложь)
	СведенияОбОбновлении				= сОИБ_СведенияОбОбновленииИнформационнойБазы();
...
Процедура сОИБ_НачальноеЗаполнениеПредопределенныхДанных(ОбновитьМультиязычныеСтроки = Ложь) Экспорт
	ОбъектыСНачальнымЗаполнением	= сОИБ_ОбъектыСНачальнымЗаполнением();
	Языки							= БазоваяПодсистемаСервер.М_ЯзыкиКонфигурации();
	СведенияОбЯзыках				= БазоваяПодсистемаСерверПовтИсп.М_СведенияОбЯзыках();

	Для Каждого МетаданныеОбъекта Из ОбъектыСНачальнымЗаполнением Цикл
		ЛокализуемыеРеквизитыОбъекта	= БазоваяПодсистемаСервер.М_МультиязычныеРеквизитыОбъекта(МетаданныеОбъекта);
		МультиязычныеСтрокиВРеквизитах	= БазоваяПодсистемаСервер.М_МультиязычныеСтрокиВРеквизитах(МетаданныеОбъекта);
			...
			Для каждого СтрокаТаблицы Из ПредопределенныеДанные Цикл
				...
				Если Не ОбновитьМультиязычныеСтроки Тогда
					...
					Если ПоддерживаетсяИерархия И ЗаполняемыйЭлемент.ЭтоГруппа Тогда
						КромеРеквизитов = РеквизитыИсключения + ?(ПустаяСтрока(РеквизитыИсключения), "", ",") + СтрСоединить(РеквизитыСЭлементами, ",");
					КонецЕсли;
...
Функция сОИБ_ОбъектыСНачальнымЗаполнением()
...
Процедура сОИБ_ДобавитьКолонкуТаблицыПредопределенныхДанных(ПредопределенныеДанные, Реквизит, ЛокализуемыеРеквизиты, Языки)
	...
	Для каждого Язык Из Языки Цикл
		ПредопределенныеДанные.Колонки.Добавить(Реквизит.Имя + "_" + Язык, Реквизит.Тип, "Локализуемый");
	КонецЦикла;
КонецПроцедуры
