...
Функция ОИБ_СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных(Очередь, ПолноеИмяОбъекта, МенеджерВременныхТаблиц, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Иначе
		ТекстИсключения = "rДля этого типа метаданных не поддерживается проверка в функции ОбновлениеВерсииИБСервер.ОИБ_СоздатьВременнуюТаблицуЗаблокированныхДляЧтенияИИзмененияДанных.";
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Иначе
		...
		Для Каждого КлючЗначение Из ДополнительныеПараметры.ДополнительныеИсточникиДанных Цикл
			...
			Для Каждого ТипИсточника Из ТипыИсточника Цикл
				Если ОИБ_ЭтоПримитивныйТип(ТипИсточника) Тогда
					Продолжить;
				КонецЕсли;
...
Процедура ОИБ_ДобавитьПроверкуБлокировкиДополнительныхИсточников(Очередь, ТекстЗапроса, ПолноеИмяОбъекта, ПолноеИмяРегистра, МенеджерВременныхТаблиц, ЭтоСозданиеВременнойТаблицы, ДополнительныеПараметры)
	...
	Иначе
		...
		Для Каждого КлючЗначение Из ДополнительныеПараметры.ДополнительныеИсточникиДанных Цикл
			...
			Если БазоваяПодсистемаСервер.СП_ЭтоТаблицаРегистра(ИсточникДанных)
...
Функция ОИБ_ОтложенноеОбновлениеЗавершено(Знач ИменаПодсистем = Неопределено) Экспорт
	...
	Сведения = сОИБ_СведенияОбОбновленииИнформационнойБазы();
...
Процедура сОИБ_СоставитьПланОтложенногоОбновления(СведенияОбОбновленииИБ, ПовторноеФормирование = Ложь) Экспорт
	ЭтоПодчиненныйУзелРИБ			= БазоваяПодсистемаСервер.ОН_ЭтоПодчиненныйУзелРИБ();
	...
	СведенияОБлокируемыхОбъектах = сОИБ_НовыеСведенияОБлокируемыхОбъектах();
	Для Каждого Подсистема Из ОписанияПодсистем.Порядок Цикл
		...
		Для Каждого ДеревоОбработчиковВерсия Из ДеревоОбработчиковБиблиотека.Строки Цикл
			Если Не ПовторноеФормирование Тогда
				сОИБ_ЗаполнитьБлокируемыеОбъекты(ДеревоОбработчиковВерсия, СведенияОбОбновленииИБ, СведенияОБлокируемыхОбъектах);
			КонецЕсли;
...
Процедура сОИБ_ЗаполнитьБлокируемыеОбъекты(СтрокаВерсия, СведенияОбОбновлении, СведенияОБлокируемыхОбъектах)
...
Процедура сОИБ_ЗаписатьСведенияОБлокируемыхОбъектах(Сведения)
...
Процедура сОИБ_ВыполнитьОтложенноеОбновлениеСейчас(ПараметрыОбновления = Неопределено) Экспорт
	...
	Пока ОбработчикиВыполнялись Цикл
		ОбработчикиВыполнялись = сОИБ_ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении, ПараметрыОбновления);
	КонецЦикла;

	сОИБ_ЗаписатьСведенияОбОбновленииИнформационнойБазы(СведенияОбОбновлении);
КонецПроцедуры

Функция сОИБ_ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении, ПараметрыОбновления = Неопределено)
	КонтекстОбработчика		= сОИБ_НовыйКонтекстОбработчика();
	ОбработчикОбновления	= сОИБ_НайтиОбработчикОбновления(КонтекстОбработчика, СведенияОбОбновлении, ПараметрыОбновления);
		...
		Попытка
			Если ОбработчикОбновления.Многопоточный Тогда
				...
				Пока ИтераторОбхода <> Неопределено Цикл
					...
					КонецЕсли;
					сОИБ_ЗавершитьВыполнениеОтложенногоОбработчика(КонтекстОбработчика, АдресРезультата, СведенияОбОбновлении);
...
Процедура сОИБ_ЗавершитьВыполнениеОтложенногоОбработчика(КонтекстОбработчика, АдресРезультата, СведенияОбОбновлении) Экспорт
	ОбработчикОбновления = сОИБ_НайтиОбработчикВДереве(СведенияОбОбновлении.ДеревоОбработчиков.Строки,
	...
	Если КонтекстОбработчика.ЗапускВыполненБезОшибок Тогда
		сОИБ_ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, СведенияОбОбновлении);
	КонецЕсли;

	сОИБ_ЗавершитьПроцедуруОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, СведенияОбОбновлении);
	ПараметрыСеанса.ПараметрыОбработчикаОбновления = Новый ФиксированнаяСтруктура(сОИБ_НовыеПараметрыОбработчикаОбновления());
	сОИБ_ЗавершитьВыполнениеОтложенногоОбработчикаОбновления(КонтекстОбработчика, СведенияОбОбновлении);
	сОИБ_РассчитатьВремяВыполненияПроцедурыОбработчика(КонтекстОбработчика, ОбработчикОбновления);

	Если ОбработчикОбновления.Многопоточный Тогда
		сОИБ_ЗавершитьВыполнениеМногопоточногоОбработчика(КонтекстОбработчика, СведенияОбОбновлении);
	КонецЕсли;
КонецПроцедуры

Процедура сОИБ_РассчитатьВремяВыполненияПроцедурыОбработчика(КонтекстОбработчика, ОбработчикОбновления)
...
Процедура сОИБ_ЗавершитьВыполнениеМногопоточногоОбработчика(КонтекстОбработчика, СведенияОбОбновлении)
...
Процедура сОИБ_ЗавершитьВыполнениеОтложенногоОбработчикаОбновления(КонтекстОбработчика, СведенияОбОбновлении)
	...
	ОписаниеОбработчика = сОИБ_НайтиОбработчикВТаблице(ОписанияОбработчиков,
		КонтекстОбработчика.ИдентификаторОбработчика,
		КонтекстОбработчика.ИмяОбработчика);
	...
	Если ПараллельныйРежим
		...
		И ОбработчикОбновления.ЧислоПопыток >= сОИБ_МаксимумПопытокОбновления(СведенияОбОбновлении, ОбработчикОбновления) Тогда
	...
	Иначе
		...
		Попытка
			...
			НовыеСведенияОбОбновлении								= сОИБ_СведенияОбОбновленииИнформационнойБазы();
...
Процедура сОИБ_ПослеЗапускаПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, СведенияОбОбновлении)
	...
	Попытка
		...
		ИначеЕсли ПараметрыОбновления.ПараллельныйРежим И Не КонтекстОбработчика.ПропуститьПроверкуОбработанныхДанных Тогда
			...
			Если Не ЕстьОбработанныеОбъекты Тогда
				МинимальнаяОчередь = сОИБ_МинимальнаяОчередьОтложеннойОбработки(ПараметрыОбновления.ПланОбновления);
			КонецЕсли;
			...
			Если Не ЕстьОбработанныеОбъекты
				...
				МаксимумПопыток = сОИБ_МаксимумПопытокОбновления(СведенияОбОбновлении, ОбработчикОбновления) - 1;
		...
		КоличествоЗапусковПроцедурыОбновления	= сОИБ_КоличествоЗапусковПроцедурыОбновления(ОбработчикОбновления) + 1;
...
Функция сОИБ_НайтиОбработчикОбновления(КонтекстОбработчика, СведенияОбОбновлении, ПараметрыОбновления = Неопределено)
	сОИБ_ВключитьОтключитьОтложенныеОбработчикиОбновления(СведенияОбОбновлении);
		...
		Для Каждого ОписаниеЦиклаОбновления Из ПланОбновления Цикл
			...
			Иначе
				...
				Для Каждого ОписаниеОбработчика Из ОписаниеЦиклаОбновления.Обработчики Цикл
					...
					ОбработчикОбновления	= сОИБ_НайтиОбработчикВДереве(ДеревоОбработчиков,
					...
					Если ОбработчикОбновления.Статус = "Выполняется" И Не ОбработчикОбновления.ОбработкаПорцииЗавершена Тогда
						Если ОбработчикОбновления.Многопоточный Тогда
							Если сОИБ_ЕстьПорцииДляОбновления(ОбработчикОбновления, СведенияОбОбновлении) Тогда
					...
					МаксимумПопытокОбновления = сОИБ_МаксимумПопытокОбновления(СведенияОбОбновлении, ОбработчикОбновления);
					Если ОбработчикОбновления.ЧислоПопыток >= МаксимумПопытокОбновления Тогда
						Если ОбработчикОбновления.Статус = "Ошибка" Тогда
							ЕстьОшибки = Истина;

							Продолжить;
						ИначеЕсли сОИБ_ВсеОбработчикиЗациклились(СведенияОбОбновлении) Тогда
							сОИБ_ОтметитьЗациклившиесяОбработчики(СведенияОбОбновлении);
							ЕстьОшибки = Истина;
	...
	сОИБ_УстановитьПараметрыОбработчикаОбновления(ОбработчикОбновления, Истина, ПараллельныйРежим);
	сОИБ_ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика,
...
Функция сОИБ_ЕстьПорцииДляОбновления(ОбработчикОбновления, СведенияОбОбновлении)
...
Функция сОИБ_НайтиОбработчикВДереве(ДеревоОбработчиков, Идентификатор, ИмяОбработчика)
...
Функция сОИБ_НайтиОбработчикВТаблице(ТаблицаОбработчиков, Идентификатор, ИмяОбработчика)
...
Процедура сОИБ_ОтметитьЗациклившиесяОбработчики(СведенияОбОбновлении)
	...
	Для каждого ОбработчикОбновления Из Выполняющиеся Цикл
		...
		Если ОбработчикОбновления.ЧислоПопыток >= МаксимумПопыток Тогда
			...
			ОбработчикОбновления.ИнформацияОбОшибке	= "Произошло зацикливание процедуры обработки данных. Выполнение прервано.";
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Функция сОИБ_ВсеОбработчикиЗациклились(СведенияОбОбновлении)
	Отбор				= Новый Структура("Статус", "Выполняется");
	ДеревоОбработчиков	= СведенияОбОбновлении.ДеревоОбработчиков;
...
Процедура сОИБ_ВключитьОтключитьОтложенныеОбработчикиОбновления(СведенияОбОбновлении)
	ПланОбновления				= СведенияОбОбновлении.ПланОтложенногоОбновления;
...
Процедура сОИБ_ПередЗапускомПроцедурыОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, ПараметрыОбновления, СведенияОбОбновлении)
	...
	Попытка
		...
		КоличествоЗапусковПроцедурыОбновления = сОИБ_КоличествоЗапусковПроцедурыОбновления(ОбработчикОбновления);
		...
		Если КоличествоЗапусковПроцедурыОбновления > 10000 Тогда // Защита от зацикливания.
			...
			ПланОбновления     = СведенияОбОбновлении.ПланОтложенногоОбновления;
			МинимальнаяОчередь = сОИБ_МинимальнаяОчередьОтложеннойОбработки(ПланОбновления);
			...
			Если Не ПараметрыОбновления.ПараллельныйРежим
				...
				ОбработчикОбновления.ЧислоПопыток = сОИБ_МаксимумПопытокОбновления(СведенияОбОбновлении, ОбработчикОбновления);
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
		КонецЕсли;
...
Процедура сОИБ_ЗавершитьПроцедуруОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления, СведенияОбОбновлении)
	Параметры = КонтекстОбработчика.Параметры;
...
Функция сОИБ_КоличествоЗапусковПроцедурыОбновления(ОбработчикОбновления)
...
Функция сОИБ_МинимальнаяОчередьОтложеннойОбработки(ПланОбновления)
	МинимальнаяОчередь = 0;
	Для Каждого ЦиклОбновления Из ПланОбновления Цикл
...
Функция сОИБ_МаксимумПопытокОбновления(СведенияОбОбновлении, ОбработчикОбновления)
	Если ОбработчикОбновления.Многопоточный Тогда
		ОбрабатываемыеДанные	= СведенияОбОбновлении.ОбрабатываемыеДанные[ОбработчикОбновления.ИмяОбработчика];
		ПараметрыВыборки		= ОбрабатываемыеДанные.ПараметрыВыборки;
...
Процедура сОИБ_ОбработатьИсключениеОбработчика(КонтекстОбработчика, ОбработчикОбновления, ИнформацияОбОшибке)
	...
	МаксимумПопытокОбновления = сОИБ_МаксимумПопытокОбновления(сОИБ_СведенияОбОбновленииИнформационнойБазы(), ОбработчикОбновления);
	...
	Попытка
		Если сОИБ_ФорсироватьОбновление(СведенияОбОбновлении) Тогда
			Если СведенияОбОбновлении.ОписаниеПотоков <> Неопределено Тогда
				сОИБ_ОтменитьВыполнениеВсехПотоков(СведенияОбОбновлении.ОписаниеПотоков, СведенияОбОбновлении);
			КонецЕсли;

			ОписаниеПотоков							= сОИБ_НовоеОписаниеПотоковОбработчиковОтложенногоОбновления();
			СведенияОбОбновлении.ОписаниеПотоков	= ОписаниеПотоков;

			Пока ОбработчикиВыполнялись Цикл
				Поток = сОИБ_ДобавитьПотокОбработчикаОтложенногоОбновления(ОписаниеПотоков, СведенияОбОбновлении);

				Если ТипЗнч(Поток) = Тип("СтрокаТаблицыЗначений") Тогда
					сОИБ_ВыполнитьПоток(ОписаниеПотоков, Поток);
					сОИБ_ОжидатьСвободныйПоток(ОписаниеПотоков, СведенияОбОбновлении);
				ИначеЕсли Поток = Истина Тогда
					сОИБ_ОжидатьЗавершениеЛюбогоПотока(ОписаниеПотоков, СведенияОбОбновлении);
				ИначеЕсли Поток = Ложь Тогда
					ОбработчикиВыполнялись = Ложь;
					сОИБ_ОжидатьЗавершениеВсехПотоков(ОписаниеПотоков, СведенияОбОбновлении);
					сОИБ_СохранитьСостояниеПотоковВСведенияОбОбновлении(ОписаниеПотоков, СведенияОбОбновлении);
				...
				Если Не ТребуетсяВыполнение Или Не сОИБ_ФорсироватьОбновление(СведенияОбОбновлении) Тогда
					сОИБ_ОжидатьЗавершениеВсехПотоков(ОписаниеПотоков, СведенияОбОбновлении);
					сОИБ_СохранитьСостояниеПотоковВСведенияОбОбновлении(Неопределено, СведенияОбОбновлении);

					Прервать;
				КонецЕсли;
			КонецЦикла;
		Иначе
			ОбработчикиВыполнялись = сОИБ_ВыполнитьОтложенныйОбработчикОбновления(СведенияОбОбновлении);
			сОИБ_ЗаписатьСведенияОбОбновленииИнформационнойБазы(СведенияОбОбновлении);
		КонецЕсли;
	Исключение
		БазоваяПодсистемаСервер.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Обновление информационной базы", УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		сОИБ_ОтменитьВыполнениеВсехПотоков(СведенияОбОбновлении.ОписаниеПотоков, СведенияОбОбновлении);
		сОИБ_СохранитьСостояниеПотоковВСведенияОбОбновлении(Неопределено, СведенияОбОбновлении);
	КонецПопытки;
...
Процедура сОИБ_ОтменитьВыполнениеВсехПотоков(ОписаниеПотоков, ПараметрыОтмены = Неопределено) Экспорт
...
Функция сОИБ_НовоеОписаниеПотоковОбработчиковОтложенногоОбновления()
	...
	Описание						= сОИБ_НовоеОписаниеПотоков();
	Описание.Группы["Обновление"]	= ГруппаОбновления;
	Описание.Группы["Поиск"]		= ГруппаПоиска;
...
Процедура сОИБ_ПриАварийномЗавершенииПотокаОтложенногоОбработчика(Поток, ИнформацияОбОшибке, СведенияОбОбновлении) Экспорт
	ОбработчикОбновления = сОИБ_НайтиОбработчикВДереве(СведенияОбОбновлении.ДеревоОбработчиков.Строки, Поток.ПараметрыПроцедуры.ИдентификаторОбработчика, Поток.ПараметрыПроцедуры.ИмяОбработчика);
	сОИБ_ОбработатьИсключениеОбработчика(Поток.ПараметрыПроцедуры, ОбработчикОбновления, ИнформацияОбОшибке);

	Если ОбработчикОбновления.Многопоточный Тогда
		сОИБ_ОтменитьОбработкуДанныхМногопоточногоОбработчика(Поток, ОбработчикОбновления, СведенияОбОбновлении);
	КонецЕсли;
КонецПроцедуры

Процедура сОИБ_ПриОтменеПотокаОтложенногоОбработчика(Поток, СведенияОбОбновлении) Экспорт
	ОбработчикОбновления = сОИБ_НайтиОбработчикВДереве(СведенияОбОбновлении.ДеревоОбработчиков.Строки, Поток.ПараметрыПроцедуры.ИдентификаторОбработчика, Поток.ПараметрыПроцедуры.ИмяОбработчика);
...
Функция сОИБ_ДобавитьПотокОбработчикаОтложенногоОбновления(ОписаниеПотоков, СведенияОбОбновлении)
	Поток = Неопределено;

	Пока Поток = Неопределено Цикл
		КонтекстОбработчика		= сОИБ_НовыйКонтекстОбработчика();
		ОбработчикОбновления	= сОИБ_НайтиОбработчикОбновления(КонтекстОбработчика, СведенияОбОбновлении);
		...
		Если ТипЗнч(ОбработчикОбновления) = Тип("СтрокаДереваЗначений") Тогда
			Если КонтекстОбработчика.ВыполнитьОбработчик Тогда
				...
				КонецЕсли;
			Иначе
				сОИБ_ЗавершитьВыполнениеОтложенногоОбработчика(КонтекстОбработчика, Неопределено, СведенияОбОбновлении);
				Поток = Неопределено;
			КонецЕсли;
		Иначе
...
Процедура сОИБ_ВыполнитьПоток(ОписаниеПотоков, Поток, ИдентификаторФормы = Неопределено)
...
Процедура сОИБ_ОжидатьСвободныйПоток(ОписаниеПотоков, Параметры = Неопределено)
	МаксимумПотоков	= сОИБ_КоличествоПотоковОбновленияИнформационнойБазы();
	Потоки			= ОписаниеПотоков.Потоки;

	Пока Потоки.Количество() >= МаксимумПотоков Цикл
		Если сОИБ_ЗавершитьПотокиВыполнившиеФЗ(ОписаниеПотоков, Параметры) Тогда
			Продолжить;
		КонецЕсли;
...
Функция сОИБ_ЗавершитьПотокиВыполнившиеФЗ(ОписаниеПотоков, Параметры = Неопределено)
	...
	Пока Индекс >= 0 Цикл
		...
		Если ИдентификаторЗадания = Неопределено Или ЗаданиеВыполнено <> Ложь Тогда
			Выполнять = Не ПустаяСтрока(ОписаниеПотока.ПроцедураЗавершения) И Поток.ПараметрыПроцедурыЗавершения <> Неопределено И (ИдентификаторЗадания = Неопределено Или ЗаданиеВыполнено = Истина);
...
Процедура сОИБ_ОжидатьЗавершениеЛюбогоПотока(ОписаниеПотоков, Параметры = Неопределено)
	Потоки				= ОписаниеПотоков.Потоки;
	КоличествоПотоков	= Потоки.Количество();

	Пока КоличествоПотоков > 0 И Потоки.Количество() >= КоличествоПотоков Цикл
		Если Не сОИБ_ЗавершитьПотокиВыполнившиеФЗ(ОписаниеПотоков, Параметры) Тогда
			сОИБ_ОжидатьЗавершениеПотока(Потоки[0]);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

Процедура сОИБ_ОжидатьЗавершениеВсехПотоков(ОписаниеПотоков, Параметры = Неопределено)
	Потоки = ОписаниеПотоков.Потоки;
...
Процедура сОИБ_СохранитьСостояниеПотоковВСведенияОбОбновлении(ОписаниеПотоков, СведенияОбОбновлении)
	СведенияОбОбновлении.ОписаниеПотоков = ОписаниеПотоков;
	сОИБ_ЗаписатьСведенияОбОбновленииИнформационнойБазы(СведенияОбОбновлении);
КонецПроцедуры
...
Процедура сОИБ_ОтменитьОбработкуДанныхМногопоточногоОбработчика(Поток, ОбработчикОбновления, СведенияОбОбновлении)
