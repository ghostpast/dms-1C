...
Процедура ПриНачалеРаботыСистемы(Параметры) Экспорт
	...
	Обработчик	= Новый ОписаниеОповещения("ВО_ОбработатьДействияСообщения", ВариантыОтчетовКлиент,, "ВО_ПриОшибкеОбработкиДействияСообщения", ВариантыОтчетовКлиент);

	Попытка
		СистемаВзаимодействия.ПодключитьОбработчикДействияСообщения(Обработчик);
	Исключение
		ИнформацияОбОшибке	= ИнформацияОбОшибке();
		БазоваяПодсистемаКлиент.ЖР_ДобавитьСообщениеДляЖурналаРегистрации("Варианты отчетов.Ошибка подключения обработчика действия сообщения",,"Ошибка", ОбработкаОшибок.ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
	КонецПопытки;
КонецПроцедуры
...
Процедура ВО_ПоделитьсяПользовательскимиНастройками(ОписаниеНастроек) Экспорт
	Если ОписаниеНастроек.Настройки.Элементы.Количество() = 0 Тогда
		БазоваяПодсистемаКлиент.ОН_СообщитьПользователю("Настройки (пользовательские) не установлены.");
...
Процедура сВО_ПоказатьКонтекстнуюНастройкуОтчета(Форма, Элемент, Область, СтандартнаяОбработка) Экспорт
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.НастройкаПоля", ПараметрыФормы, Форма, Форма.УникальныйИдентификатор);
...
Процедура сВО_ДобавитьФормулу(Настройки, КоллекцияПолей, ОписаниеФормулы) Экспорт
	...
	Если Не ЭтоПользовательскоеПолеВыражениеКомпоновкиДанных И (ТипЗнч(ОписаниеФормулы) <> Тип("Структура") Или Не ОписаниеФормулы.Свойство("Формула")) Тогда
		Возврат;
	КонецЕсли;
...
Процедура сВО_ИзменитьФормулу(Форма, Настройки, ПутьКДанным, ИмяКоллекции = "ДоступныеПоляВыбора") Экспорт
	...
	Если ТипЗнч(Формула) <> Тип("ПользовательскоеПолеВыражениеКомпоновкиДанных") Тогда
		ТекстСообщения = СтрШаблон("Редактирование поля ""%1""доступно в форме настроек для технического специалиста на закладке ""Пользовательские поля""", Формула.Заголовок);
	...
	ПараметрыРедактированияФормулы							= КонструкторФормулКлиентСервер.КФ_ПараметрыРедактированияФормулы();
	...
	КонструкторФормулКлиент.КФ_НачатьРедактированиеФормулы(ПараметрыРедактированияФормулы, Обработчик);
КонецПроцедуры
...
Процедура сВО_Сортировать(Форма, Команда, СвойстваЗаголовка = Неопределено) Экспорт
	...
	сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, сВО_ДействиеКоманды(Команда), Неопределено, Ложь)
КонецПроцедуры
...
Процедура сВО_Фильтровать(Форма, ВидСравнения, СвойстваЗаголовка, ДанныеРасшифровки)
	...
	сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, "Фильтровать", СвойстваЗаголовка.Текст);
КонецПроцедуры
...
Процедура сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, Действие, ТекущееПоле = Неопределено, РазвернутьАвтоПоля = Истина)
	...
	сВО_ДобавитьНастройкиВСтек(Форма, ИспользуемыеНастройки, Действие, ТекущееПоле);

	ИдентификаторВладельца = ?(ЭтоФормОтчета, Форма.УникальныйИдентификатор, Форма.ВладелецФормы.УникальныйИдентификатор);

	Результат = сВО_РезультатКонтекстнойНастройки(КомпоновщикНастроек, Действие, ИдентификаторВладельца);
...
Функция сВО_РезультатКонтекстнойНастройки(КомпоновщикНастроек, Действие, ИдентификаторВладельца) Экспорт
...
Процедура сВО_ДобавитьНастройкиВСтек(Форма, Настройки, Действие, ТекущееПоле = Неопределено) Экспорт
	СтекНастроек = сВО_СтекНастроекОтчета(Форма);

	Если СтекНастроек = Неопределено Тогда
		Возврат;
	КонецЕсли;

	сВО_УдалитьРазрывСтекаНастроек(СтекНастроек);
	сВО_СброситьПометкиСтекаНастроек(СтекНастроек);
	...
	Запись.ПредставлениеДействия	= сВО_ПредставлениеДействияСтекаНастроек(Форма, Запись, ТекущееПоле);

	сВО_УдалитьИзбыточныеЗаписиСтекаНастроек(СтекНастроек);
КонецПроцедуры

Функция сВО_ПредставлениеДействияСтекаНастроек(Форма, ЗаписьСтекаНастроек, ТекущееПоле)
	...
	НастройкиОтчета			= Форма.НастройкиОтчета; // см. ВариантыОтчетовСервер.ВО_НастройкиФормыОтчета
	ПредставлениеСобытия	= СтрЗаменить(НастройкиОтчета.СобытияНастроек[Действие], "...", "");

	Если ЗначениеЗаполнено(ТекущееПоле) Тогда
		Возврат СтрШаблон("%1 '%2'", ПредставлениеСобытия, ТекущееПоле);
	КонецЕсли;

	Возврат ПредставлениеСобытия;
КонецФункции

Функция сВО_СтекНастроекОтчета(Форма)
	Если  СтрЗаканчиваетсяНа(Форма.ИмяФормы, "Форма") Или СтрЗаканчиваетсяНа(Форма.ИмяФормы, "ФормаОтчета") Тогда
	...
	Если ВладелецФормы <> Неопределено И (СтрЗаканчиваетсяНа(ВладелецФормы.ИмяФормы, "Форма") Или СтрЗаканчиваетсяНа(ВладелецФормы.ИмяФормы, "ФормаОтчета")) Тогда
...
Процедура сВО_СброситьПометкиСтекаНастроек(СтекНастроек) Экспорт
	Для Каждого Запись Из СтекНастроек Цикл
		Запись.Пометка = Ложь;
	КонецЦикла;
КонецПроцедуры

Процедура сВО_УдалитьИзбыточныеЗаписиСтекаНастроек(СтекНастроек)
	Если СтекНастроек.Количество() <= 10 Тогда
		Возврат;
	КонецЕсли;
...
Процедура сВО_УдалитьРазрывСтекаНастроек(СтекНастроек)
...
Процедура сВО_ПереместитьПолеГоризонтально(Форма, Команда) Экспорт
	...
	сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, сВО_ДействиеКоманды(Команда), СвойстваЗаголовка.Текст);
КонецПроцедуры
...
Функция сВО_КонтекстноеМенюОбластиДанных(СвойстваЗаголовка, ДоступныеВидыСравнения)
	...
	// Расшифровать
	КонтекстноеМеню.Добавить("РасшифроватьПоДетальнымЗаписям", "Расшифровать по детальным записям");
...
Процедура сВО_ОчиститьОформление(Форма, СвойстваЗаголовка) Экспорт
	...
	сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, "ОчиститьОформление", СвойстваЗаголовка.Текст);
КонецПроцедуры
...
#Если ВебКлиент Тогда
	...
	Если БазоваяПодсистемаКлиентСервер.ОН_СравнитьВерсии(СистемнаяИнформация.ВерсияПриложения, "8.3.20.1859") > 0 Тогда
...
Процедура сВО_ПослеВыбораПоля(ВыбранноеПоле, ДополнительныеПараметры) Экспорт
	...
	сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, Действие, СвойстваЗаголовка.Текст);
КонецПроцедуры
...
Процедура сВО_ПоказатьРасширеннуюНастройкуФильтра(Форма, СвойстваЗаголовка) Экспорт
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ФильтроватьПоПолю", ПараметрыФормы, Форма, Форма.УникальныйИдентификатор);
КонецПроцедуры
...
Процедура сВО_СнятьФильтр(Форма, СвойстваЗаголовка, ДанныеРасшифровки = Неопределено) Экспорт
	...
	сВО_ОповеститьОЗавершенииКонтекстнойНастройки(Форма, "СнятьФильтр", СвойстваЗаголовка.Текст);
КонецПроцедуры
...
Процедура сВО_ВыполнитьРасшифровку(ВыполненноеДействие, ПараметрВыполненногоДействия, ДополнительныеПараметры) Экспорт
	...
	Если ВыполненноеДействие =  "РасшифроватьПоДетальнымЗаписям" Тогда
		сВО_ПодготовитьОтборОбработчикаРасшифровки(ДополнительныеПараметры);
		сВО_ПодготовитьПрименяемыеНастройкиОбработчикаРасшифровки(ВыполненноеДействие, ДополнительныеПараметры);
...
Процедура сВО_ПодготовитьОтборОбработчикаРасшифровки(ДополнительныеПараметры)
	Отбор = Новый Структура;

	Данные = ДополнительныеПараметры.Данные;

	Для Каждого Элемент Из Данные.Отбор Цикл
		СвойстваОтбора = сВО_СвойстваЭлементаОтбораОбработчикаРасшифровки(Данные.Настройки.Отбор, Элемент);
		Отбор.Вставить(Элемент.Ключ, СвойстваОтбора);
	КонецЦикла;

	ДополнительныеПараметры.Вставить("Отбор", Отбор);

	сВО_ДополнитьОтборОбработчикаРасшифровки(ДополнительныеПараметры.Отбор, ДополнительныеПараметры.Данные.Настройки.Структура);
КонецПроцедуры

Функция сВО_СвойстваЭлементаОтбораОбработчикаРасшифровки(Отбор, ОсновныеСвойства)
	...
	СвойстваЭлементаОтбора			= ВариантыОтчетовКлиентСервер.сВО_СвойстваОтбораОбработчикаРасшифровкиПоДетальнымЗаписям();
	СвойстваЭлементаОтбора.Значение	= ЗначениеЭлементаОтбора;

	ЭлементыОтбора = БазоваяПодсистемаКлиентСервер.ОН_НайтиЭлементыИГруппыОтбора(Отбор, ОсновныеСвойства.Ключ);
...
Процедура сВО_ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементыСтруктуры)
	Для Каждого ЭлементСтруктуры Из ЭлементыСтруктуры Цикл
		...
		Если ТипЗнч(ЭлементСтруктуры) = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
			сВО_ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементСтруктуры.Настройки.Структура);
		ИначеЕсли ТипЗнч(ЭлементСтруктуры) = Тип("ТаблицаКомпоновкиДанных") Тогда
			сВО_ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементСтруктуры.Строки);
		ИначеЕсли ТипЗнч(ЭлементСтруктуры) = Тип("ГруппировкаКомпоновкиДанных") Или ТипЗнч(ЭлементСтруктуры) = Тип("ГруппировкаТаблицыКомпоновкиДанных") Тогда
			Для Каждого ЭлементОтбора Из ЭлементСтруктуры.Отбор.Элементы Цикл
				сВО_ДобавитьЭлементОтбораОбработчикаРасшифровки(Отбор, ЭлементОтбора);
			КонецЦикла;

			сВО_ДополнитьОтборОбработчикаРасшифровки(Отбор, ЭлементСтруктуры.Структура);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры


Процедура сВО_ДобавитьЭлементОтбораОбработчикаРасшифровки(Отбор, ЭлементОтбора)
	...
	СвойстваЭлементаОтбора = ВариантыОтчетовКлиентСервер.сВО_СвойстваОтбораОбработчикаРасшифровкиПоДетальнымЗаписям();
	ЗаполнитьЗначенияСвойств(СвойстваЭлементаОтбора, ЭлементОтбора, "ВидСравнения, РежимОтображения, Представление, ИдентификаторПользовательскойНастройки");
...
Процедура сВО_ПодготовитьПрименяемыеНастройкиОбработчикаРасшифровки(ВыполненноеДействие, ДополнительныеПараметры)
...
Функция О_ЭтоВыборОбъектовМетаданных(ДоступныеТипы, Знач ОтмеченныеЗначения, Обработчик) Экспорт
	...
	Если ЭтоВыборОбъектовМетаданных Тогда
		О_ПроверитьОтмеченныеЗначения(ОтмеченныеЗначения, ДоступныеТипы);
...
Функция О_ЭтоВыборПользователей(Форма, ЭлементФормы, ДоступныеТипы, Знач ОтмеченныеЗначения, ПараметрыВыбора, Обработчик) Экспорт
	...
	Если ЭтоВыборПользователей Тогда
		...
		Если Не ВыбратьТип Тогда
			...
			КонецЕсли;
			О_ВыбратьПользователейПослеВыбораТипа(СписокТиповПользователей[0], Контекст);
		Иначе
			СписокТиповПользователей.Добавить(Тип("СправочникСсылка.Пользователи"));
			СписокТиповПользователей.Добавить(Тип("СправочникСсылка.ВнешниеПользователи"));
			Форма.ПоказатьВыборИзМеню(Новый ОписаниеОповещения("О_ВыбратьПользователейПослеВыбораТипа", ЭтотОбъект, Контекст), СписокТиповПользователей, ЭлементФормы);
		КонецЕсли;
	КонецЕсли;
...
Процедура О_ПроверитьОтмеченныеЗначения(ОтмеченныеЗначения, ДоступныеТипы)
...
Процедура О_ВыбратьПользователейПослеВыбораТипа(ВыбранныйЭлемент, Контекст) Экспорт
	...
	Иначе
		...
		Если Контекст.ДоступныеТипы.СодержитТип(Тип("СправочникСсылка.ГруппыПользователей")) Тогда
			...
			ЗаголовокФормыПодбора = "Подбор групп и внешних пользователей";
		Иначе
			ЗаголовокФормыПодбора = "Подбор внешних пользователей";
		КонецЕсли;
	КонецЕсли;
...
Функция О_ВыборПереопределен(ФормаОтчета, Знач Обработчик, ЭлементКомпоновки, ДоступныеТипы, ОтмеченныеЗначения, ПараметрыВыбора) Экспорт
	...
	ИнтеграцияПодсистемКлиент.ПриНачалеВыбораЗначений(ФормаОтчета, УсловияВыбора, Обработчик, ОткрытьСтандартнуюФорму);
