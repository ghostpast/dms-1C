...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Источник = "ПанельАдминистрированияБСП" Тогда
		Элементы.ОК.Заголовок = "Изменить";
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	Если ИнформационнаяБазаФайловая И СтрНайти(ПараметрЗапуска, "ВыполнитьОбновлениеИЗавершитьРаботу") > 0 Тогда
...
Процедура УстановитьОсновнойЯзык()
	...
	Если ПустаяСтрока(ОсновнойЯзык) Или Элементы.ОсновнойЯзык.СписокВыбора.НайтиПоЗначению(ОсновнойЯзык) = Неопределено Тогда
		ОсновнойЯзык = БазоваяПодсистемаСервер.ОН_КодОсновногоЯзыка();
	КонецЕсли;
КонецПроцедуры
...
Процедура ОтобразитьНастройкиДополнительныхЯзыков(Настройки, КоличествоЯзыков)
	...
	ИспользоватьДополнительныйЯзык1 = Константы.ИспользоватьДополнительныйЯзык1.Получить() = Истина;
	ИспользоватьДополнительныйЯзык2 = Константы.ИспользоватьДополнительныйЯзык2.Получить() = Истина;

	ДополнительныйЯзык1 = БазоваяПодсистемаСервер.М_КодПервогоДополнительногоЯзыкаИнформационнойБазы();
	ДополнительныйЯзык2 = БазоваяПодсистемаСервер.М_КодВторогоДополнительногоЯзыкаИнформационнойБазы();
...
Процедура ОК(Команда)
	Если ДанныеКорректны() Тогда
		...
		КонецЕсли;
	Иначе
		ПоказатьПредупреждение(Неопределено, "Указаны некорректные значения региональных настроек");
	КонецЕсли;
КонецПроцедуры
...
Функция ЗапуститьФоновоеПерезаполнениеНаСервере(Знач УникальныйИдентификатор)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания	= "Перезаполнение строк предопределенных элементов и классификаторов.";

	ФоновоеЗадание = БазоваяПодсистемаСервер.ДО_ВыполнитьВФоне("БазоваяПодсистемаСервер.М_ПерезаполнитьМультиязычныеСтрокиВОбъектах", ПараметрыМетода, ПараметрыВыполнения);
...
Процедура ПослеФоновогоПерезаполнения(Задание, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Задание.Статус = "Ошибка" Тогда
		...
		ТекстОшибки = "Не удается перезаполнить строки предопределенных элементов и классификаторов.";
		ТекстОшибки = ТекстОшибки + Символы.ПС + "Техническая информация:" + Задание.ПодробноеПредставлениеОшибки;
		БазоваяПодсистемаКлиентСервер.сОН_СообщитьПользователю(ТекстОшибки, Неопределено, "", "", Ложь);
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПрочитатьПрогресс(ИдентификаторЗадания)
	Возврат БазоваяПодсистемаСервер.ДО_ПрочитатьПрогрессИСообщения(ИдентификаторЗадания, "Прогресс").Прогресс;
КонецФункции
...
Функция ЗначенияКонстантИзменены()
	...
	Если (Константы.ИспользоватьДополнительныйЯзык1.Получить() = Ложь И ИспользоватьДополнительныйЯзык1 = Истина) Или (Константы.ИспользоватьДополнительныйЯзык2.Получить() = Ложь И ИспользоватьДополнительныйЯзык2 = Истина) Тогда
			Возврат Истина;
	КонецЕсли;

	Если ИспользоватьДополнительныйЯзык1 И СтрСравнить(Константы.ДополнительныйЯзык1.Получить(), ДополнительныйЯзык1) <> 0 Тогда
			Возврат Истина;
	КонецЕсли;
	
	Если ИспользоватьДополнительныйЯзык2 И СтрСравнить(Константы.ДополнительныйЯзык2.Получить(), ДополнительныйЯзык2) <> 0 Тогда
			Возврат Истина;
	КонецЕсли;
