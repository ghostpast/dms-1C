...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ТекущийПользователь = ПользователиКлиентСервер.сП_ТекущийПользователь(ПользователиСервер.сП_АвторизованныйПользователь());
...
Процедура ВставитьШаблон(Команда)
	Если ТипЗнч(ТекущийЭлемент) = Тип("ПолеФормы") И ТекущийЭлемент.Вид = ВидПоляФормы.ПолеВвода И СтрНайти(ТекущийЭлемент.Имя, "Текст") > 0 Тогда
...
Процедура ОпределитьПоведениеВМобильномКлиенте()
	...
	Элементы.ТекстСлеваСверху.ПодсказкаВвода	= "Слева сверху";
	Элементы.ТекстВЦентреСверху.ПодсказкаВвода	= "В центре сверху";
	Элементы.ТекстСправаСверху.ПодсказкаВвода	= "Справа сверху";

	Элементы.ТекстСлеваСнизу.ПодсказкаВвода		= "Слева снизу";
	Элементы.ТекстВЦентреСнизу.ПодсказкаВвода	= "В центре снизу";
	Элементы.ТекстСправаСнизу.ПодсказкаВвода	= "Справа снизу";
...
Процедура ОбновитьНастройки()
	...
	СтатусНастроек	= ВариантыОтчетовСервер.УК_СтатусНастроекКолонтитулов(Настройки);

	Если Не ЭтоИндивидуальныеНастройки Тогда
		ВариантыОтчетовСервер.УК_СохранитьНастройкиКолонтитулов(Настройки);
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьСтандартныеНастройкиСервер()
	Если Настройки = Неопределено Тогда
		Настройки = ВариантыОтчетовСервер.УК_НастройкиКолонтитулов();
	КонецЕсли;
	...
	Элементы.ВертикальноеПоложениеВерхнегоКолонтитулаВерх.Пометка	= ВертикальноеПоложениеСверху = ВертикальноеПоложение.Верх;
	Элементы.ВертикальноеПоложениеВерхнегоКолонтитулаЦентр.Пометка	= ВертикальноеПоложениеСверху = ВертикальноеПоложение.Центр;
	Элементы.ВертикальноеПоложениеВерхнегоКолонтитулаНиз.Пометка	= ВертикальноеПоложениеСверху = ВертикальноеПоложение.Низ;

	Элементы.ВертикальноеПоложениеНижнегоКолонтитулаВерх.Пометка	= ВертикальноеПоложениеСнизу = ВертикальноеПоложение.Верх;
	Элементы.ВертикальноеПоложениеНижнегоКолонтитулаЦентр.Пометка	= ВертикальноеПоложениеСнизу = ВертикальноеПоложение.Центр;
...
Процедура ПодготовитьОбразец()
	...
	Образец.Область(3, 2).Текст      = Символы.ПС + "Образец отчета" + Символы.ПС + " ";
...
Процедура ОбновитьОбразец()
	...
	КоличествоСтрокСверху	= Макс(2, КоличествоСтрокВТексте(ТекстСлеваСверху), КоличествоСтрокВТексте(ТекстВЦентреСверху), КоличествоСтрокВТексте(ТекстСправаСверху));

	КоличествоСтрокСнизу	= Макс(2, КоличествоСтрокВТексте(ТекстСлеваСнизу), КоличествоСтрокВТексте(ТекстВЦентреСнизу), КоличествоСтрокВТексте(ТекстСправаСнизу));
