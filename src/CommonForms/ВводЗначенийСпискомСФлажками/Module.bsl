...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	БыстрыйВыбор = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "БыстрыйВыбор", Ложь);

	ИнформацияОТипах = ВариантыОтчетовСервер.О_РасширенноеОписаниеТипов(Параметры.ОписаниеТипов, Истина);
	...
	ЗначенияДляВыбора	= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ЗначенияДляВыбора");
	Отмеченные			= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "Отмеченные");
	...
	Если Не ОграничиватьВыборУказаннымиЗначениями И БыстрыйВыбор И Не Параметры.ЗначенияДляВыбораЗаполнены Тогда
		ЗначенияДляВыбора = ВариантыОтчетовСервер.О_ЗначенияДляВыбора(Параметры);
	КонецЕсли;

	Заголовок = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "Представление");
	...
	ВыборГруппИЭлементов							= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ВыборГруппИЭлементов");
	Элементы.СписокЗначение.ВыборГруппИЭлементов	= ВариантыОтчетовКлиентСервер.О_ЗначениеТипаГруппыИЭлементы(ВыборГруппИЭлементов);
	...
	Если ТипЗнч(ЗначенияДляВыбора) = Тип("СписокЗначений") Тогда
		...
		ВариантыОтчетовКлиентСервер.О_ДополнитьСписок(Список, ЗначенияДляВыбора, Истина, Истина);
	КонецЕсли;
	Если ТипЗнч(Отмеченные) = Тип("СписокЗначений") Тогда
		Отмеченные.ЗаполнитьПометки(Истина);
		ВариантыОтчетовКлиентСервер.О_ДополнитьСписок(Список, Отмеченные, Истина, Не ОграничиватьВыборУказаннымиЗначениями);
	КонецЕсли;
	...
	ПараметрыВыбора = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ПараметрыВыбора");
	...
	КлючСохраненияПоложенияОкна = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "КлючУникальности");
	Если ПустаяСтрока(КлючСохраненияПоложенияОкна) Тогда
		КлючСохраненияПоложенияОкна = БазоваяПодсистемаСервер.ОН_СократитьСтрокуКонтрольнойСуммой(Строка(Список.ТипЗначения), 128);
	КонецЕсли;
...
Процедура СписокОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Дополнение = ВариантыОтчетовКлиентСервер.О_ДополнитьСписок(Список, Выбранные, Истина, Истина);
