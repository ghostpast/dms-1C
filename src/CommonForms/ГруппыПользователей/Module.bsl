...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Пользователь <> Неопределено Тогда
		...
		ЭтоВнешниеПользователи = ?(ТипЗнч(Параметры.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи"), Истина, Ложь);

		Элементы.ФормаЗаписатьИЗакрыть.Заголовок	= "Записать";
	...
	Если КоличествоПользователей = 0 Тогда
		ВызватьИсключение "Не выбрано ни одного пользователя.";
	КонецЕсли;
	...
	Для Каждого ПользовательИзМассива Из МассивПользователей Цикл
		...
		Если ТипПользователяИзМассива <> Тип("СправочникСсылка.Пользователи") И ТипПользователяИзМассива <> Тип("СправочникСсылка.ВнешниеПользователи") Тогда
		...
		Если ТипПользователей <> ТипПользователяИзМассива Тогда
			ВызватьИсключение "Команда не может быть выполнена сразу для двух разных видов пользователей.";
		КонецЕсли;
	КонецЦикла;

	Если КоличествоПользователей > 1 И Параметры.Пользователь = Неопределено Тогда
		Заголовок									= "Группы пользователей";
...
Процедура ЗаписатьИЗакрытьНачало(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если УведомлениеПользователя.ЕстьОшибки = Ложь Тогда
		Если УведомлениеПользователя.Сообщение <> Неопределено Тогда
			ПоказатьОповещениеПользователя("Перемещение пользователей", , УведомлениеПользователя.Сообщение, БиблиотекаКартинок.Информация32);
		КонецЕсли;
	Иначе
		Если УведомлениеПользователя.ПолныйТекстСообщения <> Неопределено Тогда
			ТекстВопроса	= УведомлениеПользователя.Сообщение;
			КнопкиВопроса	= Новый СписокЗначений;
			КнопкиВопроса.Добавить("Ок",			"ОК");
			КнопкиВопроса.Добавить("ПоказатьОтчет",	"Показать отчет");
			Оповещение		= Новый ОписаниеОповещения("ЗаписатьИЗакрытьОбработкаВопроса", ЭтотОбъект, УведомлениеПользователя.ПолныйТекстСообщения);
			...
			Если Группа.Родитель = ПустаяГруппа Тогда
				...
				Если СписокПользователей.КоличествоПользователей = 1 Тогда
					...
					Если ТипПользователя = "ВнешнийПользователь" Тогда
						...
						Назначение			= БазоваяПодсистемаСервер.ОН_ЗначениеРеквизитаОбъекта(Группа.Ссылка, "Назначение").Выгрузить();

						Отбор = Новый Структура;
						Отбор.Вставить("ТипПользователей", Значение);

						ПользовательКосвенноВключенВГруппу	= Группа.ВсеОбъектыАвторизации И Назначение.НайтиСтроки(Отбор).Количество() <> 0;
				...
				КонецЕсли;
			Иначе
				ГруппаРодитель							= ДеревоГруппПриемник.Строки.НайтиСтроки(Новый Структура("Группа", Группа.Родитель), Истина);
				...
				Если СписокПользователей.КоличествоПользователей = 1 Тогда
					НоваяСтрокаПодчиненныхГрупп.Пометка	= ?(Группа.Ссылка.Состав.Найти(СписокПользователей.МассивПользователей[0], ТипПользователя) = Неопределено, 0, 1);
				Иначе
...
Процедура ПолучитьПодчиненныеГруппы(ГруппыПользователей, ПодчиненныеГруппы, ГруппаРодитель)
	...
	Для Каждого Элемент Из ОтобранныеСтроки Цикл
		Если Элемент.Ссылка = Справочники.ГруппыПользователей.ВсеПользователи Или Элемент.Ссылка = Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого СтрокаДерева Из ДеревоГруппИсточник Цикл
		Если СтрокаДерева.Пометка = 1 И Не СтрокаДерева.ГруппаНеИзменяется Тогда
			Для Каждого ПользовательСсылка Из МассивПользователей Цикл
				...
				Иначе
					ТипПользователя		= "ВнешнийПользователь";
					ПеремещениеВозможно	= ПользователиСервер.сП_ВозможноПеремещениеПользователя(СтрокаДерева.Группа, ПользовательСсылка);
				...
				Добавлять = ?(СтрокаДерева.Группа.Состав.Найти(ПользовательСсылка, ТипПользователя) = Неопределено, Истина, Ложь);
				Если Добавлять Тогда
					ПользователиСервер.сП_ДобавитьПользователяВГруппу(СтрокаДерева.Группа, ПользовательСсылка, ТипПользователя);
		...
		ИначеЕсли СтрокаДерева.Пометка = 0 И Не СтрокаДерева.ГруппаНеИзменяется Тогда
			Для Каждого ПользовательСсылка Из МассивПользователей Цикл
				...
				Удалять = ?(СтрокаДерева.Группа.Состав.Найти(ПользовательСсылка, ТипПользователя) <> Неопределено, Истина, Ложь);
				Если Удалять Тогда
					ПользователиСервер.сП_УдалитьПользователяИзГруппы(СтрокаДерева.Группа, ПользовательСсылка, ТипПользователя);
...
Процедура СформироватьТекстСообщения(МассивПеремещенныхПользователей, УведомлениеПользователя, НеПеремещенныеПользователи)
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			...
			КонецЦикла;
			СообщениеПользователю = СтрШаблон("Пользователя ""%1"" не удалось включить в выбранные группы,
				           |т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".",
				Предмет);
		Иначе
			Предмет					= СтрокаСЧислом(";%1 пользователю;;%1 пользователям;%1 пользователям;осталось %1 пользователю", КоличествоНеПеремещенныхПользователей, ВидЧисловогоЗначения.Количественное, "L=ru");
			СообщениеПользователю	= "Не всех пользователей удалось включить в выбранные группы,
				           |т.к. у них различается тип или у групп установлен признак ""Все пользователи заданного типа"".";
			Для Каждого НеПеремещенныйПользователь Из НеПеремещенныеПользователи Цикл
				СтрокаПользователей = СтрокаПользователей + Строка(НеПеремещенныйПользователь.Ключ) + " : " + СтрСоединить(НеПеремещенныйПользователь.Значение, ",") + Символы.ПС;
			КонецЦикла;
			УведомлениеПользователя.ПолныйТекстСообщения = "Следующие пользователи не были включены в группы:" + Символы.ПС + Символы.ПС + СтрокаПользователей;
		КонецЕсли;
	...
	ИначеЕсли КоличествоПользователей = 1 Тогда
		НаименованиеПользователя = БазоваяПодсистемаСервер.ОН_ЗначениеРеквизитаОбъекта(МассивПеремещенныхПользователей[0], "Наименование");

		УведомлениеПользователя.Сообщение = СтрШаблон("Изменен состав групп у пользователя ""%1""", НаименованиеПользователя);
	ИначеЕсли КоличествоПользователей > 1 Тогда
		СтрокаОбъект						= СтрокаСЧислом(";%1 пользователю;;%1 пользователям;%1 пользователям;осталось %1 пользователю", КоличествоПользователей, ВидЧисловогоЗначения.Количественное, "L=ru");

		УведомлениеПользователя.Сообщение	= СтрШаблон("Изменен состав групп у %1", СтрокаОбъект);
	КонецЕсли;
...
Процедура ЗаписатьИЗакрытьОбработкаВопроса(Ответ, ПолныйТекстСообщения) Экспорт
	Если Ответ = "Ок" Тогда
		...
		ЗаписатьИЗакрытьЗавершение();
	Иначе
		ЗаголовокСообщения = "Пользователи, не включенные в группы";
