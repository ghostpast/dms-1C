...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ВариантНаименование", ВариантНаименование) Тогда
		ВызватьИсключение "Не передан служебный параметр ""ВариантНаименование"".";
	КонецЕсли;
	...
	НастройкиКД = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "Вариант");
	...
	Если ТипЗнч(ИдентификаторЭлементаСтруктурыНастроек) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		...
		Если Не Параметры.Свойство("Заголовок", Заголовок) Тогда
			ВызватьИсключение "Не передан служебный параметр ""Заголовок"".";
		КонецЕсли;

		Если Не Параметры.Свойство("ТипЭлементаСтруктурыНастроек", ТипЭлементаСтруктурыНастроек) Тогда
			ВызватьИсключение "Не передан служебный параметр ""ТипЭлементаСтруктурыНастроек"".";
		КонецЕсли;
	Иначе
	...
	Если РежимИзмененияЭлементаСтруктурыНастроек Тогда
		ИмяСтраницы			= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ИмяСтраницы", "СтраницаСоставГруппировки");
		РасширенныйРежим	= 1;
	Иначе
		РасширенныйРежим	= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ОписаниеНастроекОтчета, "ФормаНастроекРасширенныйРежим", 0);
		ИмяСтраницы			= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ОписаниеНастроекОтчета, "ФормаНастроекИмяСтраницы", "СтраницаОтборы");
	КонецЕсли;
...
Процедура СоставГруппировкиВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.СоставГруппировкиПоле Тогда
		...
		КонецЕсли;
	ИначеЕсли Поле = Элементы.СоставГруппировкиТипГруппировки Или Поле = Элементы.СоставГруппировкиТипДополнения Тогда
		СтандартнаяОбработка = Истина;
	КонецЕсли;
КонецПроцедуры
...
Функция ПараметрыГруппировкиОтборов()
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если Родитель.ПолучитьЭлементы().Индекс(Строка) < 0 Или ТипЗнч(Строка.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПоказатьСписокВыбора(Строка, СтандартнаяОбработка)
	...
	Если ВариантыОтчетовКлиент.О_ЭтоВыборОбъектовМетаданных(Строка.ТипЗначения, ТекущееЗначение, Обработчик) Или ВариантыОтчетовКлиент.О_ЭтоВыборПользователей(Строка.ТипЗначения, ТекущееЗначение, ПолеЗначения.ПараметрыВыбора, Обработчик) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗначенияДляВыбора(Строка)
	...
	ВариантыОтчетовКлиентСервер.О_ОбновитьПредставленияСписка(ЗначенияДляВыбора, Строка.ДоступныеЗначения);
...
Процедура ЗавершитьЗаполнениеСписка(ВыбранныеЗначения, ПараметрыЗаполнения) Экспорт
	...
	Для Каждого Элемент Из ВыбранныеЗначения Цикл
		ВариантыОтчетовКлиентСервер.О_ДобавитьУникальноеЗначениеВСписок(Список, Элемент.Значение, Неопределено, Истина);
	КонецЦикла;
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	ОтчетОбъект = ВариантыОтчетовСервер.О_ОтчетОбъект(ПараметрыЗагрузки.ОтчетОбъектИлиПолноеИмя);
	...
	Если НастройкиЗагружены И ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") Тогда
		ВариантыОтчетовСервер.О_УстановитьФиксированныеОтборы(ФормаПараметры.Отбор, ДоступныеНастройки.Настройки, НастройкиОтчета);
	КонецЕсли;
