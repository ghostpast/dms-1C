...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФормаПараметры		= ВариантыОтчетовСервер.ВО_СохраняемыеПараметрыФормыОтчета(Параметры);
	...
	Если ТипЗнч(ИдентификаторЭлементаСтруктурыНастроек) = Тип("ИдентификаторКомпоновкиДанных") Тогда
		...
		ПутьКЭлементуСтруктурыНастроек = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ПутьКЭлементуСтруктурыНастроек");

		ЭлементСтруктуры = ВариантыОтчетовСервер.О_ЭлементНастроекПоПолномуПути(НастройкиКД, ПутьКЭлементуСтруктурыНастроек);
	...
	Иначе
		...
		Заголовок = СтрШаблон("Настройки отчета ""%1""", ВариантНаименование);
	КонецЕсли;

	ГлобальныеНастройки = ВариантыОтчетовСервер.ВО_ГлобальныеНастройки();
...
Процедура ПередЗагрузкойВариантаНаСервере(НовыеНастройкиКД)
	Если ФормаПараметры.НачальныйКлючВарианта = КлючТекущегоВарианта И ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") Тогда
		ВариантыОтчетовСервер.О_УстановитьФиксированныеОтборы(ФормаПараметры.Отбор, НовыеНастройкиКД, НастройкиОтчета);
	КонецЕсли;
...
Процедура Подключаемый_Список_ПриИзменении(Элемент)
	...
	ВариантыОтчетовКлиент.О_КэшироватьЗначениеОтбора(КомпоновщикНастроек, ЭлементНастройки, Список);
...
Процедура Подключаемый_Список_ОбработкаВыбора(Элемент, РезультатВыбора, СтандартнаяОбработка)
	...
	Выбранные = ВариантыОтчетовКлиентСервер.О_ЗначенияСписком(РезультатВыбора);
	Выбранные.ЗаполнитьПометки(Истина);

	Дополнение = ВариантыОтчетовКлиентСервер.О_ДополнитьСписок(Список, Выбранные, Ложь, Истина);

	ЗначениеЭлементаНастройки	= БазоваяПодсистемаКлиент.ОН_СкопироватьРекурсивно(Список);
...
Процедура ВыбранныеПоля_Сгруппировать(Команда)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ГруппаВыбранныхПолей", ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор,,, Обработчик, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ВыбранныеПоля_Разгруппировать(Команда)
	...
	Если ИдентификаторыСтрок.Количество() <> 1 Тогда
		ПоказатьПредупреждение(, "Выберите одну группу.");
	...
	Если ТипЗнч(РодительСтрокиИсточника.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Тогда
		ПоказатьПредупреждение(, "Выберите группу.");

		Возврат;
	КонецЕсли;

	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "Выбор", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
...
Процедура ОтборыПриАктивизацииЯчейки(Элемент)
	...
	ЭтоПолеСписка = Строка.ДоступенСписокЗначений Или ВариантыОтчетовКлиентСервер.О_ЭтоВидСравненияСписка(Строка.ВидСравнения);
...
Процедура ОтборыВидСравненияПриИзменении(Элемент)
	...
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, КлючСвойства, ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	...
	ПолеЗначения						= ?(Строка.ЭтоПараметр, Элементы.ОтборыЗначение, Элементы.ОтборыПравоеЗначение);
	ПолеЗначения.ВыборГруппИЭлементов	= ВариантыОтчетовКлиентСервер.О_ЗначениеТипаГруппыИЭлементы(Строка.ВыборГруппИЭлементов, Условие);
...
Процедура ОтборыПравоеЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВариантыОтчетовКлиентСервер.О_ЭтоВидСравненияСписка(Строка.ВидСравнения) Тогда
		ПоказатьСписокВыбора(Строка, СтандартнаяОбработка, Элемент);
	КонецЕсли;
КонецПроцедуры
...
Процедура ИзменитьФормулу(Команда)
	...
	Формула = ВариантыОтчетовКлиентСервер.сВО_ФормулаПоПутиКДанным(Отчет.КомпоновщикНастроек.Настройки, Строка(Строка.Поле));
	...
	ВариантыОтчетовКлиент.сВО_ИзменитьФормулу(ЭтотОбъект, Отчет.КомпоновщикНастроек.Настройки, Строка(Строка.Поле));
КонецПроцедуры
...
Процедура СтруктураВариантаВыбор(Элемент, СтрокаИдентификатор, Поле, СтандартнаяОбработка)
	...
	Если Строка.Тип = "НастройкиКомпоновкиДанных" Или Строка.Тип = "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных" Или Строка.Тип = "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных" Тогда
		СтандартнаяОбработка = Ложь;

		Возврат;
	КонецЕсли;

	Если Поле = Элементы.СтруктураВариантаПредставление Или Поле = Элементы.СтруктураВариантаСодержитОтборы Или Поле = Элементы.СтруктураВариантаСодержитПоляИлиСортировки Или Поле = Элементы.СтруктураВариантаСодержитУсловноеОформление Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура СтруктураВариантаНачалоПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
	...
	Если Строка = Неопределено Тогда
		...
		Возврат;
	КонецЕсли;
	Если Строка.Тип = "КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных" Или Строка.Тип = "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных" Или Строка.Тип = "НастройкиКомпоновкиДанных" Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура СтруктураВариантаПроверкаПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, ИдентификаторЦели, Поле)
	...
	Если НовыйРодитель.Тип = "КоллекцияЭлементовСтруктурыТаблицы" Или НовыйРодитель.Тип = "КоллекцияЭлементовСтруктурыДиаграммы" Или НовыйРодитель.Тип = "ГруппировкаТаблицы" Или НовыйРодитель.Тип = "ГруппировкаДиаграммы" Тогда
		ДопустимоТолькоНаличиеГруппировок = Истина;
	КонецЕсли;

	Если ДопустимоТолькоНаличиеГруппировок И Строка.Тип <> "Группировка" И Строка.Тип <> "ГруппировкаТаблицы" И Строка.Тип <> "ГруппировкаДиаграммы" Тогда
		ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Отмена;
	...
	Пока Количество > 0 Цикл
		...
		Для Каждого ВложеннаяСтрока Из Коллекция Цикл
			Если ВложеннаяСтрока = НовыйРодитель Тогда
				...
				Возврат;
			КонецЕсли;
			Если ДопустимоТолькоНаличиеГруппировок И ВложеннаяСтрока.Тип <> "Группировка" И ВложеннаяСтрока.Тип <> "ГруппировкаТаблицы" И ВложеннаяСтрока.Тип <> "ГруппировкаДиаграммы" Тогда
				ПараметрыПеретаскивания.Действие = ДействиеПеретаскивания.Отмена;
...
Процедура СтруктураВарианта_СохранитьВФайл(Команда)
	...
	ИмяФайла	= "Настройки.xml";

	ПараметрыСохранения						= БазоваяПодсистемаКлиент.ФС_КонтекстОперации(РежимДиалогаВыбораФайла.Сохранение);
	ПараметрыСохранения.Диалог.Заголовок	= "Выберите файл для сохранения настроек отчета";
	ПараметрыСохранения.Диалог.Фильтр		= "Настройки отчета (*.xml)|*.xml";

	БазоваяПодсистемаКлиент.ФС_СохранитьФайл(Неопределено, Адрес, ИмяФайла, ПараметрыСохранения);
КонецПроцедуры

&НаСервере
Функция АдресНастроекВСтрокеXML()
	Возврат ПоместитьВоВременноеХранилище(
		БазоваяПодсистемаСервер.ОН_ЗначениеВСтрокуXML(Отчет.КомпоновщикНастроек.Настройки), УникальныйИдентификатор);
КонецФункции
...
Процедура СоставГруппировкиТипГруппировкиПриИзменении(Элемент)
	...
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "ПоляГруппировки", ИдентификаторЭлементаСтруктурыНастроек);
...
Процедура ОформлениеВыбор(Элемент, ИдентификаторСтроки, Поле, СтандартнаяОбработка)
	...
	Если Строка.ЭтоПараметрВывода Тогда
		Если Строка(Строка.Идентификатор) = "TITLE" И Поле = Элементы.ОформлениеЗаголовок Тогда
			...
			ПоказатьВводСтроки(Обработчик, Строка.Значение, "Заголовок для печати",, Истина);
		КонецЕсли;
	...
	ИначеЕсли Поле = Элементы.ОформлениеИндексКартинкиДоступа Тогда // Изменение быстрого доступа к отбору.
		СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "УсловноеОформление", ИдентификаторЭлементаСтруктурыНастроек);
...
Процедура НастроитьКолонтитулы(Команда)
	...
	ОткрытьФорму("ОбщаяФорма.НастройкаКолонтитулов", Новый Структура("Настройки", Настройки), ЭтотОбъект, УникальныйИдентификатор,,, Новый ОписаниеОповещения("ЗапомнитьНастройкиКолонтитулов", ЭтотОбъект), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура Подключаемый_СдвинутьПериодНазад(Команда)
	ВариантыОтчетовКлиент.О_СдвинутьПериод(ЭтотОбъект, Команда.Имя);
КонецПроцедуры
...
Процедура Подключаемый_Список_ВставитьИзБуфера(Команда)
	...
	Сведения					= ВариантыОтчетовКлиент.О_СведенияОЭлементеНастройки(Отчет.КомпоновщикНастроек, Индекс);
	...
	Список.ТипЗначения	= ВариантыОтчетовКлиент.О_ТипЗначенияОграниченныйСвязьюПоТипу(Сведения.Настройки, ПользовательскиеНастройки, Сведения.Элемент, Сведения.Описание);
	...
	ЗагрузкаДанныхИзФайлаКлиент.ЗДФ_ПоказатьФормуЗаполненияСсылок(ПараметрыПоиска, Обработчик);
КонецПроцедуры
...
Процедура ОбновитьПоляГруппировки()
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоПолеГруппировкиКомпоновкиДанных") Тогда
			Строка.Заголовок	= "Авто (по всем полям)";
			Строка.Картинка		= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Элемент", "Предопределенный");
		...
		Если ОписаниеНастройки.Ресурс Тогда
			Строка.Картинка = ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Ресурс");
		ИначеЕсли ОписаниеНастройки.Таблица Тогда
			Строка.Картинка = ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Таблица");
		ИначеЕсли ОписаниеНастройки.Папка Тогда
			Строка.Картинка = ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Группа");
		Иначе
			Строка.Картинка = ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Элемент");
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура СоставГруппировкиПослеВыбораПоля(ОписаниеНастройки, ИдентификаторСтроки) Экспорт
	...
	ВариантыОтчетовКлиент.сВО_ДобавитьФормулу(Настройки, Настройки.ДоступныеПоляГруппировок, ОписаниеНастройки);
...
Процедура ОбновитьПараметрыДанных()
	...
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "ПараметрыДанных", ИдентификаторЭлементаСтруктурыНастроек);

	Раздел					= Отборы.ПолучитьЭлементы().Добавить();
	Раздел.ЭтоРаздел		= Истина;
	Раздел.Заголовок		= "Параметры";
	Раздел.Картинка			= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("ПараметрыДанных");
...
Процедура ОбновитьОтборы(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда
		Раздел					= Отборы.ПолучитьЭлементы().Добавить();
		Раздел.ЭтоРаздел		= Истина;
		Раздел.Заголовок		= "Отборы";
		Раздел.Картинка			= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Отборы");
...
Функция УстановитьДанныеСтрокиОтборов(Строка, СвойствоЭлементаСтруктуры, ЭлементНастройки, ОписаниеНастройки = Неопределено)
	...
	Если ДоступныеВидыСравнения <> Неопределено И ДоступныеВидыСравнения.Количество() > 0 И ДоступныеВидыСравнения.НайтиПоЗначению(ЭлементНастройки.ВидСравнения) = Неопределено Тогда
	...
	Иначе
		ВариантыОтчетовКлиентСервер.О_ПривестиЗначениеКТипу(ЭлементНастройки.ПравоеЗначение, ОписаниеНастройки.ТипЗначения);
	КонецЕсли;
...
Процедура ОтборыПередВыборомПоля(ИдентификаторСтроки = Неопределено)
	Если Не РежимИзмененияЭлементаСтруктурыНастроек Тогда
		...
		Если (Строка = Неопределено) Или (Строка.ЭтоПараметр) Или (Строка.ЭтоРаздел И Строка.Идентификатор = "ПараметрыДанных") Тогда
...
Процедура ОтборыПослеВыбораПоля(ОписаниеНастройки, ИдентификаторСтроки) Экспорт
	...
	Настройки = Отчет.КомпоновщикНастроек.Настройки;
	ВариантыОтчетовКлиент.сВО_ДобавитьФормулу(Настройки, Настройки.ДоступныеПоляОтбора, ОписаниеНастройки);
...
Функция ПараметрыГруппировкиОтборов()
	...
	Для Каждого ИдентификаторСтроки Из ИдентификаторыСтрок Цикл
		...
		Если Родитель = Неопределено Или Родитель.ПолучитьЭлементы().Индекс(Строка) < 0 Или ТипЗнч(Строка.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
	...
	Если Строки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, "Выберите элементы.");
		Возврат Неопределено;
	КонецЕсли;

	Родители = БазоваяПодсистемаКлиентСервер.ОН_СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда
		ПоказатьПредупреждение(, "Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.");
...
Функция ПараметрыСдвигаОтборов()
	...
	Родители = БазоваяПодсистемаКлиентСервер.ОН_СвернутьМассив(Родители);
	Если Родители.Количество() > 1 Тогда
		ПоказатьПредупреждение(, "Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.");
...
Процедура ОбновитьВыбранныеПоля(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда
		...
		Раздел.ЭтоРаздел		= Истина;
		Раздел.Заголовок		= "Поля";
		Раздел.Картинка			= ВариантыОтчетовКлиентСервер.сВО_ИндексКартинкиПоля(Неопределено);
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоВыбранноеПолеКомпоновкиДанных") Тогда
			Строка.Заголовок	= "Авто (поля родителя)";
			Строка.Картинка		= 18;
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных") Тогда
			...
			Строка.Картинка		= ВариантыОтчетовКлиентСервер.сВО_ИндексКартинкиПоля(Неопределено, Истина);
		...
		Иначе
			...
			Иначе
				ЗаполнитьЗначенияСвойств(Строка, ОписаниеНастройки);
				Строка.Картинка = ВариантыОтчетовКлиентСервер.сВО_ИндексКартинкиПоля(ОписаниеНастройки.ТипЗначения);
...
Процедура ВыбранныеПоляПослеВыбораПоля(ОписаниеНастройки, ИдентификаторСтроки = Неопределено) Экспорт
	...
	Настройки = Отчет.КомпоновщикНастроек.Настройки;
	ВариантыОтчетовКлиент.сВО_ДобавитьФормулу(Настройки, Настройки.ДоступныеПоляВыбора, ОписаниеНастройки);
...
Процедура УстановитьИндикаторФормулы(Настройки, ОписаниеНастройки, Строка)
	Формула				= ВариантыОтчетовКлиентСервер.сВО_ФормулаПоПутиКДанным(Настройки, Строка(ОписаниеНастройки.Поле));
...
Процедура ВыбранныеПоляПередГруппировкойПолей(СвойстваГруппы, ПараметрыГруппировки) Экспорт
	...
	СтрокаПриемник.Картинка			= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Группа");
...
Процедура ПеретащитьВыбранныеПоля(ВыбранныеПоляУзлаНастроек, Индекс, Строки, НаследникиЭлементовНастроек, НаследникиСтрок)
	Для Каждого СтрокаИсточник Из Строки Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаПриемник, СтрокаИсточник);
		СтрокаПриемник.Идентификатор	= ВыбранныеПоляУзлаНастроек.ПолучитьИдентификаторПоОбъекту(ЭлементНастройкиПриемник);
		СтрокаПриемник.Картинка			= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Элемент");
		СтрокаПриемник.ЭтоГруппа		= (ТипЗнч(ЭлементНастройкиПриемник) = Тип("ГруппаВыбранныхПолейКомпоновкиДанных"));
...
Процедура ПривестиЗначениеКВидуСравнения(Строка, ЭлементНастройки = Неопределено)
	...
	Если Строка.ДоступенСписокЗначений Или ВариантыОтчетовКлиентСервер.О_ЭтоВидСравненияСписка(Строка.ВидСравнения) Тогда

		Значение = ВариантыОтчетовКлиентСервер.О_ЗначенияСписком(ТекущееЗначение);
	...
	Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Или ТипЗнч(ЭлементНастройки) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
...
Процедура УстановитьПараметрыРедактирования(Строка)
	...
	Иначе
		...
		СвойствоЭлементаСтруктуры	= СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, "Отбор", ИдентификаторЭлементаСтруктурыНастроек);
	КонецЕсли;
	...
	ЭлементНастройки			= ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
	ОписаниеЭлементаНастройки	= ВариантыОтчетовКлиентСервер.О_НайтиДоступнуюНастройку(ТекущиеНастройки, ЭлементНастройки);

	ПолеЗначения.ПараметрыВыбора = ВариантыОтчетовКлиентСервер.О_ПараметрыВыбора(ТекущиеНастройки, ПользовательскиеНастройки, ЭлементНастройки, РасширенныйРежим = 1);

	Строка.ТипЗначения = ВариантыОтчетовКлиент.О_ТипЗначенияОграниченныйСвязьюПоТипу(ТекущиеНастройки, ПользовательскиеНастройки, ЭлементНастройки, ОписаниеЭлементаНастройки);
...
Функция ВыборПереопределен(Строка, СтандартнаяОбработка)
	...
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, КлючСвойства, ИдентификаторЭлементаСтруктурыНастроек);

	ЭлементНастройки	= ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
	Описание			= ВариантыОтчетовКлиентСервер.О_НайтиДоступнуюНастройку(Отчет.КомпоновщикНастроек.Настройки, ЭлементНастройки);

	Если ВариантыОтчетовКлиент.О_ВыборПереопределен(ЭтотОбъект, Обработчик, Описание, Строка.ТипЗначения, ВариантыОтчетовКлиентСервер.О_ЗначенияСписком(ТекущееЗначение), ПолеЗначения.ПараметрыВыбора) Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура ПоказатьСписокВыбора(Строка, СтандартнаяОбработка, Элемент)
	...
	Если ВариантыОтчетовКлиент.О_ЭтоВыборОбъектовМетаданных(Строка.ТипЗначения, ТекущееЗначение, Обработчик) Или ВариантыОтчетовКлиент.О_ЭтоВыборПользователей(ЭтотОбъект, Элемент, Строка.ТипЗначения, ТекущееЗначение, ПолеЗначения.ПараметрыВыбора, Обработчик) Тогда
		Возврат;
	КонецЕсли;
	...
	ПараметрыОткрытия.Вставить("Отмеченные",							ВариантыОтчетовКлиентСервер.О_ЗначенияСписком(ТекущееЗначение));
...
Функция ЗначенияДляВыбора(Строка)
	ЗначенияДляВыбора = БазоваяПодсистемаКлиент.ОН_СкопироватьРекурсивно(Строка.ДоступныеЗначения);
	...
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, ?(Строка.ЭтоПараметр, "ПараметрыДанных", "Отбор"));
	...
	ВариантыОтчетовКлиент.О_ОбновитьПредставленияСписка(ЗначенияДляВыбора, Строка.ДоступныеЗначения);
...
Процедура ОбновитьСортировку()
	СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "Порядок", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ТипЗнч(ЭлементНастройки) = Тип("АвтоЭлементПорядкаКомпоновкиДанных") Тогда
			Строка.Заголовок	= "Авто (сортировки родителя)";
...
Процедура СортировкаПослеВыбораПоля(ОписаниеНастройки, ИдентификаторСтроки = Неопределено) Экспорт
	...
	Настройки = Отчет.КомпоновщикНастроек.Настройки;
	ВариантыОтчетовКлиент.сВО_ДобавитьФормулу(Настройки, Настройки.ДоступныеПоляПорядка, ОписаниеНастройки);
...
Процедура ПеретащитьВыбранныеПоляВСортировку(Строки)
	ВыбранныеПоляЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "Выбор", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);

	СортировкаЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(Отчет.КомпоновщикНастроек, "Порядок", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	...
	Для Каждого СтрокаИсточник Из Строки Цикл
		...
		Иначе
			...
			СтрокаПриемник.Картинка			= ВариантыОтчетовКлиентСервер.сВО_ИндексКартинкиПоля(ОписаниеНастройки.ТипЗначения);
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбновитьОформление()
	...
	Для Каждого ЭлементНастройки Из СвойствоЭлементаСтруктуры.Элементы Цикл
		...
		Строка.Представление = ВариантыОтчетовКлиентСервер.О_ПредставлениеЭлементаУсловногоОформления(ЭлементНастройки, ЭлементНастройки, ?(Строка.ПометкаУдаления, "ПометкаУдаления", ""));
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Заголовок					= СтрШаблон("Выводить заголовок для печати: %1", ?(ЗначениеЗаполнено(Объект.Значение), Объект.Значение, "<Отсутствует>"));
	Строка.Представление				= "Выводить заголовок для печати";
	...
	Строка								= Оформление.ПолучитьЭлементы().Добавить();
	Строка.Использование				= (Объект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить Или СвязанныйОбъект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
	Строка.Заголовок					= "Выводить фильтры";
	Строка.Представление				= "Выводить фильтры";
...
Процедура ОформлениеИзменитьЭлемент(ИдентификаторСтроки = Неопределено, Строка = Неопределено)
	...
	КонецЕсли;
	ПараметрыФормы.Вставить("Заголовок", СтрШаблон("Элемент условного оформления отчета ""%1""", ВариантНаименование));

	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ЭлементУсловногоОформленияОтчета",ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор,,, Обработчик);
КонецПроцедуры
...
Процедура ОформлениеИзменитьЭлементЗавершение(Результат, ИдентификаторСтроки) Экспорт
	...
	Если Результат.Свойство("ОписаниеФормул") Тогда
		ВариантыОтчетовКлиент.сВО_ДобавитьФормулы(Настройки, Настройки.ДоступныеПоляВыбора, Результат.ОписаниеФормул);
	КонецЕсли;
	...
	ВариантыОтчетовКлиентСервер.О_ЗаполнитьСвойстваРекурсивно(СвойствоЭлементаСтруктуры, ЭлементНастройки, Результат.ЭлементКД);
	ЭлементНастройки.ИдентификаторПользовательскойНастройки = Новый УникальныйИдентификатор;
...
Процедура ОформлениеВводЗаголовкаЗавершение(Значение, Идентификатор) Экспорт
	...
	Строка.Заголовок		= СтрШаблон("Выводить заголовок для печати: %1", ?(ЗначениеЗаполнено(Значение), Значение, "<Отсутствует>"));
...
Процедура ОбновитьКоллекциюСтруктуры(Знач Узел, Знач ИмяКоллекции, Строки)
	...
	Если ТипКоллекции = Тип("КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных") Тогда
		...
		КонецЕсли;
	ИначеЕсли ИменаКоллекций.Найти(ИмяКоллекции) <> Неопределено И (ТипКоллекции = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипКоллекции = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных")) Тогда
		Строка					= Строки.Добавить();
...
Функция СвойстваЭлементаКоллекцииСтруктуры(Элемент)
	...
	Если ТипЭлемента = Тип("ГруппировкаКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаТаблицыКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаДиаграммыКомпоновкиДанных") Тогда
		СвойстваЭлемента.Представление = ВариантыОтчетовСервер.сВО_ПредставлениеПолейГруппировки(Элемент, СвойстваЭлемента.ПометкаУдаления);
	ИначеЕсли ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
		...
		Если Не ЗначениеЗаполнено(СвойстваЭлемента.Представление) Тогда
			СвойстваЭлемента.Представление = "Вложенная группировка";
		КонецЕсли;
	ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Тогда
		СвойстваЭлемента.Представление = "Таблица";
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПользовательскойНастройки) Тогда
			СвойстваЭлемента.Представление = Элемент.ПредставлениеПользовательскойНастройки;
		КонецЕсли;
	ИначеЕсли ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Тогда
		СвойстваЭлемента.Представление = "Диаграмма";
		Если ЗначениеЗаполнено(Элемент.ПредставлениеПользовательскойНастройки) Тогда
			СвойстваЭлемента.Представление = Элемент.ПредставлениеПользовательскойНастройки;
		КонецЕсли;
	Иначе
	...
	ПредставлениеТипаЭлемента	= ВариантыОтчетовКлиентСервер.О_ТипНастройкиСтрокой(ТипЭлемента);
	СостояниеЭлемента			= ?(СвойстваЭлемента.ПометкаУдаления, "ПометкаУдаления", Неопределено);
	СвойстваЭлемента.Картинка	= ВариантыОтчетовКлиентСервер.О_ИндексКартинки(ПредставлениеТипаЭлемента, СостояниеЭлемента);
...
Процедура УстановитьПризнакиВложенныхЭлементовНастроек(ЭлементСтруктуры, СвойстваЭлементаСтруктуры)
	...
	СодержитПоля		= ВложенныеЭлементы.Количество() > 0 И Не (ВложенныеЭлементы.Количество() = 1 И ТипЗнч(ВложенныеЭлементы[0]) = Тип("АвтоВыбранноеПолеКомпоновкиДанных"));

	ВложенныеЭлементы	= Элемент.Порядок.Элементы;
	СодержитСортировки	= ВложенныеЭлементы.Количество() > 0 И Не (ВложенныеЭлементы.Количество() = 1 И ТипЗнч(ВложенныеЭлементы[0]) = Тип("АвтоЭлементПорядкаКомпоновкиДанных"));
	...
	Если Строка <> Неопределено Тогда
		...
		Если СледующийУровень Тогда
			...
			ИначеЕсли Строка.ПолучитьЭлементы().Количество() > 1 Тогда
				ПараметрыВыполнения.Оборачивать = Ложь;
			ИначеЕсли Строка.Тип = "КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных" И Строка.Подтип = "ТаблицаКолонки" Тогда
				ПараметрыВыполнения.Оборачивать = Ложь;
			КонецЕсли;
		КонецЕсли;

		Пока Строка <> Неопределено Цикл
			Если Строка.Тип = "НастройкиКомпоновкиДанных" Или Строка.Тип = "НастройкиВложенногоОбъектаКомпоновкиДанных" Или Строка.Тип = "ГруппировкаКомпоновкиДанных" Или Строка.Тип = "ГруппировкаТаблицыКомпоновкиДанных" Или Строка.Тип = "ГруппировкаДиаграммыКомпоновкиДанных" Тогда
...
Процедура ДобавитьЭлементСтруктурыНастроек(ТипЭлемента)
	...
	ПредставлениеТипаЭлемента	= ВариантыОтчетовКлиентСервер.О_ТипНастройкиСтрокой(ТипЗнч(ЭлементНастройки));
	Строка.Картинка				= ВариантыОтчетовКлиентСервер.О_ИндексКартинки(ПредставлениеТипаЭлемента);
	...
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		...
		Строка.Представление = "Диаграмма";
		...
		ПодчиненнаяСтрока.Представление	= "Точки";
		...
		ПодчиненнаяСтрока.Идентификатор	= СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка		= -1;
		ПодчиненнаяСтрока.Представление	= "Серии";
	ИначеЕсли Строка.Тип = "ТаблицаКомпоновкиДанных" Тогда
		Строка.Представление			= "Таблица";
		...
		ПодчиненнаяСтрока.Идентификатор	= СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка		= -1;
		ПодчиненнаяСтрока.Представление	= "Строки";
		...
		ПодчиненнаяСтрока.Идентификатор	= СвойствоЭлементаСтруктуры.ПолучитьИдентификаторПоОбъекту(ПодчиненныйЭлементНастройки);
		ПодчиненнаяСтрока.Картинка		= -1;
		ПодчиненнаяСтрока.Представление	= "Колонки";
	КонецЕсли;
...
Процедура СтруктураВариантаПослеВыбораПоля(ОписаниеНастройки, ПараметрыВыполнения) Экспорт
	...
	Если ОписаниеНастройки = "<>" Тогда
		...
		Представление = "<Детальные записи>";
	Иначе
	...
	Строка.Картинка				= ВариантыОтчетовКлиентСервер.О_ИндексКартинки(ПредставлениеТипаЭлемента, СостояниеЭлемента);
...
Функция ВставитьЭлементСтруктурыНастроек(ТипЭлемента, Строка, СледующийУровень)
	...
	Если ВариантыОтчетовКлиент.О_ПриДобавленииВКоллекциюНужноУказыватьТипЭлемента(ТипЗнч(ЭлементыНастроек)) Тогда
...
&НаКлиенте
Функция ПереместитьЭлементыСтруктурыВарианта(Знач Строка, Знач НовыйРодитель, Знач ПередЧемВставить = Неопределено, Знач Индекс = Неопределено, Знач ИндексЭлементаНастройки = Неопределено)
	...
	Иначе
		...
		Результат.ЭлементНастройки	= ВариантыОтчетовКлиентСервер.О_СкопироватьРекурсивно(УзелКД, ЭлементКД, КудаВставитьКД, ИндексЭлементаНастройки, ПоискЭлементовКД);

		ПоискСтрокТаблицы	= Новый Соответствие;
		Результат.Строка	= ВариантыОтчетовКлиентСервер.О_СкопироватьРекурсивно(Неопределено, Строка, КудаВставить, Индекс, ПоискСтрокТаблицы);
...
Процедура ИзменитьЭлементСтруктуры(Строка, ИмяСтраницы = Неопределено, ИспользоватьФормуВарианта = Неопределено)
	...
	Если ИспользоватьФормуВарианта = Неопределено Тогда
		ИспользоватьФормуВарианта = (Строка.Тип = "ТаблицаКомпоновкиДанных" Или Строка.Тип = "НастройкиВложенногоОбъектаКомпоновкиДанных");
	КонецЕсли;

	Обработчик = Новый ОписаниеОповещения("ИзменитьЭлементСтруктурыЗавершение", ЭтотОбъект);

	ШаблонЗаголовка = "Настройка %1 отчета ""%2""";
	Если Строка.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		ПредставлениеЭлемента = "диаграммы";
	Иначе
		ПредставлениеЭлемента = "группировки";
	КонецЕсли;
	...
	ПутьКЭлементуСтруктурыНастроек = ВариантыОтчетовКлиент.О_ПолныйПутьКЭлементуНастроек(Отчет.КомпоновщикНастроек.Настройки, ЭлементНастройки);
	...
	ПараметрыФормы.Вставить("Заголовок",								СтрШаблон(ШаблонЗаголовка, ПредставлениеЭлемента, ВариантНаименование));
...
Процедура НачатьЗаполнениеСписка(Элемент, ПараметрыЗаполнения, ВыборПереопределен = Неопределено)
	...
	Сведения = ВариантыОтчетовКлиент.О_СведенияОЭлементеНастройки(Отчет.КомпоновщикНастроек, ПараметрыЗаполнения.Индекс);
	...
	Условие										= ВариантыОтчетовКлиентСервер.О_УсловиеЭлементаНастройки(ЭлементПользовательскойНастройки, Сведения.Описание);
	ПолеЗначения.ВыборГруппИЭлементов			= ВариантыОтчетовКлиентСервер.О_ЗначениеТипаГруппыИЭлементы(ВыборГруппИЭлементов, Условие);
	ПараметрыЗаполнения.ВыборГруппИЭлементов	= ВариантыОтчетовКлиент.О_ЗначениеТипаИспользованиеГруппИЭлементов(ВыборГруппИЭлементов, Условие);

	РасширенноеОписаниеТипов					= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства, "РасширенноеОписаниеТипов", Новый Соответствие);
	...
	Список.ТипЗначения = ВариантыОтчетовКлиент.О_ТипЗначенияОграниченныйСвязьюПоТипу(Сведения.Настройки, ПользовательскиеНастройки, Сведения.Элемент, Сведения.Описание, Список.ТипЗначения);
	...
	Если ВариантыОтчетовКлиент.О_ВыборПереопределен(ЭтотОбъект, Обработчик, Сведения.Описание,
	...
	Если БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ПараметрыЗаполнения, "ЭтоПодбор", Ложь) И (ВариантыОтчетовКлиент.О_ЭтоВыборОбъектовМетаданных(Список.ТипЗначения, ОтмеченныеЗначения, Обработчик) Или ВариантыОтчетовКлиент.О_ЭтоВыборПользователей(ЭтотОбъект, Элемент, Список.ТипЗначения, ОтмеченныеЗначения, ПараметрыВыбора, Обработчик)) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПродолжитьЗаполнениеСписка(ВыбранныйЭлемент, ПараметрыЗаполнения) Экспорт
	...
	ПараметрыПодбора = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Отчет.КомпоновщикНастроек.Настройки.ДополнительныеСвойства, "ПараметрыПодбора", Новый Соответствие);
...
Процедура ЗавершитьЗаполнениеСписка(ВыбранныеЗначения, ПараметрыЗаполнения) Экспорт
	...
	Для Каждого Элемент Из ВыбранныеЗначения Цикл
		ВариантыОтчетовКлиентСервер.О_ДобавитьУникальноеЗначениеВСписок(Список, Элемент.Значение, Элемент.Представление, Истина);
	КонецЦикла;
...
Процедура ВставитьИзБуфераЗавершение(НайденныеОбъекты, ПутьКСписку) Экспорт
	...
	Для Каждого Значение Из НайденныеОбъекты Цикл
		ВариантыОтчетовКлиентСервер.О_ДобавитьУникальноеЗначениеВСписок(Список, Значение, Неопределено, Истина);
		ВариантыОтчетовКлиентСервер.О_ДобавитьУникальноеЗначениеВСписок(Отмеченные, Значение, Неопределено, Истина);
	КонецЦикла;
...
Функция СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, Ключ, ИдентификаторЭлемента = Неопределено, Режим = Неопределено)
	...
	Если Режим = 0 И (ТипЗнч(СвойствоЭлементаСтруктуры) = Тип("ВыбранныеПоляКомпоновкиДанных") Или ТипЗнч(СвойствоЭлементаСтруктуры) = Тип("ПорядокКомпоновкиДанных")) И ЗначениеЗаполнено(СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки) Тогда
		СвойствоЭлементаСтруктуры = КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки);
	КонецЕсли;
...
Процедура УдалитьСтроки(Элемент, Отказ)
	...
	Пока Индекс >= 0 Цикл
		...
		Если ТипЗнч(Строка.Идентификатор) <> Тип("ИдентификаторКомпоновкиДанных") Или (Строка.Свойство("ЭтоРаздел") И Строка.ЭтоРаздел) Или (Строка.Свойство("ЭтоПараметр") И Строка.ЭтоПараметр) Или (Строка.Свойство("ЭтоПараметрВывода") И Строка.ЭтоПараметрВывода) Тогда
			Продолжить;
		КонецЕсли;
		...
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
		Если ТипЗнч(ЭлементНастройки) = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипЗнч(ЭлементНастройки) = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ИзменитьИспользование(ИмяКоллекции, Использование = Истина, Строки = Неопределено)
	...
	Для Каждого Строка Из Строки Цикл
		...
		ЭлементНастройки = ЭлементНастройки(СвойствоЭлементаСтруктуры, Строка);
		Если ТипЗнч(ЭлементНастройки) <> Тип("НастройкиКомпоновкиДанных") И ТипЗнч(ЭлементНастройки) <> Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") И ТипЗнч(ЭлементНастройки) <> Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
			ЭлементНастройки.Использование = Использование;
		КонецЕсли;
...
Процедура ИзменитьИспользованиеЭлементаНастройки(ИмяКоллекции)
	...
	КлючСвойства				= КлючСвойстваЭлементаСтруктурыНастроек(ИмяКоллекции, Строка);
	СвойствоЭлементаСтруктуры	= СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, КлючСвойства, ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	...
	Если Строка.Свойство("ЭтоПараметрВывода") И Строка.ЭтоПараметрВывода И Строка(Строка.Идентификатор) = "DATAPARAMETERSOUTPUT" Тогда
		ЭлементНастройки.Использование	= Истина;
		ЭлементНастройки.Значение		= ?(Строка.Использование, ТипВыводаТекстаКомпоновкиДанных.Авто, ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
	КонецЕсли;
...
Процедура ИзменитьИспользованиеПоУсловию(ИмяКоллекции, Условие, Использование)
	...
	Найденные = ВариантыОтчетовКлиентСервер.О_НайтиСтрокиТаблицы(Коллекция, Условие);
...
Процедура УстановитьПометкуУдаления(ИмяКоллекции, Строка)
	...
	Если ИмяКоллекции = "Оформление" Тогда
		Строка.Картинка = ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Ошибка");
	Иначе
		Строка.Картинка = ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Элемент", "ПометкаУдаления");
	КонецЕсли;
КонецПроцедуры
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	Особенности = Новый Структура("МогутБытьРазделы, МогутБытьПараметры, МогутБытьПараметрыВывода, МогутБытьГруппы, ТребоватьОдногоРодителя");
	Особенности.МогутБытьРазделы			= (Контекст.ТаблицаИмя = "Отборы" Или Контекст.ТаблицаИмя = "ВыбранныеПоля" Или Контекст.ТаблицаИмя = "Сортировка" Или Контекст.ТаблицаИмя = "Оформление");
	...
	Особенности.ТребоватьОдногоРодителя		= (Контекст.Действие = "Переместить" Или Контекст.Действие = "ПереместитьВИерархии" Или Контекст.Действие = "Сгруппировать");
...
Функция СтрокаДобавлена(Строки, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьРазделы И СтрокаДерева.ЭтоРаздел Тогда
		Возврат Истина; // Пропустить строку.
	КонецЕсли;
	Если (Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр) Или (Особенности.МогутБытьПараметрыВывода И СтрокаДерева.ЭтоПараметрВывода) Тогда
		Если Строки.Действие = "Переместить" Тогда
			Строки.ПричинаОтказа = "Параметры не могут быть перемещены.";
		ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
			Строки.ПричинаОтказа = "Параметры не могут быть участниками групп.";
		ИначеЕсли Строки.Действие = "Удалить" Тогда
			Строки.ПричинаОтказа = "Параметры не могут быть удалены.";
		КонецЕсли;
	...
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Строки.ТекущийРодитель <> Родитель Тогда
			Если Строки.Действие = "Переместить" Тогда
				Строки.ПричинаОтказа = "Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.";
			ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
				Строки.ПричинаОтказа = "Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.";
			КонецЕсли;
...
Процедура ПереместитьВИерархии(Контекст)
	...
	Если НовыйРодитель = Неопределено Или НовыйРодитель.Тип = "ТаблицаКомпоновкиДанных" Или НовыйРодитель.Тип = "ДиаграммаКомпоновкиДанных" Тогда
		Возврат;
	КонецЕсли;
...
Функция ДоступныеРежимыОтображения(ПоказыватьРежимыФлажков)
	...
	ДоступныеРежимыОтображения.Добавить("ПоказыватьВШапкеОтчета", "В шапке отчета", , БиблиотекаКартинок.БыстрыйДоступ);

	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", "Только флажок в шапке отчета", , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;

	ДоступныеРежимыОтображения.Добавить("ПоказыватьВНастройкахОтчета", "В настройках отчета", , БиблиотекаКартинок.Реквизит);

	Если ПоказыватьРежимыФлажков Тогда
		ДоступныеРежимыОтображения.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", "Только флажок в настройках отчета", , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;

	ДоступныеРежимыОтображения.Добавить("НеПоказывать", "Не показывать", , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Функция КартинкаРежимаОтображенияЭлементаНастройки(ЭлементНастройки)
	...
	Если ЗначениеЗаполнено(ЭлементНастройки.ИдентификаторПользовательскойНастройки) Тогда
		Если ЭлементНастройки.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.БыстрыйДоступ Тогда
			Если ТипЗнч(ЭлементНастройки) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Или ТипЗнч(ЭлементНастройки) = Тип("ЭлементУсловногоОформленияКомпоновкиДанных") Тогда
...
Функция ЭлементыНастроек(Знач СвойствоЭлементаСтруктуры, Знач ЭлементНастройки = Неопределено, Знач ИмяКоллекции = Неопределено)
	...
	Если ИмяКоллекции <> Неопределено Тогда
		...
		Если ТипОбъекта = Тип("ТаблицаКомпоновкиДанных") И СтрРазделить("Строки, Колонки", ", ", Ложь).Найти(ИмяКоллекции) <> Неопределено Или ТипОбъекта = Тип("ДиаграммаКомпоновкиДанных") И СтрРазделить("Серии, Точки", ", ", Ложь).Найти(ИмяКоллекции) <> Неопределено Тогда
			Возврат ЭлементНастройки[ИмяКоллекции];
		КонецЕсли;
	КонецЕсли;

	Если ТипОбъекта = Тип("НастройкиКомпоновкиДанных") Или ТипОбъекта = Тип("ГруппировкаКомпоновкиДанных") Или ТипОбъекта = Тип("ГруппировкаТаблицыКомпоновкиДанных") Или ТипОбъекта = Тип("ГруппировкаДиаграммыКомпоновкиДанных") Тогда
		Возврат ЭлементНастройки.Структура;
	ИначеЕсли ТипОбъекта = Тип("КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных") Или ТипОбъекта = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Или ТипОбъекта = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
		Возврат ЭлементНастройки;
...
Функция ПолучитьРодителяЭлементаНастройки(Знач СвойствоЭлементаСтруктуры, Знач ЭлементНастройки)
	...
	Если ЭлементНастройки <> Неопределено И ТипЭлемента <> Тип("ПолеГруппировкиКомпоновкиДанных") И ТипЭлемента <> Тип("АвтоПолеГруппировкиКомпоновкиДанных") И ТипЭлемента <> Тип("АвтоЭлементПорядкаКомпоновкиДанных") И ТипЭлемента <> Тип("ЭлементПорядкаКомпоновкиДанных") И ТипЭлемента <> Тип("ЭлементУсловногоОформленияКомпоновкиДанных") И ТипЭлемента <> Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Тогда
...
Процедура ВыбратьПоле(ИмяКоллекции, Обработчик, Поле = Неопределено, ИдентификаторУзлаНастроек = Неопределено)
	...
	ПараметрыВыбора.Вставить("ИдентификаторЭлементаСтруктурыНастроек",	?(ИдентификаторУзлаНастроек = Неопределено, ИдентификаторЭлементаСтруктурыНастроек, ИдентификаторУзлаНастроек));

	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.ВыборПоляОтчета", ПараметрыВыбора, ЭтотОбъект, УникальныйИдентификатор,,, Обработчик, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
КонецПроцедуры
...
Процедура ОбновитьФорму(ПараметрыОбновления = Неопределено)
	...
	Если РасширенныйРежим = 0 Тогда
		ВариантыОтчетовСервер.О_ОбновитьЭлементыФормыНастроек(ЭтотОбъект, Элементы.Основное, ПараметрыОбновления);
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	ОтображатьСоставГруппировки = (ЭтоРасширенныйРежим И РежимИзмененияЭлементаСтруктурыНастроек И ТипЭлементаСтруктурыНастроек <> "ДиаграммаКомпоновкиДанных");
	...
	Если ЭтоРасширенныйРежим Тогда
		Элементы.СтраницаОтборы.Заголовок = "Фильтры";
	Иначе
		Элементы.СтраницаОтборы.Заголовок = "Основное";
	КонецЕсли;
	...
	Элементы.ОтборыПерейтиКНастройкамДляТехническогоСпециалиста.Видимость = ЭтоРасширенныйРежим И Не РежимИзмененияЭлементаСтруктурыНастроек;

	#КонецОбласти

	#Область СвойстваЭлементовСтраницыПоляИСортировки

	СвойствоЭлементаСтруктуры	= СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, "Выбор", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	ОтображатьВыбранныеПоля		= СвойствоЭлементаСтруктуры <> Неопределено И (ЗначениеЗаполнено(СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки) Или ЭтоРасширенныйРежим);

	СвойствоЭлементаСтруктуры	= СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, "Порядок", ИдентификаторЭлементаСтруктурыНастроек, РасширенныйРежим);
	ОтображатьСортировки		= ТипЭлементаСтруктурыНастроек <> "ДиаграммаКомпоновкиДанных" И СвойствоЭлементаСтруктуры <> Неопределено И (ЗначениеЗаполнено(СвойствоЭлементаСтруктуры.ИдентификаторПользовательскойНастройки) Или ЭтоРасширенныйРежим);

	Если ОтображатьВыбранныеПоля И ОтображатьСортировки Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = "Поля и сортировки";
	ИначеЕсли ОтображатьВыбранныеПоля Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = "Поля";
	ИначеЕсли ОтображатьСортировки Тогда
		Элементы.СтраницаВыбранныеПоляИСортировки.Заголовок = "Сортировки";
	КонецЕсли;
	...
	Элементы.ВыбранныеПоляПерейтиКНастройкамДляТехническогоСпециалиста.Видимость = ЭтоРасширенныйРежим И Не РежимИзмененияЭлементаСтруктурыНастроек;
	...
	Элементы.ГруппаЕстьВложенноеОформление.Видимость	= ОтображатьОформление И СодержитВложенноеУсловноеОформление И ОтображатьИнформацию;

	Элементы.ОформлениеПерейтиКНастройкамДляТехническогоСпециалиста.Видимость = ЭтоРасширенныйРежим И Не РежимИзмененияЭлементаСтруктурыНастроек;

	#КонецОбласти

	#Область СвойстваЭлементовСтраницыСтруктураВарианта

	ОтображатьСтруктуруВарианта	= НастройкиОтчета.РазрешеноИзменятьСтруктуру И Не РежимИзмененияЭлементаСтруктурыНастроек И (СодержитПользовательскиеЭлементыСтруктуры Или ЭтоРасширенныйРежим);
	...
	Элементы.СтруктураВариантаПерейтиКНастройкамДляТехническогоСпециалиста.Видимость = ЭтоРасширенныйРежим И Не РежимИзмененияЭлементаСтруктурыНастроек;
	...
	Иначе
		ОтображатьСтраницы = ОтображатьСоставГруппировки Или ОтображатьОтборы Или ОтображатьВыбранныеПоля Или ОтображатьСортировки Или ОтображатьОформление Или ОтображатьСтруктуруВарианта;
	КонецЕсли;
	...
	Если РежимИзмененияЭлементаСтруктурыНастроек Тогда
		Элементы.ЗакрытьИСформировать.Заголовок	= "Завершить редактирование";
		Элементы.Закрыть.Заголовок				= "Отмена";
	Иначе
		Элементы.ЗакрытьИСформировать.Заголовок	= "Закрыть и сформировать";
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	ОбновитьНастройкиВарианта = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ПараметрыЗагрузки, "ОбновитьНастройкиВарианта", Ложь);
	...
	ВариантыОтчетовСервер.О_ЗаполнитьДополнительныеСвойства(ОтчетОбъект, ДоступныеНастройки.Настройки, КлючТекущегоВарианта, НастройкиОтчета.КлючПредопределенногоВарианта);
	...
	Если НастройкиЗагружены И ФормаПараметры.НачальныйКлючВарианта = КлючТекущегоВарианта И ТипЗнч(ФормаПараметры.Отбор) = Тип("Структура") Тогда
	...
	ВариантыОтчетовСервер.О_ИнициализироватьПредопределенныеПараметрыВывода(НастройкиОтчета, ДоступныеНастройки.Настройки);
	...
	ВариантыОтчетовСервер.О_УстановитьУсловияОтборов(ПараметрыЗагрузки, КомпоновщикНастроек);
...
Процедура ИнициализироватьРеквизитыПредопределенныхПараметровВывода()
	...
	Если ПолеВыводаЗаголовка <> Неопределено Тогда
		ПолеВыводаЗаголовка.Заголовок = СтрШаблон("Выводить заголовок для печати: %1", ?(ЗначениеЗаполнено(Объект.Значение), Объект.Значение, "<Отсутствует>"));
	КонецЕсли;

	Объект			= ПредопределенныеПараметры.Найти("DATAPARAMETERSOUTPUT");
	СвязанныйОбъект	= ПредопределенныеПараметры.Найти("FILTEROUTPUT");

	ВыводитьФильтры	= (Объект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить Или СвязанныйОбъект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
КонецПроцедуры
...
Процедура ОформитьСписок(Элемент, ЭлементНастройки)
	...
	Иначе
		ПараметрКлиента		= БазоваяПодсистемаКлиентПовтИсп.СП_ПараметрыРаботыКлиента();
...
Функция РазрешитьИзменятьУсловияОтборов()
	...
	Для Каждого ЭлементПользовательскойНастройки Из ПользовательскиеНастройки.Элементы Цикл
		...
		Если ТипЗнч(ЭлементНастройки) <> Тип("ЭлементОтбораКомпоновкиДанных") Или ТипЗнч(ЭлементНастройки.ПравоеЗначение) = Тип("СтандартныйПериод") Или ЭлементНастройки.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный Тогда
			Продолжить;
		КонецЕсли;
...
Процедура НайтиПомеченныеНаУдалениеПоляОтбора(Настройки, Знач ЭлементСтруктуры = Неопределено, Группа = Неопределено)
	...
	Для Каждого ЭлементНастройки Из ЭлементыНастроек Цикл
		...
		Если ТипЗнч(ЭлементНастройки.ЛевоеЗначение) <> Тип("ПолеКомпоновкиДанных") Или ДоступныеПоля.НайтиПоле(ЭлементНастройки.ЛевоеЗначение) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Функция ИменаКоллекцийЭлементаСтруктуры(Элемент)
	...
	ТипЭлемента = ТипЗнч(Элемент);
	Если ТипЭлемента = Тип("ГруппировкаКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаТаблицыКомпоновкиДанных") Или ТипЭлемента = Тип("ГруппировкаДиаграммыКомпоновкиДанных") Или ТипЭлемента = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Или ТипЭлемента = Тип("КоллекцияЭлементовСтруктурыНастроекКомпоновкиДанных") Тогда
		ИменаКоллекций = "Структура";
	ИначеЕсли ТипЭлемента = Тип("ТаблицаКомпоновкиДанных") Или ТипЭлемента = Тип("КоллекцияЭлементовСтруктурыТаблицыКомпоновкиДанных") Тогда
		ИменаКоллекций = "Строки, Колонки";
	ИначеЕсли ТипЭлемента = Тип("ДиаграммаКомпоновкиДанных") Или ТипЭлемента = Тип("КоллекцияЭлементовСтруктурыДиаграммыКомпоновкиДанных") Тогда
...
Процедура УдалитьПомеченныеНаУдалениеПоля()
	...
	Для Каждого Запись Из ПомеченныеНаУдаление Цикл
		СвойствоЭлементаСтруктуры = СвойствоЭлементаСтруктурыНастроек(КомпоновщикНастроек, Запись.КлючСвойстваЭлементаСтруктуры, Запись.ИдентификаторЭлементаСтруктуры, РасширенныйРежим);
		...
		Если ЭлементыНастроек.Количество() = 0 И ТипЗнч(РодительЭлементаНастройки) = Тип("ПоляГруппировкиКомпоновкиДанных") Тогда
...
Процедура ОпределитьДанныеВыбораЭлементаОтбораПоЗапросу(ИсходныйЗапрос, ДанныеВыбора, ДоступныеПоля, Запрос = Неопределено, Родитель = Неопределено, Индекс = 0)
	...
	Пока Индекс <= ОписаниеЗапроса.ВГраница() Цикл
		...
		ИначеЕсли ДанныеВыбора.Количество() = 0 Тогда
			...
			Если Родитель <> Неопределено Тогда
				ПутьКДанным = СтрШаблон("%1.%2", Родитель.Поле, ФрагментЗапроса);
			КонецЕсли;
			...
			ОпределитьДанныеВыбораЭлементаОтбораПоЗапросу(ИсходныйЗапрос, ДанныеВыбора, ДоступныеПоля, НовыйЗапрос, НайденноеПоле, Индекс);
		КонецЕсли;
...
Функция ФрагментЗапросаЭлементаОтбора(Запрос)
	ШаблонФрагментаЗапроса = СтрШаблон(
...
#Иначе
	ФрагментЗапроса = БазоваяПодсистемаКлиент.СФ_ФорматированнаяСтрока(ШаблонФрагментаЗапроса);
#КонецЕсли
...
Функция ПредставлениеКоллекцииЭлементаСтруктуры(ИмяКоллекции)
	...
	ПредставлениеКоллекций.Вставить("Структура","Структура");
	ПредставлениеКоллекций.Вставить("Строки",	"Строки");
	ПредставлениеКоллекций.Вставить("Колонки",	"Колонки");
	ПредставлениеКоллекций.Вставить("Точки",	"Точки");
	ПредставлениеКоллекций.Вставить("Серии",	"Серии");
...
Процедура ОткрытьФормуНастроекДляТехническогоСпециалиста(ИмяСтраницы, ОбработчикЗавершения = Неопределено)
	...
	ПараметрыФормы.Вставить("ПользовательскиеНастройкиМодифицированы",	ВариантИзменен Или ПользовательскиеНастройкиМодифицированы);
