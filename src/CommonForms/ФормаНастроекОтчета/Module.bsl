...
Процедура ОбновитьВыбранныеПоля(Строки = Неопределено, ЭлементыНастроек = Неопределено)
	...
	Если Строки = Неопределено Тогда
		...
		Раздел.Картинка			= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("ВыбранныеПоля");
		Раздел.Идентификатор	= "ВыбранныеПоля";
		Строки					= Раздел.ПолучитьЭлементы();
	КонецЕсли;

	КартинкаГруппы		= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Группа");
	КартинкаЭлемента	= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Элемент");
...
Процедура ОбновитьСортировку()
	...
	Раздел.Картинка		= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("Сортировки");
	Строки				= Раздел.ПолучитьЭлементы();
...
Процедура ОбновитьОформление()
	...
	Иначе
		...
		Раздел.Заголовок		= "Условное оформление";
		Раздел.Представление	= "Условное оформление";
		Раздел.Картинка			= ВариантыОтчетовКлиентСервер.О_ИндексКартинки("УсловноеОформление");
		Раздел.ЭтоРаздел		= Истина;
		Строки					= Раздел.ПолучитьЭлементы();
	КонецЕсли;
...
Процедура ПрочитатьПредопределенныеПараметрыОформления()
	...
	Строка.Представление				= "Заголовок";
	...
	Строка.Заголовок					= "Выводить параметры и отборы";
	Строка.Представление				= "Выводить параметры и отборы";
...
Функция ПредставлениеЗаголовкаДляПечати(Знач ЗаголовокДляПечати)
	Если Не ЗначениеЗаполнено(ЗаголовокДляПечати) Тогда
		ЗаголовокДляПечати = "<Отсутствует>";
	КонецЕсли;

	Возврат СтрШаблон("%1: %2", "Заголовок для печати", ЗаголовокДляПечати);
КонецФункции
...
Процедура ВыбратьРежимОтображения(ОтборыУзлаНастроек, ИмяКоллекции, ИдентификаторСтроки, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков, ТекущийРежимОтображения = Неопределено)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВШапкеОтчета", "В шапке отчета", , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", "Только флажок в шапке отчета", , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВНастройкахОтчета", "В настройках отчета", , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", "Только флажок в настройках отчета", , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить("НеПоказывать", "Не показывать", , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура ОбновитьСвойстваЭлементовФормы()
	...
	Если ЭтоРасширенныйРежим Тогда
		Элементы.СтраницаОтборы.Заголовок = "Отборы";
	Иначе
...
Процедура ИнициализироватьРеквизитыПредопределенныхПараметровВывода()
	...
	ВыводитьПараметрыИОтборы = (Объект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить Или СвязанныйОбъект.Значение <> ТипВыводаТекстаКомпоновкиДанных.НеВыводить);
КонецПроцедуры
