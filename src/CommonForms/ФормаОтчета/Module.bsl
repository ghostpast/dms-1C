...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоиска(ПодходящиеЗначения, ИспользуемыеПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Разделитель		= СтрШаблон("%1ОБЪЕДИНИТЬ ВСЕ%2", Символы.ПС, Символы.ПС);
...
Функция ПолеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтрШаблон("КОГДА ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 ТОГДА ""%3""", Поле.Имя, """%" + ПараметрыПоиска.СтрокаПоиска + "%""", Поле.Имя);
...
Функция УсловиеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если ТипРеквизита.СодержитТип(Тип("Строка")) Тогда
			Условие = СтрШаблон("ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2", Поле.Имя, """%" + ПараметрыПоиска.СтрокаПоиска + "%""");
		ИначеЕсли ТипРеквизита.СодержитТип(Тип("Число")) Тогда
			Условие = СтрШаблон("ВЫРАЗИТЬ(%1 КАК ЧИСЛО) = %2", Поле.Имя, ПараметрыПоиска.ЧислоПоиска);
		Иначе
			Продолжить;
		КонецЕсли;
...
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, ПолеПоиска, Условие, СтрокаПоиска)
	...
	ФрагментСтрокиПоиска	= ФрагментСтрокиПоиска(Сред(ПредставлениеЗначения, НачалоСтрокиПоиска, СтрДлина(СтрокаПоиска)));
...
Функция ФрагментУсловия(Условие, ОтступДоУсловия = " ", ОтступПослеУсловия = " ")
	...
	ШаблонФрагментаУсловия = СтрШаблон("%1<span style=""color: СкрытыйВариантОтчетаЦвет"">%2</span>%3", ОтступДоУсловия, УсловиеНормализованное, ОтступПослеУсловия);
...
Функция ФрагментСтрокиПоиска(СтрокаПоиска)
	ШаблонФрагментаСтрокиПоиска = СтрШаблон("<span style=""color: МоиВариантыОтчетовЦвет; font: ВажнаяНадписьШрифт"">%1</span>", СтрокаПоиска);
...
Функция ОписаниеПолейОтчета()
	...
	ИндексСтруктурыОтчета = ВариантыОтчетовСервер.сВО_ИндексСтруктурыОтчета(ЭтотОбъект);
...
Процедура СохранитьВыбранныйУровеньГруппировок()
	...
	БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеНастроекДанныхФорм, КлючОбъектаВыбранногоУровняГруппировок(), "ВыбранныйУровеньГруппировок", ВыбранныйУровеньГруппировок, Неопределено, Неопределено , Ложь);
КонецПроцедуры
...
Процедура ВосстановитьВыбранныйУровеньГруппировок()
	...
	СохраненноеЗначение = БазоваяПодсистемаСервер.ОН_ХранилищеЗагрузить(ХранилищеНастроекДанныхФорм, КлючОбъектаВыбранногоУровняГруппировок(), "ВыбранныйУровеньГруппировок", Неопределено, Неопределено, Неопределено);
...
Функция КлючОбъектаВыбранногоУровняГруппировок()
	Возврат СтрШаблон("%1/%2", НастройкиОтчета.ПолноеИмя, КлючТекущегоВарианта);
КонецФункции
...
Процедура ЗагрузитьВариант(КлючВарианта)
	Если Не РежимРасшифровки И Не ВариантМодифицирован Тогда
		КлючОбъекта = СтрШаблон("%1/%2/ТекущиеПользовательскиеНастройки", НастройкиОтчета.ПолноеИмя, КлючТекущегоВарианта);
