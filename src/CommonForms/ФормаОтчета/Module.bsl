...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РежимРасшифровки	= (Параметры.Свойство("Расшифровка") И Параметры.Расшифровка <> Неопределено) Или Параметры.Свойство("ПредставлениеВарианта") И Параметры.ПредставлениеВарианта = "Расшифровка";
	...
	Если Не ВариантыОтчетовКлиентСервер.сВО_РежимВариантаОтчета(КлючТекущегоВарианта) Тогда
...
Процедура ОбработкаВыбора(Результат, ПодчиненнаяФорма)
	...
	Если ТипЗнч(ПодчиненнаяФорма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
			...
			Если ИмяФормыИсточника = ВРег("ФормаНастроекОтчета") Или ИмяФормыИсточника = ВРег("ФормаНастроек") Или ИмяФормыИсточника = ВРег("ФормаВариантаОтчета") Или ИмяФормыИсточника = ВРег("ФормаВарианта") Тогда
...
Процедура ПередЗагрузкойВариантаНаСервере(НовыеНастройкиКД)
	...
	Если НовыеНастройкиКД = Неопределено Или Не ВариантыОтчетовКлиентСервер.сВО_РежимВариантаОтчета(КлючТекущегоВарианта) Тогда
		Возврат;
	КонецЕсли;
	...
	Если РежимРасшифровки Тогда
		Если ТипЗнч(Параметры.Расшифровка) = Тип("ОписаниеОбработкиРасшифровкиКомпоновкиДанных") Тогда
			ВариантыОтчетовСервер.сВО_ПодготовитьНастройкиОтчетаКРасшифровкеПоДетальнымЗаписям(ЭтотОбъект, НовыеНастройкиКД, Параметры.Расшифровка.ПрименяемыеНастройки);
		КонецЕсли;
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	// Ничего не делать если отчет не на СКД и никаких настроек не загружено.
	Если Не ВариантыОтчетовКлиентСервер.сВО_РежимВариантаОтчета(КлючТекущегоВарианта) И НовыеНастройкиКД = Неопределено Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПоказательРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Иначе
		ПоказатьПредупреждение(, "Расширенные возможности настройки фильтров недоступны");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПрименитьНастройкиИПереформироватьОтчет(Результат, ПараметрыВыполнения) Экспорт
	...
	Если Результат.Свойство("ИмяСобытия") И Результат.ИмяСобытия = "ФормаНастроек" И Результат.ВариантМодифицирован Тогда
		ВариантыОтчетовКлиент.сВО_ДобавитьНастройкиВСтек(ЭтотОбъект, Результат.КомпоновщикНастроекКД.Настройки, Результат.ИмяСобытия);
	КонецЕсли;
...
Процедура НастройкиПоУмолчанию(Команда)
	...
	ВариантыОтчетовКлиент.сВО_СброситьПометкиСтекаНастроек(СтекНастроек);
...
Процедура СохранитьВариант(Команда)
	...
	ПоказатьОповещениеПользователя("Вариант отчета сохранен",, Заголовок);
КонецПроцедуры
...
Процедура ИзменитьСоставБыстрыхНастроек(Команда)
	...
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.БыстрыеНастройкиОтчета",ПараметрыФормы, ЭтотОбъект, УникальныйИдентификатор,,, Обработчик);
КонецПроцедуры
...
#Иначе
	ПоказатьПредупреждение(, "Для того чтобы редактировать схему компоновки, запустите приложение в режиме толстого клиента.");
#КонецЕсли
КонецПроцедуры
...
Процедура СохранитьВариантОтчетаВФайл(Команда)
	ОткрытьФорму("ХранилищеНастроек.ХранилищеВариантовОтчетов.Форма.СохранениеВариантаОтчетаВФайл", ВариантыОтчетовКлиент.сВО_ПараметрыСохраненияВариантаОтчетаВФайл(ЭтотОбъект), ЭтотОбъект);
КонецПроцедуры
...
Процедура СохранитьОтчет(Команда)
	ТекстПредложения = "Для сохранения результата отчета в файл рекомендуется
...
Процедура ОчиститьОформление(Команда)
	ВариантыОтчетовКлиент.сВО_ОчиститьОформление(ЭтотОбъект, СвойстваЗаголовка());
КонецПроцедуры
...
Процедура Подключаемый_ПерейтиМеждуИзменениямиНастроек(Команда)
	ПредставлениеПорядка = СтрЗаменить(Команда.Имя, "ПерейтиМеждуИзменениямиНастроек", "");
	ОписаниеЧисла = Новый ОписаниеТипов("Число");
	...
	Результат = ВариантыОтчетовКлиент.сВО_РезультатКонтекстнойНастройки(Отчет.КомпоновщикНастроек, ТекущиеНастройкиСтека.Действие, УникальныйИдентификатор);
...
Процедура Сформировать()
	...
	Результат = РезультатФормированияОтчета(ФормированиеПриОткрытии, НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный, Ложь);
	...
	Если Результат.Свойство("ИмяПараметраХраненияОтказаОтПредупреждения") Тогда
		...
		Если Предупредить Тогда
			...
			БазоваяПодсистемаКлиент.СП_ПоказатьВопросПользователю(Новый ОписаниеОповещения("СформироватьПослеПредупреждения", ЭтотОбъект, Контекст), Результат.ТекстПредупреждения, Кнопки, ПараметрыВопроса);
...
Процедура СформироватьПослеПредупреждения(Ответ, Контекст) Экспорт
	...
	ИначеЕсли ТипЗнч(Ответ) = Тип("Структура") Тогда
		...
		Если Ответ.Значение = "Продолжить" Тогда
			...
			Результат = РезультатФормированияОтчета(ФормированиеПриОткрытии,НастройкиОтчета.Внешний Или НастройкиОтчета.Безопасный, Истина);
		Иначе
			Возврат;
		КонецЕсли;
	Иначе
...
Функция РезультатФормированияОтчета(Знач ФормированиеПриОткрытии, Непосредственно, ПослеПредупреждения)
	...
	Результат = БазоваяПодсистемаСервер.ДО_ВыполнитьВФоне("ВариантыОтчетовСервер.ВО_СформироватьОтчетВФоне", ПараметрыФормирования, ПараметрыВыполнения);
...
Процедура ПоказатьСписокВыбора(Элемент, СтандартнаяОбработка)
	...
	ЗначенияДляВыбора = ЗначенияДляВыбора(Элемент.СписокВыбора, Сведения.ЭлементПользовательскойНастройки, Элемент.ДоступныеТипы, ДоступныеЗначения);
	...
	Если ВариантыОтчетовКлиент.О_ВыборПереопределен(ЭтотОбъект, Обработчик, Сведения.Описание, Элемент.ДоступныеТипы, ОтмеченныеЗначения, ПараметрыВыбора) Тогда
		Возврат;
	КонецЕсли;

	Если ВариантыОтчетовКлиент.О_ЭтоВыборОбъектовМетаданных(Элемент.ДоступныеТипы, ОтмеченныеЗначения, Обработчик) Или ВариантыОтчетовКлиент.О_ЭтоВыборПользователей(ЭтотОбъект, Элемент, Элемент.ДоступныеТипы, ОтмеченныеЗначения, ПараметрыВыбора, Обработчик) Тогда
		Возврат;
	КонецЕсли;
...
Функция ЗначенияДляВыбора(СписокВыбора, ЭлементНастройки, ДоступныеТипы, ДоступныеЗначения)
	ЗначенияДляВыбора = БазоваяПодсистемаКлиент.ОН_СкопироватьРекурсивно(СписокВыбора);
	...
	ВариантыОтчетовКлиент.О_ОбновитьПредставленияСписка(ЗначенияДляВыбора, ДоступныеЗначения);
...
Процедура РассчитатьПоказатели(ТекущаяКоманда = "")
	...
	Показатель = "";
	БазоваяПодсистемаКлиент.сОН_РассчитатьПоказатели(ЭтотОбъект, "ОтчетТабличныйДокумент", ТекущаяКоманда, 2);
КонецПроцедуры
...
Функция ПодходящиеЗначенияУниверсальногоПоиска(Запрос)
	...
	ПоляПоискаПоТипам = ПоляУниверсальногоПоискаПоТипам(НастройкиОтчета.АдресСхемы, Отчет.КомпоновщикНастроек.Настройки);
...
Функция ПараметрыУниверсальногоПоиска(Запрос)
	...
	Если БазоваяПодсистемаКлиентСервер.ОН_ЭтоЧисло(ПараметрыПоиска.СтрокаПоиска) Тогда
		ОписаниеЧисла				= Новый ОписаниеТипов("Число");
...
Функция ПараметрыУниверсальногоПоискаПрименимыКСсылкам(ПараметрыПоиска)
	...
	Возврат СтрДлина(ПараметрыПоиска.СтрокаПоиска) > 2 Или БазоваяПодсистемаКлиентСервер.ОН_ЭтоЧисло(ПараметрыПоиска.СтрокаПоиска);
КонецФункции

&НаКлиенте
Процедура НайтиПодходящиеЗначенияУниверсальногоПоискаЧисла(ПодходящиеЗначения, ПоляПоискаПоТипам, ПараметрыПоиска)
	Если Не БазоваяПодсистемаКлиентСервер.ОН_ЭтоЧисло(ПараметрыПоиска.СтрокаПоиска) Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ПолеПоиска Из ДоступныеПоляПоиска Цикл
		...
		ПредставлениеПоиска	= ПредставлениеЗначенияУниверсальногоПоискаСтроки(ПолеПоиска.Значение, СвойстваПоиска.ВидСравнения, СвойстваПоиска.ПравоеЗначение);
	...
	ВариантыОтчетовКлиентСервер.О_ДополнитьСписок(ПодходящиеЗначения, ПодходящиеЗначенияДляЧисла);
КонецПроцедуры
...
Функция ДоступныеПоляПоискаЧисла(ПоляПоиска, ЗначениеПоиска, УправляющиеСимволы)
	...
	Если Условие = ВидСравненияКомпоновкиДанных.Равно Тогда
		Возврат БазоваяПодсистемаКлиент.ОН_СкопироватьРекурсивно(ПоляПоиска);
	КонецЕсли;
...
Процедура НайтиПодходящиеЗначенияУниверсальногоПоиска(ПодходящиеЗначения, ПоляПоискаПоТипам, ПараметрыПоиска)
	...
	Для Каждого Элемент Из ПоляПоискаПоТипам Цикл
		Если Не БазоваяПодсистемаСервер.ОН_ЭтоСсылка(Элемент.Ключ) Тогда
			Продолжить;
		КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			...
			Если ВариантыПоиска = Неопределено Тогда
				ПредставлениеПоиска = ПредставлениеУниверсальногоПоискаСсылки(Выборка, ПолеПоиска.Значение, СвойстваПоиска.ВидСравнения, ПараметрыПоиска.СтрокаПоиска);
			...
			Если Не ЗначениеЗаполнено(Выборка.ПолеПоУсловию) Или ИндексДополнительныхЗначений[ПолеПоиска.Ключ] <> Неопределено Тогда
				Продолжить;
			КонецЕсли;

			ПутьКДанным			= СтрШаблон("%1.%2", ПолеПоиска.Ключ, Выборка.ПолеПоУсловию);
			...
			ПредставлениеПоиска	= ПредставлениеЗначенияУниверсальногоПоискаСтроки(ПолеПоиска.Значение, СвойстваПоиска.ВидСравнения, ПараметрыПоиска.СтрокаПоиска);
	...
	ВариантыОтчетовКлиентСервер.О_ДополнитьСписок(ПодходящиеЗначения, ДополнительныеЗначения);
...
Функция ПолеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		ПолеПоУсловию = СтрШаблон("КОГДА ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2 ТОГДА ""%3""", Поле.Имя, """%" + ПараметрыПоиска.СтрокаПоискаНормализованная + "%""", Поле.Имя);
	...
	ОписаниеПоляПоУсловию.Вставить(0, "ВЫБОР");
	ОписаниеПоляПоУсловию.Добавить("ИНАЧЕ """"");
	ОписаниеПоляПоУсловию.Добавить("КОНЕЦ");
...
Функция УсловиеТекстаЗапросаПоОбъектуПоиска(Объект, ПараметрыПоиска)
	...
	Для Каждого Поле Из Поля Цикл
		...
		Если ТипРеквизита.СодержитТип(Тип("Строка")) Тогда
			Условие = СтрШаблон("ВЫРАЗИТЬ(%1 КАК СТРОКА(150)) ПОДОБНО %2", Поле.Имя, """%" + ПараметрыПоиска.СтрокаПоискаНормализованная + "%""");
		ИначеЕсли ТипРеквизита.СодержитТип(Тип("Число")) И ПараметрыПоиска.ЧислоПоиска <> Неопределено Тогда
			Условие = СтрШаблон("ВЫРАЗИТЬ(%1 КАК ЧИСЛО) = &ЧислоПоиска", Поле.Имя);
		Иначе
			Продолжить;
		КонецЕсли;
...
Процедура УточнитьТекстПервогоЗапросаУниверсальногоПоиска(ТекстыЗапросов)
	...
	ОписаниеОператоров.Вставить(1, "РАЗРЕШЕННЫЕ");
...
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, ПолеПоиска, Условие, СтрокаПоиска)
	...
	Если ЗначениеЗаполнено(Данные.ДополнительныеДанные) Тогда
		ПредставлениеЗначения = СтрШаблон("%1 (%2)", Данные.Представление, Данные.ДополнительныеДанные);
	КонецЕсли;
	...
	ФрагментСтрокиПоиска	= ФрагментСтрокиПоиска(Сред(ПредставлениеЗначения, НачалоСтрокиПоиска, ДлинаНайденногоФрагмента));

	ФрагментПослеСтрокиПоиска = Сред(ПредставлениеЗначения, КонецСтрокиПоиска);

	Возврат Новый ФорматированнаяСтрока(ПолеПоиска, ФрагментУсловия, ФрагментДоСтрокиПоиска, ФрагментСтрокиПоиска, ФрагментПослеСтрокиПоиска);
КонецФункции
...
Функция ФрагментУсловия(Условие, ОтступДоУсловия = " ", ОтступПослеУсловия = " ")
	...
	ШаблонФрагментаУсловия = СтрШаблон("%1<span style=""color: %2"">%3</span>%4", ОтступДоУсловия, "СкрытыйВариантОтчетаЦвет", УсловиеНормализованное, ОтступПослеУсловия);

#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ФрагментУсловия = БазоваяПодсистемаСервер.СФ_ФорматированнаяСтрока(ШаблонФрагментаУсловия);
#Иначе
	ФрагментУсловия = БазоваяПодсистемаКлиент.СФ_ФорматированнаяСтрока(ШаблонФрагментаУсловия);
#КонецЕсли
...
Функция ФрагментСтрокиПоиска(СтрокаПоиска)
	ШаблонФрагментаСтрокиПоиска = СтрШаблон("<span style=""color: %1; font: %2"">%3</span>", "МоиВариантыОтчетовЦвет", "ВажнаяНадписьШрифт", СтрокаПоиска);

#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ФрагментСтрокиПоиска = БазоваяПодсистемаСервер.СФ_ФорматированнаяСтрока(ШаблонФрагментаСтрокиПоиска);
#Иначе
	ФрагментСтрокиПоиска = БазоваяПодсистемаКлиент.СФ_ФорматированнаяСтрока(ШаблонФрагментаСтрокиПоиска);
#КонецЕсли
...
Функция УсловиеУниверсальногоПоиска(ЗначениеПоиска, УправляющиеСимволы)
	Если ТипЗнч(ЗначениеПоиска) = Тип("Строка") И (Не ЗначениеЗаполнено(УправляющиеСимволы) Или УправляющиеСимволы = "=") Тогда
...
Функция ПоляУниверсальногоПоискаПоТипам(АдресСхемы, Знач Настройки)
	...
	ВариантыОтчетовСервер.О_ИнициализироватьКомпоновщикНастроек(КомпоновщикНастроек, АдресСхемы);
...
Функция ОписаниеПолейОтчета(КомпоновщикНастроек)
	...
	ИндексСтруктурыОтчета = ВариантыОтчетовСервер.сВО_ИндексСтруктурыОтчета(КомпоновщикНастроек);
...
Функция ЗначениеУниверсальногоПоискаПримененоКПараметрам(ВариантыПоиска, ЭтоПользовательскийПоиск)
	...
	Если ОписаниеПараметра.ДоступенСписокЗначений Тогда
		НайденныйПараметр.Значение = ВариантыОтчетовКлиентСервер.О_ЗначенияСписком(ПодходящийВариантПоиска.ПравоеЗначение);
	Иначе
	...
	ПользовательскийПараметр = КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(НайденныйПараметр.ИдентификаторПользовательскойНастройки);
...
Процедура ПрименитьЗначениеУниверсальногоПоискаКОтборам(ВариантыПоиска, ЭтоПользовательскийПоиск)
	...
	Для Каждого ВариантПоиска Из ВариантыПоиска Цикл
		Фильтр = ВариантыОтчетовКлиентСервер.сВО_ФильтрРазделаОтчета(Фильтры, ВариантПоиска.ЛевоеЗначение);
	...
	ПользовательскийФильтр	= КомпоновщикНастроек.ПользовательскиеНастройки.Элементы.Найти(Фильтр.ИдентификаторПользовательскойНастройки);
...
Процедура УточнитьСвойстваФильтра(Фильтр, Поиск)
	...
	Значения = ВариантыОтчетовКлиентСервер.О_ЗначенияСписком(Фильтр.ПравоеЗначение);

	Если Значения.НайтиПоЗначению(Поиск.ПравоеЗначение) = Неопределено Тогда
		Значения.Добавить(Поиск.ПравоеЗначение);
	КонецЕсли;

	Если Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно И Поиск.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно И Значения.Количество() > 1 Тогда
		Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
		Фильтр.ПравоеЗначение = Значения;
	ИначеЕсли Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке И Поиск.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно Тогда
		Фильтр.ПравоеЗначение = Значения;
	ИначеЕсли Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно И Поиск.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно И Значения.Количество() > 1 Тогда
		Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке;
		Фильтр.ПравоеЗначение = Значения;
	ИначеЕсли Фильтр.Использование И Фильтр.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке И Поиск.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно Тогда
		Фильтр.ПравоеЗначение = Значения;
	Иначе
...
Процедура ЗаполнитьМенюУровнейГруппировок(ИндексСтруктурыОтчета)
	...
	Если ИндексСтруктурыОтчета = Неопределено Или ИндексСтруктурыОтчета.Количество() = 0 Тогда
		ИндексСтруктурыОтчета	= ВариантыОтчетовСервер.сВО_НовыйИндексСтруктурыОтчета();
...
Функция СвойстваМенюУровнейГруппировокПростогоОтчета(ИндексСтруктурыОтчета)
	...
	ГруппировкиОтчета = ГруппировкиОтчетаСУчетомПоляСИерархическойГруппировкой(ИндексСтруктурыОтчета, ПоляСИерархическойГруппировкой, КоличествоУровнейГруппировок);
...
Функция ГруппировкиОтчетаСУчетомПоляСИерархическойГруппировкой(ИндексСтруктурыОтчета, ПоляСИерархическойГруппировкой, КоличествоУровнейГруппировок)
	...
	ШаблонПредставленияГруппировки			= "%1 - уровень %2";
	...
	Пока КоличествоУровнейПоляСИерархическойГруппировкой > 0 Цикл
		...
		ДополнительнаяГруппировка.ПредставлениеГруппировки	= СтрШаблон(ШаблонПредставленияГруппировки, ДополнительнаяГруппировка.ПредставлениеГруппировки, УровеньПоляСИерархическойГруппировкой);
	...
	НайденнаяГруппировка.ПредставлениеГруппировки	= СтрШаблон(ШаблонПредставленияГруппировки, НайденнаяГруппировка.ПредставлениеГруппировки, УровеньПоляСИерархическойГруппировкой);
...
Процедура СохранитьВыбранныйУровеньГруппировок(КлючХраненияЗначения, ВыбранныйУровеньГруппировок, РежимРасшифровки)
	...
	БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеНастроекДанныхФорм, КлючХраненияЗначения, "ВыбранныйУровеньГруппировок", ВыбранныйУровеньГруппировок, Неопределено, Неопределено , Ложь);
КонецПроцедуры
...
Процедура ВосстановитьВыбранныйУровеньГруппировок(КлючХраненияЗначения, ВыбранныйУровеньГруппировок, РежимРасшифровки)
	...
	СохраненноеЗначение = БазоваяПодсистемаСервер.ОН_ХранилищеЗагрузить(ХранилищеНастроекДанныхФорм, КлючХраненияЗначения, "ВыбранныйУровеньГруппировок", Неопределено, Неопределено, Неопределено);
...
Процедура УстановитьВидимостьДоступность(ПриСохраненииВарианта = Ложь)
	ПоказыватьКомандыВыбораВариантов = ВариантыОтчетовКлиентСервер.сВО_РежимВариантаОтчета(КлючТекущегоВарианта) И НастройкиОтчета.РазрешеноВыбиратьВарианты;
	...
	Если Не ПриСохраненииВарианта Тогда
		...
		Элементы.ГруппаСохранениеВыборВариантаОтчета.Видимость	= Истина;

		Элементы.ИзменитьСоставБыстрыхНастроек.Видимость = ПоказыватьКомандыИзмененияВарианта И Не БазоваяПодсистемаСервер.ОН_ЭтоМобильныйКлиент();
		...
		ЭтоПредопределенныйВариант		= Не НастройкиОтчета.Пользовательский И ЗначениеЗаполнено(НастройкиОтчета.ПредопределенныйСсылка);

		РазрешеноСохранятьВариант		= ПоказыватьКомандыИзмененияВарианта И Не НастройкиОтчета.РазрешеноВыбиратьИНастраиватьВариантыБезСохранения;

		ДоступноСохранятьТекущийВариант	= ЗначениеЗаполнено(НастройкиОтчета.ВариантСсылка) И Не ЭтоПредопределенныйВариант;

		БазоваяПодсистемаКлиентСервер.ОН_УстановитьСвойствоЭлементаФормы(Элементы, "СохранитьВариант", "Доступность", ДоступноСохранятьТекущийВариант);
		БазоваяПодсистемаКлиентСервер.ОН_УстановитьСвойствоЭлементаФормы(Элементы, "СохранитьВариантЕще", "Доступность", ДоступноСохранятьТекущийВариант);
		...
		БазоваяПодсистемаКлиентСервер.ОН_УстановитьСвойствоЭлементаФормы(Элементы, "СохранитьВариантКак", "Видимость", РазрешеноСохранятьВариант);
		БазоваяПодсистемаКлиентСервер.ОН_УстановитьСвойствоЭлементаФормы(Элементы, "СохранитьВариантКакЕще", "Видимость", РазрешеноСохранятьВариант);
...
Процедура ПроверитьДоступностьОбменаНастройкамиВарианта(МетаданныеОтчета)
	ОбменНастройкамиВариантаДоступен = ВариантыОтчетовКлиентСервер.сВО_РежимВариантаОтчета(КлючТекущегоВарианта) И ВариантыОтчетовСервер.ВО_ОтчетПодключенКХранилищу(МетаданныеОтчета);
...
Процедура ЗагрузитьВариант(КлючВарианта, ОчиститьСтекНастроек = Истина)
	Если Не РежимРасшифровки И Не ВариантМодифицирован Тогда
		КлючОбъекта = СтрШаблон("%1/%2/%3", НастройкиОтчета.ПолноеИмя, КлючТекущегоВарианта, "ТекущиеПользовательскиеНастройки");

		БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеСистемныхНастроек, КлючОбъекта, "", Отчет.КомпоновщикНастроек.ПользовательскиеНастройки, Неопределено, Неопределено, Ложь);
	КонецЕсли;
...
Процедура УстановитьКлючТекущегоВарианта(ОтчетПолноеИмя, ОтчетОбъект)
	...
	Если ТипЗнч(Расшифровка) = Тип("ОписаниеОбработкиРасшифровкиКомпоновкиДанных") Тогда
		...
		КлючТекущегоВариантаРасшифровки	= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Настройки.ДополнительныеСвойства, "КлючВарианта", "");
	КонецЕсли;
...
Функция НастройкиОтчета(ОтчетОбъект)
	...
	Настройки.ИспользуемыеТаблицы	= БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(Параметры, "ИспользуемыеТаблицы");
...
Функция СобытияНастроек()
	...
	НайтиСобытияНастроек( Элементы.ОтчетТабличныйДокумент.КонтекстноеМеню, УточненияСобытийНастроек(), СобытияНастроекИсключения(), СобытияНастроек);

	СобытияНастроек.Вставить("ФильтроватьИСформировать", "Фильтровать...");

	СобытияНастроек.Вставить("ФормаНастроек", "Расширенная настройка");

	СобытияНастроек.Вставить("ИзменитьСоставБыстрыхНастроек", "Изменение состава быстрых настроек");
...
Функция УточненияСобытийНастроек()
	...
	Префиксы.Вставить("ОформитьЖелтым",		"Оформить желтым");
	Префиксы.Вставить("ОформитьКрасным",	"Оформить красным");
	Префиксы.Вставить("ОформитьЗеленым",	"Оформить зеленым");
	Префиксы.Вставить("ОформитьЕще",		"Оформить (еще)");
...
Процедура НайтиСобытияНастроек(Меню, Уточнения, Исключения, СобытияНастроек)
	Для Каждого Элемент Из Меню.ПодчиненныеЭлементы Цикл
		...
		ИначеЕсли СобытияНастроек[Элемент.ИмяКоманды] = Неопределено И Исключения.Найти(Элемент.ИмяКоманды) = Неопределено Тогда
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления = Неопределено)
	...
	Если ПараметрыОбновления.ИмяСобытия <> "ПослеФормирования" Тогда
		...
		ИначеЕсли Переформировать Тогда
			ВариантыОтчетовКлиентСервер.О_ОтобразитьСостояниеОтчета(ЭтотОбъект, "Отчет формируется...", БиблиотекаКартинок.ДлительнаяОперация48);
		ИначеЕсли Не ОтчетСформирован И (ПараметрыОбновления.ВариантМодифицирован Или ПараметрыОбновления.ПользовательскиеНастройкиМодифицированы) Тогда
			ВариантыОтчетовКлиентСервер.О_ОповеститьОИзмененииНастроек(ЭтотОбъект);
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗагрузитьНастройкиВКомпоновщик(ПараметрыЗагрузки)
	...
	ВариантыОтчетовСервер.О_ЗаполнитьДополнительныеСвойства(ОтчетОбъект,
	...
	Если Не НастройкиОтчета.Пользовательский Тогда
		ОтчетОбъект.КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("ВариантНаименование", ОтчетНаименованиеТекущегоВарианта);
	КонецЕсли;

	ВариантыОтчетовСервер.О_УстановитьДоступныеЗначения(ОтчетОбъект, ЭтотОбъект);
	ВариантыОтчетовСервер.О_ИнициализироватьПредопределенныеПараметрыВывода(НастройкиОтчета, Отчет.КомпоновщикНастроек.Настройки, ВариантыОтчетовСервер.О_ТребуетсяСброситьПредопределенныеПараметрыВывода(ПараметрыЗагрузки));
...
Процедура СохранитьТекущийВариантОтчета()
	...
	ХранилищаНастроек.ХранилищеВариантовОтчетов.Сохранить(КлючОтчета, КлючТекущегоВарианта, Отчет.КомпоновщикНастроек.Настройки, ОписаниеНастроек);
...
Процедура ОбновитьИнформациюОВариантеОтчета()
	...
	ДополнительныеСвойства.Вставить("ВариантНаименование",				ОтчетНаименованиеТекущегоВарианта);
	ДополнительныеСвойства.Вставить("КлючПредопределенногоВарианта",	ОписаниеНастроекОтчета.КлючПредопределенногоВарианта);
КонецПроцедуры
...
Функция ВариантОтчетаПоСсылкеИКлючу(ОтчетСсылка, КлючВарианта = Неопределено)
	...
	Если КлючВарианта <> Неопределено Тогда
		Запрос.Текст = Запрос.Текст +" И ВариантыОтчетов.КлючВарианта = &КлючВарианта";
...
Функция КлючНастроекВариантаОтчета(ПолноеИмяОтчета, КлючВарианта)
	Возврат СтрШаблон("%1/%2", ПолноеИмяОтчета, КлючВарианта);
КонецФункции
...
Процедура УдалитьКомандыМенюПереходаМеждуИзменениямиНастроек()
	МаксимальныйРазмерСтекаНастроек = 10;

	Для Порядок = 1 По МаксимальныйРазмерСтекаНастроек Цикл
		ИмяКоманды = СтрШаблон("%1%2", "ПерейтиМеждуИзменениямиНастроек", Порядок);
...
Процедура ОпределитьДоступностьКомандПереходаМеждуИзменениямиНастроек()
	...
	КнопкаУстановкиСтандартныхНастроек.Доступность	= КнопкаУстановкиСтандартныхНастроек.Видимость И ТекущиеНастройкиСтекаОпределены;
...
Процедура ДобавитьКомандуПереходаМеждуИзменениямиНастроек(ЗаписьСтекаНастроек, ГруппаДействий, СледующаяКоманда)
	ИмяКоманды			= СтрШаблон("%1%2", "ПерейтиМеждуИзменениямиНастроек", ЗаписьСтекаНастроек.Порядок);
	ЗаголовокКоманды	= СтрШаблон("%1. %2", ЗаписьСтекаНастроек.Порядок, ЗаписьСтекаНастроек.ПредставлениеДействия);
...
Процедура ОбновитьСочетаниеКлавишКомандПереходаМеждуИзменениямиНастроек()
	...
	Если ТекущиеНастройкиСтека.Количество() > 0 Тогда
		...
		СочетаниеКлавишУстановлено = УстановитьСочетаниеКлавишКомандыПереходаМеждуИзменениямиНастроек(ПорядокКомандыТекущихНастроекСтека - 1, СочетаниеКлавишОтменыДействия);

		Если Не СочетаниеКлавишУстановлено И ПорядокКомандыТекущихНастроекСтека = 1 И Не КнопкаУстановкиСтандартныхНастроек.Пометка Тогда
			КнопкаУстановкиСтандартныхНастроек.СочетаниеКлавиш = СочетаниеКлавишОтменыДействия;
		КонецЕсли;

		УстановитьСочетаниеКлавишКомандыПереходаМеждуИзменениямиНастроек(ПорядокКомандыТекущихНастроекСтека + 1, СочетаниеКлавишПовтораДействия);
...
Функция УстановитьСочетаниеКлавишКомандыПереходаМеждуИзменениямиНастроек(ПорядокКоманды, СочетаниеКлавиш)
	ИмяКоманды = СтрШаблон("%1%2", "ПерейтиМеждуИзменениямиНастроек", ПорядокКоманды);
	Кнопка = Элементы.Найти(ИмяКоманды);
...
Процедура СохранитьСостояниеОпцииВыводитьЗаголовкиНастроек(КлючХраненияЗначения, ВыводитьЗаголовкиНастроек, РежимРасшифровки)
	...
	БазоваяПодсистемаСервер.ОН_ХранилищеСохранить(ХранилищеНастроекДанныхФорм, КлючХраненияЗначения, "ВыводитьЗаголовкиНастроек", ВыводитьЗаголовкиНастроек, Неопределено, Неопределено, Ложь);
КонецПроцедуры
...
Процедура ВосстановитьСостояниеОпцииВыводитьЗаголовкиНастроек(КлючХраненияЗначения, ВыводитьЗаголовкиНастроек, РежимРасшифровки)
	...
	СохраненноеЗначение = БазоваяПодсистемаСервер.ОН_ХранилищеЗагрузить(ХранилищеНастроекДанныхФорм, КлючХраненияЗначения, "ВыводитьЗаголовкиНастроек", Неопределено, Неопределено, Неопределено);
...
Процедура ПослеУстановкиРасширенияРаботыСФайлами(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Диалог.Заголовок			= "Сохранение результата отчета";

	Обработчик = Новый ОписаниеОповещения("ПослеВыбораИмениФайла", ЭтотОбъект, Контекст);
	БазоваяПодсистемаКлиент.ФС_ПоказатьДиалогВыбора(Обработчик, Диалог);
КонецПроцедуры
...
Процедура ПослеСохраненияРезультатаОтчета(Результат, ПолноеИмяФайлаОтчета) Экспорт
	...
	ПоказатьОповещениеПользователя("Отчет сохранен в файл", Обработчик, ПолноеИмяФайлаОтчета);
КонецПроцедуры

&НаКлиенте
Процедура ПриВыбореИмениФайлаОтчета(ПолноеИмяФайлаОтчета) Экспорт
	БазоваяПодсистемаКлиент.ФС_ОткрытьФайл(ПолноеИмяФайлаОтчета);
КонецПроцедуры
...
Функция ДоступныеФорматыСохраненияОтчета(ИндексФорматовСохраненияОтчета)
	...
	ФорматыСохранения = БазоваяПодсистемаСервер.СП_НастройкиФорматовСохраненияТабличногоДокумента();
	...
	Для Каждого ФорматСохранения Из ФорматыСохранения Цикл
		...
		ПредставлениеФорматаСохранения = СтрЗаменить(ФорматСохранения.Представление, "." + ФорматСохранения.Расширение, РасширениеФорматаСохранения);
...
Процедура УточнитьПризнакАвтоформированияОтчета(ПараметрыОбновления)
	Если Не БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(ПараметрыОбновления, "Переформировать", Ложь) Тогда
