...
Функция ЗначенияДляВыбора(СписокВыбора, ЭлементНастройки, ДоступныеТипы, ДоступныеЗначения)
	...
	ВариантыОтчетовКлиентСервер.О_ОбновитьПредставленияСписка(ЗначенияДляВыбора, ДоступныеЗначения);
...
Процедура УстановитьКлючТекущегоВарианта(ОтчетПолноеИмя, ПредопределенныеВарианты)
	...
	Если ЗначениеЗаполнено(КлючНазначенияИспользования) Или (ЗначениеЗаполнено(Параметры.КлючВарианта) И КлючТекущегоВарианта <> Параметры.КлючВарианта) Тогда
...
Процедура ОбновитьЭлементыФормыНастроекНаСервере(ПараметрыОбновления = Неопределено)
	...
	ОтчетСформирован = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(НастройкиОтчета, "ОтчетСформирован", Ложь);
...
Функция ФорматыСохраненияОтчета(Знач ПолноеИмяФайлаОтчета)
	...
	ИндексФорматовСохраненияОтчета = БазоваяПодсистемаКлиентСервер.ОН_СвойствоСтруктуры(НастройкиОтчета, "ИндексФорматовСохраненияОтчета", Новый Соответствие);
