...
Функция ПодходящиеЗначенияУниверсальногоПоиска(Запрос)
	...
	Если ПодходящиеЗначения.Количество() = 0 И СтрДлина(ПараметрыПоиска.СтрокаПоиска) > 2 Тогда
		Представление = Новый ФорматированнаяСтрока("Совпадений не найдено",, WebЦвета.НейтральноСерый);
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			...
			Если ВариантыПоиска = Неопределено Тогда
				ПредставлениеПоиска = ПредставлениеУниверсальногоПоискаСсылки(Выборка, СвойстваПоиска.ВидСравнения, ПараметрыПоиска.СтрокаПоиска);
...
Функция ПредставлениеУниверсальногоПоискаСсылки(Данные, Условие, СтрокаПоиска)
	...
	Возврат Новый ФорматированнаяСтрока(ФрагментУсловия, ФрагментДоСтрокиПоиска, ФрагментСтрокиПоиска, ФрагментПослеСтрокиПоиска);
КонецФункции
...
Функция ФрагментУсловия(Условие, ОтступДоУсловия = " ", ОтступПослеУсловия = " ")
	ШаблонФрагментаУсловия = СтрШаблон("%1<span style=""color: СкрытыйВариантОтчетаЦвет"">%2</span>%3", ОтступДоУсловия, НРег(Условие), ОтступПослеУсловия);
