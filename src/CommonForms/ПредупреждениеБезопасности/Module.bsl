...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Не ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ТекстОшибки =
			"Общая форма ""Предупреждение безопасности"" является вспомогательной и открывается из служебных механизмов программы.";
	...
	Если Не ПустаяСтрока(Параметры.ИмяФайла) Тогда
		Элементы.ПредупреждениеПриОткрытииФайла.Заголовок = СтрШаблон(Элементы.ПредупреждениеПриОткрытииФайла.Заголовок, Параметры.ИмяФайла);
	КонецЕсли;

	// Зарезервировано для новых подсистем

	Если Параметры.МножественныйВыбор Тогда
		Элементы.ПредупреждениеПередУдалениемРасширенияСДаннымиТекстУдалить.Заголовок = "Удалить выделенные расширения?";
	Иначе
		Элементы.ПредупреждениеПередУдалениемРасширенияСДаннымиТекстУдалить.Заголовок = "Удалить расширение?";
	КонецЕсли;
...
&НаКлиенте
Процедура ПредупреждениеПередУдалениемРасширенияРезервноеКопированиеОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура УправлениеРольюНаСервере()
	...
	БазоваяПодсистемаСервер.СП_УстановитьПараметрыАдминистрирования(ПараметрыАдминистрирования);
